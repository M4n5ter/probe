...
通过 HTTP JSON hook 委托 protective decision，通过显式 route 或 opt-in DNS discovery 解析 allow-path
upstream target，为转发响应和 proxy-generated deny/gateway response 写入 plaintext bridge event，
返回 proxy-side response，并记录 durable delegated decision。
...
- product proxy routed 和 DNS-discovered allow/deny path 已覆盖透明入站和出站 HTTPS MITM。
  product proxy TLS 会 advertise 并 gate HTTP/1.1 ALPN；已支持 exact 和 suffix-wildcard
  upstream route；显式 opt-in DNS upstream discovery 默认过滤 IANA special-purpose/special-use
  address range。
