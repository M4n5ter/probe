...
同一 Traffic tab 也提供 `Watch`、`Auto`、`eBPF` 和 `libpcap` 操作。`Watch`
为选中进程写入或移除默认 `Auto` 的双向 process observation profile；`Auto`、`eBPF`
和 `libpcap` 显式选择该 profile 的 data-path mode。保存时，如果有 active admin socket，
会进入 runtime reload contract。Traffic tab 会展示 MITM data-path readiness，但 reliable MITM
本身通过 Enforcement 和 TLS 页面配置，因为 transparent interception 同时拥有 nftables、
proxy lifecycle、client trust 和 plaintext bridge 等 setup-time data path 资源。
...
`apply_config_reload`；capture、observation、config version 和支持在线 generation 的 TLS material
变更会进入 runtime generation，不要求用户手动重启 TUI。policy-only 主配置变更在
...
`enforcement.selector` 或启用中的 transparent interception setup selector 引用时会与 runtime
generation 一起排队；若 action-gated selector consumer 引用了变更 entry，则仍需要重启，直到这些
owner 能够在同一个事务中应用。capture、observation、config version、TLS plaintext instrumentation 和 TLS
...
ref 会先在 manifest 命名空间内解析，再与主配置 `enforcement.selector` 合成。
这个 effective policy selector 不会收窄或扩大单独配置的
`enforcement.interception.selector`；后者拥有 transparent interception setup scope。
...
`apply-config-reload` 只会对兼容的 plan-only online owner、单个 action-gated online owner 更新
active plan，或提交 runtime generation request；generation request 可携带兼容的 plan-only storage
retention 和 export plan 更新，但不会提前替换 active plan。
policy-only 主配置变更在本地 watcher 和远程 poller topology 未启用时可在线应用；enforcement
policy source 和 `enforcement.selector` 变更在 enforcement reload watcher/poller topology 未启用，
且 transparent interception 未启用或由显式 `enforcement.interception.selector` 固定 setup scope
时可在线应用。顶层 `[selectors]` registry 变更只有在变更 entry 未被 enabled policy selector、
`enforcement.selector` 或启用中的 transparent interception setup selector 引用时才会进入 runtime
generation；被这些 action-gated selector consumer 引用时仍需要重启。
...
预算耗尽后的成功切换会在 outcome 中标记 forced handoff。capture、observation、data-path-only selector
registry、config version、TLS plaintext instrumentation 和 TLS decrypt-hint material 变更会构建候选
capture generation，只有候选 provider 打开成功后才交换到 live loop、更新 runtime status 并替换共享
active plan。plan-only 的 storage retention 和 export plan 变更可在 active plan 未独立前进时随
generation request 携带；action-gated online/data-path 混合变更保持 `restart_required`，直到存在能整体
应用候选配置且不会产生 partial commit 的事务型 generation owner。generation request 无法入队时，旧
