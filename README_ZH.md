...
product proxy 下游 TLS termination、基于 native roots 或导入 trust anchors 的 upstream TLS relay，
strategy-specific target recovery、outbound upstream socket-mark bypass，以及 loopback HTTP JSON policy hook，
用于 proxy-side enforcement delegation。
first-party `product_proxy` backend 会从 typed MITM readiness、bridge、policy hook
leaf certificate material refs 和 upstream trust material refs 生成 proxy CLI。
...
- crate-level downstream/upstream TLS relay 不等于完整透明 HTTPS MITM；transparent rule-path
  end-to-end validation、动态 SNI/ALPN routing、强原始归因和自动修改 trust store 仍是明确能力边界。
- 不隐藏长期保存原始流量。
