...
decision，写入 plaintext bridge event，返回 proxy-side deny response，并记录 durable delegated
decision。
...
name；TCP 连接目标仍来自配置或 original destination，不表示已经支持 hostname-to-upstream
target routing。
first-party `product_proxy` backend 会从 typed MITM readiness、bridge、policy hook
CA 或 leaf TLS material refs 和 upstream trust material refs 生成 proxy CLI。
...
- product proxy deny path 已覆盖透明入站 HTTPS MITM。ALPN-aware routing、
  hostname-to-upstream target routing、强原始归因、自动修改 trust store 和通用 transparent
  allow-path upstream routing matrix 仍是明确能力边界。
- 不隐藏长期保存原始流量。
