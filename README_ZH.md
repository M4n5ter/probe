...
name；也可以用 reconcile 后的 SNI 或 HTTP Host 命中显式 host-to-upstream route。Host/SNI
不一致会 fail closed，route 未命中时回退配置目标或 recovered original destination。
first-party `product_proxy` backend 会从 typed MITM readiness、bridge、policy hook
CA 或 leaf TLS material refs、upstream trust material refs 和 host route table 生成 proxy CLI。
...
- product proxy routed allow path 和 deny path 已覆盖透明入站和出站 HTTPS MITM。product proxy TLS
  ...
  ALPN-based multi-protocol routing、DNS-discovered upstream route selection、强原始归因、
  自动修改 trust store 和非 HTTP transparent allow-path matrix 仍是明确能力边界。
- 不隐藏长期保存原始流量。
