...
product proxy 下游 TLS termination、静态 leaf material 或 CA-backed dynamic SNI certificate、
基于 native roots 或导入 trust anchors 的 upstream TLS relay、strategy-specific target recovery、
HTTP/1.1 ALPN gate、outbound upstream socket-mark bypass，以及 loopback HTTP JSON policy hook，
用于 proxy-side enforcement delegation。
first-party product proxy 还具备透明入站和出站 HTTPS E2E 路径。入站 TPROXY 可以将客户端连接送入
proxy；出站 OUTPUT redirect 可以将本机发起的连接送入同一个 proxy，并让 proxy 自身的 upstream
socket 通过保留 mark 绕过再次捕获。两个方向都会用 CA-backed dynamic SNI certificate 终止下游 TLS，
通过 HTTP JSON hook 委托 protective decision，将显式 allow-path host 路由到配置的 upstream target，
为转发响应和 proxy-generated deny/gateway response 写入 plaintext bridge event，返回 proxy-side
response，并记录 durable delegated decision。
CA-backed dynamic certificate mode 要求下游 TLS client 发送 DNS SNI。启用 upstream TLS 时，
product proxy 会使用该 SNI 作为 upstream TLS server name，除非显式 pin 了 upstream server
name；也可以用 reconcile 后的 SNI 或 HTTP Host 命中显式 host-to-upstream route。配置固定 upstream
时，不能再组合 route table 或 DNS discovery；route table 可以把 DNS discovery 作为显式 fallback。
Host/SNI 不一致会 fail closed；route mode 下 route 未命中时可先经过显式 opt-in DNS upstream
discovery，再回退 recovered target。DNS discovery 默认拒绝 IANA special-purpose/special-use
address range，除非 operator 显式允许。
first-party `product_proxy` backend 会从 typed MITM readiness、bridge、policy hook
CA 或 leaf TLS material refs、upstream trust material refs、host route table 和 upstream discovery
policy 生成 proxy CLI。
...
- product proxy routed allow path 和 deny path 已覆盖透明入站和出站 HTTPS MITM。product proxy TLS
  会 advertise 并 gate HTTP/1.1 ALPN；已支持 exact 和 suffix-wildcard upstream route；
  product proxy 数据面支持显式 opt-in DNS upstream discovery，且默认过滤 IANA
  special-purpose/special-use address range。
  ALPN-based multi-protocol routing、强原始归因、自动修改 trust store 和非 HTTP transparent
  allow-path matrix 仍是明确能力边界。
- 不隐藏长期保存原始流量。
