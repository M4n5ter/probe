...
CA-backed dynamic certificate mode 要求下游 TLS client 发送 DNS SNI；这个 SNI 只用于下游
certificate selection，不表示已经支持 upstream SNI-based routing。
...
- crate-level downstream/upstream TLS relay 不等于完整透明 HTTPS MITM；transparent rule-path
  end-to-end validation、ALPN-aware routing、SNI-based upstream routing、强原始归因和自动修改
  trust store 仍是明确能力边界。
- 不隐藏长期保存原始流量。
