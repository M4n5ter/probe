...
outbound upstream socket-mark bypass，以及 loopback HTTP JSON policy hook，用于 proxy-side
...
- product proxy routed allow path 和 deny path 已覆盖透明入站 HTTPS MITM。ALPN-aware
  routing、通配或 DNS-discovered upstream route selection、强原始归因、自动修改 trust
  store 和非 HTTP transparent allow-path matrix 仍是明确能力边界。
- 不隐藏长期保存原始流量。
