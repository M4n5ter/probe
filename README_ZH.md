...
- product proxy routed allow path 和 deny path 已覆盖透明入站和出站 HTTPS MITM。product proxy TLS
  会 advertise 并 gate HTTP/1.1 ALPN；ALPN-based multi-protocol routing、通配或
  DNS-discovered upstream route selection、强原始归因、自动修改 trust store 和非 HTTP
  transparent allow-path matrix 仍是明确能力边界。
- 不隐藏长期保存原始流量。
