...
first-party product proxy 还具备透明入站 HTTPS E2E 路径：TPROXY 将客户端连接送入 proxy，
proxy 使用 CA-backed dynamic SNI certificate 终止下游 TLS，通过 HTTP JSON hook 委托 protective
decision，将显式 allow-path host 路由到配置的 upstream target，写入 plaintext bridge event，
...
- product proxy routed allow path 和 deny path 已覆盖透明入站 HTTPS MITM。product proxy TLS
