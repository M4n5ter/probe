...
| TLS plaintext | libssl uprobe 明文、TLS 1.3 key log/session secret material、auto-binding 和 plaintext bridge。 |
...
- transparent interception 和 L7 MITM 是显式 strategy，拥有独立 readiness、self-bypass、
  material、lifecycle 和 audit contract。
...
capture-event plaintext bridge provenance，以及 loopback HTTP JSON policy hook，
