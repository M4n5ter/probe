...
本地 bundle 是一个包含 `manifest.toml`、`main.lua` 和可选 bundle-local Lua module 的目录：
...
`manifest.toml` 声明 bundle 身份和 Probe 需要调用的 hook。policy 使用 bundle-local module
时，每个 module name 都必须显式列出：
...
bundle-local module 通过 `require("guard.matcher")` 加载。module name 必须是 dotted Lua
identifier，并在本地 bundle 中映射到 `modules/guard/matcher.lua`。未声明 module、重复 module
name、symlink module file 和超出大小限制的 module source 都会被拒绝。每个 bundle 最多声明
64 个 module。所有已声明 module 会在 bundle 激活前完成语法检查，但 module body 仍在首次
`require` 时才执行。module source 是 Lua chunk，需要用 `return M` 显式返回 module value，
不能使用裸表达式作为 module。
...
允许使用的 Lua 标准库是 `table`、`string`、`math` 和 `bit`。`require` 会被替换为只允许加载
manifest 声明 module 的 bundle-local loader。Host capability 会被移除：`io`、`os`、
`package`、`debug`、`ffi`、`jit`、`dofile`、`loadfile`、`load` 和 `collectgarbage` 都不可用。
...
| `on_websocket_message` | 重组后的有界 WebSocket message。 |
...
- `opcode`
- `payload_len`
- `payload_text`，仅在完整 UTF-8 text payload 不超过 64 KiB 时存在
- `payload_fingerprint`
...
1-indexed integer byte value 数组；fingerprint 不代表保留明文 payload。WebSocket message
payload bytes 会保留在 export event 中，但 Lua 不接收裸 `payload` byte array；超过 64 KiB
的 text message 和 binary message 应通过 `payload_len`、`payload_fingerprint` 与 `opcode`
判断。
...
bundle-local module 通过带 `name` 和 `source` 的 `[[modules]]` entry 携带。未知的顶层字段、
manifest 字段或 module 字段都会被拒绝。
