...
- `capture.deep_observe_selector` 可按方向授权 outbound `write(2)` / `sendto(2)` / `writev(2)` / `sendmsg(2)` 的
  single-buffer 或 bounded iovec-prefix syscall argument sample。
- 同一 selector 授权 inbound `read(2)` / `recvfrom(2)` / `readv(2)` / `recvmsg(2)` 的 single-buffer 或
  bounded iovec-prefix syscall result sample。
- 内核只在 userspace allow map 的 fd-table epoch 与 read/write direction mask 同时匹配时读取 payload。
- vector syscall 在固定 scan limit 内跳过 zero-length iovec，并将可读 iovec 拼接成 bounded prefix。
...
- 不保存完整 WebSocket message payload；`websocket_message` metadata 上限为 16 MiB；超限 message 保留 frame metadata
...
| Protocol | `http1`、`sse`、`websocket_handoff`、`websocket_frame`、`websocket_message` | WebSocket 输出 handoff、frame metadata 和 16 MiB 有界 text/binary message metadata；超限 message 保留 frame metadata 并省略 message event；不保存完整 message payload，不解压 extension payload。 | 第 12-14 节 |
...
- `ebpf`
  ...
  - 已实现：vector syscall 在固定 scan limit 内跳过 zero-length iovec，并拼接可读 iovec 的 bounded prefix。
...
- `http1` / `sse` / `websocket_handoff` / `websocket_frame` / `websocket_message`
  - Runtime status：HTTP/1、SSE、WebSocket handoff/frame 为 Available；`websocket_message` 为 Degraded，因为只提供
    16 MiB 有界 metadata，不保存完整 payload，也不解压 extension payload。
  ...
  - 已实现：对无 extension RSV 的 text/binary payload 输出 16 MiB 有界完整 message metadata，包括 message sequence、
    首尾 frame sequence、payload length 和 payload fingerprint。
  ...
  - 验收：`xtask e2e-websocket-plaintext-feed` 覆盖 HTTP Upgrade、101 response、WebSocket text frame、message metadata、
    handoff/frame/message export、Lua hook、connection lifecycle 顺序、无 protocol error 和 durable spool 输出。
  - 边界：不保存完整 WebSocket message payload，不解压 extension payload；不支持 HTTP/2、HTTP/3/QUIC。
...
- 对无 extension RSV 的 text/binary frame 和 continuation frame，parser 会输出 `WebSocketMessage` metadata。
- message metadata 包括方向、HTTP stream sequence、message sequence、首尾 frame sequence、opcode、payload length 和 payload fingerprint。
- message metadata 上限为 16 MiB；超限 message 会保留每个 `WebSocketFrame` metadata，并省略对应
  `WebSocketMessage` event。
- parser 不保存完整 WebSocket message payload。
...
- outbound 支持 single-buffer 与 bounded iovec-prefix syscall argument sample。
- inbound 支持 single-buffer 与 bounded iovec-prefix syscall result sample。
...
- enter tracepoint 在 allow gate 后立即持久化 single-buffer 或 bounded iovec-prefix snapshot。
...
- 将 scan window 内的可读 iovec 拼接成一个 bounded prefix。
...
- TLS ringbuf saturation E2E：
  ...
  并会向 previous output-loss checkpoint 之后观察到的 plaintext flows 发出 conservative unknown-offset `Gap` fan-out；
  仍缺人为制造真实 ringbuf saturation 的 privileged E2E。
- strong fd ownership：
...
- `TRAFFIC_PROBE_PENDING_WRITES`：
  保存已通过内核侧 allow+epoch+write-direction gate 的 enter-time pending outbound single-buffer/bounded iovec-prefix sample。
- `TRAFFIC_PROBE_PENDING_READS`：
  保存已通过 allow+epoch+read-direction gate 的 inbound single-buffer/bounded iovec-prefix attempt。
...
- message 事件记录方向、HTTP stream sequence、message sequence、首尾 frame sequence、opcode、payload length 和解 mask 后 payload fingerprint；
  fingerprint 增量计算，不为了 message metadata 缓存完整 payload。
- message metadata 上限为 16 MiB；超限 message 会保留 frame metadata，并省略对应 `websocket_message`。
...
| `hooks` | Lua callback 列表。 | 使用 typed policy hook enum；TOML/JSON 边界表现为 callback name，例如 `on_http_request_headers`、`on_websocket_handoff`、`on_websocket_frame`、`on_websocket_message`。 |
...
也不会被复制进 planner 生成的 `EnforcementDecision.reason`。当前 eBPF single-buffer/bounded iovec-prefix syscall
...
- eBPF object 路径放在 `[capture.ebpf] object_path` 下。这个字段作为 `aya-obj` object preflight 输入；process observation runtime 使用
  ...
  fd lifecycle、single-buffer/bounded iovec-prefix payload tracepoint sections，并拒绝夹带 TLS maps/programs 的混装
  object。高层用户态 `aya` process observation loader、connect/accept observation 到单个 `ConnectionOpened` `CaptureEvent` 的纯转换
  bridge、`capture.deep_observe_selector` 按方向授权的 outbound single-buffer/bounded iovec-prefix syscall argument
...
- `xtask e2e-ebpf-process-loopback`
  ...
  - 覆盖 selector-authorized outbound single-buffer/bounded iovec-prefix syscall argument sample。
  - 覆盖 selector-authorized inbound single-buffer/bounded iovec-prefix syscall result sample。
  ...
  - vector 模式验证带 zero-length leading iovec 和 split payload 的 bounded iovec-prefix `writev(2)` / `readv(2)` /
...
- `readv-writev` 验证 bounded iovec-prefix `writev(2)` / `readv(2)` path。
...
- 命中 selector 后，才能验证 selector-authorized outbound/inbound single-buffer/bounded iovec-prefix syscall sample。
...
- 验证带 zero-length leading iovec 和 split payload 的 bounded iovec-prefix `writev(2)` / `readv(2)`。
- 验证带 zero-length leading iovec 和 split payload 的 bounded iovec-prefix `sendmsg(2)` / `recvmsg(2)`。
...
- 断言 selector-authorized degraded outbound single-buffer/bounded iovec-prefix sample。
- 断言 selector-authorized degraded inbound single-buffer/bounded iovec-prefix sample。
...
- 覆盖 allow-map 授权。
- 覆盖 bounded outbound single-buffer/bounded iovec-prefix syscall argument sample。
- 覆盖 bounded inbound single-buffer/bounded iovec-prefix syscall result sample。
...
- vector 模式覆盖带 zero-length leading iovec 和 split payload 的 bounded iovec-prefix `writev(2)` / `readv(2)` /
...
- 该 E2E 覆盖 selector-authorized outbound/inbound single-buffer/bounded iovec-prefix payload sample degraded semantics。
...
E2E：人为制造真实 eBPF/TLS ringbuf/provider loss 时必须产生 provider-scoped `capture_loss`；存在 eligible flow target 时必须产生
conservative unknown-offset `Gap` fan-out。provider loss 信号必须进入 durable export、admin metrics/status 与 Prometheus counters。
...
| WebSocket | 识别 Upgrade、定义 handoff，解析 frame metadata/fingerprint，并输出 16 MiB 有界 text/binary message metadata；超限 message 保留 frame metadata 并省略 message event；不保存完整 message payload，不解压 extension payload | 第 13、31 节 |
