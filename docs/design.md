...
无进程条件的 `match` 或 `all(match, ...)` 交集在至少包含 local port、remote port 或 remote IP literal 约束时可投影为 `host_rules`。带进程条件的 selector 会显式报告 `requires_process_classifier`；process-only selector 不再被误报成 unsupported；`all()` 下的 process glob/regex/systemd/container 约束不会被压平成字符串交集，也不会用空 traffic selector 伪装 process-only scope，而是保留为专门的 process expression。`remote_addresses` 使用 IP 地址值匹配和规范化，不表达 hostname 或 CIDR；非法 remote address 是 setup input error，不会被伪装成 flow classifier 能力缺口。

执行层在加载外部 manifest 后，会把主 `enforcement.selector`、manifest selector 和 `enforcement.interception.selector` 的交集作为最终 effective setup selector 重新投影。process-scoped setup 只有在 process classifier capability 可用、存在 host-rule boundary、local ports 显式、当前每个目标 TCP listener inode 的所有 holder 进程都可经完整 procfs owner scan 归因且明确命中 process selector 时才会安装 host rules；其它 process-scoped 情况和所有 flow-aware setup plan 都以 setup error fail closed，避免把“按应用拦截”的配置静默扩大成全机拦截。当前 executable inbound TPROXY 对 host-rule scope 仍要求具备显式 local port 集合；wildcard local port scope 会 fail closed，因为它会把 proxy listen port 也纳入拦截范围，而当前还没有完整 proxy self-bypass lifecycle 或规则排除契约。远程 manifest 可以进一步收窄本地 scope，但不能成为安装 host rules 的唯一 scope。
...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；出站透明代理当前只覆盖 host-rule setup，后续若要支持 process/flow-scoped outbound setup，需要把 classifier、target recovery、self-bypass 和 audit/status contract 作为同一个 executable backend 的门控条件；若启用 L7 MITM backend，还必须额外接入证书材料、MITM CA 注入和 L7 分类。
...
| `transparent-interception` | root/net-admin | 透明拦截 lifecycle，覆盖 nftables/TPROXY 入站 host rule、process-scoped setup-time listener proof、outbound transparent proxy managed relay data-plane 和 external proxy reserved-mark self-bypass 路径。 |
...
19. `e2e-transparent-linux-outbound-redirect-artifact-netns` 在 root/net-admin 环境下从 explicit `transparent-linux` artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，再执行真实 `nft --check`、安装、规则列表断言和 cleanup 验证。它属于 `linux-artifacts` profile，不启动 agent，不证明 agent runtime outbound proxy self-bypass、original destination recovery 或证书/MITM lifecycle。
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_transparent_proxy`，并在 runtime plan/status/check 中报告 proxy mode/self-bypass/listen port、独立 selector、按方向投影的 `local_setup_projection`、process/flow classifier capability 和 agent-owned proxy runtime snapshot；`inbound_tproxy` 有 executable Linux lifecycle，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，配置化 active health probe 会主动检查显式 TCP endpoint 并在连续失败达到阈值后降级 health。`outbound_transparent_proxy` 按第 18 节 proxy lifecycle contract 支持 host-rule OUTPUT/dstnat redirect、managed relay 或 external reserved-mark self-bypass；agent-owned webhook/control-plane HTTP connector 也会使用 outbound proxy bypass mark，使 planned webhook exporter 可与 outbound transparent proxy 共存；RuntimePlan/check/status 会表达 OUTPUT/dstnat artifact、proxy port 和 proxy bypass mark。缺少显式 remote port 的 outbound host rules、process-scoped outbound setup、flow-aware outbound setup 仍 fail closed。
