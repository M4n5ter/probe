...
5. 运行 `cargo run -p xtask --locked -- e2e-file-exporter`，验证 plaintext feed 产生的 HTTP/policy export events 通过 configured file exporter 在 bounded run tail drain 中以 zstd-compressed protobuf batch 写入 JSON Lines file record，解码后 local-file sink cursor 前进且该 sink 没有 pending records。
6. 后续 SSE/overload E2E 覆盖 streaming response、metrics 中无静默 gap；如人为制造过载，则出现明确 degraded/gap。
...
| 验收 | HTTP/SSE demo、webhook/file exporter E2E、libssl TLS demo、pcap fallback、dry-run enforcement | 第 31 节 |
