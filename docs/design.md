...
| L7 MITM | `l7_mitm` | 目标能力；默认全机 MITM 被拒绝，显式 selector-scoped backend 尚未实现。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 验收：尚无 L7 MITM backend E2E；当前只验证透明转发、plain TCP relay、external proxy reserved-mark contract 和出站 host-rule lifecycle。
...
- `RuntimePlan.enforcement` 是能力事实源：`mode_capability` 只表达 mode 自身的能力需求，当前只有 `dry_run` 需要 `dry_run_enforcement`；
  ...
  `interception.capability` 只由可执行透明拦截 strategy 产生，当前包括入站 TPROXY 和 host-rule 出站透明代理 lifecycle；proxy 进程生命周期由 `proxy.mode`
...
当前已落地的是透明转发、plain TCP relay、external proxy reserved-mark contract 和出站 host-rule lifecycle；L7 MITM backend 本身仍未实现。
...
- `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_transparent_proxy`。
- runtime plan/status/check 报告 proxy mode/self-bypass/listen port、独立 selector、`local_setup_projection`、process/flow classifier capability 和
  agent-owned proxy runtime snapshot。
