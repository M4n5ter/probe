...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、`client_trust.mode = "operator_managed"`、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；真实 TLS MITM 数据面、proxy 内部动作执行和自动 client trust store 安装仍缺失。 | 第 11、18、32 节 |
...
- L7 MITM lifecycle audit 已进入 durable export；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  RuntimePlan/agent wiring 和本机 HTTP JSON hook transport 已接入。
- 自动 client trust store 安装、TLS MITM 数据面和 proxy 内部保护动作执行仍缺失。
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  proxy-side hook transport、`delegated` planner outcome 和 typed execution evidence readback 已由真实 agent/netns/spool E2E 覆盖。
...
RuntimePlan/agent proxy hook wiring、本机 HTTP JSON hook transport 和 typed delegated decision readback 已进入控制面能力；
真实 TLS MITM 数据面、proxy 内部保护动作执行和自动 client trust store 安装不属于当前控制面能力。
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、operator-managed client trust contract、plaintext bridge、proxy-side hook wiring 和 typed execution evidence。 | 内置 TLS MITM data-plane、proxy 内部动作执行、自动 client trust store 安装。 |
...
出站 selector redirect canary、MITM plaintext bridge live fan-in，以及本机 HTTP JSON policy hook typed delegated decision。
