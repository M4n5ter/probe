...
| Operator surface | admin/status/prometheus、ratatui TUI | TUI 是服务器本地 agent 工作台：优先复用已运行 agent，否则托管启动本地 agent，并通过 active admin socket 浏览选中进程的解析事件；policy source 编辑和大型 MITM backend contract 编辑不由 TUI 代替。 | 本节、Admin 章节 |
...
- eBPF object 路径放在 `[capture.ebpf] object_path` 下。这个字段作为 `aya-obj` object preflight 输入；process observation runtime 使用
  ...
  accept/accept4、close、close_range、process exit/exec、descriptor lease lifecycle、single-buffer/bounded multi-iovec prefix/available sendfile family
