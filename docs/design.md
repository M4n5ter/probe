...
| `e2e-suite --include-privileged` | 取决于 case | 运行 `product` profile，包括 baseline、libpcap loopback、admin reload live capture、eBPF process observation、libssl plaintext 和 transparent TPROXY。 |
...
| `transparent-interception` | root/net-admin | 透明拦截 lifecycle，覆盖 nftables/TPROXY 入站 host rule 路径和 process-scoped setup-time listener proof 路径。 |
...
17. `e2e-transparent-linux-outbound-redirect-artifact-netns` 在 root/net-admin 环境下从 explicit `transparent-linux` artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，再执行真实 `nft --check`、安装、规则列表断言和 cleanup 验证。它属于 `linux-artifacts` profile，不启动 agent，不证明 agent runtime outbound proxy self-bypass、original destination recovery 或证书/MITM lifecycle。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle、可选 managed TCP relay、基础运行态指标、配置化 active proxy health probe、relay-connect upstream connect telemetry、出站 selector projection、Linux original-destination resolver、proxy pre-connect `SO_MARK` 原语、RuntimePlan/check/status outbound redirect artifact、shared Linux OUTPUT/dstnat artifact planning、agent outbound host-rule managed relay lifecycle 和 root/net-admin netns Linux artifact E2E 已接入；L7 MITM 作为后续独立 backend | 入站用 nftables/TPROXY 透明转发到本地代理是已验证可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理当前只支持 host-rule scope + managed relay；`external` proxy self-bypass、agent control-plane outbound bypass、process/flow-scoped outbound setup、真实 agent outbound 成功 E2E 和 L7 MITM 证书/分类 lifecycle 仍是缺口。 |
...
| 当前拦截验收 | dry-run typed verdict、remote external manifest GET、manifest selector 与主配置 selector 合成、process/traffic scoped dry-run decision、manifest profile unsupported audit、online enforcement manifest reload active planner swap/invalid reload retention/setup-time surface rejection、显式 backend 下的 owner-verified TCP socket destroy audit decision，以及透明拦截 nftables/TPROXY plan/probe/lifecycle、managed relay runtime metrics、隔离 netns 入站截获/转发 E2E、process-scoped setup-time listener proof E2E、outbound host-rule lifecycle fail-closed 单测和 outbound redirect Linux artifact acceptance | 第 18、31 节 |
