...
| `e2e-suite --profile product` | user、root/CAP_NET_RAW、root/bpffs、root/net-admin | 当前产品能力全链路验证入口；显式运行产品级端到端 case，覆盖 replay/plaintext、live libpcap、admin reload、eBPF process observation、TLS plaintext/material 和 transparent interception。 |
| `e2e-suite --include-privileged` | 取决于 case | 运行 `product` profile，包括 baseline、libpcap loopback、admin reload live capture、eBPF process observation、libssl plaintext 和 transparent interception。 |
...
| `live-core` | root/CAP_NET_RAW | 真实 libpcap loopback、admin policy/enforcement reload，以及 `session_secret_file` / `key_log_file` TLS material auto-binding 的预置与 refresh 路径。 |
