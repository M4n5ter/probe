...
| attribution bridge | `capture` 提供 connect observation + TGID+thread PID+fd lookup 到单个 `ConnectionOpened` `CaptureEvent` 的纯转换；agent 侧 procfs resolver 已支持读取 `/proc/<thread_pid>/fd/<fd>`，线程 fd 消失时回退到 `/proc/<tgid>/fd/<fd>`，并按 TGID 读取进程身份。write sample 只会在内核 allow 后产生，并且只合并到 provider 已跟踪的 `(thread pid, fd)` flow；未跟踪写入不会产生未知归因 payload；close observation 不做 post-close procfs fd lookup，只关闭 provider 已跟踪的同 thread `(pid, fd)` flow；TGID epoch 是保守失效机制，不是强 fd-table/socket identity，跨线程写采样覆盖率会降级为 fail-closed，后续应引入 socket cookie 或 fd-table identity。 | accept/socket-cookie/fd-table identity/exit/error/lost-event conversion，以及跨 fd dup/fork/fd passing 的强生命周期归因。 |
| build gate | `xtask check-ebpf` 执行 eBPF fmt、BPF target clippy、locked release build 和 object contract preflight；`xtask ebpf-build` 执行 locked build 与 preflight。 | CI 环境矩阵、真实特权 process observation e2e、payload privileged e2e 和 TLS privileged dynamic lifecycle e2e。 |
