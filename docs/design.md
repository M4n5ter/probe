...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_mitm`，并在 runtime plan/status 中报告独立 selector、`local_setup_scope` 和 `transparent_interception` capability；当前仅 `inbound_tproxy` 有 executable Linux lifecycle，`outbound_mitm` runtime 会 fail closed，不安装 output redirect 规则。
...
| 拦截抽象 | PolicyRuntime + external enforcement manifest + TargetScope + ScopedEnforcementPlanner + EnforcementBackend boundary + 显式 linux socket destroy backend + connection_enforcement capability + transparent_interception `local_setup_scope` 分类 + selector-projected rule lifecycle | 第 18 节 |
| 当前拦截验收 | dry-run typed verdict、remote external manifest GET、manifest selector 与主配置 selector 合成、process/traffic scoped dry-run decision、manifest profile unsupported audit、显式 backend 下的 owner-verified TCP socket destroy audit decision，以及透明拦截 nftables/TPROXY plan/probe/lifecycle 和隔离 netns 入站截获 E2E | 第 18、31 节 |
