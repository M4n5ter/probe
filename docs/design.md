...
dynamic libssl load、同 PID replacement-style 第二 mapped path 和可卸载 libssl-like 旧映射消失已有 E2E；per-target health、
...
- 该入口不把系统 OpenSSL `NODELETE` 行为视为失败，也不直接观察 `SSSA_TLS_STATE_EPOCHS` 写 `0` 或新 epoch。
...
- xtask 同时把真实系统 libssl 复制成文件名不含 `libssl` 的 TLS engine，并通过 `SSSA_E2E_REAL_TLS_ENGINE_PATH` 传给 fixture。
