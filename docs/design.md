...
当前 `transparent_interception` capability 的含义是“入站 TPROXY 规则/策略路由生命周期入口可执行”。它只由 Linux/root/nft/ip probe 决定，不把 selector projectability 编码为 capability，也不表示已具备 MITM、process classifier 或 flow-aware classifier。
...
- transparent interception plan 会保留 proxy mode/listen port、resolved nft table、mark、route table、selector 是否配置、`local_setup_projection` 和 capability。`local_setup_projection` 是 typed setup plan：`host_rules` 携带 projected host rule scope，`requires_process_classifier` 携带 process expression 与显式 `host_rule_boundary`，`requires_flow_classifier` 携带 transparent-interception-owned classifier selector view 与同样显式的 `host_rule_boundary`。这两种 classifier requirement 都不是当前 executable backend，也不会把 capability 改写成 unavailable；执行层会以 setup error fail closed。
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_mitm`，并在 runtime plan/status 中报告 proxy mode/listen port、独立 selector、`local_setup_projection`、`transparent_interception` capability 和 agent-owned proxy runtime snapshot；当前 `inbound_tproxy` 有 executable Linux lifecycle，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，配置化 active health probe 会主动检查显式 TCP endpoint 并在连续失败达到阈值后降级 health，`outbound_mitm` runtime 会 fail closed，不安装 output redirect 规则。
...
| 拦截抽象 | PolicyRuntime + external enforcement manifest + TargetScope + ScopedEnforcementPlanner + agent-owned RuntimeEnforcementPlanner + EnforcementBackend boundary + 显式 linux socket destroy backend + connection_enforcement capability + transparent_interception `local_setup_projection` 分类 + selector-projected rule lifecycle + 可选 agent-managed TCP relay lifecycle | 第 18 节 |
