...
- 已实现 `check` 的 TLS plaintext material 内容核验：对显式引用的 `key_log_file` 读取有界 regular file，并按 NSS/SSLKEYLOGFILE 三字段语法解析 label、context 和 secret hex；解析结果和 check report 只输出 entries 与 label counts，不保留 secret bytes，也不在错误中泄漏 secret 字段值。`session_secret_file` 当前只做同一 bounded source read，并以 reserved bytes 摘要出现在 check report 中，等待后续 session secret schema 定义。
