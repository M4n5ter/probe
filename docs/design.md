...
| TLS 路线 | 非 MITM 优先，当前已有 explicit key log/session secret material typed store、secret-free check summary、TLS 1.3 session-secret protected-record decrypt core、`PlaintextEvent` bridge、capture-layer ciphertext stream adapter、显式 flow-binding decrypt core 与 libssl uprobe demo；material store 可按 client random 等非 secret key 查找 secret，session secret core 可在有 cipher suite metadata 和 sequence 的前提下认证解密 TLS 1.3 application data record，并以 `tls_session_secret` provenance 进入 plaintext 抽象；stream/flow core 已处理 record framing、ciphertext/plaintext offset、sequence、gap/poison 和 flow close cleanup，但仍不声称已具备 keylog/session decrypt live provider、material lookup wiring 或 agent 接入 | 第 11、31 节 |
