...
| `transparent_flow_classifier` | Unavailable until a classifier backend is wired | RuntimePlan/check/status 已显式报告 flow classifier capability；`not`、named ref，以及无法表示为单个 setup-time host-rule scope 的 `any` setup requirement 会落到该 capability。 | 还没有 flow-aware classifier 或 registry-backed setup resolution；`any` 只有在所有分支都不依赖 process/flow classifier、每个分支都有可投影 host scope，且分歧不超过一个 host-rule 维度时才会直接投成 host rules；其它 flow-aware transparent interception 仍 fail closed。 |
...
| --- | --- | --- |
| `host_rules` | projected host rule scope。 | 可按当前 strategy/direction 进入 executable host-rule lifecycle：inbound TPROXY 仍要求最终 effective scope 具备显式 local port 集合，outbound transparent proxy 仍要求显式 remote port 集合。 |
| `requires_process_classifier` | process expression 和 `host_rule_boundary`。`host_rule_boundary` 明确区分 `no_host_rule_boundary` 与 `scope`，不使用 nullable host scope。 | 对本地 projection 只做静态 typed 预分类；执行层只对最终 effective setup scope 运行 setup-time procfs listener proof。当 final `host_rule_boundary = scope` 且 scope 包含显式 local ports 时，classifier 可消费该 typed 输入；否则 fail closed。未来 cgroup/owner mark 或 proxy-side dynamic classification 仍应复用这个输入，不能丢弃 process selector。 |
| `requires_flow_classifier` | transparent-interception-owned classifier selector view 和 `host_rule_boundary`。 | 当前 fail closed；`not`、named ref 和不能无扩大地表示为单个 host-rule scope 的 `any` 需要 flow-aware classifier 或 registry-backed resolution。 |
...
执行层在加载外部 manifest 后，会把主 `enforcement.selector`、manifest selector 和 `enforcement.interception.selector` 的交集作为最终 effective setup selector 重新投影。Outbound UID/GID-only process-scoped setup 不走 classifier，而是作为 nft socket-owner host rule 安装；其它 process-scoped setup 只有在 process classifier capability 可用、存在 host-rule boundary、local ports 显式、当前每个目标 TCP listener inode 的所有 holder 进程都可经完整 procfs owner scan 归因且明确命中 process selector 时才会安装 host rules。其它 process-scoped 情况和所有 flow-aware setup plan 都以 setup error fail closed，避免把“按应用拦截”的配置静默扩大成全机拦截。当前 executable inbound TPROXY 对 host-rule scope 仍要求具备显式 local port 集合；wildcard local port scope 会 fail closed，因为它会把 proxy listen port 也纳入拦截范围，而当前还没有完整 proxy self-bypass lifecycle 或规则排除契约。远程 manifest 可以进一步收窄本地 scope，但不能成为安装 host rules 的唯一 scope。
...
- transparent interception activation 更晚：在 ingress recovery 之后、打开 live capture provider 之前，用最终 effective selector 生成 executable nft 脚本。executable inbound TPROXY 要求该 final scope 具备显式 local port 集合，wildcard local port scope 会 fail closed，直到完整 proxy self-bypass lifecycle 或规则排除契约存在；通过 final scope 校验后执行 `nft --check`，随后获取 `sssa_probe` table owner lock 并清理当前保留 host resources 的所有 policy routing 遗留；若配置 `managed_tcp_relay`，会在 lock 持有期间按同一 inbound TPROXY plan 启动透明 listener，通过后再安装规则。executable outbound transparent proxy 要求 final scope 是可投影 host-rule outbound scope 且具备显式 remote port 集合；通过 final scope 校验后执行 `nft --check`、获取同一个 `sssa_probe` table owner lock、清理旧 owned table 和保留 policy routing 遗留，最后安装 OUTPUT/dstnat rules。出站 `managed_tcp_relay` 模式会在规则安装前准备 marked managed relay plan、本机地址自循环保护并启动 marked managed relay；出站 `external` 模式只在 `uses_reserved_mark` self-bypass contract 已声明时进入 activation，不启动 managed relay。provider 构造或 pipeline 失败后仍会先清理 host rules，再停止本次启动的 managed relay，最后释放 owner lock，guard 也有 best-effort Drop fallback；Ctrl-C/SIGTERM 会先设置 pipeline shutdown flag，让主 run 路径停止轮询并进入 cleanup。
...
20. `e2e-transparent-linux-outbound-redirect-artifact-netns` 在 root/net-admin 环境下从 explicit `transparent-linux` artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，再执行真实 `nft --check`、安装、规则列表断言和 cleanup 验证。它属于 `linux-artifacts` profile，不启动 agent，不证明 agent runtime outbound proxy self-bypass、original destination recovery 或证书/MITM lifecycle。
