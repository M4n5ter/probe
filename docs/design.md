...
- Runtime status fact：
  ...
  不能解释为内核 link firing heartbeat、持续探活后的当前健康或强 socket lifetime。
- Failure behavior：
...
- kernel link liveness and firing heartbeat：
  ...
  `reconcile_health` 说明 reconcile loop 最近一次尝试是否失败。
  这些状态字段不证明内核仍在触发该 uprobe，也不证明任意 flow 的 socket lifetime 强绑定到 attach target。
...
已在 status/health 中表达；kernel link liveness/firing heartbeat、TLS ringbuf saturation E2E 和 strong ownership 仍是后续边界。
...
| 优先级 | 状态 | 目标 | 主要交付 | 验收信号 |
| --- | --- | --- | --- | --- |
| P0 | Active | 收敛主要缺口，避免 best-effort 被误读成强保证。 | 强 socket lifetime、precise flow-specific lost-event reconstruction、TLS ringbuf saturation E2E、TLS kernel link liveness/firing heartbeat。 | capability、metrics、export 和 E2E 状态一致。 |
