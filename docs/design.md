...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    透明 HTTPS E2E 覆盖 product proxy 的入站和出站 routed allow/deny path。入站路径中，
...
- L7 MITM lifecycle audit 由 durable export 表达；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  和透明入站/出站 HTTPS product proxy routed allow/deny path 由对应测试覆盖。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  ...
  显式 host-to-upstream route、policy hook 和 durable decision readback；
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  product proxy 透明入站/出站 HTTPS routed allow/deny data-plane verdict/audit 由 privileged netns E2E 覆盖；
...
下游/上游 TLS relay、HTTP/1.1 ALPN gate 以及 product proxy 透明入站/出站 HTTPS routed allow/deny path
...
可执行路径。出站 case 证明 OUTPUT redirect 规则路径可以接管本机发起的 HTTPS 连接，且 product proxy
upstream route 目标也命中同一 selector，upstream socket 必须使用保留 mark 才能绕过自身透明捕获。
该 privileged E2E 使用无 ALPN legacy client，
证明透明规则路径不要求客户端发送 ALPN。
