...
当前实现不是运行中动态 reconcile。当前 `agent run` 只做启动时一次性 procfs scan；startup attach 被建模为从空 attach state 到当前 attach plan 的第一次 reconcile，然后把 libssl plaintext provider 作为 best-effort sidecar 接入 `CaptureMultiplexer`。后续重复扫描、对新增 target 执行 attach、对 stale target 执行 detach/unattach 和 process-exit 清理仍属于缺口。
...
4. attach reconcile model 可区分 new、retained 和 stale targets，避免 retained target 在后续动态阶段被重复 attach。
