...
| `transparent_interception` | Available only for explicitly configured and probed inbound TPROXY | `enforcement.interception.strategy` 已提供 `none`、`inbound_tproxy`、`outbound_mitm` 三种 typed 配置，支持独立 selector 和显式 proxy plan；proxy plan 的 `health_probe` 是 `disabled`/`enabled` typed intent，enabled 已把 target 解析成 IP socket address，并携带 bounded timing/threshold。`proxy.mode = "external"` 是默认模式，`proxy.mode = "managed_tcp_relay"` 会让 agent 为入站 TPROXY 启动 plain TCP relay。nftables table、TPROXY mark 和 policy route table 是内部保留 host resources，不暴露为用户配置。`RuntimePlan.enforcement.execution_surfaces`、`RuntimePlan.enforcement.interception`、capability matrix、`check` 和 enforcement status 都会报告 strategy、proxy mode/listen port、resolved nftables resources、selector、`local_setup_scope` 与 capability requirement；`local_setup_scope` 只对 RuntimePlan 可见的本地配置 selector 做预分类，区分 `not_configured`、`host_rules`、`requires_process_classifier`、`requires_flow_classifier` 和 `unsupported`，避免把未来按应用拦截需求误报成当前可安装 host rules。agent composition root 已接入 Linux inbound TPROXY lifecycle probe/runtime：固定可信路径查找 `nft` 和 `ip`，要求 Linux/root；`run` 在加载外部 manifest 并得到最终 effective setup selector 后，对最终 nft 脚本执行 `nft --check`，再获取单一 `inbound_tproxy` host owner lock，清理当前保留 table/mark/route table 的所有 IPv4/IPv6 policy routing 遗留；若使用 managed relay，会在 lock 持有期间按同一 inbound TPROXY plan 的 rule family 启动透明 TCP listener，然后按 nft rule family 安装必要的 policy routing，退出时删除 owned table、清理本次 route/rule，再停止 managed relay 并释放 lock。managed relay 对 TPROXY accepted socket 使用 `local_addr()` 作为原始上游目标；`SO_ORIGINAL_DST` 不是依赖，因为 nftables/TPROXY 不是 NAT redirect。agent-owned proxy runtime state 会在在线 admin status 中报告 mode、listener family、配置化 active proxy health probe 状态、upstream connect counters 和最近一次失败原因，并在 admin metrics 和 Prometheus 中报告 active probe mode/check counters、active relay、accepted/rejected relay、relay failure、listener failure 与 upstream connect success/failure counters。`xtask e2e-transparent-tproxy-loopback` 已在隔离 network namespace 中建立 pid-backed client netns/veth，启动 agent 安装 TPROXY 与 managed relay，验证 client 命中两个原始服务端口后分别由 agent relay 转发到对应 upstream，并断言 upstream peer 是 host-side relay 地址以避免直连假阳性。 | 当前 capability 只表示入站 TPROXY 规则/策略路由 lifecycle 入口可执行，不表示当前 selector 可投影、不表示已完成 MITM、proxy-side L7 分类，或提前校验了带外部 manifest selector 的最终脚本；`external` proxy mode 仍要求用户提供本地代理。managed relay 是有界 thread-per-connection plain TCP passthrough，不解析 L7、不注入证书、不执行 MITM CA lifecycle；它只证明 agent 可以拥有入站 TPROXY 代理进程生命周期、基础运行态计数，以及由真实 relay connect 尝试驱动的 per-flow upstream connect telemetry；这些 connect counters 不是代理全局健康信号，不会改变 agent health；配置化 active proxy health probe 当前只属于可执行的 inbound TPROXY proxy lifecycle，通过显式 `health_probe.target`、`interval_ms`、`timeout_ms` 和 `failure_threshold` 主动连接用户声明的 TCP endpoint，config validation 和 runtime typed plan 构建会在执行层前解析 target、校验 timing/threshold/static self-target invariant，并拒绝非法 health probe intent，agent 预探测路径不会因非法 target panic 或误报 healthy；`interval_ms`、`timeout_ms` 和阈值都有受控范围，`timeout_ms` 不能超过 `interval_ms`，managed relay 模式会先在配置/typed plan 层拒绝 loopback/unspecified 同端口 target，再通过可信 `ip -j address show` 在 activation 和每次 same-port active probe 前读取本机接口地址，只在目标地址族会命中实际 managed listener 时拒绝把本机非 loopback 地址作为自身 listen port 的 probe target，达到连续失败阈值后才把 agent health 降为 degraded。当前 local setup scope 只预分类无进程条件的显式本地 selector：`match` 或 `all(match, ...)` 交集，并且必须至少包含端口或 remote IP literal 约束；但当前 executable inbound TPROXY lifecycle 进一步要求 final effective scope 具备显式 local port 集合，remote-only 或 remote-port-only 的 wildcard local port scope 会 fail closed，直到 proxy self-bypass / 规则排除契约存在。`remote_addresses` 使用 IP 地址值匹配和规范化，不表达 hostname 或 CIDR。进程名/exe/cmdline/systemd/container 会被 `local_setup_scope` 标记为需要 process classifier，`any`、`not` 和 named ref 会标记为需要 flow classifier。执行层会在加载外部 manifest 后用最终 effective selector 再做一次 rule projection 校验，classifier 未实现前仍 fail closed，避免误变成全机拦截。nft table、mark 和 route table 固定为当前保留默认值（`sssa_probe`、`0x53534101`、`53534`），避免误删常见 nft table 或改写外部 policy routing；当前单一 `inbound_tproxy` owner lock 覆盖这个可执行 lifecycle 操作的共享 host resources，避免同一 host namespace 内两个 agent 互相清理。`outbound_mitm` 当前只有 typed config/status 表达，runtime 和 executable lifecycle 均 fail closed；在 proxy self-bypass、证书/MITM lifecycle、output redirect 和 L7 分类明确整合前不会作为 executable backend，也不会由 agent run path 安装 output redirect 规则。 |
...
当前 `transparent_interception` capability 的含义是“入站 TPROXY 规则/策略路由生命周期入口可执行”，只由 Linux/root/nft/ip probe 决定，不把 selector projectability 编码为 capability，也不表示已具备 MITM。`external` proxy mode 下显式配置的 proxy listen port 仍是外部前置条件；`managed_tcp_relay` mode 下 agent 会启动有界 plain TCP relay；在线 admin status 暴露 mode、listener family、配置化 active proxy health probe 状态、upstream connect counters 和最近一次失败原因，admin metrics 和 Prometheus 暴露 active probe mode/check counters、active relay、accepted/rejected relay、relay failure、listener failure 以及 upstream connect success/failure counters。这些 upstream connect counters 只来自真实 relay connect 尝试，是每条流连接原始目标的 telemetry，不是代理全局健康信号；connect failure 不会增加 relay failure counter，也不会改变 agent health。配置化 active proxy health probe 当前只属于可执行的 inbound TPROXY proxy lifecycle，只连接用户显式声明的 TCP endpoint，`interval_ms`、`timeout_ms` 和阈值都有受控范围，`timeout_ms` 不能超过 `interval_ms`，managed relay 模式会先在配置/typed plan 层拒绝 loopback/unspecified 同端口 target，再通过可信 `ip -j address show` 在 activation 和每次 same-port active probe 前读取本机接口地址，只在目标地址族会命中实际 managed listener 时拒绝把本机非 loopback 地址作为自身 listen port 的 probe target，达到连续失败阈值后才把 agent health 降为 degraded；它不从 per-flow relay connect 结果推断健康。不加载 MITM CA、不注入证书、不把 MITM 作为默认 TLS plaintext 路线。managed relay 解析上游目标时使用 TPROXY accepted socket 的 `local_addr()`，并拒绝转发到自身 listen port，避免 direct proxy connect 或错误规则造成自连递归；`SO_ORIGINAL_DST` 不能作为必需信号，因为 TPROXY 路径不一定设置 NAT original destination。relay stop 会主动 shutdown 已登记的 downstream/upstream streams，避免长连接把 agent cleanup 生命周期无限拖住；relay 会在 downstream write half-close 后向 upstream 传播 write half-close，避免 EOF-delimited TCP 协议卡住。当前 active relay 数量固定有界，后续若要承载高并发代理面，应替换为 async/bounded worker 模型和可观测 backpressure，而不是继续扩大 thread-per-connection 模式。RuntimePlan 的 `local_setup_scope` 只根据本地配置可见的主 `enforcement.selector` 与 `enforcement.interception.selector` 做预分类；它不加载本地/远程 manifest，也不等于最终 executable scope。该字段会把本地 selector 分成 `host_rules`、`requires_process_classifier`、`requires_flow_classifier` 或 `unsupported`：可投影形态是 `match` 或 `all(match, ...)`，并且必须至少包含 local port、remote port 或 remote IP literal 约束；`remote_addresses` 使用 IP 地址值匹配和规范化，不表达 hostname 或 CIDR；进程名、exe glob、cmdline regex、systemd service、container id 不会被丢弃，而是显式标记为需要 cgroup/owner mark 或 proxy-side process classification；`any`、`not` 和 named ref 显式标记为需要 flow-aware classifier。执行层在加载外部 manifest 后，会把主 `enforcement.selector`、manifest selector 和 `enforcement.interception.selector` 的交集作为最终 effective setup selector 重新投影；当前 executable inbound TPROXY 要求该最终 scope 具备显式 local port 集合，wildcard local port scope 会 fail closed，因为它会把 proxy listen port 也纳入拦截范围，而当前还没有 proxy self-bypass 或规则排除契约。classifier 未实现前仍 fail closed，避免把“按应用拦截”的配置静默扩大成全机拦截。远程 manifest 可以进一步收窄本地 scope，但不能成为安装 host rules 的唯一 scope。后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 unsupported process selector 丢掉；后续要实现出站 MITM，应把 proxy self-bypass 契约、证书材料、MITM CA 注入、output redirect lifecycle 和 L7 分类作为同一个 executable backend 的门控条件。
...
| 透明拦截 strategy 启用但 `transparent_interception` capability 不可用 | 无运行态。 | `RuntimePlan` validation fail closed；典型原因是缺少 root、可信 `nft`/`ip` 等平台入口。selector projectability 由 `local_setup_scope` 和 configured enforcement 的最终 setup scope 校验表达；最终 effective selector 生成的 nft 脚本会在 activation 前执行 `nft --check`，失败时不会安装规则。 |
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_mitm`，并在 runtime plan/status 中报告 proxy mode/listen port、独立 selector、`local_setup_scope`、`transparent_interception` capability 和 agent-owned proxy runtime snapshot；当前 `inbound_tproxy` 有 executable Linux lifecycle，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，配置化 active health probe 会主动检查显式 TCP endpoint 并在连续失败达到阈值后降级 health，`outbound_mitm` runtime 会 fail closed，不安装 output redirect 规则。
...
| 拦截抽象 | PolicyRuntime + external enforcement manifest + TargetScope + ScopedEnforcementPlanner + agent-owned RuntimeEnforcementPlanner + EnforcementBackend boundary + 显式 linux socket destroy backend + connection_enforcement capability + transparent_interception `local_setup_scope` 分类 + selector-projected rule lifecycle + 可选 agent-managed TCP relay lifecycle | 第 18 节 |
