...
当前也已实现外部 enforcement policy manifest 的第一版装配边界。`enforcement.policy.source` 可以指向单个 TOML manifest 文件，或指向包含 `manifest.toml` 的目录；`RuntimePlan.enforcement.policy_source` 会把目录 source 解析为实际 `manifest.toml` 路径，后续 `check`、`run` 和 offline `status` 都消费 plan，而不是重新解释 raw config。`remote` source 只作为控制面预留，当前 `check`/`run` 会 fail closed。manifest 当前字段为：
...
主配置中的 `enforcement.selector` 与 manifest 内的 `selector` 使用 AND 语义合成；因此可以把全局 agent 防护边界和外部下发的应用/服务边界分开管理。manifest 不执行代码，只是声明保护 profile 和作用域；Lua policy 仍负责产生 typed verdict。`check` 和 `run` 会显式读取并校验 manifest；offline `status` 只做 metadata-only 检查并把健康状态标为 degraded，避免把 offline status 变成隐式执行或启用防护的入口；在线 admin status 使用 `run` 启动时已加载的 manifest snapshot，报告 `loaded`，不会重新读取磁盘上可能已变化的 source。
...
- `RuntimePlan.enforcement` 保留 `mode`、主配置 selector 是否配置、以及 `policy_source` 的 typed plan；目录 source 会在 plan 中解析为 `manifest.toml` 路径。runtime plan 阶段只做配置形状和 capability validation，不打开 manifest 文件；`check`/`run` 在 composition root 加载 external enforcement manifest，offline `status` 只做 metadata-only source inspection，online admin status 报告运行中已加载的 manifest。
...
- enforcement status 使用 typed enum 报告 configured mode、effective status、主配置 selector、manifest selector、effective selector、external policy source metadata 和 capability requirement。`disabled`/`audit_only` 标记 capability `not_required`；`dry_run` 标记需要 `DryRunEnforcement` capability。外部 enforcement manifest source 可用时 offline status 解析 TOML manifest metadata，报告 id/version、manifest selector 是否配置和 protective actions，但不执行动作；source 缺失、remote source 或 manifest 非法时 health 为 unavailable，source 可用但只是 metadata-only 时 health degraded。在线 admin status 报告启动时已加载的 manifest 为 `loaded`，不因磁盘 source 后续变化而误报当前运行保护范围。`enforce` 和缺少 dry-run capability 的配置在 `RuntimePlan` 构建阶段 fail closed，因此 CLI status 对这类配置直接返回 validation error，而不是输出一个伪可用 snapshot。
...
3. `check`/`run` 显式加载 external enforcement manifest，合成主配置 selector 与 manifest selector。
