...
- 强 socket lifetime。
- 内置 TLS MITM 明文数据面、proxy 内部保护动作执行和自动修改客户端 trust store。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、`client_trust.mode = "operator_managed"`、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；managed backend-owned hook/action-report fixture 覆盖代理侧执行声明的验收形态。真实 TLS MITM 数据面、生产 proxy 内部动作执行和自动 client trust store 安装仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 可选 policy hook 使用本机 HTTP JSON endpoint，把每流量保护动作委托给 MITM proxy。
    ...
    managed-process MITM fixture 还会把 policy hook endpoint 放在同一个 backend 进程内；
    只有该 backend 已生成对应 plaintext bridge flow 时，fixture 才会写入 action report 并返回 delegated deny。
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    managed backend-owned fixture 证明 action execution contract 可由同一个 backend 进程持有；
    仍需独立实现内置 TLS MITM 数据面、生产 proxy 内部动作执行和自动 client trust store 安装。
  ...
  - 验收：managed-process MITM policy hook E2E 覆盖 backend-owned hook endpoint、同 backend 生成的 plaintext bridge flow、
    action report 和 typed `EnforcementDecision.execution` readback。
...
- L7 MITM lifecycle audit 已进入 durable export；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  RuntimePlan/agent wiring、本机 HTTP JSON hook transport 和 managed backend-owned action report 验收已接入。
- 自动 client trust store 安装、TLS MITM 数据面和生产 proxy 内部保护动作执行仍缺失。
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  proxy-side hook transport、`delegated` planner outcome、typed execution evidence readback，以及 managed backend-owned action report
  已由真实 agent/netns/spool E2E 覆盖。
  真实 TLS MITM data-plane verdict/audit、生产 proxy 内部保护动作执行和 managed-process health-transition audit 的 privileged/E2E
  覆盖仍是缺口。
...
RuntimePlan/agent proxy hook wiring、本机 HTTP JSON hook transport、typed delegated decision readback 和
managed backend-owned action report 验收已进入控制面能力；
真实 TLS MITM 数据面、生产 proxy 内部保护动作执行和自动 client trust store 安装不属于当前控制面能力。
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、operator-managed client trust contract、plaintext bridge、proxy-side hook wiring、typed execution evidence 和 managed backend-owned action report。 | 内置 TLS MITM data-plane、生产 proxy 内部动作执行、自动 client trust store 安装。 |

MITM lifecycle coverage 包含入站/出站 strategy plan/status、出站 selector redirect canary、
external backend、managed-process backend、capture-event plaintext bridge、HTTP JSON policy hook 和 backend-owned action report。
...
出站 selector redirect canary、MITM plaintext bridge live fan-in、本机 HTTP JSON policy hook typed delegated decision，
以及 managed backend-owned action report。
...
它不是 proxy 内部实现的密码学证明，也不替代真实 TLS MITM 数据面验证。
自动 client trust store 安装、真实 TLS MITM 数据面和 proxy 内部保护动作执行不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | TLS MITM 数据面、自动 client trust store 安装、proxy 内部动作执行、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM lifecycle、operator-managed client trust contract 与 plaintext bridge 可配置和报告；内置 TLS MITM 数据面仍缺失 | 第 11、18、32 节 |
