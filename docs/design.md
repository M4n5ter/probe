...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、typed material refs、TCP readiness probe、durable lifecycle audit 和明确的 `l7_mitm_plaintext` data-plane provenance；真实 TLS MITM 数据面仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；仍需独立实现 client trust/install instructions、
    内置 TLS MITM 数据面和同步 proxy-side policy hook transport。
...
同步 proxy-side policy hook transport 尚未实现。该入口实现时应消费现有 `EnforcementDecision.effective_action`，
由 MITM/proxy 边界把 `observe`/`allow`/`alert` 解释为继续处理，把 `deny`、`reset`、`quarantine`
解释为代理可执行的保护动作；没有 decision 或 control transport 失败必须显式建模，不应被静默等同为允许转发。
该入口不引入新的策略语言。
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle 和 plaintext bridge。 | 内置 TLS MITM data-plane、出站 MITM audit。 |

MITM lifecycle coverage 包含 external backend、managed-process backend 和 capture-event plaintext bridge。
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM lifecycle 与 plaintext bridge 可配置和报告；内置 TLS MITM 数据面仍缺失 | 第 11、18、32 节 |
