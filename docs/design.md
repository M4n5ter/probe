...
- 验证面：入站 external/managed MITM backend 已有真实 agent E2E，证明 backend lifecycle、managed fixture
  ...
  lifecycle audit 与 health-transition audit 由 core/agent 单元测试覆盖；出站 MITM、proxy-side policy hook、真实 TLS MITM
  data-plane verdict/audit 和 health-transition audit 的 privileged/E2E 覆盖仍是验收缺口。
