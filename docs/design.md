...
| Operator surface | admin/status/prometheus、ratatui TUI | TUI 是服务器本地配置工作台，也可通过在线 admin socket 浏览选中进程的解析事件；policy source 编辑和大型 MITM backend contract 编辑不由 TUI 代替。 | 本节、Admin 章节 |
