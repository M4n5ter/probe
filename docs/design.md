...
- enforcement demo：Lua 策略返回 `deny`、`reset`、`quarantine` 等 typed verdict；默认 `audit_only`/`dry_run` 只记录 requested action、effective action、planner outcome 和 audit event，显式 `enforcement.backend = "linux_socket_destroy"` 可在 root 下通过固定系统路径 `ss -K` 销毁 selector 命中的现有 TCP socket。
...
| keylog/session decrypt hints | Planned config only | TLS plaintext config/runtime/status plan 可表达 provider、selector、capability requirement、keylog/session secret material refs。 | 尚未实现 keylog/session 解密 provider；证书/私钥不会被误称为通用解密材料。 |
...
| `connection_enforcement` | Available only when explicitly configured and probed | `EnforcementBackend` trait、planner delegation boundary、`RuntimePlan.enforcement.capability`、显式 `enforcement.backend = "linux_socket_destroy"`、固定系统路径 `ss -K` TCP socket destroy backend 和 status/backend reporting 已存在。 | 默认 `backend = "none"` 仍 unavailable；Linux socket destroy probe 只确认 Linux、受信系统路径中的 `ss -K` 入口和 root 执行上下文，实际每条 flow 仍可能因事件不是 live host observation、内核/namespace/匹配窗口返回 `unsupported`；它只支持销毁已存在的 TCP IPv4/IPv6 socket，不提供 pre-connect deny、payload 级阻断、非 TCP 阻断或强规则生命周期。 |
...
当前实现状态：已实现 libssl uprobe attach candidate discovery 和 attach plan，但尚未接入 runtime provider/loader。`capture::LibsslUprobeTargetDiscovery` 可按 PID 读取 `/proc/<pid>/maps`，跳过非 executable 和非文件映射，识别 libssl/boringssl shared object，保留 mapped path、`/proc/<pid>/root` read path、typed mapped file identity（device major/minor + inode）和 deleted 状态，并通过单次打开的 fd 校验当前 read path 仍匹配 maps 中的 mapped file identity，然后用 `object` crate 解析 ELF symbol table，确认当前支持的 `SSL_read`、`SSL_write`、`SSL_read_ex`、`SSL_write_ex` 已定义 text symbol 集合。`capture::LibsslUprobeAttachPlan` 把 discovery report 转成后续 Aya loader 可消费的 attach recipe：每个 supported SSL symbol 都规划 entry + return 两个 probe，recipe 层记录 inbound/outbound plaintext direction，attach offset 当前保持 `0` 并使用 Aya 的 symbol resolution。entry probe 用于捕获 buffer/length 参数，return probe 用于判断实际读写字节数和失败结果；当前只生成 attach intent，不读取明文。deleted mapping、不可读 library、超过当前大小上限的 library、mapped file identity 不匹配或非 ELF library 都进入 typed discovery degraded reasons，而不是把它们伪装成可 attach target。TLS plaintext config/runtime/status plan 已能表达 provider、selector、capability requirement 和 keylog/session secret refs，但尚未实现 Aya uprobe loader、动态 attach lifecycle、SSL fd/socket 关联、ring buffer 和 plaintext event provider，因此 capability matrix 中 `libssl_uprobe` 必须保持 unavailable；`external_plaintext_feed` available 只表示 agent 已能消费外部已解密明文 feed。
...
5. 默认 agent 不真实阻断，`audit_only`/`dry_run` 的 `effective_action` 仍为 `observe`；显式 `enforcement.backend = "linux_socket_destroy"` 且 capability 可用时，`enforce` 委托 backend 销毁命中的现有 TCP socket，否则 fail closed。
...
| 默认启用真实连接级阻断 | 推迟 | 默认 destructive 行为风险过高；当前只允许显式 `linux_socket_destroy` backend 销毁已存在 TCP socket，更强内核级 pre-connect/payload 阻断继续后置。 |
...
| 当前拦截验收 | dry-run typed verdict、external manifest protection profile、selector 合成，以及显式 backend 下的 TCP socket destroy audit decision | 第 18、31 节 |
