...
- Fjall ingress/export lanes、webhook/file/Unix HTTP exporter、status/admin/metrics 已可持续运行和观测。
...
- 主配置 reload 已覆盖 policy-only 在线应用、在稳定 transparent interception setup scope 下的 enforcement policy config
  在线应用和 runtime generation 请求；capture/observations 变更会在 capture safe point validate-then-swap 新 capture provider，并更新
  agent-wide active `RuntimePlan` handle。
...
- `capture.deep_observe_selector` 可按方向授权 outbound `write(2)` / `sendto(2)` / `writev(2)` / `sendmsg(2)` 的
  single-buffer 或 bounded multi-iovec prefix syscall argument sample。
...
- 超出 bounded multi-iovec scan/sample 的 scatter/gather continuation。
...
- 不实现动态远程控制面或长连接下发；主配置热重载限定为已有明确 lifecycle owner 的资源。
  policy-only 主配置变更、export 主配置变更、以及在稳定 transparent interception setup scope 下的
  enforcement policy config 变更可在线应用；
...
- `PROBE_HOME` 是本地 state root。默认值遵循用户 state 目录：优先使用
  `$XDG_STATE_HOME/traffic-probe`；未设置 `XDG_STATE_HOME` 时使用
  `$HOME/.local/state/traffic-probe`。
- 机器级 Linux 服务部署可以显式设置 `PROBE_HOME=/var/lib/traffic-probe`，或在 TOML 中写出绝对路径。
...
- 动作能力：dry-run verdict、enforcement policy source reload、本地 enforcement manifest watcher、connection-level enforcement
...
| Storage / export | durable spool、ingress/export queue、webhook/file/Unix HTTP exporter | spool 负责恢复与投递；active parser state 不落盘；export 是 per-sink at-least-once。 | 第 21-24 节 |
...
| Transparent interception | `transparent_interception`、classifier capability | 入站 TPROXY、出站透明代理、setup-time process classifier、finite-boundary proxy-side flow classifier 和 managed L7 MITM lifecycle 已接入；强 socket lifetime、no-boundary classifier 和 kernel/cgroup classifier 仍缺失。 | 第 18、31 节 |
| Operator surface | admin/status/prometheus、ratatui TUI | TUI 是服务器本地 agent 工作台：复用或托管启动本地 agent，浏览全量或 watched process 事件，展示 provider/open-failure 诊断和 MITM 路线提示；policy source 与大型 MITM backend contract 不由 TUI 代替。 | 本节、Admin 章节 |
...
- TUI 启动时向配置中的 admin socket 发送轻量 `ping`。若 socket 响应 `pong`，TUI 复用该运行中 agent，
  ...
  `agent.log` 路径和短 tail，避免只暴露 exit status。
- TUI model 将 editable config 和 active runtime socket 分开。Traffic 与 Runtime tab 使用 active
  admin socket；用户编辑 admin socket path 后，新路径不会被误认为已经生效。TUI-managed
  runtime overlay 只服务当前 TUI 会话，不把 admin enablement 或私有 socket path 持久化到用户配置文件。
...
- `tail_events` 是 non-mutating admin command。它读取 `after_sequence` 之后的 export records，按可选 selector
  ...
  `selector = null` 且 candidate selector 为空表示全量读取；candidate-only 请求不会匹配强归因事件。
...
- Runtime tab 保存配置后通过 admin Unix socket 调用 `apply_config_reload`。该动作先复用主配置 reload planning，
  ...
  未启用，且 transparent interception 未启用或由显式 `enforcement.interception.selector`
  固定 setup scope 时可在线生效。顶层 `[selectors]` registry 变更在变更 entry 未被 enabled policy
  selector、`enforcement.selector` 或启用中的 transparent interception setup selector 引用时进入
  runtime generation；若 action-gated selector consumer 引用了变更 entry，则仍需要重启，直到这些
  owner 能够在同一个事务中应用。
...
- 保存路径优先用 `toml_edit` 定点更新 TUI-managed 字段，保留用户配置中的非相关内容和注释。
  ...
  路径替换成普通文件。
  写入前必须获取同目录 advisory lock，确认磁盘内容未被外部修改，把渲染后的 TOML 重新解析为
  `AgentConfig`，执行 static runtime config validation 和 L7 MITM contract validation；校验失败不得写文件。
  写入使用同目录临时文件、原文件 mode/owner preservation、file sync、rename 和 directory sync。
...
- `product_proxy` backend 从 typed bridge、policy hook、target recovery、CA 或 leaf TLS material、
  upstream TLS mode、upstream trust material、非空 application protocol policy、显式 upstream route table
  和 upstream discovery policy 生成 first-party proxy CLI。
  缺省 protocol policy 是 HTTP/1.1；显式空列表视为配置错误。
- 内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、
  CA-backed dynamic SNI certificate generation、per-connection upstream TLS mode、downstream SNI-derived
  upstream TLS server name、由 application protocol policy 派生的 HTTP/1.1 ALPN advertise/gate、
...
- `ebpf`
  ...
  - 已实现：bounded inbound `read(2)` / `recvfrom(2)` / `readv(2)` / `recvmsg(2)` result sample 到 always-degraded
    `CapturedBytes`/`Gap` bridge。
  - 已实现：payload bridge 和 close bridge 只按 descriptor generation 匹配 tracked flow；
    exact descriptor 不可见时，不使用 process-hint+fd alias 把 socket-specific payload 或 close lifecycle 强接回已跟踪 flow。
    隐藏 PID / PID 撞号只能在 procfs resolver 阶段形成明确 flow-start descriptor lease。
  ...
  - 已实现：vector syscall 在固定 scan limit 内跳过 zero-length iovec，并把落在固定 verifier-friendly append slot
    边界上的非空 iovec segment 拼接为 bounded prefix；若连续 prefix 没有落在可追加 slot 边界，
    则在该 prefix 处截断，不跳过字节后继续拼接。
  ...
  - 边界：process exit/exec lifecycle 不生成 `ConnectionClosed`。它只为 userspace 已跟踪且已授权 payload 的同 TGID flow
    ...
    cancellation 使用 `EbpfUnresolvedFlow` evidence；process lifecycle 造成的 tracked flow boundary 使用
    `EbpfProcessLifecycleBoundary` evidence；userspace tracker displacement 使用 event-local terminal
    `ProviderStateBoundary` evidence，并释放 parser/evidence flow state。
  ...
  - 缺口：超出 bounded multi-iovec scan/sample 的 scatter/gather continuation、precise flow-specific lost-event reconstruction、partial-write retry 语义和完整
...
- `procfs_socket_attribution`
  ...
  - 已实现：通过 `/proc/net/tcp`、best-effort `/proc/net/tcp6`、socket inode、`/proc/<pid>/fd` 和 TGID+thread PID+fd
    lookup 做 TCP 连接归因。
  - 已实现：fd lookup 支持 thread pid、TGID fallback、`NStgid` PID namespace alias，以及 observed TGID 隐藏或与
    host PID 撞号场景下的 unique fd/process-hint candidate。
  - 已实现：flow-start 的 procfs socket lookup 失败但 BPF process hint 可唯一映射到 host-visible process 时，
    degraded observed flow 使用该 host-visible process context，以便进程过滤、TUI 和 export 看到同一进程身份。
...
- `l7_mitm`
  ...
  - 验收：`xtask e2e-mitm-policy-hook-plaintext-bridge-live-sidecar` 使用同一 fresh network namespace
    入站 MITM bridge harness，验证 enforcement policy manifest 的 protective action profile、Lua `deny`
...
- `export_queue`
  ...
  - 已实现：sink ACK 只推进对应 per-sink cursor；export queue 物理删除只由 storage retention 执行。
...
- `webhook_exporter`
  ...
  - 已实现：支持 cursor-safe ACK advance、trust anchors、client identity refs。
  ...
  - 验收：`xtask e2e-webhook-exporter` 覆盖 configured webhook sink 的 bounded-run tail drain、非默认 gzip codec、
    ...
    完整预期 export event set、JSON structured ack、custom headers、collector sink cursor 前进和 sink pending set 为空。
  - 边界：当前 E2E 不证明 periodic worker loop、rejection/retry/partial ACK 或 retention-driven record prune；这些由 agent export/storage
    单元与集成测试覆盖。
...
- `file_exporter`
  ...
  - 验收：`xtask e2e-file-exporter` 覆盖真实 agent bounded run tail drain，以 zstd-compressed protobuf batch 写入 JSON Lines
    file record，随后解码 record、验证 batch range/id/schema/event set、sink cursor 前进且该 sink pending set 为空。
...
- `unix_http_exporter`
  - Runtime status：Available。
  - 已实现：`unix_http` transport 作为 planned sink 从同一 export queue drain protobuf batch envelope。
  - 已实现：复用 webhook 的 protobuf batch、codec header、idempotency key 和 JSON structured ACK 契约。
  - 已实现：配置包含 absolute `socket_path`、HTTP path `endpoint`、custom headers 和 codec；TLS material 不适用于该 transport。
  - 已实现：agent status 通过 exporter preflight 验证 socket path 存在、不是 symlink，且是 Unix domain socket。
  - 验收：config/runtime/agent drain/status/exporter 测试覆盖 `unix_http` 解析、plan、socket target preflight、failure reason 分类和 status target。
  - 验收：`xtask e2e-unix-http-exporter` 覆盖真实 agent bounded run tail drain，经 Unix domain socket HTTP
    发送 deflate-compressed protobuf batch，校验 custom header、batch range/id/schema/event set、structured ACK、sink cursor 前进且该 sink pending set 为空。
  - 边界：Unix HTTP 适合本机 collector sidecar，不等同于跨主机远端 transport。
...
- `dry_run_enforcement`
  ...
  - 已实现：enforcement policy manifest 的 selector/profile 会进入 active planner。
...
- `connection_enforcement`
  ...
  - Metrics contract：online admin metrics 在 outcome counters 之外提供 execution surface counters。
    `metrics.pipeline.enforcement.execution.connection_backend` 统计 connection backend 执行面，
    其中 `linux_socket_destroy` 表达真实 socket destroy backend 被调用并产出 execution evidence；
    `metrics.pipeline.enforcement.execution.proxy_side_hook` 统计 proxy-side hook 执行面，
    其中 `l7_mitm` 表达 MITM policy hook delegated execution。
...
- `xtask e2e-transparent-linux-outbound-cgroup-artifact-netns` 在存在非 root cgroup v2 path 且 nft socket-cgroup resolver
  可用时，覆盖 cgroup-path-only selector 生成的 `socket cgroupv2 level N "path"` rule 能通过真实 `nft --check`、
  安装、规则列出断言和清理；resolver 不可用时该 case skip。
...
- `xtask e2e-transparent-outbound-flow-classifier-loopback` 覆盖有限 remote-port boundary 下的出站 proxy-side
  flow classifier、accepted/rejected relay metrics 和 rejected branch upstream isolation。
...
- `xtask e2e-transparent-linux-outbound-cgroup-artifact-netns` 覆盖真实 cgroupfs path 下的 socket-cgroup rule
  `nft --check`、安装、规则列出断言和清理。
...
- capability 不表示 process/flow classifier 是否满足当前 selector；这两类 classifier 由独立 capability 表达。
...
- cgroup-path-only outbound process selector 已可投影为 nft OUTPUT socket cgroupv2 host rules。
- nft socket cgroupv2 path 在 ruleset 加载时解析为内核 cgroup 对象；它不跟踪同名 cgroup 的删除和重建。
- cgroup-path host rule 是静态安装边界；动态应用生命周期跟踪需要 cgroup lifecycle watcher、mark classifier 或 proxy-side classifier。
- cgroup-path host rule 形态由 privileged linux-artifacts E2E 覆盖；在 nft socket-cgroup resolver 可用时，
  它验证真实 nft path 解析、安装、列出和清理，不证明同名 cgroup 删除和重建后的规则刷新。
- 不能下推为 UID/GID 或 cgroup path 的 outbound process selector 仍 fail closed，除非满足 `transparent_flow_classifier` executable contract。
...
- `transparent_flow_classifier`
  - Runtime status：完整 procfs TCP table 与 fd owner scan 可用时 degraded，否则 unavailable。
  - 已实现：RuntimePlan/check/status 已显式报告 flow classifier capability。
  - 已实现：agent 已接入 finite-boundary proxy-side procfs TCP owner classifier。
  ...
  - 边界：没有有限 `host_rule_boundary` 的 flow-aware selector 仍 fail closed，不安装全机宽规则。
  - 边界：classifier 依赖 procfs 当前 owner 归因，不提供强 socket lifetime；procfs race、namespace、
    PID reuse、fd scan 不完整或 TCP table 不可读都会导致 unavailable 或代理侧 fail closed。
  - 边界：缺失或循环 ref 会在配置/manifest validation 阶段 fail closed；
  ...
  - 验收：`e2e-transparent-tproxy-flow-classifier-loopback` 覆盖有限 host boundary 下的入站 proxy-side
    flow classifier。
  - 验收：`e2e-transparent-outbound-flow-classifier-loopback` 覆盖有限 remote-port boundary 下的出站
    proxy-side flow classifier：匹配 `xtask` 进程名的 branch 被 managed relay 接受并转发；同一
    setup-time superset 中不匹配 selector 的 branch 被 proxy 拒绝、不连接 upstream，并在 admin metrics
    中表现为预期的 rejected relay 计数。
  - 缺口：no-host-boundary flow classifier、external proxy flow classifier、强 socket lifetime、
    kernel/cgroup-backed classifier 和 L7 semantic classifier 仍缺失。

Flow classifier executable contract：

| 维度 | 契约 |
| --- | --- |
| Activation gate | 只有当 setup projection 产生 `requires_flow_classifier`、`host_rule_boundary` 是有限 host rules、proxy mode 为 `managed_tcp_relay`、且 `transparent_flow_classifier` capability 非 `unavailable` 时可执行。 |
| Host boundary | nft 只安装 setup-time 可证明的有限安全 superset；`no_host_rule_boundary`、规则上限溢出或 analysis budget 溢出都 fail closed，不安装全机宽规则。 |
| Selector source | classifier 使用 validation/composition 后的 canonical `Selector`，不维护另一个 selector AST。named ref 必须先由来源 registry 展开。 |
| Inbound attribution | 入站 TPROXY relay 在 upstream connect 前按 original target 的 local port 查询当前 TCP listener holder；目标地址必须能命中归因 listener，任一相关 listener 未归因或 selector 不命中都拒绝。 |
| Outbound attribution | 出站 transparent proxy relay 用 recovered original destination 与 downstream peer 构造 TCP connection，通过 procfs 查询当前连接 owner；无法归因或 selector 不命中都拒绝。 |
| Reject semantics | classifier 拒绝会关闭 downstream，不连接 upstream，只计入 managed proxy rejected relay，不计入 accepted relay 或 relay failure。 |
| Unsupported surfaces | external proxy、no-boundary flow-aware setup、强 socket lifetime 证明、kernel/cgroup classifier、L7 semantic classifier 不由该 backend 声称支持。 |
...
- 已实现 replay CLI。
- replay CLI 用单向输入文件驱动 capture provider、ingress journal、parser、policy、export queue 和可选 webhook/file/Unix HTTP exporter。
...
- 若 eBPF PID 与当前 procfs namespace 不可映射，或 observed TGID 与 host 上无关 PID 撞号导致 direct fd lookup
  不能形成 socket 证据，但 flow-start observation 提供 process hint，resolver 只在严格条件下做低置信度归因。
- 低置信度归因要求候选扫描完整。
- 低置信度归因要求同 fd socket。
...
- agent build 默认把第一方 process-observation 和 TLS uprobe artifact 嵌入 agent binary；当运行配置需要对应能力且未配置
  override path 时，agent 会把内置 artifact 物化到 `PROBE_HOME/artifacts/ebpf/` 下的 content-addressed 文件。
- `capture.ebpf.object_path` 和 `tls.plaintext.instrumentation.libssl_uprobe_object_path` 是高级 override，不是普通用户首次启用路径。
...
- capture input activity 不证明 eBPF per-link kernel liveness，也不证明强 socket-object lifetime；process eBPF
  tracepoint firing counters 是独立的 provider diagnostics。
...
- `traffic_probe_ebpf_process_observation_tracepoint_firing_metrics_available` 表示当前 status snapshot
  是否包含 process eBPF provider 的 tracepoint firing counter 数值投影。
- `traffic_probe_ebpf_process_observation_tracepoint_firing_mode{mode="available|degraded|unavailable"}`
  是 `capture.provider.tracepoint_firings.mode` 的 one-hot gauge。
- `traffic_probe_ebpf_process_observation_tracepoint_firings_total` 只在
  `capture.provider.tracepoint_firings.mode = "available"` 时输出，表示从 provider counter 读取到的
...
- 已实现 enforcement policy source 的 agent 运行时加载路径。
...
- 该 reload 不承担远程控制面语义。
- 如果 transparent interception 未启用，或显式 `enforcement.interception.selector`
  固定了 setup scope，该 reload 可以替换 planner policy。
- 如果 transparent interception 缺少显式 `enforcement.interception.selector`，setup scope
  会继承 effective policy selector；这类 plan 会 fail closed，直到 host rule mutation owner
  能安全调和数据面范围。
...
- 该 E2E 用四条 flow 覆盖 `enforcement.selector ∧ manifest selector` 控制矩阵。
...
缺失 container id 或缺失 cgroup path 都按未知处理，只能保留候选，不能排除候选。最终是否命中仍由完整 flow + direction selector 决定。`TargetScope` 是 enforcement 层的一等目标边界：
...
- Kernel object
  ...
  - process artifact 已覆盖 connect enter/exit、accept/close/close_range、process exit/exec、fd-table epoch、per-fd
    descriptor generation、bounded outbound/inbound syscall samples、available sendfile family kernel-transfer gap、process output loss counter
    和 per-tracepoint firing counter。
...
- Userspace loader/provider
  - `capture::EbpfProcessObservationProbe` load/attach process tracepoints、打开 ringbuf/allow map/loss counter/firing counter，
    并解码 typed observation。
...
sequenceDiagram
    ...
    alt resolved before retry budget is exhausted
        ...
        else lifecycle is untracked or was displaced from bounded userspace state
...
- outbound 支持 single-buffer 与 bounded multi-iovec prefix syscall argument sample。
...
- inbound 支持 single-buffer 与 bounded multi-iovec prefix syscall result sample。
...
- userspace bounded tracker 发生容量淘汰或 descriptor replacement 时，不伪造 `ConnectionClosed`；
  对已经授权 payload 方向输出 event-local terminal `provider_state_boundary` evidence 的 unknown-offset `Gap`。
  pipeline 在该 boundary 后释放对应 parser/evidence flow state，不等待后续 close。
- allow-map revoke 只在 eviction 后没有 retained tracked flow 仍持有同一 `(tgid, fd)` payload allowance 时执行。
...
- enter tracepoint 在 allow gate 后立即持久化 single-buffer 或 bounded multi-iovec prefix snapshot。
...
- 这不证明完整 socket 流、超出 bounded scan/sample 的 scatter/gather reassembly 或应用最终消费语义。

vector syscall 设计刻意保守：

- 最多扫描固定数量的 iovec。
- 跳过 zero-length iovec。
- 将 scan window 内落在固定 append slot 边界上的非空 iovec segment 按顺序拼接到同一个 bounded sample buffer。
- 如果 HTTP start-line/header 跨越多个已扫描且边界对齐的 segment，parser 可以看到拼接后的 prefix。
- 如果连续 prefix 没有落在可追加 slot 边界，则在该 prefix 后截断，不跳过未捕获字节去拼接后续 segment。
...
未解析、未跟踪、已从 bounded userspace tracker 移出或 lifecycle event 自身丢失的 socket lifetime 不会被伪造。
如果 displaced flow 已经启用 payload 采样，provider 会输出 event-local terminal provider-state boundary `Gap`
表达 payload continuity 中断。pipeline 在该 boundary 后释放对应 parser/evidence flow state，不依赖后续 close。
...
- live fd 在真实 `/proc` 上可通过 `pidfd_getfd` + `SO_COOKIE` 增强成功归因后的 flow identity。
  eBPF process hint 存在时，direct、`NStgid` alias 和 full procfs hint scan 进入同一候选集合；候选必须同时匹配
  fd、remote endpoint、name、uid、gid，且结果唯一。该规则覆盖 observed TGID 不可见和 observed TGID 与 host
  上无关 PID 撞号的场景；仍不满足时不根据 remote endpoint 猜测进程。
- eBPF provider 会把已解析 connect/accept flow 按 bounded descriptor lease `(tgid, fd, fd_generation)` 跟踪
  payload attribution：
  - payload sample 和 close 只命中同 generation 的 tracked flow。
  - close_range 按同 TGID fd range 终止已跟踪 flow。
  - process exit/exec 按同 TGID 为 active payload-tracked flow 输出 lifecycle boundary gap。
  - userspace tracked-flow capacity eviction 或 descriptor replacement 对已授权 payload 方向输出 event-local terminal
    provider-state boundary gap，并在 pipeline 中释放对应 parser/evidence flow state。
...
- `TRAFFIC_PROBE_ALLOWED_SOCKET_FDS`：
  descriptor-level userspace allow map。
- `TRAFFIC_PROBE_ALLOWED_PROCESS_TGIDS`：
  process-level TGID allow map。
...
- `TRAFFIC_PROBE_PENDING_WRITES`：
  保存已通过内核侧 allow+epoch+write-direction gate 的 enter-time pending outbound single-buffer/bounded multi-iovec prefix sample。
- `TRAFFIC_PROBE_PENDING_READS`：
  保存已通过 allow+epoch+read-direction gate 的 inbound single-buffer/bounded multi-iovec prefix attempt。
...
- reload 不改变 enforcement mode、planner execution surface 或 backend。显式 `enforcement.interception.selector`
  让 transparent interception setup scope 与 policy selector 独立，因此 planner-only reload 可以替换 policy。
  缺少显式 interception selector 时，setup scope 会继承 effective policy selector，这类 planner-only
  reload 会 fail closed，直到 host mutation owner 能安全调和 host rules。
...
pipeline metrics 同时按 execution kind/surface 计数，方便运维区分“产生了 enforcement decision”和“真实执行面被委托或应用”。
...
也不会被复制进 planner 生成的 `EnforcementDecision.reason`。当前 eBPF single-buffer/bounded multi-iovec prefix syscall
...
flow blocker，避免没有 close 的弱 flow 永久阻塞 cursor。`ProviderStateBoundary` 是 terminal provider-state boundary：
它的 evidence 只作用于当前 event，但 pipeline 会在该 gap 后释放对应 parser/evidence flow state，不等待后续 close。
这类事件仍可进入 parser、policy、dry-run 和 audit，但 `ScopedEnforcementPlanner` 在真实
...
relay。`enforcement.interception.selector` 是这一 strategy 的透明接管 setup-time 作用域；配置后由它单独拥有
nft/TPROXY/redirect 规则范围。主配置 `enforcement.selector` 和外部 manifest selector 保持 policy/planner
作用域，不会收窄或扩大显式 interception selector。
...
- 选择面：显式 `enforcement.interception.selector` 拥有 MITM/transparent interception 数据面接管范围；
  缺省时接管范围回退到 effective policy selector。主 `enforcement.selector` 与外部 manifest selector
  使用 AND 语义合成为 planner/proxy-hook policy scope。
...
| Transparent nftables rules | 可信系统路径中的 `nft` CLI | `nft` 是常见发行版的标准 nftables 管理入口，规则语言覆盖 TPROXY、redirect、mark、socket-owner、socket cgroupv2 与 `nft --check` 预检。 | 可替换为 typed nf_tables backend；替换前必须证明 TPROXY/redirect/socket-owner/socket-cgroup/mark 表达、原子批处理、错误语义和 E2E 行为。 |
...
- `run` 在通过完整 RuntimePlan validation 并构造 effective setup scope 后，
  对 inbound nft 脚本执行 `nft --check`，再获取
...
- `run` 用 effective setup scope 投影出的 outbound host-rule set 构造 `transparent-linux` artifact。
- 任一 scope 缺少 explicit remote port 时拒绝。
- 非 UID/GID/cgroup-path 可下推的 process-scoped outbound setup 需要满足 process/flow classifier executable contract，否则拒绝。
- flow-aware outbound setup 按 `transparent_flow_classifier` 的 executable contract 进入 activation；
  不满足该契约时拒绝。
- UID/GID-only 和 cgroup-path-only outbound process selector 会分别下推为 nft socket-owner 与 socket-cgroup host rules。
...
- `xtask e2e-transparent-linux-outbound-cgroup-artifact-netns` 属于 `linux-artifacts` profile。
- 该测试枚举真实非 root cgroup v2 path，选择可被 nft socket-cgroup resolver 接受的 path，并在隔离 network namespace
  中复用同一 path 验证 `socket cgroupv2 level N "path"` rule 的 `nft --check`、安装、规则列表断言和清理。
- 若主机没有可用非 root cgroup v2 path，或 nft/kernel socket-cgroup resolver 不接受任何候选 path，该测试 skip。

当前 `transparent_interception` capability 的含义是“所选透明拦截 strategy 的 Linux/root/nft/RTNETLINK lifecycle 可用”。`nft`
对应发行版 `nftables` package，host routing 通过 RTNETLINK 执行。它只由平台 probe 和 agent composition root 的 backend
wiring 决定，不把 selector projectability 编码为 capability，也不表示已具备 L7 MITM proxy/data-plane、普通 external proxy
runtime health proof 或非 UID/GID/cgroup-path 可下推的 process-scoped outbound setup。

出站透明代理要求该 capability available，并且只在最终 selector 可投影为 host-rule outbound rule set，或满足
`transparent_flow_classifier` executable contract 时进入 activation。process/flow classifier readiness 由独立的
`transparent_process_classifier` 和 `transparent_flow_classifier` capability 表达。

process classifier 由 agent 的 procfs TCP listener owner proof probe 注入，在 TCP/tcp6 listener 表和 fd owner scan
都完整可用时以 degraded 方式提供 setup-time listener classification。flow classifier 由 agent 的 procfs TCP owner
proof probe 注入，在同样的 procfs 完整性条件满足时以 degraded 方式提供 finite-boundary managed-proxy fail-closed classification。
...
RuntimePlan 的 `local_setup_projection` 只根据本地配置可见的 transparent interception setup scope 做预分类：
显式 `enforcement.interception.selector` 优先；缺省时回退到主 `enforcement.selector`。它不加载本地/远程
manifest，也不等于最终 executable scope。该字段是 typed setup plan，而不是字符串原因集合：
...
- `requires_flow_classifier`
  - Payload：canonical `Selector` 和 `host_rule_boundary`。
  - 当前语义：执行层按 `transparent_flow_classifier` executable contract 安装有限 host-rule boundary，
    并在托管代理 relay 前运行 proxy-side procfs TCP owner classifier。其它情况 fail closed。
...
UID/GID owner 维度会作为 socket-owner scope 保留在 host-rule boundary 中；cgroup path 维度会先规范化为相对 `/sys/fs/cgroup` 的 path prefix，
再作为 socket cgroupv2 scope 保留在 host-rule boundary 中。当 selector 只有 UID/GID 或 cgroup path 时可直接成为 `host_rules`，
与其它进程条件混合时仍会显式报告 `requires_process_classifier`，同时保留可下推的 owner/cgroup boundary。process-only selector 不再被误报成 unsupported；`all()` 下的 process
...
执行层在加载外部 manifest 后会分别构造两个 scope。setup scope 使用显式 `enforcement.interception.selector`；
缺省时回退到 effective enforcement selector。policy/planner scope 使用主 `enforcement.selector` 与 manifest
selector 的 AND 合成。显式 interception selector 因此可以为透明接管数据面提供稳定边界，而不阻止 policy
source 或 `enforcement.selector` 在线替换。

- Outbound UID/GID-only 或 cgroup-path-only process-scoped setup 不走 classifier，而是作为 nft socket owner/cgroup host rule 安装。
- nft socket cgroupv2 rule 是静态 host boundary：`nft --check` 会在安装前验证 path 可解析，但已安装规则不会自动跟随同名 cgroup
  的删除和重建。需要跨 systemd service/scope 重启保持应用级拦截时，应使用 cgroup lifecycle watcher、owner/cgroup mark classifier
  或 proxy-side classifier，而不是把静态 socket-cgroup rule 声称为动态进程跟踪。
...
- 其它 process-scoped outbound setup 仍以 setup error fail closed，除非满足 flow classifier executable contract。
- flow-aware setup 按 `transparent_flow_classifier` executable contract 进入 activation；
  没有 finite boundary 的 flow-aware setup 仍 fail closed，避免把“按应用拦截”的配置静默扩大成全机拦截。
...
成功时为 degraded，表示只支持 setup-time listener proof；flow classifier 在完整 procfs TCP table 与 fd owner scan probe
成功时为 degraded，表示只支持 finite-boundary managed-proxy fail-closed classification。需要 classifier 但超出当前能力边界的 setup
composition 会在执行前以 setup error fail closed，并把对应 `transparent_process_classifier` 或 `transparent_flow_classifier`
及原因写进 block reason。

后续要实现 UID/GID socket-owner 和静态 cgroupv2 socket path 之外的动态 process-scoped transparent interception，应引入 cgroup lifecycle
watcher、cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；出站透明代理当前覆盖 host-rule setup、UID/GID-only
socket-owner setup 和 cgroup-path-only socket-cgroup setup，后续若要支持
...
- 透明拦截 strategy 启用但 `transparent_interception` capability 不可用：
  ...
  selector projectability 由 `local_setup_projection` 和 configured enforcement 的 effective setup scope 校验表达。
  effective setup scope 生成的 nft 脚本会在 activation 前执行 `nft --check`，失败时不会安装规则。
...
- 在线 `plan_config_reload` 可读取本地候选 TOML，执行 parse/schema/static runtime validation，并输出
  `no_change`、`apply_online`、`queue_runtime_generation`、`restart_required` 或 `invalid_candidate`。
...
- process observation eBPF object 默认随 agent binary 内嵌，并在需要 eBPF capture 且未配置 override 时物化到
  ...
  observation object 的高级 override；无 override 时普通用户不需要手动构建或填写 object path。
- process observation object 作为 `aya-obj` object preflight 输入；process observation runtime 使用 strict process artifact contract，
  校验 ringbuf、allow map、fd-table epoch map、fd generation map、pending connect、pending accept、pending write/read、pending
  write scratch、process write/read event scratch 和 process output loss counter map shape，校验 connect enter/exit、accept/accept4、
  close、close_range、process exit/exec、descriptor lease lifecycle、single-buffer/bounded multi-iovec prefix/available sendfile family
  payload tracepoint sections，并拒绝夹带 TLS maps/programs 的混装 object。高层用户态 `aya` process observation loader、
  connect/accept observation 到单个 `ConnectionOpened` `CaptureEvent` 的纯转换
  bridge、`capture.deep_observe_selector` 按方向授权的 outbound single-buffer/bounded multi-iovec prefix syscall argument
  sample、outbound available sendfile family kernel-transfer byte-count gap，以及 inbound single-buffer/bounded
  multi-iovec prefix syscall result sample 到 always-degraded `CapturedBytes`/`Gap` 的 payload bridge 均已存在。
  descriptor-generation close/close_range 到 best-effort `ConnectionClosed` lifecycle event、TGID-level process
  exit/exec 到 lifecycle boundary `Gap`、unresolved connect/accept 到 degraded `Gap` 的 provider path、userspace tracked-flow
  displacement 到 event-local terminal provider-state boundary `Gap` 的转换，以及 output ringbuf failure counter 到 degraded `capture_loss`
  event 的转换和 active tracked payload flow 的 conservative unknown-offset `Gap` fan-out 已存在。
  超出 bounded scan window、sample buffer 或固定 append slot 的 continuation、
  precise flow-specific lost-event reconstruction 和完整 kernel/socket-path traffic capture program 仍是缺口。
...
- 任一 scope 缺少 explicit remote port 时拒绝。
- 非 UID/GID/cgroup-path 可下推的 process-scoped outbound setup 需要满足 classifier contract，否则拒绝。
- flow-aware outbound setup 按 `transparent_flow_classifier` executable contract 执行或拒绝。
- UID/GID-only outbound selector 会渲染为 socket-owner rule；cgroup-path-only outbound selector 会渲染为 socket cgroupv2 rule。
...
- `interception.mitm.backend.process.upstream_tls_mode` 仅适用于 `product_proxy` backend：
  `auto` 表示下游 TLS 连接对上游使用 TLS、下游明文连接对上游使用明文；
  `never` 表示 TLS 终止后连接明文上游；`always` 表示无论下游是否 TLS 都连接 TLS 上游。
  默认是 `auto`，适合作为透明出站 MITM 的通用可靠路径；入站 TLS 终止到本地明文应用时应显式使用 `never`。
...
- `requires_flow_classifier` 携带 canonical `Selector` 与显式 `host_rule_boundary`。
...
- flow requirement 按 `transparent_flow_classifier` executable contract 执行或 fail closed。
...
- activation 用 effective setup scope 生成 executable nft 脚本。
...
Export cursor and lifecycle boundary：

- planned export worker drain 和 tail drain 只负责读取 export queue、发送 batch、校验 ACK 并推进对应 sink cursor。
- sink ACK 是投递状态，不是本地生命周期信号；ACK 不删除 export record，也不影响 admin `tail_events` 的响应 cursor。
- export queue 的物理删除只由 `storage.retention.export` 执行。
- retention 删除连续前缀时，会在同一个 Fjall batch 中退休 planned sink cursor。
- cursor retirement 表示这些事件因本地生命周期策略不可再投递，不等同于对应 sink 成功 ACK。
- 没有配置 export retention 时，已 ACK record 仍留在 export queue；生产配置必须显式设置 `max_age_ms` 或 `max_records`。
...
- export queue 当前只按 `[storage.retention.export]` 的 `max_age_ms`、`max_records`、`sweep_interval_ms`
  和 `prune_batch_limit` 周期清理可退休连续前缀；exporter ACK 不触发物理删除。
...
外发 transport 必须可扩展，后续支持常见方式，例如 HTTP、gRPC、Kafka、OTLP 等。当前配置面只暴露已实现的 `webhook`、`file` 与 `unix_http`；runtime plan、drain worker 和
...
| `unix_http` | `socket_path`、`endpoint`、`headers`、`codec`。 | 经 Unix domain socket 发送 HTTP/1.1 `POST` protobuf batch envelope，要求同 webhook 的 JSON structured ack 推进 cursor。 | `target.transport = "unix_http"`，包含 codec、socket path 和 endpoint。 |
...
- send：
  当前通过 webhook 或 Unix HTTP sink 发送 protobuf batch envelope，或通过 file sink 写入 JSON Lines batch record。
...
- lifecycle retire：
  ...
  ACK 不等于删除；慢 sink 不会因为其它 sink 成功 ACK 而被跳过。
  retention retire 只处理连续前缀，并在同一个 storage batch 中推进 planned sinks 的读 cursor，
  明确表示这些事件因生命周期策略被丢弃。
...
- 当前已实现 per-sink cursor、per-sink batch quota、storage retention worker 的 max-age/max-records lifecycle
  retention 和内存态 exponential backoff，避免某个慢或失败 sink 阻塞其它 sink；在线 admin status 会从 exporter
...
- ingress recovery 重新生成的语义事件
  ...
  - 不保证：exactly-once。
  - 不保证：已被 retention 删除的本地 record 不保留永久 tombstone。
...
- 当前 exporter worker 采用配置化固定间隔、全局/每 sink 每轮 batch budget、配置化 sink timeout 和内存态 per-sink exponential failure backoff 的
  best-effort drain；每个 batch 受 1024 records 和 16 MiB stored payload bytes soft limit 约束。
  若配置了 `storage.retention.export.max_age_ms` 或 `max_records`，统一 storage retention worker 会按
  `sweep_interval_ms` 维护 export queue 生命周期；exporter worker 和 tail drain 不执行物理删除，避免 exporter
  drain 层重新 owning storage lifecycle policy。
...
- TUI 只为托管 agent 的 `PROBE_HOME/run/tui/<session>` runtime overlay 创建私有 socket parent。
- 保存启用默认 admin socket 的配置时，TUI 会创建默认 `PROBE_HOME/run` 私有目录。
...
- `apply_online` 表示 changed section 由在线 runtime owner 管理。
  ...
  watcher/poller topology 未启用，且 transparent interception 未启用或由显式
  `enforcement.interception.selector` 固定 setup scope。
...
- `queue_runtime_generation` 表示 changed section 属于 runtime generation owner，或属于可随候选
  plan 携带的 plan-only owner。
- action-gated online owner 不能与 data-path generation owner 混合；这类候选保持
...
- `apply_config_reload` 复用相同 planning contract；当 decision 为 `apply_online` 时执行对应在线动作，
  所有在线动作成功后替换 agent-wide active `RuntimePlan` handle。
- `apply_config_reload` 返回 `config_reload_apply`，其中包含原始 plan、每个 apply action 的 outcome，
  以及 `active_plan_updated`。
- 对涉及 data path generation 的 rebuild verdict，`apply_config_reload` 会向 runtime generation owner 提交
  `request_runtime_generation` action；action outcome 为 `queued` 并携带 `request_id`，表示候选配置已进入运行时
  请求队列但 data-path active generation 尚未替换。
...
- `apply_config_reload` 应用 planning 判定为 `apply_online` 的候选主配置段；任一在线 action
  失败时不替换 active plan。
- `apply_config_reload` 对 planning 判定为 `queue_runtime_generation` 的候选主配置提交 runtime
  generation request，但不提前替换 active plan。
- action-gated online/data-path 候选配置保持 `restart_required`，避免在线 owner 成功而 data-path
  generation 未提交或未应用造成 partial commit；plan-only section 可随 generation candidate 携带。
- reload commands 只作用于 active `RuntimePlan` 已经拥有可交换 runtime owner 的资源。
- `reload_runtime_actions` 不重读主 TOML，不替换 active config，也不改变 capture/export/TLS/admin/interception 配置。
...
- `apply_config_reload` 的 owner 是兼容的 plan-only online owners、单个 action-gated
  online owner 或 runtime generation owner；成功返回 `config_reload_apply`，
  ...
  failed action；setup-time、不兼容 mixed-owner 或 action-gated online/data-path section
  返回 restart verdict。
...
- `apply_config_reload` 在线应用 policy-only 变更、export 主配置变更、同一 `storage.path` 下的
  retention 变更，以及在稳定 transparent interception setup scope 下的 enforcement policy source 和
  主配置 `enforcement.selector` 变更。
  顶层 `[selectors]` registry 变更，包括被 `enforcement.selector` 或
  `enforcement.interception.selector` 引用的条目变更，
...
- transparent interception 缺少显式 `enforcement.interception.selector` 时，setup-time host rules 会继承
  effective policy selector；这类计划的 enforcement policy reload 会 fail closed，直到存在 host-rule
  mutation owner。
...
- watcher 启动失败会让显式启用该能力的 `run` fail fast。
- transparent interception 缺少显式 `enforcement.interception.selector` 时，该 watcher 不启动；host rule
  动态 mutation 需要独立 owner。
...
- capture、observations、data-path-only selector registry、config version、TLS plaintext instrumentation
  和 TLS decrypt-hint material 变更会在旧 provider drain 成功或有界预算耗尽后消费 queued request，随后构建并打开候选 capture provider。
...
- generation executor 的二次校验只接受 runtime generation owner 和 plan-only owner 的组合 diff。
  action-gated online/data-path 候选配置保持 `restart_required`，不能在 admin apply 阶段拆成两个非事务提交；
  plan-only storage retention 和 export plan 变更可在 active plan 未独立前进时随 generation request 携带。
- 被 enabled policy selector、`enforcement.selector` 或启用中的 transparent interception setup selector
  引用的 selector registry changes、TLS material registry/source changes 和 enforcement execution surface
  仍缺完整 lifecycle owner；这些 section 保持 `restart_required`，不进入 runtime generation queue。
  未被 action-gated selector consumer 引用的 selector registry changes 由 runtime generation owner
  validate-then-swap。enforcement policy source 和主配置 `enforcement.selector` 已由 enforcement reload
  owner 在线 validate-then-swap。
...
- main TOML enforcement policy config 可在无 enforcement reload watcher/poller topology，
  且 transparent interception 未启用或由显式 `enforcement.interception.selector`
  固定 setup scope 时应用 policy source 和主配置 `enforcement.selector` 变更，并更新 active plan。
  未被 action-gated selector consumer 引用的 `[selectors]` registry 变更可进入 runtime generation；
  enabled policy selector、`enforcement.selector` 或启用中的 interception setup selector 引用的变更仍为
  restart-required。
...
- action-gated online/data-path mixed config 保持 `restart_required`；需要完整事务型 generation owner
  后才能整体 validate-then-swap。plan-only storage retention 和 export plan 变更可作为 generation
  candidate 的一部分携带，并通过 active-plan conflict 检查避免覆盖更新。
...
- 当前 runtime generation queue 接收 capture、observations、data-path-only selector registry、agent
  `config_version`、TLS plaintext instrumentation 和 TLS decrypt-hint material 变更。
- 当前主配置 watcher 可触发 policy-only 在线应用、export 主配置在线应用、storage retention 在线应用、enforcement policy source /
  `enforcement.selector` 在线应用，以及可携带 plan-only 更新的 runtime generation queue。
- 被 enabled policy selector、`enforcement.selector` 或启用中的 transparent interception setup selector
  引用的 selector registry changes、TLS material registry/source changes、enforcement execution surface、
  MITM bridge、transparent interception、storage path 和 runtime/admin topology 仍由 startup/setup-time
  owner 持有；planning 必须保持 `restart_required`，不能因为目标模型存在 data path generation 概念而提前排队。

目标运行时约束：

- capture data path、selector registry、TLS plaintext、MITM bridge 和 transparent interception host rules
  属于目标完整 generation 生命周期；当前实现只允许已具备 owner 的 data-path-only selector registry、
  TLS plaintext instrumentation 和 decrypt-hint material 进入 runtime generation queue。
- 完整 data path generation 应包含 capture backend、process observations、selectors、TLS plaintext、MITM bridge
  和 transparent interception；enabled policy selector、`enforcement.selector` 或启用中的 interception
  setup selector 引用的 selectors、MITM bridge 和 transparent interception host-rule owner 存在前，
...
- `capture`
  ...
  - 负责 bounded sample：selector-authorized outbound/inbound single-buffer 或 bounded multi-iovec prefix sample 到
...
- `xtask e2e-ebpf-process-loopback`
  ...
  - 覆盖 selector-authorized outbound single-buffer/bounded multi-iovec prefix syscall argument sample。
  - 覆盖 selector-authorized outbound available sendfile family kernel-transfer byte-count gap。
  - 覆盖 selector-authorized inbound single-buffer/bounded multi-iovec prefix syscall result sample。
  ...
  - vector 模式验证带 zero-length leading iovec 和 split payload 的 bounded multi-iovec prefix `writev(2)` /
    `readv(2)` / `sendmsg(2)` / `recvmsg(2)` tracepoint path。
    aligned split case 让 HTTP start-line/header 横跨多个 iovec segment，并断言不会产生 truncated gap；
    unaligned split case 让 prefix 落在非 append slot 边界，并断言 bytes prefix 后紧跟同 offset 的 truncated gap。
- `xtask e2e-ebpf-process-output-loss`
  ...
  - 将实际观测到的 eBPF loss/gap event contract 送入 agent capture-event feed，验证 durable ingress/export、
    admin metrics JSON 和 Prometheus counters。
...
- process eBPF 真实 ringbuf saturation 与实际 eBPF loss/gap event contract 的 agent projection 由
  `e2e-ebpf-process-output-loss` 覆盖；它不证明 live agent drain 时必然触发 ringbuf saturation。
...
- 默认/显式 live capture：
  ...
  - eBPF observation：已有 `aya-obj` strict process artifact preflight、shared ABI、最小内核 object scaffold、高层用户态 `aya` loader、
    ...
    degraded `Gap` 的 provider path、userspace tracked-flow displacement 到 event-local terminal provider-state boundary `Gap` 的转换、
    output ringbuf failure delta 到 degraded `capture_loss` export event 的 provider/pipeline path，以及 active tracked payload flow 的
    conservative unknown-offset `Gap` fan-out。fd resolution
...
| Export | `e2e-webhook-exporter`、`e2e-file-exporter`、`e2e-unix-http-exporter` | user | spool-backed exporter、codec、ACK/cursor 和 sink drain。 | 跨主机 gRPC/Kafka/OTLP transport、exactly-once。 |
...
admin metrics 还必须暴露 connection backend execution counters，证明运行期状态面能观察到 backend execution surface。
...
- `--io-mode readv-writev` 使用 zero-length leading `IoSlice`/`IoSliceMut` 加 split payload；默认 aligned split
  会让 HTTP start-line/header 横跨多个 iovec segment。
- `readv-writev` 验证 bounded multi-iovec prefix `writev(2)` / `readv(2)` path。
- `--io-mode sendmsg-recvmsg` 使用同样的 split iovec 形态。
- `sendmsg-recvmsg` 验证 `sendmsg(2)` / `recvmsg(2)` path。
- `--vector-first-payload-slice-bytes` 可调整 vector fixture 的第一个非空 payload segment 长度；
  eBPF 验收使用 aligned split 证明跨 iovec stitching，使用 unaligned split 证明固定 append slot 外不会跳过字节继续拼接。
...
- 命中 selector 后，才能验证 selector-authorized outbound/inbound single-buffer/bounded multi-iovec prefix syscall sample。
...
- 验证带 zero-length leading iovec 和 aligned split payload 的 bounded multi-iovec prefix `writev(2)` / `readv(2)`；
  HTTP start-line/header 横跨多个 iovec segment，且不会产生 truncated gap。
- 验证带 zero-length leading iovec 和 aligned split payload 的 bounded multi-iovec prefix `sendmsg(2)` / `recvmsg(2)`；
  HTTP start-line/header 横跨多个 iovec segment，且不会产生 truncated gap。
- 验证 `readv(2)` / `writev(2)` unaligned split payload：bytes prefix 长度等于第一个非空 segment，
  truncated gap 的 `expected_offset` 等于该 prefix 长度。
...
- 断言 selector-authorized degraded outbound single-buffer/bounded multi-iovec prefix sample。
- 断言 selector-authorized outbound available sendfile family kernel-transfer gap。
- 断言 selector-authorized degraded inbound single-buffer/bounded multi-iovec prefix sample。
...
- 覆盖 allow-map 授权。
- 覆盖 bounded outbound single-buffer/bounded multi-iovec prefix syscall argument sample。
- 覆盖 bounded inbound single-buffer/bounded multi-iovec prefix syscall result sample。
...
- vector 模式覆盖带 zero-length leading iovec 和 split payload 的 bounded multi-iovec prefix `writev(2)` /
  `readv(2)` / `sendmsg(2)` / `recvmsg(2)` path。
  aligned split 验证 HTTP start-line/header 跨 iovec stitching 且无 truncated gap；
  unaligned split 验证固定 append slot 外的 prefix 后 gap。
...
- `xtask e2e-ebpf-process-output-loss` 在 privileged 环境中打开真实 process eBPF provider，
  ...
  `capture_loss` 与 active tracked flow 的 conservative unknown-offset `Gap` fan-out；随后把实际观测到的
  eBPF loss/gap event contract 送入 agent capture-event feed，验证 durable ingress/export、admin metrics JSON
  和 Prometheus counters。
...
- 该 E2E 覆盖 selector-authorized outbound/inbound single-buffer/bounded multi-iovec prefix payload sample degraded semantics。
...
- `xtask e2e-ebpf-process-output-loss` 已验证真实 process eBPF output ringbuf saturation 会产生
  provider-scoped `capture_loss`，并向 active tracked payload flow fan out unknown-offset `Gap`；同一 case
  已验证实际 eBPF loss/gap event contract 的 durable/admin/Prometheus 投影。
...
- 失败 sink 未确认时，其它 sink 的 ACK 不会删除它的待投递事件。
...
- 校验 collector sink cursor 前进，且该 sink pending set 为空。
...
- 校验 JSON payload schema 和完整预期 export event set。
- 校验 `0600` 文件权限。
- 校验 local-file sink cursor 前进，且该 sink pending set 为空。

#### Unix HTTP exporter E2E 覆盖

- `xtask e2e-unix-http-exporter` 使用同一 plaintext scenario。
- 该 case 启动 configured Unix HTTP exporter 的真实 agent 和本地 Unix domain socket receiver。
- bounded run tail drain 后校验 Unix socket 上的 HTTP `POST /probe/batches?tenant=local`。
- 校验 `content-type = application/x-protobuf` 和 `x-traffic-probe-codec = deflate`。
- 校验 protobuf batch body、batch id range 与实际 sequence 对齐。
- 校验全局 export sequence 连续无重复。
- 校验 JSON payload schema 和完整预期 export event set。
- 校验每个 batch 的 custom header。
- 校验按 batch sequence 返回的 JSON ack。
- 校验 local-sidecar sink cursor 前进，且该 sink pending set 为空。

#### Exporter E2E 不证明

- periodic worker loop。
- rejection / retry / partial ACK。
- retention-driven record prune。
...
- 跨主机 gRPC/Kafka/OTLP transport 已完成。
...
- admin 测试覆盖 `apply_config_reload` 对 enforcement policy source 和主配置 `enforcement.selector`
  变更执行在线应用、更新 active plan，并让运行中的 planner 和 status 使用新 manifest 或 selector。
- admin 测试覆盖 `enforce` 模式删除 enforcement policy source 会被候选配置校验拒绝，并保持旧 active
  plan、planner 和 status。
...
- 显式 `enforcement.interception.selector` 固定 setup scope 时，transparent interception plan 允许
  planner-only reload；缺少显式 interception selector、setup scope 继承 effective policy selector 时
  fail closed。
...
- enforcement reload watcher 测试覆盖本地 manifest 文件变更、manifest 文件替换、manifest directory 替换后继续监听、
  运行期 symlink directory replacement 保留旧 policy、symlink manifest 启动拒绝、显式 interception selector
  下的 watcher eligibility，以及继承 setup scope 的 transparent interception plan fail-fast。
...
- Prometheus 测试覆盖 `traffic_probe_pipeline_enforcement_decisions_total{outcome="..."}` 和
  ...
  `traffic_probe_pipeline_enforcement_execution_total{kind="proxy_side_hook",surface="l7_mitm"}`。
...
| Process eBPF privileged E2E | `xtask e2e-ebpf-process-output-loss` | 真实 process eBPF output ringbuf saturation 产生 provider-scoped `capture_loss`，并向 active tracked flow 输出 unknown-offset `Gap`；随后把实际观测到的 eBPF loss/gap event contract 通过 agent capture-event feed 投影到 durable ingress/export、admin metrics JSON 和 Prometheus counters。 | live agent drain 时必然触发 ringbuf saturation；精确丢失字节重建。 |
...
`xtask validate-local` 是普通用户可运行的单机验收入口。它启动真实 agent，从 follow 模式 capture-event feed
追加 typed HTTP traffic，经 HTTP parser、Lua policy、durable export queue、file exporter 和 admin `tail_events`
验证闭环。该入口还通过 admin `plan_config_reload` 和 `apply_config_reload` 在线切换 policy bundle，
并验证切换后的后续流量使用新的 policy version。

`xtask e2e-suite` 是能力矩阵验收入口。

入口语义：

- `cargo run -p xtask --locked -- e2e-suite`
  - 等价于 `--profile baseline`。
  - 覆盖 local validation、plaintext feed、typed capture-event feed/provider loss、flow-level gap/degraded 语义、
    SSE streaming response、WebSocket handoff/frame、webhook/file/Unix HTTP exporter、remote policy bundle
...
- `e2e-suite`
  ...
  - 覆盖：非特权 agent durable output、HTTP/SSE/WebSocket parser、flow-level gap/degraded 语义、
    local validation、typed provider loss event、policy、remote policy bundle 一次性加载、
    remote policy bundle polling、remote enforcement manifest、webhook/file/Unix HTTP exporter，
    以及不依赖 transparent host rules 的 first-party product MITM proxy 明文/TLS feed ingestion
    和同一 listener 上的明文/TLS 混合入口。
...
- `baseline`
  ...
  - 具体包括：plaintext feed、typed capture-event feed provider loss、SSE、flow-level gap、WebSocket、
    webhook/file/Unix HTTP exporter、remote policy bundle 一次性加载、remote policy bundle polling、
    remote enforcement manifest，以及 first-party product MITM proxy 明文/TLS feed ingestion。
- `live-core`
...
- `transparent-interception`
  ...
  - 覆盖：nftables/TPROXY 入站 host rule、process-scoped setup-time listener proof、
    process-only listener-derived host rules、finite-boundary proxy-side flow classifier、
    outbound transparent proxy managed relay data-plane、external proxy reserved-mark self-bypass，
    UID/GID socket-owner-scoped outbound 路径，以及 product MITM 透明 HTTPS HTTP/WebSocket 数据面。
- `linux-artifacts`
  - 默认权限：root/net-admin。
  - 覆盖：透明 Linux artifact 语法、安装和 cleanup 验证，以及 cgroup-path socket-cgroup outbound 规则形态在 resolver
    可用环境中的真实 `nft --check`、安装、规则列出断言和清理。
  - 边界：不声明真实 agent runtime 成功闭环；cgroup-path socket-cgroup 规则形态不证明同名 cgroup 删除和重建后的规则刷新。
- `product`
  - 默认权限：user、root/CAP_NET_RAW、root/bpffs、root/net-admin。
...
- Provider loss
  ...
  - `e2e-ebpf-process-output-loss`，root/bpffs：
    ...
    并向 active tracked flow 输出 unknown-offset `Gap`；case 会把实际观测到的 eBPF loss/gap event contract
    送入 agent capture-event feed，验证 durable ingress/export、admin metrics JSON 和 Prometheus counters。
    不证明 live agent drain 时必然触发 ringbuf saturation，也不证明精确丢失字节重建。
...
- Export
  - `e2e-local-validation`，user：
    真实 `agent run` 从 capture-event feed follow 模式持续接收 typed `CaptureEvent`，
    经 HTTP parser、Lua policy、durable export queue、file exporter 和 admin `tail_events`
    形成单机非特权闭环。
    该 case 验证 admin tail 可读取仍被 retention 保留的 export records；sink ACK 不会消费 tail view。
    它还在两轮流量之间通过 admin `apply_config_reload` 在线更新 active policy set，
    并验证 reload 后追加的流量使用新的
    policy version 产生 policy alert。
  - `e2e-webhook-exporter`，user：
    ...
    structured ACK 后 collector sink cursor 前进且该 sink pending set 为空。
  - `e2e-file-exporter`，user：
    ...
    解码后 local-file sink cursor 前进且该 sink pending set 为空。
  - `e2e-unix-http-exporter`，user：
    plaintext feed 产生的 HTTP/policy export events 通过 configured Unix HTTP exporter，
    以 deflate-compressed protobuf batch 经本地 Unix domain socket 发送到 receiver。
    structured ACK 后 local-sidecar sink cursor 前进且该 sink pending set 为空。
...
- Enforcement control
  - `e2e-remote-enforcement-policy`，user：
    remote enforcement policy source 一次性拉取、selector 合成，以及 manifest protective action profile 对
...
- Transparent inbound
  ...
  - 覆盖：隔离 network namespace 中的 nftables/TPROXY host rule lifecycle、managed relay 入站转发，
    process-scoped setup-time listener proof、process-only listener-derived host rules、finite-boundary proxy-side
    flow classifier、inbound relay self-bypass 和 cleanup。
  - 边界：不证明规则安装后的 listener 动态追踪、no-boundary flow classifier、强 socket lifetime 或 cgroup/owner mark classifier。
...
- Transparent outbound
  ...
  - 覆盖：OUTPUT redirect、managed/external proxy self-bypass、webhook control-plane bypass、
    remote policy bundle reload control-plane bypass、UID/GID socket-owner-scoped outbound selector、
    finite-boundary outbound flow classifier、accepted/rejected relay metrics 和 cleanup。
  - 边界：不证明 no-boundary flow classifier、external proxy flow classifier、强 socket lifetime、
    cgroup-path socket-cgroup install/list 行为、no-boundary kernel/cgroup-backed classifier 或完整 L7 MITM lifecycle。

- Transparent Linux artifacts
  - `e2e-transparent-linux-outbound-redirect-artifact-netns`
  - `e2e-transparent-linux-outbound-cgroup-artifact-netns`
  - 权限：root/net-admin。
  - 覆盖：nft rule rendering、family split、socket-owner match、socket-cgroup match、selector-projected artifact shape、
    真实 `nft --check`、安装、规则列出断言和清理；socket-cgroup resolver 不可用时 cgroup artifact case skip。
  - 边界：不启动 agent，不证明 proxy 数据面，也不证明同名 cgroup 删除和重建后的规则刷新。

- Product MITM
  - `e2e-product-mitm-proxy-local`
  - 权限：user。
  - 覆盖：正式 `traffic-probe-mitm-proxy` 作为 standalone first-party product proxy 被启动，
    直接处理普通 HTTP 明文下游连接和 operator-managed CA-backed TLS 下游连接，分别写入
    `l7_mitm_plaintext` capture-event feed；同一 TLS-capable listener 在 `upstream_tls_mode = "auto"`
    下可同时接收明文 HTTP 与 TLS HTTP，并分别连接明文上游和 TLS 上游；真实 agent 以
    `capture_event_feed` backend 消费这些 feed，并输出 HTTP request/response/body durable events。
  - 边界：不安装 nftables/TPROXY/OUTPUT 透明规则，不证明自动 client trust store 安装，也不声明原始进程归因。
...
- Linux artifacts
  ...
  - `e2e-transparent-linux-outbound-cgroup-artifact-netns`，root/net-admin：
    使用真实 cgroup v2 path 验证 socket-cgroup outbound artifact 的 `nft --check`、安装、规则列表断言和 cleanup。
    当主机没有可用非 root cgroup path，或 nft/kernel resolver 不接受任何候选 path 时 skip。
  - 两个 case 均属于 `linux-artifacts` profile，不启动 agent。
...
| `e2e-ebpf-process-output-loss` | 真实 process eBPF ringbuf/provider loss 产生 provider-scoped `capture_loss`；存在 eligible flow target 时产生 conservative unknown-offset `Gap` fan-out；实际 eBPF loss/gap event contract 可进入 agent durable/admin/Prometheus 投影。 |
...
#### Enforcement manifest 与 reload

- `check`/`run` 显式加载 enforcement policy source。
...
- 在线 admin metrics 输出 enforcement execution surface counters：
  `connection_backend.decisions`、`connection_backend.linux_socket_destroy`、
  `proxy_side_hook.decisions` 和 `proxy_side_hook.l7_mitm`。
...
- `outbound_transparent_proxy` 按第 18 节 proxy lifecycle contract 支持 host-rule OUTPUT/dstnat redirect、managed relay 或 external reserved-mark
  self-bypass。
- finite-boundary outbound flow-aware setup 可通过 managed relay proxy-side classifier 执行；selector 不命中时拒绝 downstream，
  不连接 upstream，并输出 rejected relay metrics。
...
- 非 UID/GID/cgroup-path 可下推的 process-scoped outbound setup fail closed，除非满足 classifier contract。
- flow-aware outbound setup 按 `transparent_flow_classifier` executable contract 执行或 fail closed。
- UID/GID-only outbound process selector 会下推为 nft socket-owner host rule；cgroup-path-only selector 会下推为静态 nft socket cgroupv2 host rule。
...
| 优先级 | 状态 | 目标 | 主要交付 | 验收信号 |
| --- | --- | --- | --- | --- |
| P0 | Active | 收敛主要缺口，避免 best-effort 被误读成强保证。 | kernel lifetime、loss reconstruction 和完整 per-link coverage。 | capability、metrics、export 和 E2E 状态一致。 |
...
- P0 聚焦 kernel socket-object lifetime、precise flow-specific lost-event reconstruction，
  以及超出 tracepoint firing counters 的完整 per-link coverage。
  `metrics.pipeline.capture_polls` 只能证明 provider poll loop activity，
  不证明单个 eBPF link、socket-object 或 kernel-side observer 存活。
  process eBPF tracepoint firing counters 可证明对应 handler 曾被调用，但不证明所有 attached links 都持续覆盖目标 workload。
...
| Unix HTTP exporter | 通过本地 Unix domain socket 向 sidecar 发送同一 protobuf batch/ACK 协议，按 sink cursor 推进 | 第 22、24 节 |
...
| 验收 | HTTP/SSE/WebSocket demo、webhook/file/Unix HTTP exporter E2E、libssl TLS demo、pcap fallback、dry-run enforcement | 第 31 节 |
