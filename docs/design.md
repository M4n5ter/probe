...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；后续要启用出站 MITM runtime install，应把已有 proxy pre-connect `SO_MARK`、已存在但未接入 lifecycle 的 original-destination resolver、证书材料、MITM CA 注入、output redirect activation/install 和 L7 分类作为同一个 executable backend 的门控条件。
...
- `enforcement.interception.strategy = "none"` 默认不要求额外能力；`"inbound_tproxy"` 要求本地显式 selector、proxy listen port、平台入口层面的 `transparent_interception` capability、`enforcement.mode = "enforce"`、live host capture，并且不能与 connection backend 同时启用，直到真实 composite execution backend 存在；`"outbound_mitm"` 当前会保留 outbound selector projection 和 redirect preview，RuntimePlan/check/status 可以通过真实 composition 路径表达 OUTPUT/dstnat、proxy port 与 proxy bypass mark；agent setup scope 校验已能通过 `transparent-linux` artifact planner 把最终 outbound host-rule scope 渲染成 OUTPUT/dstnat nft artifact，并拒绝 wildcard remote port；proxy 内部已有 Linux original-destination resolver 和 pre-connect `SO_MARK` 原语，但它们尚未成为 executable outbound lifecycle 或独立 status contract。`outbound_mitm` 不要求 `transparent_interception` capability，也不把 preview 或 artifact 伪装成可执行能力；activation/install 仍 fail closed，因为它还缺少 output redirect activation/install 和 MITM lifecycle。
...
17. `e2e-transparent-linux-outbound-redirect-artifact-netns` 在 root/net-admin 环境下从 explicit `transparent-linux` artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，再执行真实 `nft --check`、安装、规则列表断言和 cleanup 验证。它属于 `linux-artifacts` profile，不启动 agent，不安装 runtime outbound MITM，也不证明 proxy self-bypass、original destination recovery 或证书/MITM lifecycle。
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_mitm`，并在 runtime plan/status/check 中报告 proxy mode/listen port、独立 selector、按方向投影的 `local_setup_projection`、process/flow classifier capability 和 agent-owned proxy runtime snapshot；当前 `inbound_tproxy` 有 executable Linux lifecycle，并独占 `transparent_interception` executable capability requirement，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，配置化 active health probe 会主动检查显式 TCP endpoint 并在连续失败达到阈值后降级 health。`outbound_mitm` 已有 selector projection、内部 Linux original-destination resolver 原语、proxy pre-connect `SO_MARK` 原语、redirect preview 和 shared Linux OUTPUT/dstnat artifact planning，可在 RuntimePlan/check/status 中表达 OUTPUT/dstnat、proxy port 与 proxy bypass mark，并在 setup scope 校验中拒绝缺少显式 remote port 的 outbound host rules；但 runtime 仍 fail closed，不安装 output redirect 规则，也不要求 `transparent_interception` capability，直到 output redirect activation/install 和 MITM lifecycle 接入。
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；relay 目标恢复已拆成显式策略，出站 MITM 可复用 Linux original-destination resolver 原语，proxy connect path 已具备 pre-connect `SO_MARK` 原语；出站 MITM 已有按 outbound selector 投影的 status/check 表达、RuntimePlan redirect preview、`transparent-linux` OUTPUT/dstnat artifact planning 和 Linux artifact acceptance，但不安装 output redirect，直到证书注入、output redirect activation/install 和 L7 MITM lifecycle 一起落地 | 第 11、18、32 节 |
