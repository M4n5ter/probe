...
- link-level target liveness beyond reconcile：
  `last_reconcile.targets.*.reconcile_state` 说明最近一次成功 reconcile 的 target 状态，
  `reconcile_health` 说明 reconcile loop 最近一次尝试是否失败；二者都不证明已 attach uprobe link 在两次 reconcile 之间仍然存活。
...
已在 status/health 中表达；link-level target liveness beyond reconcile、TLS ringbuf saturation E2E 和 strong ownership
仍是后续边界。
...
| 优先级 | 状态 | 目标 | 主要交付 | 验收信号 |
| --- | --- | --- | --- | --- |
| P0 | Active | 收敛主要缺口，避免 best-effort 被误读成强保证。 | 强 socket lifetime、precise flow-specific lost-event reconstruction、TLS ringbuf saturation E2E、TLS link-level liveness beyond reconcile。 | capability、metrics、export 和 E2E 状态一致。 |
