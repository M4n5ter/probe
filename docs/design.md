...
当前实现状态：agent 已有 eBPF host probe，检查运行平台是否为 Linux、`/sys/kernel/btf/vmlinux` 是否为 regular file、`/sys/fs/bpf` 是否为目录且 Linux `statfs` filesystem magic 是否为 bpffs，并读取 `/proc/sys/kernel/unprivileged_bpf_disabled` 作为权限/安全姿态提示，其中 `0` 表示 unprivileged BPF enabled，`1` 表示 permanently disabled，`2` 表示 disabled 但管理员仍可再改。宿主前提满足后，agent 会读取 `capture.ebpf.object_path` 并通过 Aya object parser 做 eBPF object preflight，报告 object 是否能解析以及其中的 programs/maps。由于 attach、event reader 和 kernel-side capture program 还没有完成，`ebpf` provider descriptor 仍不会进入 available；`auto` 不会选择 eBPF，显式 `capture.selection = "ebpf"` 会 fail closed 并给出 object 或未实现 attach/reader 的原因。
...
- eBPF object 路径放在 `[capture.ebpf] object_path` 下。这个字段只表达 Aya object preflight 输入，不等同于 eBPF provider 已可运行；attach/event reader 完成前，preflight 成功仍然是 unavailable capability，并保留未实现 builder 的原因。
...
- 默认/显式 live capture：按 `auto`、`ebpf`、`libpcap` 解析 plan；当前实际可运行的是 libpcap，eBPF 已有 Aya object preflight，但还没有 attach/event reader，因此不会被选中。
