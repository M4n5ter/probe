...
- 已实现 external enforcement manifest 的 agent 运行时加载路径：本地 file/directory manifest 和 remote endpoint 都会在 `check`/`run` 显式解析，manifest selector 会与主配置 selector 合成 effective selector，manifest protective action profile 会进入 `ScopedEnforcementPlanner`。`xtask e2e-remote-enforcement-policy` 用 loopback remote manifest、真实 `agent run`、configured Lua verdict 和 Fjall spool readback 验证了 remote manifest 被请求一次，并用四条 flow 覆盖 `config selector ∧ manifest selector` 控制矩阵：两者都命中且 action 在 profile 内时输出 `dry_run`、只命中主配置 selector 或只命中 manifest selector 时输出 `selector_miss`、两者都命中但 action 不在 manifest profile 内时输出 `unsupported`；所有 decision 都保持 `effective_action = observe`。
...
3. `check`/`run` 显式加载 external enforcement manifest，本地 source 从文件读取，remote source 通过一次 GET 拉取 TOML，随后合成主配置 selector 与 manifest selector；`xtask e2e-remote-enforcement-policy` 已用真实 agent path 验证 remote source 拉取、主配置 selector 与 manifest selector 合成，以及 manifest protection profile 对 dry-run decision 的约束。
...
| 拦截抽象 | PolicyRuntime + external enforcement manifest + TargetScope + ScopedEnforcementPlanner + EnforcementBackend boundary + 显式 linux socket destroy backend + connection_enforcement capability + transparent_interception `local_setup_scope` 分类 + selector-projected rule lifecycle + 可选 agent-managed TCP relay lifecycle | 第 18 节 |
| 当前拦截验收 | dry-run typed verdict、remote external manifest GET、manifest selector 与主配置 selector 合成、process/traffic scoped dry-run decision、manifest profile unsupported audit、显式 backend 下的 owner-verified TCP socket destroy audit decision，以及透明拦截 nftables/TPROXY plan/probe/lifecycle、managed relay 和隔离 netns 入站截获/转发 E2E | 第 18、31 节 |
