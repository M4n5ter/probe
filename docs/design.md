...
| `libssl_uprobe` | Probed degraded/unavailable; degraded is explicitly usable for best-effort TLS plaintext | 已实现 process-scoped discovery/planning、strict TLS artifact preflight、eBPF libssl plaintext artifact contract、userspace Aya loader/provider、ringbuf sample decoder、procfs fd-to-flow resolver、startup attach planning、attach target reconcile diff model、concrete provider `reconcile_libssl_uprobes(next_plan)` 方法、target-scoped detach primitive、agent concrete sidecar owner、按 `tls.plaintext.reconcile_interval_ms` 驱动的周期性 process scan/reconcile、online runtime `last_reconcile` counters/sequence/observed timestamp，以及 primary live provider + TLS plaintext provider 的 capture multiplexer wiring；best-effort attach 按同一 process/library target 做 session link 提交，先确认 target 同时具备 fd association 与 plaintext recipe，再按 fd association -> lifecycle -> plaintext 顺序 attach，未 ready target 不提交 link，半个 recipe 会 rollback 已 attach link；一旦未提交 target 可能已经执行过会写 TLS state map 的 probe，loader 会 fail-closed 而不是继续启用 sidecar；TLS producer 在 `SSL_set_fd` state 存在时置 `FD_VALID`，userspace 只在该标志存在时用 fd 做 procfs flow resolution；详情见 TLS plaintext 小节。 | 尚未实现 per-target provider health、强 fd ownership、async fan-in/backpressure、lost-event feedback 和 privileged dynamic lifecycle e2e 覆盖；producer 的 fd 关联仍是 best-effort，`FD_VALID` 表示可尝试归因，不等于强 socket ownership 证明，解析成功的 libssl bytes 仍保持 degraded；TLS state maps 目前没有 target/generation fence，unsafe partial attach 只能通过 fail-closed 处理。 |
...
- 已实现 selector AST 的基础形态：`match`、`all`、`any`、`not`、`ref`，命名 selector 通过 registry 编译解析；`CompiledSelector::may_match_process` 提供 process-scope 保守投影，供 TLS attach、深度观测和未来拦截在 flow 尚未知时做 false-only 安全剪枝。
...
当前实现还提供 process-scope projection：`CompiledSelector::may_match_process(process)` 只保证 `false` 表示该进程可在 flow attribution 前安全排除，`true` 只是保留候选而不是证明未来一定存在匹配 flow。它会用 process selector 做可验证剪枝，但不会因为 attach 前未知的 traffic 维度误杀候选进程；traffic-only selector 保持候选，`not` 也按保守语义处理。对于 partial process context，空/`unknown` 进程名、空 exe path、空 cmdline hash、缺失 systemd service 或缺失 container id 都按未知处理，只能保留候选，不能排除候选。最终是否命中仍由完整 flow + direction selector 决定。这个边界用于 libssl uprobe 动态 attach、深度 payload 开关和未来只保护部分应用的拦截入口，不能替代 per-flow policy/enforcement 判定。
...
| object preflight | `ebpf-object` 使用 `aya-obj` 解析 object，校验 required `SSSA_EVENTS` ringbuf map、`sssa_sys_enter_connect` 与 `sssa_sys_enter_close` tracepoint program、TLS plaintext uprobe/uretprobe programs、TLS state maps、section、map shape、pinning 和同一次 hardened read bytes；通用 contract 默认允许额外 map/program，process observation runtime、TLS plaintext loader、agent provider descriptor、agent `libssl_uprobe` capability probe 和 `xtask check-ebpf` 对具体 artifact 使用 strict inventory policy，防止 process artifact 夹带 TLS maps/programs。 | 真实 payload program contract、privileged eBPF e2e。 |
...
当前 `agent run` 已进入运行中 best-effort dynamic reconcile：agent 启动时先做一次 procfs scan，生成 startup attach planning result。planning result 要么给出可用 attach plan，要么给出 blocked reason；可用 plan 可以包含 attachable probes，也可以是“当前没有 attachable target 且没有 planning/degradation reason”的空等待计划。若当前候选进程存在 planning error 或 discovery degradation 且没有任何 attachable probe，runtime 不能把 TLS sidecar 伪装成 enabled，而是记录 disabled reason。agent TLS runtime 用 concrete sidecar 持有 `LibsslUprobePlaintextProvider`、procfs attach planner 和 fixed interval schedule，并在 sidecar 的 best-effort provider slot 内按 `tls.plaintext.reconcile_interval_ms` 周期重新扫描进程、重建 attach plan、调用 capture concrete provider 的 `reconcile_libssl_uprobes(next_plan)`。session 是已提交 link 的唯一事实源，probe 基于下一轮 plan 计算 diff；对 stale target 做 target-scoped detach，对 new target 复用同一个 loaded program/session 边界执行 best-effort attach，并且只从 retained target + 实际 committed target 生成下一轮 attach state，避免把 clean-skipped target 误标成已生效。成功 reconcile 会写入 TLS runtime `last_reconcile`：`sequence`、`observed_unix_ns`、`attached_targets`、`detached_targets` 和 `active_targets`；如果后续 sidecar disabled，该字段保留最后一次成功结果用于排障，但不能被解释为当前 per-target provider health。任何 reconcile side effect 失败都会 poison 该 probe，并 best-effort detach 剩余 links；错误由 sidecar best-effort mux 捕获，TLS runtime state 进入 disabled，mux 会 drop sidecar provider 触发剩余 link cleanup，但 required primary capture 不停止。per-target provider health、async fan-in/backpressure、strong fd ownership 和 privileged dynamic lifecycle e2e 仍是缺口。
...
- Remaining lifecycle gaps：当前仍缺 per-target provider health、async fan-in/backpressure、lost-event feedback、更强的 fd ownership、TLS map target/generation fence，以及 privileged dynamic lifecycle e2e 覆盖。
...
TLS plaintext config/runtime/status/check plan 已能表达 provider、selector、capability requirement、`libssl_uprobe_object_path`、`reconcile_interval_ms` 和 keylog/session secret refs。agent registry 会对已配置的 TLS plaintext object 做 strict TLS artifact preflight，并把 host/object/contract/procfs socket attribution 失败写入 `libssl_uprobe` capability reason；如果这些前置条件都满足，能力仍保持 degraded 而不是 available，因为当前仍缺 per-target provider health、strong fd ownership 和 privileged dynamic lifecycle e2e。`tls.plaintext.enabled = true` 对 `libssl_uprobe` 要求 live capture mode，拒绝 unavailable capability，但允许 degraded best-effort capability 显式启用。best-effort attach disabled reason 会保留首个 attach/validation 失败，避免把 stale target、attach permission 和 partial recipe 全部压成无信息状态；online admin status 会额外暴露 TLS sidecar runtime pending/enabled/disabled 状态、disabled reason，以及最后一次成功 reconcile 的 counters、sequence 和 `observed_unix_ns`，并在 disabled 时把 status capability matrix 中的 `libssl_uprobe` overlay 为 unavailable。`external_plaintext_feed` available 只表示 agent 已能消费外部已解密明文 feed。
...
`tls.plaintext.selector` 与 enforcement/policy selector 使用同一 AST，用于表达“只对某些应用做 TLS plaintext instrumentation”的 attach 范围。当前 runtime 会编译校验 selector，并在 startup scan/planning/reconcile 阶段用 process-scope projection 做候选进程剪枝；运行中的动态发现和 attach/detach reconcile loop 已接入 best-effort sidecar，并通过 runtime status 暴露最近一次成功 reconcile counters、sequence 和 `observed_unix_ns`，仍缺 privileged e2e 覆盖、per-target provider health 和强 fd ownership。
...
- TLS plaintext uprobe object 路径放在 `[tls.plaintext] libssl_uprobe_object_path` 下，而不是复用 `[capture.ebpf] object_path`。前者必须满足 strict TLS plaintext artifact contract，后者必须满足 strict process observation artifact contract；两个 artifact 不能混装。`[tls.plaintext] reconcile_interval_ms` 控制 libssl sidecar 在 provider poll 路径上的周期性 process scan/reconcile，默认 `1000`，必须在 `1..=3600000` 毫秒范围内。agent registry 会用 TLS object 路径探测 `libssl_uprobe` capability：缺路径、host prerequisites、object read/parse、TLS contract 或 procfs socket attribution 失败都会进入 unavailable reason；TLS artifact 和 procfs attribution 都通过时报告 degraded，因为当前仍缺 per-target provider health、strong fd ownership、low-latency fan-in 和 privileged dynamic lifecycle e2e。`tls.plaintext.enabled = true` 要求 live capture mode 和非 unavailable `libssl_uprobe` capability；如果 startup scan 没有 attachable target 且没有 planning error/discovery degradation，agent 会保持空 attach live sidecar 并等待后续 reconcile；target attach race 导致无可用 probe，或没有同一 process/library target 上完整的 fd association + plaintext recipe set，不会停止 primary capture，但会通过 TLS plaintext runtime state 暴露 disabled/degraded reason 和最近一次成功 reconcile 的 counters、sequence 与 `observed_unix_ns`。
...
- 默认/显式 live capture：
  ...
  - Missing：仍未完成 async fan-in/backpressure、per-target provider health、lost-event feedback、强 fd ownership、TLS map target/generation fence、privileged dynamic lifecycle e2e 和完整 kernel-side capture program。
...
- ABI 与 object contract：`ebpf-abi` 单元测试覆盖 process event 与 TLS plaintext event 的 wire kind、header layout、record layout、field offset、encode/decode、invalid direction reject、read-failed payload reject、TLS map spec 表、TLS state layout，以及公共 event header decoder 能在只读 header prefix 时识别 TLS plaintext record；process observation decoder 遇到 TLS plaintext record 会明确拒绝为非 process event。`ebpf-object` 单元测试用从 ABI spec 派生的 TLS plaintext ELF fixture 验证 5 个 map 和 13 个 uprobe/uretprobe program 可被 `aya-obj` inventory 解析并满足 contract；`xtask check-ebpf` 已验证真实 `ebpf-program` process artifact 与 `ebpf-tls-plaintext` TLS artifact 分别满足 strict contract。
...
- Runtime/config/status：config/runtime/status/check 测试覆盖 `tls.plaintext.libssl_uprobe_object_path` 与 `tls.plaintext.reconcile_interval_ms` 的解析、无效路径/错误 provider 组合 fail-closed、RuntimePlan 透传、status/check metadata 输出，runtime 测试还覆盖 libssl TLS plaintext 要求 live capture mode、unavailable fail closed、degraded best-effort capability 可显式启用。agent registry 测试覆盖缺 TLS object path、无效 TLS object、TLS object preflight 成功但仍保持 degraded，以及缺 procfs socket attribution 时 unavailable；agent TLS plaintext planning/runtime 单元测试覆盖 startup process scan 跳过消失进程与 racy/invalid process metadata，同时不把全局 procfs dependency error 误当成 per-PID race，empty/error-free planning result 保持可用空 plan，empty planning report 带 error 时进入 blocked，sidecar 到期后先 reconcile 再 poll provider、blocked planning 不继续 poll、成功 reconcile 会写入 runtime `last_reconcile` counters、sequence 和 `observed_unix_ns`、disabled 后保留最后一次成功 fact，disabled TLS plaintext build 会记录 unavailable reason，以及 bounded `(pid, process starttime, SSL*, TCP connection)` flow-start tracking 对同一 SSL connection 保持稳定、对 PID generation/connection 变化产生新 identity。startup runtime builder 生成初始 attach result；capture concrete provider 已有动态 reconcile 方法，agent runtime 用 concrete sidecar 按固定 interval 调度该入口。online admin status 会报告 TLS plaintext runtime sidecar 的 pending/enabled/disabled 状态、`reconcile_interval_ms` 和最近一次成功 reconcile 的 counters、sequence 与 `observed_unix_ns`；sidecar disabled 时，status capability matrix 和 `tls.plaintext.capability` 都会把 `libssl_uprobe` overlay 为 unavailable 并保留原因，整体 health 降级但 primary capture 不被误判为失败。

该验证仍不等同于 privileged libssl uprobe attach/detach e2e、process-exit 清理或强 fd ownership 已完成。
