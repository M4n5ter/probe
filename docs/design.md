...
- 不把透明代理/MITM 作为默认核心拦截路径；当前只实现显式启用的 Linux nftables/TPROXY 规则生命周期，并可选启动 agent-managed plain TCP relay。后续 MITM backend 必须显式启用、能力门控、按进程/应用 selector 生效，并独立管理证书材料、proxy health、self-bypass 和 L7 分类，不能复用 TLS material 或 libssl uprobe instrumentation 的概念。
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_mitm`，并在 runtime plan/status 中报告 proxy mode/listen port、独立 selector、`local_setup_scope` 和 `transparent_interception` capability；当前 `inbound_tproxy` 有 executable Linux lifecycle，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，`outbound_mitm` runtime 会 fail closed，不安装 output redirect 规则。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle 与可选 managed TCP relay 已接入，出站 MITM 执行 backend 推迟 | 入站用 nftables/TPROXY 透明转发到本地代理是当前可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理/MITM 需要 proxy self-bypass、证书注入、proxy health 和 L7 分类一起建模后再安装 output redirect 规则，不能成为默认 capture 路线。 |
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，并可选启动 agent-managed plain TCP relay；出站 MITM 先保留配置与 plan/status 表达但不安装 output redirect，直到 proxy self-bypass、证书注入和 proxy health 一起落地 | 第 11、18、32 节 |
