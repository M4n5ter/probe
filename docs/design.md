...
| `transparent-interception` | root/net-admin | 透明拦截 lifecycle，覆盖 nftables/TPROXY 入站 host rule、process-scoped setup-time listener proof 和 outbound transparent proxy managed relay data-plane 路径。 |
...
18. `e2e-transparent-linux-outbound-redirect-artifact-netns` 在 root/net-admin 环境下从 explicit `transparent-linux` artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，再执行真实 `nft --check`、安装、规则列表断言和 cleanup 验证。它属于 `linux-artifacts` profile，不启动 agent，不证明 agent runtime outbound proxy self-bypass、original destination recovery 或证书/MITM lifecycle。
...
| 当前拦截验收 | dry-run typed verdict、remote external manifest GET、manifest selector 与主配置 selector 合成、process/traffic scoped dry-run decision、manifest profile unsupported audit、online enforcement manifest reload active planner swap/invalid reload retention/setup-time surface rejection、显式 backend 下的 owner-verified TCP socket destroy audit decision，以及透明拦截 nftables/TPROXY plan/probe/lifecycle、managed relay runtime metrics、隔离 netns 入站截获/转发 E2E、process-scoped setup-time listener proof E2E、outbound host-rule lifecycle fail-closed 单测、external outbound proxy self-bypass plan/check/status 单测、outbound redirect Linux artifact acceptance 和真实 agent outbound transparent proxy data-plane E2E | 第 18、31 节 |
