...
- 强 socket lifetime。
- 透明 HTTPS MITM E2E、自动修改客户端 trust store、ALPN-aware routing、hostname-to-upstream target routing
  和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、`client_trust.mode = "operator_managed"`、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；`product_proxy` backend 从 typed bridge、policy hook、target recovery、CA 或 leaf TLS material 和 upstream trust material 生成 first-party proxy CLI，内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、CA-backed dynamic SNI certificate generation、upstream TLS relay、downstream SNI-derived upstream TLS server name、dynamic flow feed、proxy-side hook deny 执行和 durable delegated decision。透明 HTTPS MITM E2E、强原始归因、ALPN-aware routing、hostname-to-upstream target routing 和自动 client trust store 安装仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 边界：readiness/health probe 只证明配置的本机 TCP endpoint 可达；它不是外部进程 supervisor，不证明 IPv4/IPv6
    ...
    hostname-to-upstream target routing 或 plaintext extraction 成功。
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    TCP 连接目标仍来自配置或 recovered original destination，不进行 hostname-to-upstream target routing。
    仍需独立实现透明 HTTPS MITM E2E、强原始 socket/process attribution、ALPN-aware routing、
    hostname-to-upstream target routing 和自动 client trust store 安装。
...
- 自动 client trust store 安装、透明 HTTPS MITM E2E、ALPN-aware routing、hostname-to-upstream target routing
  和强原始归因仍缺失。
...
构成可验证控制面能力；透明 HTTPS MITM E2E、强原始归因、ALPN-aware routing、hostname-to-upstream target routing
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、ALPN-aware routing、hostname-to-upstream target routing、
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、operator-managed client trust contract、plaintext bridge、proxy-side hook wiring、typed execution evidence、managed backend HTTP data-plane canary、正式 HTTP plaintext proxy deny 执行、正式 proxy 下游/上游 TLS relay、CA-backed dynamic SNI certificate generation、downstream SNI-derived upstream TLS server name、backend-owned action report，以及 gated Linux socket destroy destructive path。 | 透明 HTTPS MITM E2E、ALPN-aware routing、hostname-to-upstream target routing、强原始归因、自动 client trust store 安装、pre-connect deny。 |
...
ALPN-aware routing 或 hostname-to-upstream target routing。四条 privileged/live
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | 透明 HTTPS MITM E2E、自动 client trust store 安装、ALPN-aware routing、hostname-to-upstream target routing、完整 proxy-side 动作执行、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM lifecycle、operator-managed client trust contract 与 plaintext bridge 可配置和报告；`product_proxy` backend 从 typed CA 或 leaf TLS material refs 与 upstream trust refs 合成 first-party proxy TLS 参数，agent 侧 typed material refs 是 MITM material source of truth；upstream TLS 使用 downstream SNI 作为 server name，并以 HTTP Host 作为 fallback；透明 HTTPS MITM E2E、ALPN-aware routing、hostname-to-upstream target routing 和自动 client trust store 安装仍缺失 | 第 11、18、32 节 |
