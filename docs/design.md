...
- 强 socket lifetime。
- 自动修改客户端 trust store、HTTP/2+ ALPN dispatch/routing、完整 L7/TLS proxy 分类和
  DNS discovery 的 privileged transparent E2E 覆盖。
- 完整 TCP 栈恢复。
- 强进程归因。
...
- DNS-discovered upstream selection 是 product proxy 的显式 opt-in 能力：固定 upstream 不能与 route table
  ...
  deadline，应引入独立 resolver。privileged transparent E2E 仍使用显式 upstream route。
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    DNS-discovered upstream selection 已有 product proxy 配置、plan、CLI 和 crate-level data-plane 覆盖；
    privileged transparent E2E 仍使用显式 route。
...
- 自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、强原始归因，以及除 WebSocket
  101 tunnel 以外的非 HTTP transparent allow-path matrix 仍缺失。
- DNS upstream discovery 已是 product proxy 的 opt-in data-plane 能力；其 privileged transparent
  path 仍未进入 E2E matrix。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  ...
  候选地址失败后继续连接和 config/plan/CLI 参数合成。
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  强原始归因、managed-process health-transition audit、HTTP/2+ ALPN dispatch/routing、DNS discovery
  privileged transparent path，以及除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix
  的 E2E 覆盖仍是缺口。
...
以及 opt-in DNS upstream discovery 的配置/plan/CLI/data-plane route miss 与 special-purpose address 过滤行为
...
DNS discovery 的 privileged transparent path E2E 仍未覆盖。
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、HTTP/2+ ALPN dispatch/routing、DNS discovery
privileged transparent path、非 HTTP plaintext deny 的保护动作执行和 L7 分类。
...
- 已证明 product proxy DNS upstream discovery 的显式 opt-in、route 优先级、IANA special-purpose/special-use address 默认拒绝、
  候选地址失败重试和 config/plan/CLI 参数合成；该能力尚未进入 privileged transparent E2E matrix。
...
- Product MITM
  ...
  - 边界：不证明自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、DNS upstream discovery 的 privileged transparent path 或
...
IANA special-purpose/special-use address 默认拒绝和候选地址失败后的顺序重试；RuntimePlan/config tests 证明该策略会进入生成的 proxy CLI。
自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、强原始归因，以及除 WebSocket 101 tunnel
以外的非 HTTP transparent allow-path matrix 不属于当前能力边界。DNS discovery 的 privileged transparent
path E2E 覆盖仍是独立缺口。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | trust store、HTTP/2+ ALPN dispatch/routing、DNS discovery privileged path、非 HTTP transparent allow path、proxy action 和 data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
- P2 聚焦自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、DNS discovery privileged path、
