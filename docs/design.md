...
| L7 MITM | `l7_mitm` | 显式 `inbound_tproxy_mitm` / `outbound_transparent_mitm` strategy 要求外部 MITM backend contract、typed material refs 和 TCP readiness probe；agent 可验证外部 listener 可达，但尚不管理 L7 proxy 进程或明文数据面。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - external backend contract 由第 18 节的 MITM backend contract 定义，包含 backend kind、typed material refs 和
    本机 listener readiness gate。
  ...
  - agent 不管理 external L7 proxy 进程，不证明 proxy 已完成 TLS interception，也不把该 contract 等同于明文 parser 数据面。
  ...
  - 边界：仍需独立实现 client trust/install instructions、periodic external proxy health、L7 lifecycle、plaintext feed bridge、
    policy hook 和审计事件。
...
- capability 不表示 L7 MITM proxy 进程或明文数据面已实现；这些由 `l7_mitm` capability 和 MITM backend contract 单独表达。
...
- L7 MITM 还需要信任材料分发、periodic external proxy health、独立 proxy lifecycle、明文 feed bridge 和 policy hook。
...
- 能力面：MITM strategy 同时要求 `transparent_interception` 与 `l7_mitm`；缺少 external backend contract、readiness target、
  listener readiness 或 material pair 时 fail closed。
...
contract/readiness 的配置、plan、status、check 和 fail-closed 边界已经存在；agent-managed L7 proxy/data-plane 仍未实现。
...
- readiness probe 只证明配置的本机 TCP endpoint 可达；它不证明 IPv4/IPv6 全族监听、TPROXY original-destination
  recovery、TLS handshake、ALPN/SNI routing、plaintext bridge 或 policy hook。
...
- plan 保留 MITM backend、backend readiness probe 和 resolved CA/leaf/upstream trust material refs。
...
- RuntimePlan/check/status 会表达 external MITM backend contract 和 resolved MITM material refs。
...
MITM backend contract 已进入配置、规划和能力报告；agent-managed L7 proxy 进程、明文 bridge 和完整 E2E 仍是后续交付。默认全机
MITM 和 proxy-first 主采集路径不进入目标设计。
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；入站 TPROXY/managed relay 与出站 host-rule/external self-bypass/owner-scoped paths 已接入；external L7 MITM contract 已可配置和报告，agent-managed L7 proxy/data-plane 仍缺失 | 第 11、18、32 节 |
