...
- 已实现 `probe-config` crate 的 TOML runtime config schema，覆盖 capture selection、live capture fallback order、provider-specific nested config、storage、export runtime worker、exporter、TLS material registry、policy、enforcement mode 和 admin Unix socket 的第一版结构；配置解析拒绝未知字段，基础字段校验会拒绝 ambiguous external plaintext feed 配置：当前 external plaintext feed 的 source path 归 `capture.plaintext_feed.path` 所有，不能塞进 TLS material/provider 配置。TLS material 路径不能为空；可被 exporter 引用的 material 需要唯一 id，exporter TLS refs 必须存在、类型匹配，并且 client certificate refs 与 private key ref 必须在单个 exporter 上成对配置；文件存在性属于 runtime/admin status 和实际 exporter 构造观察面。
