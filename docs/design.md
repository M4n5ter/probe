...
透明拦截是独立于现有 socket destroy backend 的 strategy plan，而不是 `linux_socket_destroy` 的分支补丁。`enforcement.interception.strategy = "none"` 是默认值；`"inbound_tproxy"` 由 nftables/TPROXY 将 selector 投影后命中的入站 TCP 流量转发到显式配置的本地代理端口；`"outbound_transparent_proxy"` 由 nftables OUTPUT/dstnat 将 host-rule outbound TCP 流量 redirect 到 agent-managed relay。`enforcement.interception.proxy.mode` 明确区分代理生命周期：`external` 表示本地代理由外部进程提供，`managed_tcp_relay` 表示 agent 启动 plain TCP relay；当前出站透明代理只接受 `managed_tcp_relay`，因为 external proxy 尚无可验证 self-bypass contract。后续 L7 MITM 不能伪装成 managed TCP relay，而必须拥有独立证书、self-bypass、L7 lifecycle，并复用当前配置化 active proxy health probe 模型。`enforcement.interception.selector` 是这一 strategy 自己的作用域；它不替代主配置 selector 和外部 manifest selector。
...
当前 agent composition root 也已接入 host-rule outbound transparent proxy lifecycle：RuntimePlan/check/status 表达 OUTPUT/dstnat artifact、proxy port 和 proxy bypass mark；`run` 用最终 effective selector 投影出的 outbound host-rule scope 构造 `transparent-linux` artifact，拒绝 wildcard remote port、process-scoped outbound setup 和 flow-aware outbound setup；activation 先准备 managed relay plan，包括读取本机地址 inventory 以配置自循环保护，随后执行 `nft --check`，获取同一个 `/run/sssa-probe/transparent-interception/sssa_probe.lock` table-scoped owner lock，best-effort 删除旧 owned table 和同 owner 下保留 policy routing 遗留，启动 agent-managed relay，再安装 OUTPUT/dstnat table。出站 relay 用 Linux original-destination resolver 恢复原始目标，在 upstream connect 前设置 proxy bypass `SO_MARK`，并用本机地址 inventory 拒绝 original destination 指向本地 proxy listener 的自循环；agent-owned webhook/control-plane HTTP connector 会把 outbound proxy bypass mark 设置到连接 socket 上，因此 planned webhook exporter 可与 outbound transparent proxy 共存；setup 或 nft install 失败会 best-effort cleanup 并停止 proxy。当前 `xtask e2e-transparent-tproxy-loopback` 会在独立 network namespace 内创建 pid-backed client netns/veth，让流量真实经过 PREROUTING，并证明 agent-managed relay 能把命中两个原始服务端口的入站连接分别转发到对应 upstream；`xtask e2e-transparent-tproxy-process-loopback` 复用同一真实路径，但在 selector 中加入监听进程名约束，验证 setup-time procfs listener proof 能在所有目标 listener holder 都匹配时安装 host rules 并完成转发；`xtask e2e-transparent-outbound-proxy-loopback` 在独立 network namespace 内启动真实 agent，用 loopback outbound client 触发 OUTPUT redirect，同时让 webhook exporter 在规则仍生效时发送 batch，并通过 upstream 响应、receiver batch 和 admin transparent proxy metrics 精确证明业务连接走 managed relay、webhook control-plane socket 没有增加 relay 计数、original destination upstream connect success、proxy bypass mark 未自捕获以及 shutdown cleanup；`xtask e2e-transparent-linux-outbound-redirect-artifact-netns` 属于 `linux-artifacts` profile，在隔离 network namespace 中从 explicit artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，并执行真实 `nft --check`、安装、规则列表断言和清理。

当前 `transparent_interception` capability 的含义是“所选透明拦截 strategy 的 Linux/root/nft/ip executable lifecycle 可用”。它只由平台 probe 和 agent composition root 的 backend wiring 决定，不把 selector projectability 编码为 capability，也不表示已具备 MITM、flow-aware classifier、external proxy self-bypass 或 process-scoped outbound setup。出站透明代理要求该 capability available，并且只在最终 selector 可投影为 host-rule outbound scope 时进入 activation；process/flow classifier readiness 由独立的 `transparent_process_classifier` 和 `transparent_flow_classifier` capability 表达。process classifier 由 agent 的 procfs TCP listener owner proof probe 注入，在 TCP/tcp6 listener 表和 fd owner scan 都完整可用时以 degraded 方式提供 setup-time listener classification，flow classifier 当前默认 unavailable。
...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；出站透明代理当前只覆盖 host-rule managed relay，后续若要支持 process/flow-scoped outbound setup 或 external proxy，需要把 classifier、external proxy self-bypass contract、target recovery 和 audit/status contract 作为同一个 executable backend 的门控条件；若启用 L7 MITM backend，还必须额外接入证书材料、MITM CA 注入和 L7 分类。
...
- `enforcement.interception.strategy = "none"` 默认不要求额外能力；`"inbound_tproxy"` 要求本地显式 selector、proxy listen port、平台入口层面的 `transparent_interception` capability、`enforcement.mode = "enforce"`、live host capture，并且不能与 connection backend 同时启用，直到真实 composite execution backend 存在；`"outbound_transparent_proxy"` 当前要求 `managed_tcp_relay`、本地显式 selector、proxy listen port、平台入口层面的 `transparent_interception` capability、`enforcement.mode = "enforce"` 和 live host capture，并且同样不能与 connection backend 同时启用。RuntimePlan/check/status 可以通过真实 composition 路径表达 OUTPUT/dstnat artifact、proxy port 和 proxy bypass mark；agent setup scope 校验通过 `transparent-linux` artifact planner 把最终 outbound host-rule scope 渲染成 OUTPUT/dstnat nft artifact，并拒绝 wildcard remote port、process-scoped outbound setup 和 flow-aware outbound setup。webhook exporter 与 `outbound_transparent_proxy` 可以共存：agent-owned webhook/control-plane socket 会继承同一个 outbound proxy bypass `SO_MARK`，避免被本机 OUTPUT redirect 自捕获；file exporter 仍可作为本地交接路径使用。外部 proxy/self-bypass contract、process-scoped outbound setup 和 flow-aware outbound setup 仍 fail closed。
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_transparent_proxy`，并在 runtime plan/status/check 中报告 proxy mode/listen port、独立 selector、按方向投影的 `local_setup_projection`、process/flow classifier capability 和 agent-owned proxy runtime snapshot；`inbound_tproxy` 有 executable Linux lifecycle，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，配置化 active health probe 会主动检查显式 TCP endpoint 并在连续失败达到阈值后降级 health。`outbound_transparent_proxy` 当前要求 `managed_tcp_relay`，可在 host-rule outbound scope 下安装 OUTPUT/dstnat redirect，启动 agent-managed relay，通过 Linux original-destination resolver 恢复目标，并在 upstream connect 前设置 proxy bypass `SO_MARK` 防止自捕获；agent-owned webhook/control-plane HTTP connector 也会使用 outbound proxy bypass mark，使 planned webhook exporter 可与 outbound transparent proxy 共存；RuntimePlan/check/status 会表达 OUTPUT/dstnat artifact、proxy port 和 proxy bypass mark。缺少显式 remote port 的 outbound host rules、process-scoped outbound setup、flow-aware outbound setup 仍 fail closed；`external` proxy mode 仍被拒绝，直到外部 proxy self-bypass contract 可验证。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle、可选 managed TCP relay、基础运行态指标、配置化 active proxy health probe、relay-connect upstream connect telemetry、出站 selector projection、Linux original-destination resolver、typed TCP `SO_MARK` socket factory、RuntimePlan/check/status outbound redirect artifact、shared Linux OUTPUT/dstnat artifact planning、agent outbound host-rule managed relay lifecycle、agent-owned webhook/control-plane bypass、root/net-admin netns Linux artifact E2E 和真实 agent outbound data-plane/control-plane E2E 已接入；L7 MITM 作为后续独立 backend | 入站用 nftables/TPROXY 透明转发到本地代理是已验证可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理当前只支持 host-rule scope + managed relay；`external` proxy self-bypass、process/flow-scoped outbound setup 和 L7 MITM 证书/分类 lifecycle 仍是缺口。 |
