...
- 强 socket lifetime。
- 真实 TLS MITM 明文数据面、upstream TLS relay、自动修改客户端 trust store 和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、`client_trust.mode = "operator_managed"`、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；内置 `traffic-probe-mitm-proxy` 已覆盖 managed-process HTTP plaintext data-plane、dynamic flow feed、proxy-side hook deny 执行和 durable delegated decision。真实 TLS MITM 数据面、upstream TLS relay、强原始归因和自动 client trust store 安装仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    内置 `traffic-probe-mitm-proxy` 进一步证明正式 managed-process proxy 可生成 dynamic `l7_mitm_plaintext`
    ...
    仍需独立实现 TLS MITM 数据面、upstream TLS relay、强原始 socket/process attribution 和自动 client trust store 安装。
...
- L7 MITM lifecycle audit 已进入 durable export；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  backend-owned action report 和正式 HTTP plaintext proxy deny 执行验收已接入。
- 自动 client trust store 安装、TLS MITM 数据面、upstream TLS relay 和强原始归因仍缺失。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  当前正式 HTTP plaintext proxy 已验证本地 deny 执行，完整 TLS data-plane verdict/audit 跟随该 hook 落地，
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  真实 TLS MITM data-plane verdict/audit、upstream TLS relay、强原始归因和 managed-process health-transition audit 的 privileged/E2E
  覆盖仍是缺口。
...
managed backend HTTP data-plane canary、backend-owned action report 和正式 HTTP plaintext proxy deny 执行验收已进入控制面能力；
真实 TLS MITM 数据面、upstream TLS relay、强原始归因和自动 client trust store 安装不属于当前控制面能力。
...
MITM plaintext bridge live fan-in、本机 HTTP JSON policy hook typed delegated decision，以及 managed backend-owned action report。
...
backend-owned hook 和 downstream deny response 可由产品二进制持有。
它不是 proxy 内部实现的密码学证明，也不替代真实 TLS MITM 数据面验证。
自动 client trust store 安装、真实 TLS MITM 数据面、upstream TLS relay 和强原始归因不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | TLS MITM 数据面、upstream TLS relay、自动 client trust store 安装、完整 proxy-side 动作执行、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
