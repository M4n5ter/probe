...
| initial pending | configured refs 缺失、为空或尚未 ready；`key_log_file` 已存在但还没有 TLS 1.3 application traffic secret 时也视为 pending。 | 无 store，candidate 只保留 lookup intent。 |
| active | 全部 configured refs ready，且每个 ref 都贡献至少一个可用于 live auto-binding 的 TLS 1.3 application traffic secret。缺失 `cipher_suite` 的 record 由后续 ServerHello 事实补齐；自带 `cipher_suite` 的 record 必须与 ServerHello selected suite 一致。 | 生成新 active store，并激活 matching waiting candidate。 |
| later pending | 新一轮 refresh 缺失、为空或未 ready。 | 保留旧 active store。 |
...
`key_log_file` 和 `session_secret_file` 是 plaintext decrypt hint material，只能由 TLS plaintext plan 显式引用。`tls.plaintext.decrypt_hints.key_log_refs` 必须指向 `key_log_file` material，`tls.plaintext.decrypt_hints.session_secret_refs` 必须指向 `session_secret_file` material；引用不存在、空字符串、类型错配或同一 refs 列表内重复都会在基础配置校验阶段失败，避免引用 multiplicity 改变后续 secret lookup 语义。runtime plan 会把这些 refs 解析成 typed material plan，agent live composition root 会进一步收敛为单一 typed auto-binding material collection，而不是在 provider/check/runtime 各自拼接 refs；status 会对 configured plaintext refs 做 metadata-only source check，并把 `tls.plaintext.decrypt_hints.refresh_interval_ms` 传入 live decrypt hint wrapper。`check` 会进一步读取显式引用的 plaintext material：`key_log_file` 按 SSLKEYLOGFILE/NSS key log 语法核验三字段、label、context hex 和 secret hex，只输出 entries 与 label count 摘要；live auto-binding 模式下 keylog summary 与 runtime loader 使用同一 live snapshot parser，只把 `\n` 当作完整提交点，未以 `\n` 结束的尾行按正在写入的 partial line 忽略，单独尾随 `\r` 不视为完整行，不让短 secret 半行污染检查或 refresh。live auto-binding 模式下缺失的 `key_log_file` 或 `session_secret_file` 都会在 `check` report 中表达为 pending/not_found，空白 `session_secret_file` 表达为 pending/empty，而不是与 `run` 的 pending material 语义分叉为硬失败。`session_secret_file` 按 JSON Lines 导入契约校验，每条记录要求 `protocol = "tls12" | "tls13"`、`secret_kind`、32-byte hex `client_random`、hex `secret`，并可带 32-byte hex `server_random`、`0x` + 2-byte hex `cipher_suite`、`not_before_unix_ns` 和 `not_after_unix_ns`。当前合法组合是 `tls12 + master_secret`，以及 `tls13 + client_handshake_traffic_secret | server_handshake_traffic_secret | client_application_traffic_secret | server_application_traffic_secret | exporter_secret`；不合法组合在导入边界 fail closed。check report 只输出 entries、protocol/kind counts 和 secret byte min/max 摘要。`key_log_refs` 和 `session_secret_refs` 都会在 agent live composition root 中启用 TLS 1.3 decrypt-hint auto-binding wrapper；agent loader 在打开 primary live provider 前读取并解析 material，后续按 refresh interval 重读同一 refs，并只从可被当前 auto-binding provider 消费的 TLS 1.3 application traffic records 构建 lookup-unique store；`session_secret_file` 与 `key_log_file` 的 TLS 1.3 application traffic record 都可以先缺少 `cipher_suite`，后续在同一 flow 的明文 ServerHello 被观察到后用 selected cipher suite 形成 resolved binding，不回写原始 material record；若 material 自带 suite 与 observed ServerHello selected suite 冲突，则 fail closed。missing/empty 初始 material 会以 pending store 启动；`key_log_file` 已存在但还没有 TLS 1.3 application traffic secret 时不贡献 records，也不阻塞其它 ready material，支持 ClientHello 先到、material 后到的同一握手后续 application record auto-binding；若早期 application record 已透传，后续 record 只能在 bounded sequence candidate window 且当前 chunk 从 TLS record 边界开始时被认证绑定，绑定时会先输出 plaintext gap，不回溯重写已透传密文；非空但非法的初始 material 仍 fail closed，运行期 pending 或失败 refresh 则保留当前 store 并继续 primary capture，只有读到新的可用 store 才替换 provider store。exact duplicate live records 会被去重；同一 lookup 域、同一 secret、互补 metadata 的 records 会合并，例如 `key_log_file` record 可与携带 `cipher_suite` 的 `session_secret_file` record 归并；合并后的记录会重新与完整候选集合做冲突检查，secret、cipher suite、server random 或时间窗口语义冲突的重叠 records 会 fail closed，避免 merged store 把唯一 lookup 变成 ambiguous 后静默透传密文。TLS 1.2 master、TLS 1.3 handshake/exporter 等非 live records 仍可进入 check summary，但不会进入当前 auto-binding provider store。`key_log_refs` 不会改变 libssl uprobe attach 行为，也不会绕过 ClientHello/ServerHello/record sequence 对 best-effort 解密的要求；live keylog tail E2E 仍是后续缺口。
...
agent check 测试覆盖显式 key log refs 与 `session_secret_file` refs 通过 typed store 解析为 secret-free summary、keylog-only live auto-binding material 被 `check` 接受、缺失 live keylog 以 pending/not_found 进入 report、live keylog 未以 `\n` 提交的尾行和尾随 `\r` 半写入都不破坏 check summary、合并后重叠的 `session_secret_file` refs 复用 live loader 语义并让 `check` fail closed，以及非法 key log/session secret material 让 `check` fail closed；agent TLS decrypt hint loader 单元测试覆盖无 decrypt hint refs 时保持 primary provider、配置 `session_secret_refs` 或 `key_log_refs` 后包装 auto-binding provider、empty `session_secret_file` 可作为 pending live material 启动、`session_secret_file` application traffic secret 缺 `cipher_suite` 时仍进入 auto-binding store、`key_log_file` 无 TLS 1.3 application secret 时保持 pending、idle keylog 不阻塞其它 ready keylog material、live keylog append 的 trailing partial line 不破坏完整前缀解析、运行期 refresh 读到损坏 material 时保留当前 store、运行期 refresh 读到 empty material 时不清空当前 store、多 ref refresh 遇到 blocking 缺失/empty 或非 live ref 时不部分替换 active store、多个 `session_secret_file` 合并为同一 store、exact duplicate records 去重、同一 lookup key 的冲突重叠记录 fail closed，以及非法 material fail closed 且不泄漏 secret。capture TLS material 单元测试覆盖 keylog application traffic entries 归一为 session-secret records、无 application traffic keylog 不生成 live store、keylog record 与携带 `cipher_suite` 的等价 session-secret record 合并、合并扩大有效时间窗口后重新检查冲突；capture keylog parser 单元测试覆盖 live snapshot 忽略未以 `\n` 提交的尾行，即使尾行本身刚好满足三字段语法，并把尾随 `\r` 视为未完成行。`Tls13SessionSecretFlowBindingPlanner` 单元测试覆盖 material 缺 `cipher_suite` 时可由 observed ServerHello cipher suite 补齐且不回写 material record、缺两者时 fail closed、material suite 与 observed ServerHello suite 冲突时 fail closed；`Tls13SessionSecretAutoBindingProvider` 单元测试覆盖 ClientHello 后保留不可解密外层 application_data、ServerHello selected cipher suite 通过 handshake facts 写入缺 `cipher_suite` 的 keylog/session-secret candidate intent 并激活 candidate、store 在 ClientHello 后到达仍可绑定后续 application record、store 在首条 application record 已 raw 透传后到达时可用 bounded sequence candidate window 绑定第二条 application record 并先输出 plaintext gap、pre-auth candidate 在认证绑定前使用刷新后的当前 material、认证成功后的 bound stream 保留已冻结 material + resolved cipher suite、同一 `CapturedBytes` 中途绑定、首个 application record 跨 `CapturedBytes` 时先缓冲再绑定，并在认证成功的 application record 处输出 plaintext。当前仍没有 live keylog tail E2E、unbounded/general sequence resynchronization、TLS 1.2 key block decrypt 或 MITM/proxy lifecycle；`e2e-tls-session-secret-auto-binding-loopback` 已覆盖预置 `session_secret_file` 的 privileged/live auto-binding，`e2e-tls-session-secret-material-refresh-auto-binding-loopback` 已覆盖 ClientHello 后写入 `session_secret_file` 的 privileged/live auto-binding；两者都不证明 keylog-derived live path。
...
11. `e2e-tls-session-secret-auto-binding-loopback` 在 root/CAP_NET_RAW 下验证预置 `session_secret_file` material、真实 `agent run`、live libpcap、TLS 1.3 ClientHello observation、session-secret auto-binding、decrypted `tls_session_secret` plaintext、HTTP parser、Lua policy 和 Fjall durable output 的闭环；它使用 synthetic TLS-over-TCP fixture，不证明 keylog-derived live path。
12. `e2e-tls-session-secret-material-refresh-auto-binding-loopback` 在 root/CAP_NET_RAW 下验证 `session_secret_file` 初始为空、通过 admin pipeline metrics 确认至少一个 capture event 在 material 写入前进入 agent，再通过 admin status 等待 `session_secret_refresh.generation` 进入 active，证明后续 material refresh 激活 pending candidate；随后 application record 被 authenticated decrypt，并进入 HTTP parser、Lua policy 和 Fjall durable output。fixture 在 material 写入前只发送 ClientHello、不发送 application record，因此该 capture event 是 ClientHello payload 观察。它不证明已经透传出去的密文可回溯解密。
13. `e2e-transparent-tproxy-loopback` 在 root/net-admin 环境下验证隔离 network namespace 中的 nftables/TPROXY host rule lifecycle 与 managed relay 入站转发。
...
11. `xtask e2e-tls-session-secret-auto-binding-loopback` 使用预置 `session_secret_file` 和 synthetic TLS-over-TCP fixture，在真实 agent + live libpcap primary 下覆盖 TLS 1.3 ClientHello observation、application record authenticated auto-binding、ciphertext suppression、decrypted `tls_session_secret` plaintext HTTP request、policy alert 和 durable spool readback；它证明 preloaded session-secret live path，不证明 keylog-derived live path。
12. `xtask e2e-tls-session-secret-material-refresh-auto-binding-loopback` 使用初始为空的 `session_secret_file` 和 synthetic TLS-over-TCP fixture，在真实 agent + live libpcap primary 下覆盖 ClientHello 发送后、application record 发送前，admin pipeline metrics 已观测到 capture event，material 随后才原子替换写入，admin status 中 `session_secret_refresh.generation` 进入 active，runtime refresh 激活 pending candidate，后续 application record authenticated auto-binding、ciphertext suppression、decrypted `tls_session_secret` plaintext HTTP request、policy alert 和 durable spool readback；它证明同次握手后续 record 的 live `session_secret_file` refresh path，不证明已透传密文可回溯、不证明 keylog-derived live path。
