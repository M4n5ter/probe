...
| `transparent_interception` | Available only when the executable inbound TPROXY Linux/root/nft/ip lifecycle is usable | typed strategy/proxy plan、按方向投影的 `local_setup_projection` reporting、inbound TPROXY nftables/policy-route lifecycle、host owner lock、managed plain TCP relay、proxy runtime status/metrics、active proxy health probe，以及 outbound MITM redirect preview 已进入 RuntimePlan/check/status；preview 记录 OUTPUT/dstnat、proxy port 与 proxy bypass mark，但不生成或安装 nft 脚本。proxy relay 已把 target recovery 建模为显式策略：入站 TPROXY 继续使用 accepted socket `local_addr()`，出站透明代理后续可复用 Linux `SO_ORIGINAL_DST` / `IP6T_SO_ORIGINAL_DST` resolver 原语。`xtask e2e-transparent-tproxy-loopback` 覆盖隔离 network namespace 中的真实 PREROUTING + managed relay 路径，`xtask e2e-transparent-tproxy-process-loopback` 在同一路径上覆盖 process-scoped setup-time listener proof。 | capability 只表示入站 TPROXY executable lifecycle 的 Linux/root/nft/ip 前置条件满足。它不表示 selector 可投影、flow classifier 已实现、MITM 已实现、outbound redirect 可安装或外部 manifest 的最终 selector 已提前校验。`external` proxy mode 仍要求用户提供本地代理；超出 setup-time procfs listener proof 的 process-scoped setup、flow-aware setup 和 outbound MITM runtime install 当前仍 fail closed。outbound redirect preview 不等于 MITM：original-destination resolver 已存在但尚未接入 executable outbound lifecycle；proxy self-bypass socket marking、output redirect lifecycle、证书注入和 L7 MITM proxy lifecycle 仍缺失。 |
...
执行层在加载外部 manifest 后，会把主 `enforcement.selector`、manifest selector 和 `enforcement.interception.selector` 的交集作为最终 effective setup selector 重新投影。process-scoped setup 只有在 process classifier capability 可用、存在 host-rule boundary、local ports 显式、当前每个目标 TCP listener inode 的所有 holder 进程都可经完整 procfs owner scan 归因且明确命中 process selector 时才会安装 host rules；其它 process-scoped 情况和所有 flow-aware setup plan 都以 setup error fail closed，避免把“按应用拦截”的配置静默扩大成全机拦截。当前 executable inbound TPROXY 对 host-rule scope 仍要求具备显式 local port 集合；wildcard local port scope 会 fail closed，因为它会把 proxy listen port 也纳入拦截范围，而当前还没有 proxy self-bypass 或规则排除契约。远程 manifest 可以进一步收窄本地 scope，但不能成为安装 host rules 的唯一 scope。
...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；后续要启用出站 MITM runtime install，应把 proxy self-bypass socket marking、已存在但未接入 lifecycle 的 original-destination resolver、证书材料、MITM CA 注入、output redirect lifecycle 和 L7 分类作为同一个 executable backend 的门控条件。
...
- 不把透明代理/MITM 作为默认核心拦截路径；当前只实现显式启用的 Linux nftables/TPROXY 规则生命周期，并可选启动 agent-managed plain TCP relay。后续 MITM backend 必须显式启用、能力门控、按进程/应用 selector 生效，并独立管理证书材料、proxy self-bypass、output redirect lifecycle 和 L7 分类，不能复用 TLS material 或 libssl uprobe instrumentation 的概念。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle、可选 managed TCP relay、基础运行态指标、配置化 active proxy health probe、relay-connect upstream connect telemetry、出站 selector projection、Linux original-destination resolver 与 RuntimePlan/check/status redirect preview 已接入，出站 MITM 执行 backend 推迟 | 入站用 nftables/TPROXY 透明转发到本地代理是当前可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理/MITM 已能形成 typed redirect preview 并具备 target recovery 原语，但还需要 proxy self-bypass socket marking、证书注入、output redirect lifecycle 与 L7 分类一起落地后才可安装规则，不能成为默认 capture 路线。 |
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；relay 目标恢复已拆成显式策略，出站 MITM 可复用 Linux original-destination resolver 原语；出站 MITM 已有按 outbound selector 投影的 status/check 表达与 RuntimePlan redirect preview，但不安装 output redirect，直到 proxy self-bypass socket marking、证书注入、output redirect lifecycle 和 L7 MITM lifecycle 一起落地 | 第 11、18、32 节 |
