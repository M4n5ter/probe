...
- Runtime status fact：
  ...
  `reconcile_state { mode, state, reason }`。
  ...
  它只能说明最近一次 reconcile 的 target 集合和当次 target provider state，不能解释为持续探活后的当前健康。
- Failure behavior：
...
- continuous per-target provider health：
  `last_reconcile.targets.*.reconcile_state` 说明最近一次成功 reconcile 的 target 状态，
  但尚未做持续 target probe 或 link-level runtime liveness check。
...
- Admin observable status：
  ...
  per-target last-reconcile state。
- Failure overlay：
...
dynamic libssl load、同 PID replacement-style 第二 mapped path 和可卸载 libssl-like 旧映射消失已有 E2E；continuous per-target
health、TLS ringbuf saturation E2E 和 strong ownership 仍是后续边界。
...
| 优先级 | 状态 | 目标 | 主要交付 | 验收信号 |
| --- | --- | --- | --- | --- |
| P0 | Active | 收敛主要缺口，避免 best-effort 被误读成强保证。 | 强 socket lifetime、precise flow-specific lost-event reconstruction、TLS ringbuf saturation E2E、continuous TLS target health。 | capability、metrics、export 和 E2E 状态一致。 |
