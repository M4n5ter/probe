...
| `libssl_uprobe` | Probed degraded/unavailable; degraded is explicitly usable for best-effort TLS plaintext | 已实现 process-scoped discovery/planning、strict TLS artifact preflight、eBPF libssl plaintext artifact contract、userspace Aya loader/provider、ringbuf sample decoder、procfs fd-to-flow resolver、startup attach sidecar builder、attach target reconcile diff model、concrete provider `reconcile_libssl_uprobes(next_plan)` 方法、target-scoped detach primitive，以及 primary live provider + TLS plaintext provider 的 capture multiplexer wiring；best-effort attach 按同一 process/library target 做 session link 提交，先确认 target 同时具备 fd association 与 plaintext recipe，再按 fd association -> lifecycle -> plaintext 顺序 attach，未 ready target 不提交 link，半个 recipe 会 rollback 已 attach link；一旦未提交 target 可能已经执行过会写 TLS state map 的 probe，loader 会 fail-closed 而不是继续启用 sidecar；TLS producer 在 `SSL_set_fd` state 存在时置 `FD_VALID`，userspace 只在该标志存在时用 fd 做 procfs flow resolution；详情见 TLS plaintext 小节。 | 尚未实现 agent runtime 的周期性发现/调度 loop、process-exit 清理、强 fd ownership、async fan-in/backpressure、agent-side provider control handle retention 和 provider 级健康上报；producer 的 fd 关联仍是 best-effort，`FD_VALID` 表示可尝试归因，不等于强 socket ownership 证明，解析成功的 libssl bytes 仍保持 degraded；TLS state maps 目前没有 target/generation fence，unsafe partial attach 只能通过 fail-closed 处理。 |
...
当前 `agent run` 还不是运行中动态 reconcile：agent 只在启动时做一次 procfs scan，生成 startup attach plan，然后把 libssl plaintext provider 作为 best-effort sidecar 接入 `CaptureMultiplexer`。capture concrete provider 已有 `reconcile_libssl_uprobes(next_plan)` 方法：session 是已提交 link 的唯一事实源，probe 基于下一轮 plan 计算 diff；对 stale target 做 target-scoped detach，对 new target 复用同一个 loaded program/session 边界执行 best-effort attach，并且只从 retained target + 实际 committed target 生成下一轮 attach state，避免把 clean-skipped target 误标成已生效。任何 reconcile side effect 失败都会 poison 该 probe，并 best-effort detach 剩余 links，后续 poll/reconcile 返回终止性 provider error，避免继续运行在不可信状态上。后续重复扫描、agent-side provider control handle retention、调度该 handle、process-exit 清理和运行态健康上报仍属于 agent runtime 缺口。
...
- Remaining lifecycle gaps：当前仍缺 agent runtime 的运行中周期性进程发现与调度 loop、process-exit 清理、async fan-in/backpressure、provider 级健康上报和更强的 fd ownership。
...
- 默认/显式 live capture：
  ...
  - TLS plaintext sidecar：libssl plaintext sample ABI、capture bridge、userspace uprobe loader/provider、agent startup sidecar builder、attach target reconcile diff model、concrete provider reconcile 方法、procfs fd-to-flow resolver 和带状态生命周期清理的 eBPF uprobe producer 已存在；startup sidecar 作为 best-effort provider 接入 multiplexer，运行期失败会被禁用并更新 online runtime status/capability reason，但不会停止 required primary capture。
  - Missing：仍未完成 agent runtime 的周期性 process scan/reconcile 调度、process-exit cleanup、async fan-in/backpressure、lost-event feedback 和完整 kernel-side capture program。
...
7. agent dynamic process scan/reconcile 调度和 process-exit cleanup 完成后，再把验收升级为动态 attach 生命周期覆盖。
