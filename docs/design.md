...
| `transparent_interception` | Available only when the executable inbound TPROXY Linux/root/nft/ip lifecycle is usable | typed strategy/proxy plan、按方向投影的 `local_setup_projection` reporting、inbound TPROXY nftables/policy-route lifecycle、host owner lock、managed plain TCP relay、proxy runtime status/metrics、active proxy health probe，以及 outbound MITM redirect preview 已进入 RuntimePlan/check/status；preview 记录 OUTPUT/dstnat、proxy port 与独立 outbound proxy bypass mark，agent 侧已有可从最终 outbound host-rule scope 渲染的 nft artifact，但尚不安装规则。RuntimePlan 显式拆分 inbound TPROXY route mark、outbound proxy bypass mark 和 inbound TPROXY route table，避免把“拦截后本地路由”和“代理自身出站绕过”混成一个 host resource。proxy relay 已把 target recovery 建模为显式策略：入站 TPROXY 继续使用 accepted socket `local_addr()`，出站透明代理后续可复用 Linux `SO_ORIGINAL_DST` / `IP6T_SO_ORIGINAL_DST` resolver 原语；proxy upstream connect path 已具备 typed 可选 pre-connect `SO_MARK` 原语，当前入站 relay 使用 unmarked connect。`xtask e2e-transparent-tproxy-loopback` 覆盖隔离 network namespace 中的真实 PREROUTING + managed relay 路径，`xtask e2e-transparent-tproxy-process-loopback` 在同一路径上覆盖 process-scoped setup-time listener proof。 | capability 只表示入站 TPROXY executable lifecycle 的 Linux/root/nft/ip 前置条件满足。它不表示 selector 可投影、flow classifier 已实现、MITM 已实现、outbound redirect 可安装或外部 manifest 的最终 selector 已提前校验。`external` proxy mode 仍要求用户提供本地代理；超出 setup-time procfs listener proof 的 process-scoped setup、flow-aware setup 和 outbound MITM runtime install 当前仍 fail closed。outbound redirect artifact 不等于 MITM：original-destination recovery 和 pre-connect `SO_MARK` 原语已存在，但 executable outbound activation、证书注入和 L7 MITM proxy lifecycle 仍缺失。 |
...
- `RuntimePlan.enforcement` 是能力事实源：`mode_capability` 只表达 mode 自身的能力需求，当前只有 `dry_run` 需要 `dry_run_enforcement`；`execution_surfaces` 显式列出 `enforce` 使用的执行面；`connection.capability` 只由显式 connection backend 产生；`interception.capability` 只由可执行透明拦截 strategy 产生，当前是入站 TPROXY。`outbound_mitm` 只产生 redirect preview 和 agent-side nft artifact rendering，不把 preview 或 artifact 伪装成 capability requirement。`enforce` 表示允许真实执行，但必须配置且只能配置一个执行面，不再隐式等同于 connection enforcement。
...
`xtask e2e-suite` 是当前端到端验证的统一入口。默认 `cargo run -p xtask --locked -- e2e-suite` 等价于 `--profile baseline`，覆盖 plaintext feed、WebSocket handoff/frame、webhook exporter、file exporter 和 remote enforcement manifest。`cargo run -p xtask --locked -- e2e-suite --list` 会列出全部 case 及其 host capability requirement；`--list-profiles` 会列出命名验证 profile、权限需求和展开后的 case 列表；`--profile <name>` 运行一个命名 profile，且不能与 `--case`、`--include-privileged` 或 `--only-privileged` 混用，避免 profile 语义被调用方隐式改写。`product` profile 是当前产品能力全链路验收入口，按 registry canonical order 运行全部已注册 case；新增 E2E case 会自动进入 `product`，但 registry invariant 只把 baseline、live-core、process-ebpf、tls-plaintext 和 transparent-interception 这类 curated profile 计入专门归属，因此新增 case 仍必须显式进入某个 curated profile 或引入显式例外。`--case <name>` 可定向运行单个或多个 case，且不能与 privilege selection flag 混用；`--include-privileged` 复用 `product` profile 的全量 case selection；`--only-privileged` 只运行特权 case。已有 `e2e-*` 命令只作为定向调试入口；它们也通过同一个 suite case registry 分发，因此 case 名称、执行入口、顺序和 privilege gate 只有一个事实源。

| 入口 | 默认权限 | 覆盖能力 |
| --- | --- | --- |
| `e2e-suite` | user | 非特权 agent durable output、HTTP/WebSocket parser、policy、remote enforcement manifest、webhook/file exporter。 |
| `e2e-suite --profile product` | user、root/CAP_NET_RAW、root/bpffs、root/net-admin | 当前产品能力全链路验证入口；运行全部已注册 case，覆盖 replay/plaintext、live libpcap、admin reload、eBPF process observation、TLS plaintext/material 和 transparent interception。 |
| `e2e-suite --include-privileged` | 取决于 case | 运行全部已注册 case，包括 baseline、libpcap loopback、admin reload live capture、eBPF process observation、libssl plaintext 和 transparent TPROXY。 |
...
| `product` | user、root/CAP_NET_RAW、root/bpffs、root/net-admin | 全部已注册 case；作为阶段提交前或发布前的产品级验收入口。 |
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；relay 目标恢复已拆成显式策略，出站 MITM 可复用 Linux original-destination resolver 原语，proxy connect path 已具备 pre-connect `SO_MARK` 原语；出站 MITM 已有按 outbound selector 投影的 status/check 表达、RuntimePlan redirect preview 和 agent-side OUTPUT/dstnat nft artifact rendering，但不安装 output redirect，直到证书注入、output redirect activation/install 和 L7 MITM lifecycle 一起落地 | 第 11、18、32 节 |
