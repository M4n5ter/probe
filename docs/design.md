...
| L7 MITM | `l7_mitm` | 显式 `inbound_tproxy_mitm` / `outbound_transparent_mitm` strategy 是配置与 RuntimePlan 契约；backend capability 默认不可用并 fail closed。 | 第 11、18、32 节 |
...
| Transparent interception | `transparent_interception`、process/flow classifier capability | 入站 TPROXY 和出站透明代理已有 lifecycle；flow classifier、MITM 代理生命周期和多数 process selector 投影仍缺失。 | 第 18、31 节 |
...
- `l7_mitm`
  ...
  - runtime capability matrix 以 unavailable 状态报告该目标能力，避免后续配置或策略把缺失的 MITM backend 误读为隐式可用。
  - `inbound_tproxy_mitm` 与 `outbound_transparent_mitm` 是显式 strategy，由 config、RuntimePlan、check report 和 validation 共同表达；
    由于 `l7_mitm` 默认不可用，配置会 fail closed。
  ...
  - 边界：必须独立管理 MITM CA、leaf certificate strategy、client trust/install instructions、upstream TLS trust material、
    self-bypass、L7 lifecycle、policy hook 和审计事件。
  - 验收：尚无 L7 MITM backend E2E；当前验证覆盖配置解析、relay 伪装拒绝、双 capability plan 和 runtime validation fail-closed。
...
- L7 MITM 还需要证书注入、信任材料分发和独立 proxy lifecycle。
...
MITM 是目标能力，但它不是证书导入的隐式副作用，也不是默认全机采集路径。MITM 以独立 plaintext/enforcement backend 接入：

- 入站场景通过 nftables/TPROXY 将 selector 命中的连接导向受控本地代理。
- 出站场景通过透明代理/MITM backend 接管 selector 命中的连接。
- MITM backend 必须独立管理 CA/cert/key material、信任注入或证书分发、upstream TLS、self-bypass、L7 lifecycle、policy hook 和审计事件。
...
- `RuntimePlan.enforcement` 是能力事实源：`mode_capability` 只表达 mode 自身的能力需求，当前只有 `dry_run` 需要 `dry_run_enforcement`；
  ...
  和 `proxy.self_bypass` 契约决定。Linux artifact planning 属于 `transparent-linux` artifact owner，agent runtime 可以消费该 artifact，
  但 artifact acceptance 本身不伪装成产品 E2E。`enforce` 表示允许真实执行，但必须配置且只能配置一个执行面，不再隐式等同于 connection enforcement。
...
- 运行面：显式启用的 MITM backend，而不是 `managed_tcp_relay` 的布尔开关。
- 配置面：`inbound_tproxy_mitm` / `outbound_transparent_mitm` 是 config 和 RuntimePlan 的一等 strategy 名称。
- 能力面：MITM strategy 同时要求 `transparent_interception` 与 `l7_mitm`；`l7_mitm` 默认不可用，因此当前配置会在 runtime validation 阶段 fail closed。
- 生命周期面：当前只接受外部 L7 backend 监听端口；`managed_tcp_relay` 是 plain TCP relay，配置为 MITM strategy 时会被拒绝。
- 证书面：独立引用或生成 MITM CA、leaf certificate strategy、client trust/install instructions 和 upstream TLS trust material。
...
透明转发、plain TCP relay、external proxy reserved-mark contract、出站 host-rule lifecycle，以及显式 MITM strategy 的配置/plan/fail-closed
边界已经存在；L7 MITM backend 本身仍未实现。
...
和 agent composition root 的 backend wiring 决定，不把 selector projectability 编码为 capability，也不表示已具备 MITM、flow-aware
classifier、external proxy runtime health proof 或非 UID/GID-only 的 process-scoped outbound setup。出站透明代理要求该 capability
...
audit/status contract 作为同一个 executable backend 的门控条件；实现 L7 MITM backend 时，还必须额外接入证书材料、MITM CA 注入和 L7 分类。
...
当前实现只完成第一版 filesystem material backend：配置校验保证 material id 唯一、exporter refs 存在且 kind 匹配、client cert/private key 在
exporter 上成对出现，并要求引用 TLS material 的 exporter 使用 HTTPS webhook endpoint。runtime plan 为每个 exporter 保留 resolved material
的 `id`、`kind` 和 `path`，供 status、exporter drain 和 file store 共享同一语义。filesystem store 拒绝缺失、symlink、目录、非 regular file
和超过当前大小上限的 material；活跃 webhook exporter 引用的 TLS material source 不可用时，会把对应 exporter 和 health 标为 unavailable，并在原因中带上
material id/kind/path。planned webhook exporter 在实际 drain 时先确认该 sink 有待发送 batch，再按 per-sink refs 通过同一 file store 边界读取
trust anchor/client identity PEM 并构造 hyper-rustls transport；读取或 PEM 解析失败会让对应 drain 失败，空队列不会读取 secret bytes。尚未实现权限强校验、
路径白名单、热加载和非 filesystem secret backend；接入 Vault/KMS/TPM 前，应先把 config/runtime material source 从单一 `path` 提升为 typed
source enum，再引入高一层的 secret material store 负责分发不同 source backend。不能把非文件 backend 直接套在当前 `Path` contract 上。
...
| TLS plaintext | TLS plaintext/material cases | root/bpffs 或 CAP_NET_RAW | libssl plaintext、TLS 1.3 auto-binding、durable output。 | TLS 1.2、unbounded resync、L7 MITM。 |
| Enforcement / interception | enforcement 与 transparent cases | user 或 root/net-admin | dry-run、manifest reload、TPROXY/transparent lifecycle。 | 默认 destructive action、L7 MITM。 |
...
当前仍没有 unbounded/general sequence resynchronization、TLS 1.2 key block decrypt 或 MITM/proxy lifecycle。四条 privileged/live
...
- Transparent outbound
  ...
  - 边界：不证明非 UID/GID-only process selector、flow-aware outbound setup 或 L7 MITM lifecycle。
...
显式 MITM backend 是目标能力，必须按 selector 生效、受 capability gate 约束，并与 eBPF、libpcap、plaintext
provider 主线并行。默认全机 MITM 和 proxy-first 主采集路径不进入目标设计。
...
| P2 | Next | 建立显式 L7 MITM backend。 | MITM CA/material、入站/出站 transparent MITM、self-bypass、L7 policy hook。 | 默认全机 MITM 不可用；显式 MITM 有独立 health/status/audit/E2E。 |
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；入站 TPROXY/managed relay 与出站 host-rule/external self-bypass/owner-scoped paths 已接入；L7 MITM backend 是目标能力但尚未实现 | 第 11、18、32 节 |
...
| 当前拦截验收 | dry-run verdict、remote manifest、socket destroy audit、transparent TPROXY/relay/outbound lifecycle 与真实 agent outbound E2E 已覆盖；L7 MITM 仍未完成 | 第 18、31 节 |
