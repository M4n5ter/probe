...
- 非 UID/GID-only outbound process selector 仍 fail closed，除非满足 `transparent_flow_classifier` executable contract。
...
runtime health proof 或非 UID/GID-only 的 process-scoped outbound setup。
...
- 其它 process-scoped outbound setup 仍以 setup error fail closed。
...
- Transparent outbound
  ...
  - 边界：不证明 no-boundary flow classifier、external proxy flow classifier、强 socket lifetime、
    kernel/cgroup-backed classifier 或完整 L7 MITM lifecycle。
