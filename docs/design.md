...
- 强 socket lifetime。
- 内置 TLS MITM 明文数据面、proxy-side policy hook 和 trust/install lifecycle。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、typed material refs、TCP readiness probe 和 durable lifecycle audit；真实 TLS MITM 数据面仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - backend lifecycle 会输出 provider-level `l7_mitm_audit` event 到 durable export queue。
    event 是 backend-specific typed contract：`external` backend 可记录 `backend_starting`、
    `backend_health_probe_started`、`backend_stopping`、`backend_stopped` 和 `backend_stop_failed`；
    `managed_process` backend 可记录 `backend_starting`、`backend_ready`、`backend_start_failed`、
    `backend_stopping`、`backend_stopped` 和 `backend_stop_failed`。
    audit payload 携带 readiness probe；managed backend 额外携带 program、args_count、working_dir 和可选 process_group；
    failure phase 携带失败原因。该事件不触发 flow-level Lua policy hook。
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；仍需独立实现 client trust/install instructions、
    内置 TLS MITM 数据面和 proxy-side policy hook。
  - 验收：`xtask e2e-mitm-plaintext-bridge-live-sidecar` 在 fresh network namespace 中覆盖 libpcap live primary
    与运行期追加的 external MITM capture-event plaintext bridge sidecar 同时进入 parser/policy/spool/export pipeline。
  - 验收：`xtask e2e-managed-mitm-plaintext-bridge-live-sidecar` 在 fresh network namespace 中覆盖 agent-managed
    backend 启动、readiness 连接后 fixture-owned bridge feed 追加 typed events、admin status 中 `managed_process`
    backend 与 `healthy`/`active` runtime、graceful shutdown 后 backend 进程清理，以及同一 parser/policy/spool/export
    pipeline 闭环。
...
- capability 不表示 TLS MITM 数据面已实现；external/managed MITM listener、material contract 和可选 plaintext bridge
...
- L7 MITM backend contract 可声明 CA/leaf/upstream trust material refs，并要求本机 listener readiness/health gate 可达。
- L7 MITM lifecycle audit 已进入 durable export；信任材料分发、TLS MITM 数据面和 proxy-side policy hook 仍缺失。
...
- export event schema 是 `sssa.probe.event_envelope.subject_origin.json`。
...
- `external` backend 由外部进程提供；`managed_process` backend 由 agent 启动直接子进程。
  显式 stop 和 panic/unwind drop 都会 best-effort 清理其进程组。
- MITM backend 必须独立管理信任注入或证书分发、upstream TLS、self-bypass、TLS/L7 protocol state 和 policy hook；
...
- Agent 可验证 MITM contract、material refs 和 TCP listener readiness，并把 selector 命中流量导向 configured listener。
  `managed_process` 只覆盖进程生命周期；真实 TLS MITM 数据面仍需要 backend 自身实现并通过 plaintext bridge 或后续内置数据面进入 pipeline。
...
- 证书面：独立引用 MITM CA pair、leaf certificate pair 和 upstream TLS trust material；自动生成 CA/leaf、client trust/install instructions
  不属于当前 backend lifecycle contract。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理和 proxy-side policy hook；
  data-plane verdict/audit 跟随该 hook 落地，区别于已经存在的 backend lifecycle audit。
- 验证面：入站 external/managed MITM backend 已有真实 agent E2E，证明 backend lifecycle、managed fixture
  readiness 连接后的 bridge feed fan-in、policy decision、managed process cleanup 和 runtime status；provider-level
  lifecycle audit 由 core/agent 单元测试覆盖；出站 MITM、proxy-side policy hook 和 ongoing health-transition audit 仍是验收缺口。
...
和 fail-closed 边界构成当前控制面。
真实 TLS MITM 数据面、proxy-side policy hook 和 trust/install lifecycle 不属于当前控制面能力。
...
audit/status contract 作为同一个 executable backend 的门控条件；完善 L7 MITM backend 时，还必须接入 trust/install 生命周期、
proxy-side L7 readiness、plaintext bridge hardening、proxy-side policy hook 和 L7 分类。
...
- 落盘内容包括 `sssa.probe.capture_event.origin.json` JSON framed `CaptureEvent`。
- 落盘内容包括 `sssa.probe.event_envelope.subject_origin.json` JSON framed `EventEnvelope`。
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent/MITM control-plane。 | TLS MITM data-plane、出站 MITM audit。 |

MITM control-plane 覆盖 external backend 与 managed-process backend。

### 端到端 fixture

Enforcement/interception 验证面覆盖 TPROXY/transparent lifecycle、MITM external/managed-process
contract 配置/状态/readiness/health gate、managed backend 进程清理，以及 MITM plaintext bridge live fan-in。
eBPF process observation 不证明 ringbuf saturation 处理；TLS plaintext 不证明 unbounded resync。
Enforcement/interception 验证面不覆盖默认 destructive action。
...
当前仍没有 unbounded/general sequence resynchronization、TLS 1.2 key block decrypt 或内置 MITM TLS data-plane。四条 privileged/live
...
- L7 MITM runtime metrics 输出 backend health mode/check counters 和 plaintext bridge mode。
...
- RuntimePlan/check/status 会表达 MITM backend contract、managed process plan、backend health、capture-event plaintext bridge 和 resolved MITM material refs。
...
lifecycle、provider-level lifecycle audit，以及 capture-event plaintext bridge live provider fan-in。
proxy-side policy hook、trust/install lifecycle 和真实 TLS MITM 数据面不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | TLS MITM 数据面、trust/install、L7 policy hook、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM control-plane 可配置和报告；内置 TLS MITM 数据面仍缺失 | 第 11、18、32 节 |
| HTTP 范围 | 当前 HTTP/1.x 优先 | 第 13 节 |
| SSE | 当前一等识别 | 第 13、31 节 |
| WebSocket | 识别 Upgrade、定义 handoff，并解析 frame metadata/fingerprint；不聚合完整 message | 第 13、31 节 |
| payload 默认 | selector 命中目标默认完整 payload，不是全机完整 payload | 第 14 节 |
| 过载语义 | 有界无损 + 显式 degraded/gap | 第 14 节 |
| 策略语言 | mlua + LuaJIT | 第 15 节 |
| LuaJIT 降级 | JIT 不可用时解释模式降级，并标记 capability matrix、policy runtime status 和 metrics | 第 15 节 |
...
| policy bundle reload | 在线 admin `reload_policies` 手动触发 enabled bundles 的新 VM 校验后原子切换 | 第 17、28 节 |
| enforcement policy reload | 在线 admin 手动触发 enforcement policy source 的 validate-then-swap | 第 17、18、28 节 |
...
| 拦截验收 | dry-run verdict、remote manifest、socket destroy audit、transparent lifecycle 与 MITM control-plane 已覆盖 | 第 18、31 节 |
