...
| L7 MITM | `l7_mitm` | 显式 `inbound_tproxy_mitm` / `outbound_transparent_mitm` strategy 要求外部 MITM backend contract；agent 可验证配置和材料引用，但尚不管理 L7 proxy 进程或明文数据面。 | 第 11、18、32 节 |
...
- `l7_mitm`
  - Runtime status：默认 unavailable；显式配置 external L7 MITM backend contract 且 material contract 完整时可为 available。
  - `inbound_tproxy_mitm` 与 `outbound_transparent_mitm` 是显式 strategy，由 config、RuntimePlan、check report 和 validation 共同表达。
  - external backend contract 使用 `[enforcement.interception.mitm] backend = "external"`，并要求导入 MITM CA pair 或 leaf certificate pair。
  ...
  - 边界：仍需独立实现 client trust/install instructions、external proxy health、L7 lifecycle、plaintext feed bridge、policy hook 和审计事件。
  - 验收：尚无 L7 MITM backend E2E；当前验证覆盖配置解析、material ref 校验、relay 伪装拒绝、双 capability plan、status/check report 和 runtime validation。
...
- 当前 managed relay 只是 plain TCP transparent relay。
- external L7 MITM contract 可声明 CA/leaf/upstream trust material refs。
- L7 MITM 还需要信任材料分发、external proxy health、独立 proxy lifecycle、明文 feed bridge 和 policy hook。
...
- MITM backend 必须 capability-gated、selector-scoped，并与 libssl uprobe instrumentation、keylog/session decrypt hints 和 exporter
  trust material 保持独立。
- 当前 agent 可验证 external MITM contract 并把 selector 命中流量导向 external listener；它还没有管理 L7 proxy 进程，也没有把被代理后的明文接回 parser/policy pipeline。
...
- `managed_tcp_relay` + `none`
  ...
  - 当前边界：不做 MITM，不加载证书，不代表 L7 policy point。
- `external` + `uses_reserved_mark`
  ...
  - 当前缺口：agent 不验证外部进程健康或 socket mark 使用。
...
- 能力面：MITM strategy 同时要求 `transparent_interception` 与 `l7_mitm`；缺少 external backend contract 或 material pair 时 fail closed。
- 生命周期面：当前只接受外部 L7 backend 监听端口；agent 负责 selector-scoped redirect，不负责启动或监督 L7 proxy 进程。
...
- L7 生命周期：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、policy hook、audit event 和 health/status contract。
...
contract 的配置、plan、status、check 和 fail-closed 边界已经存在；agent-managed L7 proxy/data-plane 仍未实现。
...
- plan 保留 MITM backend 和 resolved CA/leaf/upstream trust material refs。
...
runtime plan 也会把 `interception.mitm` refs 解析成 typed material plan，供 status/check/report 表达 MITM backend contract。
