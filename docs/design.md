...
| Operator surface | admin/status/prometheus、ratatui TUI | TUI 是服务器本地配置工作台，覆盖常见配置修改和 `/proc` 进程选择；在线流量浏览、policy source 编辑和大型 MITM backend contract 编辑不由 TUI 代替。 | 本节、Admin 章节 |
