...
- 强 socket lifetime。
- 内置 TLS MITM 明文数据面、同步 proxy-side hook transport 和 trust/install lifecycle。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、typed material refs、TCP readiness probe、durable lifecycle audit 和明确的 `l7_mitm_plaintext` data-plane provenance；真实 TLS MITM 数据面和生产 proxy hook transport 仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；仍需独立实现 client trust/install instructions、
    内置 TLS MITM 数据面和同步 proxy-side hook transport。
  ...
  - 验收：fresh network namespace E2E 覆盖入站/出站 MITM strategy 的 agent-managed backend：
    ...
    readiness 连接后 fixture-owned bridge feed 追加 typed events、admin status 中 `managed_process`
    backend 与 `healthy`/`active` runtime、graceful shutdown 后 backend 进程清理，以及同一
    parser/policy/spool/export pipeline 闭环；出站 case 同样用无人监听的 selector remote port
...
- L7 MITM lifecycle audit 已进入 durable export；enforcement crate 定义了 proxy-side hook contract 和 `delegated` outcome。
- RuntimePlan/agent 生产路径、信任材料分发、TLS MITM 数据面和同步 proxy-side hook transport 仍缺失。
...
RuntimePlan/agent 生产路径和真实同步 hook transport 尚未实现；该入口应消费 `EnforcementDecision.effective_action`，
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理和同步 proxy-side hook transport；
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  同步 proxy-side hook transport、真实 TLS MITM data-plane verdict/audit 和 managed-process health-transition audit 的
  privileged/E2E 覆盖仍是验收缺口。
...
真实 TLS MITM 数据面、RuntimePlan/agent proxy hook wiring、同步 proxy-side hook transport 和 trust/install lifecycle
不属于当前控制面能力。
...
proxy-side L7 readiness、plaintext bridge hardening、同步 proxy-side hook transport 和 L7 分类。
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle 和 plaintext bridge。 | 内置 TLS MITM data-plane、RuntimePlan/agent proxy hook wiring、同步 proxy-side hook transport、trust/install lifecycle。 |

MITM lifecycle coverage 包含入站/出站 strategy plan/status、出站 selector redirect canary、
external backend、managed-process backend 和 capture-event plaintext bridge。
...
出站 selector redirect canary，以及 MITM plaintext bridge live fan-in。
...
同步 proxy-side hook transport、trust/install lifecycle 和真实 TLS MITM 数据面不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | TLS MITM 数据面、trust/install、同步 L7 policy hook transport、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
