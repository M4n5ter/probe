...
- 强 socket lifetime。
- 自动修改客户端 trust store、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection
  和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| Protocol | `http1`、`sse`、`websocket_handoff`、`websocket_frame`、`websocket_message` | HTTP/1.x、SSE 和 WebSocket metadata；详细 WebSocket 边界见下方。 | 第 12-14 节 |
...
- `product_proxy` backend 从 typed bridge、policy hook、target recovery、CA 或 leaf TLS material、
  upstream trust material 和显式 upstream route table 生成 first-party proxy CLI。
- 内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、
  ...
  proxy-side hook deny 执行、透明入站 HTTPS routed allow/deny 规则路径和 durable delegated decision。
...
- WebSocket 输出 handoff、frame metadata 和 16 MiB 有界 text/binary message metadata。
- 超限 message 保留 frame metadata 并省略 message event。
- 当前不保存完整 message payload，不解压 extension payload。
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    upstream target；route 命中时覆盖 configured/recovered target，route 未命中时回退 configured
    或 recovered original destination。
    透明入站 HTTPS E2E 覆盖 product proxy routed allow path 和 deny path：fresh network namespace
    client 经 veth/TPROXY 进入 product proxy，使用 operator-managed CA trust 完成下游 TLS handshake；
    allow path 根据 Host route 转发到 upstream TLS server 并写入 request/response plaintext feed；
    deny path 触发 HTTP JSON hook 的 deny verdict，返回代理侧 403，并写入 `l7_mitm_plaintext` feed
    与 durable delegated decision。
    TLS 连接只 advertise `http/1.1` ALPN；无 ALPN 的 legacy client 仍按 HTTP/1.x 处理，明确协商
    非 HTTP/1.1 ALPN 的下游连接会 fail closed，上游 TLS client 同样只 advertise `http/1.1`。
    强原始 socket/process attribution、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection、
    自动 client trust store 安装和非 HTTP transparent allow-path matrix 仍缺失。
  ...
  - 验收：`xtask e2e-product-mitm-proxy-transparent-https-policy-hook`
    覆盖正式 `traffic-probe-mitm-proxy` 作为 first-party `product_proxy` backend 被启动、创建
    capture-event feed、生成 dynamic L7 MITM plaintext flow、在透明入站 HTTPS 规则路径下完成下游 TLS
    handshake、通过本机 HTTP JSON hook 接收 deny action、返回带 policy reason 的 403，并写入
    durable delegated `EnforcementDecision`。
...
- L7 MITM lifecycle audit 由 durable export 表达；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  和透明入站 HTTPS product proxy routed allow/deny path 由对应测试覆盖。
- 自动 client trust store 安装、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection、
  强原始归因和非 HTTP transparent allow-path matrix 仍缺失。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  product proxy 的 HTTP plaintext deny 执行、下游 TLS termination、upstream TLS relay 和透明入站 HTTPS
  routed allow/deny path 已可验证。透明入站 HTTPS E2E 同时覆盖 transparent rule path、TLS termination、
  无 ALPN legacy client、显式 host-to-upstream route、policy hook 和 durable decision readback；
  product proxy crate-level TLS behavior tests 覆盖下游 `http/1.1` ALPN negotiation、h2-only fail closed
  和上游 `http/1.1` ALPN negotiation；
  ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection 和非 HTTP transparent
  allow-path matrix 仍是独立缺口。
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  product proxy 下游 TLS termination、HTTP/1.1 ALPN gate 与 upstream TLS relay 由 crate-level TLS
  client/server/feed 测试覆盖。
  product proxy 透明入站 HTTPS routed allow/deny data-plane verdict/audit 由 privileged netns E2E 覆盖；
  强原始归因、managed-process health-transition audit、ALPN-based multi-protocol routing、通配或 DNS-discovered
  upstream route selection 和非 HTTP transparent allow-path matrix 的 privileged/E2E 覆盖仍是缺口。
...
下游/上游 TLS relay、HTTP/1.1 ALPN gate 以及 product proxy 透明入站 HTTPS routed allow/deny path
构成可验证控制面能力；强原始归因、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream
route selection、自动 client trust store 安装和非 HTTP transparent allow-path matrix 不属于当前控制面能力。
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection、
...
- 已证明 managed backend HTTP data-plane canary、正式 HTTP plaintext proxy deny 执行、正式 proxy
  ...
  server name、显式 host-to-upstream route、product proxy 透明入站 HTTPS routed allow/deny path、
...
当前仍没有 unbounded/general sequence resynchronization、TLS 1.2 key block decrypt、
ALPN-based multi-protocol routing 或通配/DNS-discovered upstream route selection。四条 privileged/live
...
透明入站 HTTPS product proxy routed allow/deny path 由 privileged netns E2E 覆盖，证明 TPROXY
规则路径、下游 TLS handshake、显式 host-to-upstream route、upstream TLS relay、HTTP JSON hook deny、
代理侧 403、plaintext bridge、response plaintext bridge 和 durable delegated decision 属于同一可执行路径。
该 privileged E2E 使用无 ALPN legacy client，证明透明规则路径不要求客户端发送 ALPN。
product proxy crate-level TLS behavior tests 证明下游 `http/1.1` ALPN negotiation、h2-only fail closed
和上游 `http/1.1` ALPN negotiation。自动 client trust store 安装、ALPN-based multi-protocol routing、
通配或 DNS-discovered upstream route selection、强原始归因和非 HTTP transparent allow-path matrix
不属于当前能力边界。
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM lifecycle、trust、bridge、product proxy TLS 和透明入站 HTTPS routed allow/deny path 的事实源见对应章节。 | 第 11、18、32 节 |
| HTTP 范围 | 当前 HTTP/1.x 优先 | 第 13 节 |
| SSE | 当前一等识别 | 第 13、31 节 |
| WebSocket | 识别 Upgrade、定义 handoff，解析 frame metadata/fingerprint，并输出有界 message metadata；完整边界见对应章节。 | 第 13、31 节 |
