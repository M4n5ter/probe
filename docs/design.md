...
| `ebpf` | Degraded when explicit and procfs socket attribution is usable / unavailable on failed host, object, contract, or procfs preflight | host probe、`aya-obj` object preflight、strict process artifact contract、shared ABI、kernel connect、accept/accept4、close 和 payload tracepoint observation、userspace `aya` loader、ringbuf decoder、TGID+thread PID+fd lookup 到 `ConnectionOpened` bridge、`capture.deep_observe_selector` 命中 flow 后由 userspace 将 `(tgid, fd, flow-start fd-table epoch, read/write direction mask)` 写入 allow map 的 bounded outbound `write(2)` / `sendto(2)` / `writev(2)` / `sendmsg(2)` argument sample 与 inbound `read(2)` / `recvfrom(2)` / `readv(2)` / `recvmsg(2)` result sample 到 always-degraded `CapturedBytes`/`Gap` bridge、best-effort descriptor close 到 `ConnectionClosed` lifecycle event、unresolved connect/accept 到 degraded `Gap` 的 provider wiring 已存在；outbound sample 只有 write direction 被授权时才在 syscall enter 通过 allow gate 后捕获 bounded prefix，syscall exit 只根据实际返回长度裁剪或降级，避免 exit 后重读可变用户 buffer；inbound sample 只有 read direction 被授权时才在 syscall enter 通过 allow+epoch gate 后保存用户 buffer 指针，syscall exit 仅在成功返回后读取 kernel 已写回的用户 buffer bounded prefix；vector syscall 只读取第一个 iovec/buffer prefix，并把返回长度超过已捕获 prefix 的部分表达为 gap；connect/accept fd 归因 retry 已拆成跨 poll 的状态机，避免 provider 在 serial multiplexer 中长时间 sleep。 | 仍是 degraded：当前只覆盖已解析且 selector 授权 flow 上的 outbound single-buffer/first-iovec syscall argument snapshot 和 inbound single-buffer/first-iovec syscall result buffer snapshot；outbound sample 不是内核已发送字节的强证明，inbound sample 也只是 syscall 返回后的用户 buffer 观察，不等于完整 socket 流。full multi-iovec continuation、lost-event conversion、partial-write retry 语义和完整 kernel/socket-path traffic capture program 尚未完成；descriptor close + fd-table epoch 是保守生命周期边界，不是强 socket lifetime，dup/fork/fd passing、process exit 或 lost event 仍可能导致生命周期降级；`auto` 会跳过 degraded eBPF observation provider，显式 `capture.selection = "ebpf"` 可用于 process observation 验证。 |
...
| object preflight | `ebpf-object` 使用 `aya-obj` 解析 object，校验 process artifact 的 `SSSA_EVENTS` ringbuf map、`SSSA_ALLOWED_SOCKET_FDS` LRU allow map、`SSSA_FD_TABLE_EPOCHS` hash epoch map、`SSSA_PENDING_ACCEPTS` hash map、`SSSA_PENDING_WRITES` hash map、`SSSA_PENDING_WRITE_SCRATCH` per-CPU pending scratch map、`SSSA_PENDING_READS` hash map、`SSSA_PROCESS_EVENT_SCRATCH` per-CPU write output scratch map、`SSSA_PROCESS_READ_EVENT_SCRATCH` per-CPU read output scratch map，以及 connect enter、accept/accept4 enter/exit、close enter、dup/dup2/dup3/fcntl/close_range/process_exit/process_exec lifecycle、write/writev/sendto/sendmsg enter/exit、read/readv/recvfrom/recvmsg enter/exit 共 29 个 tracepoint programs。TLS plaintext artifact 仍校验 uprobe/uretprobe programs、TLS state maps、section、map shape、pinning 和同一次 hardened read bytes；process map spec 和 tracepoint spec 都由 `ebpf-abi` 的 canonical table 派生。通用 contract 默认允许额外 map/program，process observation runtime、TLS plaintext loader、agent provider descriptor、agent `libssl_uprobe` capability probe 和 `xtask check-ebpf` 对具体 artifact 使用 strict inventory policy，防止 process artifact 夹带 TLS maps/programs。 | TLS library replacement lifecycle e2e；process 侧仍缺 full multi-iovec continuation、lost-event conversion 和完整 kernel/socket-path traffic capture 验收。 |
| shared ABI | `ebpf-abi` 定义 ringbuf event envelope、公共 event header decoder、ABI revision、connect/accept/close/payload observation decoder、按 kind 区分长度的 process wire record、bounded socket write/read sample record、bounded libssl plaintext sample record，以及 TLS BPF state map contract layout；single-buffer、first-iovec outbound/inbound 和 TLS payload record 都带 original length、fixed-size payload 和 truncated/read-failed flags。超出样本上限或 vector syscall 未捕获后续 iovec 的字节必须由 userspace bridge 表达为 gap；read-failed record 不能携带 payload bytes；payload sample 若 `captured_len < original_len` 但没有 truncated/read-failed flag，decoder fail closed，避免伪造连续 payload。 | socket cookie、lost-event 等更多 socket event record 或 full scatter/gather payload collector。 |
| kernel object | `ebpf-program` 使用 `aya-ebpf` 生成独立 process observation artifact 和 TLS plaintext artifact：process artifact 包含 connect、accept/accept4、close tracepoint observation，并用 sys_enter/sys_exit `write(2)` / `sendto(2)` / `writev(2)` / `sendmsg(2)` 做 bounded outbound syscall argument sample，用 sys_enter/sys_exit `read(2)` / `recvfrom(2)` / `readv(2)` / `recvmsg(2)` 做 bounded inbound syscall result sample。connect/accept record 携带当时的 TGID-scoped fd-table epoch；payload 采样前会先在内核侧确认 `(tgid, fd)` 存在于 userspace allow map，且 allow epoch 非 0、等于当前 TGID epoch，并且 allow direction mask 覆盖当前 read/write 方向；未授权 fd、缺失 epoch 或方向未授权都不会执行 `bpf_probe_read_user_buf`。通过 gate 后，outbound enter tracepoint 用 pending scratch 读取 bounded prefix，并把专用 pending sample 放入 `SSSA_PENDING_WRITES`；outbound exit tracepoint 只根据 syscall 返回值原子更新 pending sample，再用 output scratch 组装最终 wire record，不再 post-write 重读用户 buffer；如果 partial write/send 返回长度短于已捕获长度，则把 captured prefix clamp 到实际返回长度，避免固定 record 泄漏未实际写入的尾部 bytes。inbound enter tracepoint 保存 fd、requested length、first-iovec readable length 和用户 buffer 指针到 `SSSA_PENDING_READS`，inbound exit tracepoint 只在 syscall 成功返回后从用户 buffer 读取 bounded prefix 并组装 inbound sample。vector syscall 当前只读取第一个 iovec/buffer prefix；当 syscall 返回长度超过该 prefix 或样本上限时，后续字节通过 truncated/gap 表达。outbound 样本不能证明这些 bytes 就是 socket 实际发送内容，因为用户态 buffer 可能在 enter tracepoint 与内核 copy 之间被修改；inbound 样本也只是 syscall 返回后的用户 buffer 观察，不覆盖完整 scatter/gather reassembly 或完整 TCP 流。强内容源应来自 TLS uprobe、未来 socket 内核路径、libpcap 重组或透明代理/MITM。provider 只在 connect/accept 解析成 TCP flow 且 `capture.deep_observe_selector` 命中 inbound 或 outbound 时写入 flow-start epoch 和精确方向 mask；`close(fd)` 只删除当前 TGID 下该 fd 的 allow，不再提升全 TGID epoch，避免同进程其它 fd close 误杀活跃 flow；dup/dup2/dup3、`fcntl` fd-dup commands、close_range、TGID leader 的 process exit 以及 process exec 会提升 TGID epoch，使 delayed allow、旧 allow 或 exec 后被 close-on-exec 清理但无 close tracepoint 的 fd fail closed。process artifact 用 pending scratch 承载 enter-time bounded sample，用 output scratch 组装 outbound/inbound payload sample 这种大 record；connect/accept/close 使用具体小 record 输出，避免所有事件都背最大 payload。TLS artifact 包含 libssl plaintext uprobe producer，维护 pending call、`SSL* -> fd` 和 stream offset。 | socket cookie、payload chunk continuation、full multi-iovec continuation、强 fd lifecycle、fd-table identity、kernel-side traffic capture 和 lost event。 |
...
- 默认/显式 live capture：
  ...
  - Missing：TLS lifecycle 缺口见 [Remaining TLS lifecycle gaps](#remaining-tls-lifecycle-gaps)；full multi-iovec continuation、lost-event feedback 和完整 kernel-side capture program 仍未完成。
...
- `e2e-ebpf-process-loopback` 必须先运行 `cargo run -p xtask --locked -- ebpf-build` 和 `cargo build -p agent -p e2e-fixture -p xtask --locked`，并在 root 且 `/sys/fs/bpf` 已挂载 bpffs 的 Linux 环境下运行 `sudo target/debug/xtask e2e-ebpf-process-loopback`。该入口显式选择 `capture.selection = "ebpf"`，配置 process artifact path 和 traffic-only `capture.deep_observe_selector`，用 `any(remote_port = listener, local_port = listener)` 同时命中 client-side connect flow 与 server-side accept flow 的 outbound/inbound 方向，分别用 fixture `--io-mode read-write`、`--io-mode send-recv`、`--io-mode readv-writev` 和 `--io-mode sendmsg-recvmsg` 跑完整 agent loopback；fixture 使用 `--connect-write-delay-ms 2000` 给 client-side connect flow 完成 procfs 归因与 allow-map 授权窗口，并使用 plain HTTP-only 的 `--accept-read-delay-ms 2000` 给 server-side accept flow 完成归因与 inbound read 授权窗口，同时验证 `write(2)` / `read(2)`、`sendto(2)` / `recvfrom(2)`、first-iovec `writev(2)` / `readv(2)` 和 first-iovec `sendmsg(2)` / `recvmsg(2)` payload tracepoint。最终从 Fjall spool 断言 `source = ebpf_syscall`、`provider = ebpf` 的 client/server connection opened/closed、selector-authorized degraded outbound single-buffer/first-iovec sample、selector-authorized degraded inbound single-buffer/first-iovec sample、observation-only enforcement evidence、fixture 进程归因、双方向 HTTP request headers、双方向 HTTP response headers、`ebpf-e2e-policy@e2e` policy alert，以及无 policy runtime error。该入口证明 process artifact load/attach/ringbuf/read、procfs attribution、allow map、HTTP parser、policy 和 durable output 的真实特权闭环；它不把 syscall argument/result snapshot 提升为强 payload proof，也不覆盖 full multi-iovec continuation、lost-event conversion 或 TLS library replacement lifecycle。
