...
- 不实现动态远程控制面、长连接下发或主配置热替换；policy bundle 和 enforcement manifest 只支持当前配置引用 source
  的手动 reload 或显式本地 watcher。
- 不长期保存全量原始流量。
...
- 策略能力：LuaJIT、JIT 状态、policy state API、manual online policy bundle reload、本地 policy bundle watcher，以及后续
...
| Policy | `lua_jit` | 已支持 replay/live pipeline、多个 active bundle、manual reload 和本地 bundle watcher；远程控制面未实现。 | 第 15-17 节 |
...
- 内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、
  ...
  proxy-side hook deny 执行、透明入站/出站 HTTPS routed allow/deny 规则路径、
...
- 强原始归因、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection、
  自动 client trust store 安装，以及除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix 仍缺失。
...
- `lua_jit`
  ...
  - 缺口：主配置 reload、远程控制面下发和 policy state migration 未实现。
...
- `dry_run_enforcement`
  ...
  - 边界：不执行真实连接阻断；在线 reload 不等同于主配置 reload、远程控制面下发、remote source 后台轮询或 setup-time
    host rule 动态更新。
...
- L7 MITM lifecycle audit 由 durable export 表达；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  透明入站/出站 HTTPS product proxy routed allow/deny path 和透明 HTTPS WebSocket tunnel path 由对应测试覆盖。
- 自动 client trust store 安装、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection、
  强原始归因，以及除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix 仍缺失。
...
- 当前仍未实现主配置 reload、远程控制面下发或通用后台 reload worker。
...
- watcher 只监控本地 `policies[].source.kind = "local_directory"`；remote bundle 不做后台轮询。
- watcher 触发后会重载全部 enabled policy sources，包含当前配置引用的 remote bundle。
- 任意 enabled source 加载失败都会保留旧 active policy set。
...
- policy bundle reload 不支持主配置 reload 或远程控制面下发。
...
- watcher 只监听当前 plan 解析出的本地 manifest 路径和父目录；remote source 不做后台轮询。
...
- enforcement policy reload 不支持主配置 reload 或远程控制面下发。
...
remote enforcement manifest source 不代表动态控制面、长连接下发、主配置热替换或 remote 后台轮询。
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  product proxy 透明入站/出站 HTTPS routed allow/deny data-plane verdict/audit 与 WebSocket tunnel path
  由 privileged netns E2E 覆盖；
  强原始归因、managed-process health-transition audit、ALPN-based multi-protocol routing、通配或 DNS-discovered
  upstream route selection，以及除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix
  的 privileged/E2E 覆盖仍是缺口。
...
下游/上游 TLS relay、HTTP/1.1 ALPN gate、product proxy 透明入站/出站 HTTPS routed allow/deny path
以及 product proxy 透明 HTTPS WebSocket tunnel path
构成可验证控制面能力；强原始归因、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream
route selection、自动 client trust store 安装，以及除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix
不属于当前控制面能力。

当前 agent composition root 已接入 Linux inbound TPROXY lifecycle。

- probe 检查 Linux、root、可信系统路径中的 `nft` 和 `ip`。
- 执行层调用受信系统命令生成、校验和安装 nftables/policy-route 状态；没有引入 `nftables` crate 作为 netlink API。
...
- 文件变更触发主配置 validate-then-swap 是后续 config watcher reload 目标；当前不会动态切换 active config。
...
- 不证明 ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route selection、强原始归因、
  自动 client trust store 安装、除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix 或 pre-connect deny。
...
- Product MITM
  ...
  - 覆盖矩阵：
    ...
    | --- | --- | --- | --- | --- |
    | `e2e-product-mitm-proxy-transparent-https-policy-hook` | Inbound TPROXY | Downstream TLS termination、显式 upstream route、上游 TLS relay、HTTP routed allow/deny、`l7_mitm_plaintext` feed、parser/policy/spool output。 | Proxy-side HTTP JSON policy hook 产生 delegated deny decision。 | 不覆盖 outbound original process attribution。 |
    | `e2e-product-outbound-mitm-proxy-transparent-https-policy-hook` | Outbound OUTPUT | Downstream TLS termination、显式 upstream route、上游 TLS relay、HTTP routed allow/deny、`l7_mitm_plaintext` feed、parser/policy/spool output。 | Proxy-side HTTP JSON policy hook 产生 delegated deny decision。 | 断言 original process attribution 指向 transparent TLS client process。 |
    | `e2e-product-mitm-proxy-transparent-https-websocket` | Inbound TPROXY | Downstream TLS termination、显式 upstream route、上游 TLS relay、HTTP Upgrade、101 response、WebSocket text frame tunnel、`l7_mitm_plaintext` feed、WebSocket handoff/frame/message output。 | Endpoint-only policy hook 配置不会产生 delegated decision；policy alert 仍进入 durable output。 | 不覆盖 outbound original process attribution。 |
  ...
  - 边界：不证明自动 client trust store 安装、ALPN-based multi-protocol routing、通配/DNS route discovery 或
...
- HTTP policy-hook path：证明 TPROXY/OUTPUT 规则路径、下游 TLS handshake、显式 host-to-upstream route、
...
通配或 DNS-discovered upstream route selection、强原始归因，以及除 WebSocket 101 tunnel 以外的
非 HTTP transparent allow-path matrix 不属于当前能力边界。
...
- P2 聚焦自动 client trust store 安装、ALPN-based multi-protocol routing、通配或 DNS-discovered upstream route
  selection、除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix、完整 proxy-side 动作执行和 data-plane audit。
...
| policy bundle reload | 在线 admin 手动 reload 或本地 bundle watcher 触发 enabled sources 的新 VM 校验后原子切换 | 第 17、28 节 |
| enforcement policy reload | 在线 admin 手动触发或本地 manifest watcher 触发 enforcement policy source 的 validate-then-swap | 第 17、18、28 节 |
