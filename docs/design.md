...
- 强 socket lifetime。
- 自动修改客户端 trust store、ALPN-aware routing、通配或 DNS-discovered upstream route selection
  和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
- 内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、
  ...
  server name、显式 host-to-upstream route、dynamic flow feed、proxy-side hook deny 执行、透明入站
  HTTPS routed allow/deny 规则路径和 durable delegated decision。
- 强原始归因、ALPN-aware routing、通配或 DNS-discovered upstream route selection、自动 client trust
  store 安装和非 HTTP transparent allow-path matrix 仍缺失。
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    强原始 socket/process attribution、ALPN-aware routing、通配或 DNS-discovered upstream route selection、
...
- 自动 client trust store 安装、ALPN-aware routing、通配或 DNS-discovered upstream route selection、
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  ...
  显式 host-to-upstream route、policy hook 和 durable decision readback；ALPN-aware routing、通配或
  DNS-discovered upstream route selection 和非 HTTP transparent allow-path matrix 仍是独立缺口。
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  强原始归因、managed-process health-transition audit、ALPN-aware routing、通配或 DNS-discovered
...
下游/上游 TLS relay 以及 product proxy 透明入站 HTTPS routed allow/deny path 构成可验证控制面能力；
强原始归因、ALPN-aware routing、通配或 DNS-discovered upstream route selection、自动 client trust
store 安装和非 HTTP transparent allow-path matrix 不属于当前控制面能力。
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、ALPN-aware routing、通配或 DNS-discovered upstream route selection、
...
- 不证明 ALPN-aware routing、通配或 DNS-discovered upstream route selection、强原始归因、
...
ALPN-aware routing 或通配/DNS-discovered upstream route selection。四条 privileged/live
...
自动 client trust store 安装、ALPN-aware routing、通配或 DNS-discovered upstream route selection、
强原始归因和非 HTTP transparent allow-path matrix 不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | trust store、ALPN/wildcard routing、非 HTTP transparent allow path、proxy action 和 data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
- P2 聚焦自动 client trust store 安装、ALPN-aware routing、通配或 DNS-discovered upstream route
