...
- Remaining lifecycle gaps：当前仍缺 provider 级健康上报、async fan-in/backpressure、lost-event feedback、更强的 fd ownership、TLS map target/generation fence，以及 privileged dynamic lifecycle e2e 覆盖。
...
- 默认/显式 live capture：
  ...
  - Missing：仍未完成 async fan-in/backpressure、provider 级健康上报、lost-event feedback、强 fd ownership、TLS map target/generation fence、privileged dynamic lifecycle e2e 和完整 kernel-side capture program。
