...
| `transparent_interception` | Available only for explicitly configured and probed inbound TPROXY | typed strategy/proxy plan、inbound TPROXY nftables/policy-route lifecycle、host owner lock、managed plain TCP relay、proxy runtime status/metrics、active proxy health probe，以及 `local_setup_projection` typed setup plan reporting 已接入；`xtask e2e-transparent-tproxy-loopback` 覆盖隔离 network namespace 中的真实 PREROUTING + managed relay 路径。 | capability 只表示 Linux/root/nft/ip 层面的入站 TPROXY lifecycle 入口可执行，不表示 selector 可投影、process/flow classifier 已实现、MITM 已实现或外部 manifest 的最终 selector 已提前校验。`external` proxy mode 仍要求用户提供本地代理；process-scoped、flow-aware 和 outbound MITM setup 当前均 fail closed。 |
| `transparent_process_classifier` | Unavailable until a classifier backend is wired | RuntimePlan/check/status 已显式报告 process classifier capability；当 `local_setup_projection` 或最终 effective setup selector 需要 process classifier 时，composition error 会引用该 capability，而不是只输出字符串原因。 | 还没有 cgroup/owner mark 或 proxy-side process classification backend；process-scoped transparent interception 仍 fail closed，不会退化成 host-wide rules。 |
...
当前 `transparent_interception` capability 的含义是“入站 TPROXY 规则/策略路由生命周期入口可执行”。它只由 Linux/root/nft/ip probe 决定，不把 selector projectability 编码为 capability，也不表示已具备 MITM、process classifier 或 flow-aware classifier。process/flow classifier readiness 由独立的 `transparent_process_classifier` 和 `transparent_flow_classifier` capability 表达；二者当前默认 unavailable。
...
| `requires_process_classifier` | process expression 和 `host_rule_boundary`。`host_rule_boundary` 明确区分 `no_host_rule_boundary` 与 `scope`，不使用 nullable host scope。 | 当前 fail closed；未来 cgroup/owner mark 或 proxy-side process classification 应消费这个 typed 输入，不能丢弃 process selector。 |
...
执行层在加载外部 manifest 后，会把主 `enforcement.selector`、manifest selector 和 `enforcement.interception.selector` 的交集作为最终 effective setup selector 重新投影。classifier 未实现前，process-scoped 和 flow-aware setup plan 都以 setup error fail closed，避免把“按应用拦截”的配置静默扩大成全机拦截。当前 executable inbound TPROXY 对 host-rule scope 仍要求具备显式 local port 集合；wildcard local port scope 会 fail closed，因为它会把 proxy listen port 也纳入拦截范围，而当前还没有 proxy self-bypass 或规则排除契约。远程 manifest 可以进一步收窄本地 scope，但不能成为安装 host rules 的唯一 scope。

`RuntimePlan.enforcement.interception.classification` 暴露 process classifier 与 flow classifier 的完整 capability state（kind/mode/reason）；`check` 和 status 也输出同一结构。当前这两个 classifier capability 默认 unavailable，因此需要 classifier 的 setup composition 会在执行前以 setup error fail closed，并把缺失的 `transparent_process_classifier` 或 `transparent_flow_classifier` 及其不可用原因写进 block reason。
...
- transparent interception plan 会保留 proxy mode/listen port、resolved nft table、mark、route table、selector 是否配置、`local_setup_projection`、`classification` 和 capability。`local_setup_projection` 是 typed setup plan：`host_rules` 携带 projected host rule scope，`requires_process_classifier` 携带 process expression 与显式 `host_rule_boundary`，`requires_flow_classifier` 携带 transparent-interception-owned classifier selector view 与同样显式的 `host_rule_boundary`。`classification` 报告 `transparent_process_classifier` 与 `transparent_flow_classifier` 的完整 capability state（kind/mode/reason）；这两种 classifier requirement 都不是当前 executable backend，也不会把 `transparent_interception` capability 改写成 unavailable；执行层会以 setup error fail closed。
