...
- `ebpf`
  ...
  - 边界：`SO_COOKIE` 只增强成功 fd lookup 后的身份，不解决 fd 关闭后才解析、dup/fork/fd passing、connect 失败或
    lost event。
  ...
  - 缺口：仍缺 kernel-side socket cookie、connect exit/error 语义、process exit/fd passing 的完整 socket-object lifetime。
...
- Kernel object
  ...
  - process artifact 已覆盖 connect/accept/close/close_range、fd-table epoch、per-fd descriptor generation、
...
- Attribution bridge
  ...
  - 仍缺 kernel-side socket-cookie、connect exit/error event 和 precise flow-specific lost-event reconstruction。
- Build gate
...
- eBPF object 路径放在 `[capture.ebpf] object_path` 下。这个字段作为 `aya-obj` object preflight 输入；process observation runtime 使用
  strict process artifact contract，校验 ringbuf、allow map、fd-table epoch map、fd generation map、pending accept、pending write/read、pending
  write scratch、process write/read event scratch 和 process output loss counter map shape，校验 connect、accept/accept4、close、
  descriptor lease lifecycle、single-buffer/bounded iovec-prefix payload tracepoint sections，并拒绝夹带 TLS maps/programs 的混装
...
| 优先级 | 状态 | 目标 | 主要交付 | 验收信号 |
| --- | --- | --- | --- | --- |
| P0 | Active | 收敛主要缺口，避免 best-effort 被误读成强保证。 | kernel socket-object lifetime、precise flow-specific lost-event reconstruction、TLS ringbuf saturation E2E、TLS kernel link liveness/firing heartbeat。 | capability、metrics、export 和 E2E 状态一致。 |
