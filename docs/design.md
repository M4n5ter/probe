...
| `transparent_interception` | Available only when the executable inbound TPROXY Linux/root/nft/ip lifecycle is usable | typed strategy/proxy plan、按方向投影的 `local_setup_projection` reporting、inbound TPROXY nftables/policy-route lifecycle、host owner lock、managed plain TCP relay、proxy runtime status/metrics、active proxy health probe，以及 outbound transparent proxy redirect preview 已进入 RuntimePlan/check/status；preview 记录 OUTPUT/dstnat、proxy port 与独立 outbound proxy bypass mark。`transparent-linux` artifact planner 拥有 Linux 保留资源与 nft/ip artifact 输入模型，可从显式 artifact spec 与最终 host-rule scope 渲染 nft artifact，但 runtime 尚不安装出站规则。RuntimePlan 显式拆分 inbound TPROXY route mark、outbound proxy bypass mark 和 inbound TPROXY route table，避免把“拦截后本地路由”和“代理自身出站绕过”混成一个 host resource。proxy relay 已把 target recovery 建模为显式策略：入站 TPROXY 继续使用 accepted socket `local_addr()`，出站透明代理后续可复用 Linux `SO_ORIGINAL_DST` / `IP6T_SO_ORIGINAL_DST` resolver 原语；proxy upstream connect path 已具备 typed 可选 pre-connect `SO_MARK` 原语，当前入站 relay 使用 unmarked connect。`xtask e2e-transparent-tproxy-loopback` 覆盖隔离 network namespace 中的真实 PREROUTING + managed relay 路径，`xtask e2e-transparent-tproxy-process-loopback` 在同一路径上覆盖 process-scoped setup-time listener proof；`xtask e2e-transparent-linux-outbound-redirect-artifact-netns` 是单独 Linux artifact acceptance case，在隔离 netns 中从 explicit artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，并执行 `nft --check`、安装、规则列出断言和清理。 | capability 只表示入站 TPROXY executable lifecycle 的 Linux/root/nft/ip 前置条件满足。它不表示 selector 可投影、flow classifier 已实现、MITM 已实现、outbound redirect 可安装或外部 manifest 的最终 selector 已提前校验。`external` proxy mode 仍要求用户提供本地代理；超出 setup-time procfs listener proof 的 process-scoped setup、flow-aware setup 和 outbound transparent proxy runtime install 当前仍 fail closed。outbound redirect artifact 不等于 MITM：original-destination recovery 和 pre-connect `SO_MARK` 原语已存在，但 executable outbound activation、proxy self-bypass 和 target recovery lifecycle 仍缺失；L7 MITM 还需要证书注入和独立 proxy lifecycle；Linux artifact acceptance 也不表示 agent runtime 会安装出站规则。 |
...
- `RuntimePlan.enforcement` 是能力事实源：`mode_capability` 只表达 mode 自身的能力需求，当前只有 `dry_run` 需要 `dry_run_enforcement`；`execution_surfaces` 显式列出 `enforce` 使用的执行面；`connection.capability` 只由显式 connection backend 产生；`interception.capability` 只由可执行透明拦截 strategy 产生，当前是入站 TPROXY。`outbound_transparent_proxy` 只产生 redirect preview；Linux artifact planning 属于 `transparent-linux` artifact owner，不把 preview 或 artifact 伪装成 capability requirement。`enforce` 表示允许真实执行，但必须配置且只能配置一个执行面，不再隐式等同于 connection enforcement。
...
透明拦截是独立于现有 socket destroy backend 的 strategy plan，而不是 `linux_socket_destroy` 的分支补丁。`enforcement.interception.strategy = "none"` 是默认值；`"inbound_tproxy"` 由 nftables/TPROXY 将 selector 投影后命中的入站 TCP 流量转发到显式配置的本地代理端口；`"outbound_transparent_proxy"` 是 typed strategy，当前已有 outbound selector projection、RuntimePlan/check/status 的 redirect preview，以及 `transparent-linux` 从 explicit artifact spec 与最终 outbound host-rule scope 渲染 OUTPUT/dstnat nft artifact 的 plan primitive，但 runtime 仍不可执行，因为出站透明代理必须先把已有 proxy pre-connect `SO_MARK`、original-destination resolver、output redirect activation/install、proxy self-bypass 和 target recovery 组合成同一个可验证 lifecycle，避免代理自身连接被 output redirect 回代理端口。`enforcement.interception.proxy.mode` 明确区分代理生命周期：`external` 表示本地代理由外部进程提供，`managed_tcp_relay` 表示 agent 为入站 TPROXY 启动 plain TCP relay；后续 L7 MITM 不能伪装成 managed TCP relay，而必须拥有独立证书、self-bypass、output redirect activation/install、L7 lifecycle，并复用当前配置化 active proxy health probe 模型。`enforcement.interception.selector` 是这一 strategy 自己的作用域；它不替代主配置 selector 和外部 manifest selector。当前 agent composition root 已接入 Linux inbound TPROXY lifecycle：probe 检查 Linux、root、可信系统路径中的 `nft` 和 `ip`；nft table、inbound TPROXY route mark、outbound proxy bypass mark 和 inbound TPROXY route table 不接受用户配置，执行层固定使用当前保留资源（`sssa_probe`、`0x53534101`、`0x53534102`、`53534`），并在 RuntimePlan、check 和 status 中作为 resolved fields 报告；`run` 在通过完整 RuntimePlan validation、加载外部 manifest 并得到最终 effective selector 后，对最终 inbound nft 脚本执行 `nft --check`，再获取 `/run/sssa-probe/transparent-interception/inbound_tproxy.lock` 这个 host owner lock，清理当前保留 host resources 的所有 IPv4/IPv6 policy routing 遗留；若配置 `managed_tcp_relay`，会在 lock 持有期间按同一 inbound TPROXY plan 的 rule family 启动透明 IPv4/IPv6 listener，然后安装 owned `inet` table 和 `fwmark -> route table` policy routing。policy routing 只按实际 nft rule family 安装：IPv4-only selector 不触碰 IPv6 rule/route，IPv6-only selector 不触碰 IPv4，未限定 remote address 时同时安装两族。退出时删除 owned nftables table 并清理本次安装的 route/rule，再停止 managed relay 并释放 owner lock。setup 失败会 best-effort rollback；不会使用 `flush ruleset` 这类全局破坏性命令。当前 `xtask e2e-transparent-tproxy-loopback` 会在独立 network namespace 内创建 pid-backed client netns/veth，让流量真实经过 PREROUTING，并证明 agent-managed relay 能把命中两个原始服务端口的入站连接分别转发到对应 upstream；`xtask e2e-transparent-tproxy-process-loopback` 复用同一真实路径，但在 selector 中加入监听进程名约束，验证 setup-time procfs listener proof 能在所有目标 listener holder 都匹配时安装 host rules 并完成转发；`xtask e2e-transparent-linux-outbound-redirect-artifact-netns` 属于 `linux-artifacts` profile，在隔离 network namespace 中从 explicit artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，并执行真实 `nft --check`、安装、规则列表断言和清理，证明 artifact 语法与 owned table cleanup，而不接入 agent runtime install。

当前 `transparent_interception` capability 的含义是“入站 TPROXY 规则/策略路由 lifecycle 可执行”。它只由平台 probe 决定，不把 selector projectability 编码为 capability，也不表示已具备 MITM、flow-aware classifier 或 outbound redirect 安装能力。出站透明代理允许 RuntimePlan/check/status 形成 redirect preview，并在 agent setup scope 校验路径中通过 `transparent-linux` artifact planner 把最终 outbound host-rule scope 渲染成 nft artifact；它不要求 `transparent_interception` capability，也不会让该 capability 变成 available，activation/install 继续 fail closed。process/flow classifier readiness 由独立的 `transparent_process_classifier` 和 `transparent_flow_classifier` capability 表达；process classifier 由 agent 的 procfs TCP listener owner proof probe 注入，在 TCP/tcp6 listener 表和 fd owner scan 都完整可用时以 degraded 方式提供 setup-time listener classification，flow classifier 当前默认 unavailable。
...
| --- | --- | --- |
| `host_rules` | projected host rule scope。 | 可进入 executable inbound TPROXY lifecycle；执行层仍要求最终 effective scope 具备显式 local port 集合。 |
...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；后续要启用出站透明代理 runtime install，应把已有 proxy pre-connect `SO_MARK`、已存在但未接入 lifecycle 的 original-destination resolver、proxy self-bypass、output redirect activation/install 和 target recovery 作为同一个 executable backend 的门控条件；若启用 L7 MITM backend，还必须额外接入证书材料、MITM CA 注入和 L7 分类。
...
- 不把透明代理/MITM 作为默认核心拦截路径；当前只实现显式启用的 Linux nftables/TPROXY 规则生命周期，并可选启动 agent-managed plain TCP relay。后续 MITM backend 必须显式启用、能力门控、按进程/应用 selector 生效，并独立管理证书材料、proxy self-bypass lifecycle、output redirect activation/install 和 L7 分类，不能复用 TLS material 或 libssl uprobe instrumentation 的概念。
...
- `enforcement.interception.strategy = "none"` 默认不要求额外能力；`"inbound_tproxy"` 要求本地显式 selector、proxy listen port、平台入口层面的 `transparent_interception` capability、`enforcement.mode = "enforce"`、live host capture，并且不能与 connection backend 同时启用，直到真实 composite execution backend 存在；`"outbound_transparent_proxy"` 当前会保留 outbound selector projection 和 redirect preview，RuntimePlan/check/status 可以通过真实 composition 路径表达 OUTPUT/dstnat、proxy port 与 proxy bypass mark；agent setup scope 校验已能通过 `transparent-linux` artifact planner 把最终 outbound host-rule scope 渲染成 OUTPUT/dstnat nft artifact，并拒绝 wildcard remote port；proxy 内部已有 Linux original-destination resolver 和 pre-connect `SO_MARK` 原语，但它们尚未成为 executable outbound lifecycle 或独立 status contract。`outbound_transparent_proxy` 不要求 `transparent_interception` capability，也不把 preview 或 artifact 伪装成可执行能力；activation/install 仍 fail closed，因为它还缺少 output redirect activation/install、proxy self-bypass 和 target recovery lifecycle。
...
- transparent interception activation 更晚：在 ingress recovery 之后、打开 live capture provider 之前，用最终 effective selector 生成 executable nft 脚本；当前 executable inbound TPROXY 要求该 final scope 具备显式 local port 集合，wildcard local port scope 会 fail closed，直到完整 proxy self-bypass lifecycle 或规则排除契约存在。通过 final scope 校验后执行 `nft --check`，随后获取 `inbound_tproxy` host owner lock 并清理当前保留 host resources 的所有 policy routing 遗留；若配置 `managed_tcp_relay`，会在 lock 持有期间按同一 inbound TPROXY plan 启动透明 listener，通过后再安装规则；provider 构造或 pipeline 失败后仍会先清理 host rules，再停止 managed relay，最后释放 owner lock，guard 也有 best-effort Drop fallback；Ctrl-C/SIGTERM 会先设置 pipeline shutdown flag，让主 run 路径停止轮询并进入 cleanup。
...
| `linux-artifacts` | root/net-admin | Linux artifact acceptance，覆盖尚未进入 runtime install 的透明 Linux artifact 语法、安装和 cleanup 验证。 |
...
17. `e2e-transparent-linux-outbound-redirect-artifact-netns` 在 root/net-admin 环境下从 explicit `transparent-linux` artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，再执行真实 `nft --check`、安装、规则列表断言和 cleanup 验证。它属于 `linux-artifacts` profile，不启动 agent，不安装 runtime outbound transparent proxy，也不证明 proxy self-bypass、original destination recovery 或证书/MITM lifecycle。
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_transparent_proxy`，并在 runtime plan/status/check 中报告 proxy mode/listen port、独立 selector、按方向投影的 `local_setup_projection`、process/flow classifier capability 和 agent-owned proxy runtime snapshot；当前 `inbound_tproxy` 有 executable Linux lifecycle，并独占 `transparent_interception` executable capability requirement，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，配置化 active health probe 会主动检查显式 TCP endpoint 并在连续失败达到阈值后降级 health。`outbound_transparent_proxy` 已有 selector projection、内部 Linux original-destination resolver 原语、proxy pre-connect `SO_MARK` 原语、redirect preview 和 shared Linux OUTPUT/dstnat artifact planning，可在 RuntimePlan/check/status 中表达 OUTPUT/dstnat、proxy port 与 proxy bypass mark，并在 setup scope 校验中拒绝缺少显式 remote port 的 outbound host rules；但 runtime 仍 fail closed，不安装 output redirect 规则，也不要求 `transparent_interception` capability，直到 output redirect activation/install、proxy self-bypass 和 target recovery lifecycle 接入。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle、可选 managed TCP relay、基础运行态指标、配置化 active proxy health probe、relay-connect upstream connect telemetry、出站 selector projection、Linux original-destination resolver、proxy pre-connect `SO_MARK` 原语、RuntimePlan/check/status redirect preview、shared Linux OUTPUT/dstnat artifact planning 和 root/net-admin netns Linux artifact E2E 已接入，出站透明代理执行 backend 推迟，L7 MITM 作为后续独立 backend | 入站用 nftables/TPROXY 透明转发到本地代理是当前可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理已能形成 typed redirect preview、渲染并在隔离 netns 中验证 selector-projected nft artifact，并具备 target recovery 和 pre-connect mark 原语，但还需要 output redirect activation/install、proxy self-bypass 和 target recovery lifecycle 一起落地后才可安装规则；L7 MITM 还需要额外证书注入与 L7 分类，不能成为默认 capture 路线。 |
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；relay 目标恢复已拆成显式策略，出站透明代理可复用 Linux original-destination resolver 原语，proxy connect path 已具备 pre-connect `SO_MARK` 原语；出站透明代理已有按 outbound selector 投影的 status/check 表达、RuntimePlan redirect preview、`transparent-linux` OUTPUT/dstnat artifact planning 和 Linux artifact acceptance，但不安装 output redirect，直到 output redirect activation/install、proxy self-bypass 和 target recovery lifecycle 一起落地；L7 MITM backend 还需要证书注入和 L7 lifecycle | 第 11、18、32 节 |
