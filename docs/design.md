...
- libssl uprobe plaintext sidecar 已有 provider-level、startup agent、dynamic process attach/detach、同 PID dynamic libssl load
  和同 PID replacement-style 第二 libssl mapped path privileged loopback E2E。
...
- 强 socket lifetime。
- 可强制旧 libssl 映射消失的 TLS lifecycle fixture。
...
- `libssl_uprobe`
  ...
  - 已实现：startup attach planning、periodic reconcile sidecar、dynamic process attach/detach E2E、同 PID dynamic libssl load E2E、
    同 PID replacement-style 第二 libssl mapped path E2E。
...
- Object preflight
  ...
  - 仍缺可强制旧 libssl 映射消失的 lifecycle fixture、flow-specific lost-event reconstruction 和完整 kernel/socket-path capture 验收。
- Shared ABI
...
- forced old-mapping disappearance：
  `e2e-tls-plaintext-dynamic-library-loopback` 已证明同一 PID 在 agent ready 后动态加载第一份 libssl 后会被周期
  reconcile attach，并在第一笔 durable TLS plaintext HTTP/policy 输出后释放第一份 `Library` handle、加载第二份
  libssl mapped path、等待 admin status 报告第二个 path active，再产生第二笔 durable TLS plaintext HTTP/policy 输出。
  当第一份 libssl mapping 在 `/proc/<pid>/maps` 中仍可见时，该入口要求 admin status 同时保留第一份 path 并报告第二份 path
  active，因此 OpenSSL `NODELETE` 主机上的 retained+new target 是可观察断言；在允许普通 `dlclose` 卸载旧映射的主机上，
  该入口要求旧 path 不再留在 active target 集合中，从而覆盖 stale+new active-state lifecycle。它仍不证明本机 OpenSSL 3
  的旧 libssl mapping 会从 `/proc/<pid>/maps` 消失；若需要强制验证旧映射消失，需要一个不带 `NODELETE` 的专用 fixture。
...
- 不覆盖可强制旧 libssl 映射消失的 TLS lifecycle。
...
- `e2e-tls-plaintext-loopback` 必须先运行 `cargo run -p xtask --locked -- ebpf-build` 和 `cargo build -p agent -p e2e-fixture
  ...
  output 的真实 agent 闭环；它仍不证明 keylog/session decrypt、可强制旧 libssl 映射消失的 lifecycle 或强 socket ownership。
...
- `e2e-tls-plaintext-dynamic-library-loopback` 必须先运行 `cargo run -p xtask --locked -- ebpf-build` 和 `cargo build -p agent
  ...
  因此 OpenSSL `NODELETE` 主机上的 retained+new target 是可观察断言；在可卸载旧映射的主机上，它要求旧 path 不再留在 active
  target 集合中，从而覆盖 stale+new active-state lifecycle。
  它不证明本机 OpenSSL 3 的旧 mapping 会从 `/proc/<pid>/maps` 消失，也不把 fd-valid 升级为强 socket ownership。
...
- 完整 socket lifetime。
- flow-specific lost-event reconstruction。
- 可强制旧 libssl 映射消失的 TLS lifecycle。
...
- 可强制旧 libssl 映射消失的 TLS lifecycle 仍需 privileged/e2e 覆盖。
...
- 可强制旧 libssl 映射从 `/proc/<pid>/maps` 消失的真实 Aya detach side effect 仍需专用 privileged/e2e fixture 覆盖。
...
- 真实 Aya attach/rollback/detach side effect 的动态进程路径已有 E2E 覆盖；同 PID dynamic libssl load 和同 PID
  replacement-style 第二 mapped path 路径已有 attach 与 active-state E2E 覆盖。
- 可强制旧 libssl 映射消失的同进程路径仍待专用 privileged E2E 补齐。
...
dynamic libssl load 和同 PID replacement-style 第二 mapped path 已有 E2E，可强制旧 libssl 映射消失的 lifecycle、per-target health、
TLS ringbuf saturation E2E 和 strong ownership 仍是后续边界。
...
- TLS plaintext
  - `e2e-tls-plaintext-target-lifecycle-loopback` / `e2e-tls-plaintext-dynamic-library-loopback`，root/bpffs：
    ...
    fd-to-flow 归因和 durable parser/policy 链路。
    不证明强 socket ownership 或旧 libssl 映射一定从 `/proc/<pid>/maps` 消失。
...
- 该入口证明同 PID dynamic library load 和 replacement-style 第二 mapped path。
- 该入口不证明 `dlclose` 后旧 mapping 消失。
- 第一份 libssl mapping 仍可见时，该入口要求 admin status 同时保留第一份 path 并报告第二份 path active；
  OpenSSL `NODELETE` 主机上的 retained+new target 是可观察断言。第一份 mapping 已消失时，该入口要求旧 path 不再留在
  active target 集合中，从而覆盖可卸载旧映射主机上的 stale+new active-state lifecycle。
- 该入口不直接观察 `SSSA_TLS_STATE_EPOCHS` 写 `0` 或新 epoch。
...
1. 引入可强制旧 libssl 映射消失的专用 fixture，覆盖目标进程运行中旧 mapping 从 `/proc/<pid>/maps` 消失后的同进程 stale+new library target 生命周期。
