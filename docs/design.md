...
| TLS 路线 | 非 MITM 优先，当前先完成 explicit key log/session secret material typed store、secret-free check summary 与 libssl uprobe demo；material store 可按 client random 等非 secret key 查找 secret，但仍不解析 TLS record、不执行 key schedule，也不声称已具备 keylog/session decrypt provider | 第 11、31 节 |
