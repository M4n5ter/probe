...
可选 capture-event plaintext bridge 把 backend 明文接入同一 pipeline；
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend 输出的统一 capture events；仍需独立实现 client trust/install instructions、
...
- 配置 capture-event plaintext bridge 时，MITM backend 输出的 typed capture events 可进入 parser/policy pipeline。
...
| Enforcement | enforcement、transparent、MITM cases | user 或 root/net-admin | dry-run、manifest reload、transparent lifecycle、MITM control-plane。 | 内置 TLS MITM data-plane。 |

### 端到端 fixture

Enforcement/interception 验证面覆盖 TPROXY/transparent lifecycle、MITM external/managed-process
contract 配置/状态/readiness/health gate，以及 MITM plaintext bridge live fan-in。
