...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、`client_trust.mode = "operator_managed"`、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；managed backend HTTP data-plane canary 覆盖 backend-owned plaintext feed、proxy-side hook response 和 action report 验收形态。真实 TLS MITM 数据面、生产 proxy 内部动作执行和自动 client trust store 安装仍缺失。 | 第 11、18、32 节 |
...
- L7 MITM lifecycle audit 已进入 durable export；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  RuntimePlan/agent wiring、本机 HTTP JSON hook transport、managed backend HTTP data-plane canary 和
  backend-owned action report 验收已接入。
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  canary，以及 backend-owned action report 已由真实 agent/netns/spool E2E 覆盖。
...
managed backend HTTP data-plane canary 和 backend-owned action report 验收已进入控制面能力；
...
出站 selector redirect canary、managed backend HTTP data-plane canary、MITM plaintext bridge live fan-in、
本机 HTTP JSON policy hook typed delegated decision，以及 managed backend-owned action report。
