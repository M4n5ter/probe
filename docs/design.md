...
- 强 socket lifetime。
- 自动修改客户端 trust store、HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route selection
  和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
- `product_proxy` backend 从 typed bridge、policy hook、target recovery、CA 或 leaf TLS material、
  upstream trust material、非空 application protocol policy 和显式 upstream route table 生成 first-party proxy CLI。
...
- 内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、
  ...
  显式 exact / suffix-wildcard host-to-upstream route、
  dynamic flow feed、
...
- 强原始归因、HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route selection、
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    强原始 socket/process attribution、HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route selection、
    自动 client trust store 安装，以及除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix 仍缺失。
...
- 自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route selection、
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  ...
  HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route selection 和非 HTTP transparent
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  强原始归因、managed-process health-transition audit、HTTP/2+ ALPN dispatch/routing、DNS-discovered
...
构成可验证控制面能力；强原始归因、HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route selection、
...
- 已证明 suffix-wildcard upstream route selection 的 exact 优先、longest-suffix 优先和 inbound transparent
  WebSocket path 覆盖。
- 不证明 HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route selection、强原始归因、
...
HTTP/2+ ALPN dispatch/routing 或 DNS-discovered upstream route selection。四条 privileged/live
...
- Product MITM
  ...
  - 边界：不证明自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、DNS route discovery 或
...
自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | trust store、HTTP/2+ ALPN dispatch/routing、DNS-discovered route selection、非 HTTP transparent allow path、proxy action 和 data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
- P2 聚焦自动 client trust store 安装、HTTP/2+ ALPN dispatch/routing、DNS-discovered upstream route
