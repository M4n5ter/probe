...
- 当前落盘 payload 是带 schema 的 `SpoolPayload`。schema 在代码内由 `SpoolPayloadSchema` enum 表达，写入端不接受裸字符串；unknown schema 只能通过显式 wire decode API 进入 `Other`，避免在 pipeline/export/storage 边界散落字符串。磁盘格式仍保存稳定 wire string，保留向后兼容与未知 schema 透传能力。V0 内容包括 JSON framed `CapturedBytes` 和 JSON framed `EventEnvelope`，用于 replay/debug/export 闭环。该格式必须通过 `payload_schema` 明示，不能伪装成最终 protobuf event schema。
