...
- 已实现 `check` 的 TLS plaintext material 内容核验：对显式引用的 `key_log_file` 通过 `TlsMaterialFileStore` 读取有界 regular file，并按 NSS/SSLKEYLOGFILE 三字段语法解析 label、context 和 secret hex；解析阶段会构造 secret-bearing typed material store，但 check report 只输出 entries 与 label counts，不输出 secret bytes，也不在错误中泄漏 secret 字段值。`session_secret_file` 使用 JSON Lines 导入契约，字段级 schema 集中在 TLS 材料小节说明；`check` 会校验 protocol/secret kind 组合，只输出 entries、protocol/kind counts 和 secret byte min/max 摘要，JSON parse 错误只暴露类别和位置，不回显输入片段。key log 和 session secret lookup 在不能唯一选择 material 时会显式返回 ambiguous，而不是按文件顺序隐式选择第一条；session secret record decrypt core 当前支持 TLS 1.3 application traffic secret + cipher suite metadata 下的 protected record authenticated decrypt，可通过 `Tls13SessionSecretPlaintextAdapter` 输出统一 plaintext event，可由 `Tls13SessionSecretStreamAdapter` 消费有序 ciphertext chunk/gap 后维护 TLS record stream、sequence 和 plaintext offset，可由 `Tls13SessionSecretFlowBindingPlanner` 将已知 client random、typed application traffic secret kind、typed lookup time 与 flow cursor 显式解析成 binding，可由 `Tls13SessionSecretFlowDecryptor` 在该 typed binding 后消费统一 `CaptureEvent` 输出 plaintext events，并可由 `Tls13SessionSecretDecryptingProvider` 包装任意 capture provider，对已显式绑定 stream 输出 `tls_session_secret` plaintext `CaptureEvent`、抑制对应密文、保留未绑定流原样、在 terminal decrypt gap 后继续抑制该 stream 后续密文，并把上游 enforcement evidence/propagation 传播到生成的 plaintext bytes/gap，但仍不声称已具备 keylog/session decrypt live provider、live client-random extraction 或 agent 接入。
...
| TLS 路线 | 非 MITM 优先，当前已有 explicit key log/session secret material typed store、secret-free check summary、TLS 1.3 session-secret protected-record decrypt core、`PlaintextEvent` bridge、capture-layer binding planner、ciphertext stream adapter、显式 flow-binding decrypt core、显式绑定 decrypting provider adapter 与 libssl uprobe demo；material store 可按 client random 等非 secret key 查找 secret，session secret core 可在有 cipher suite metadata 和 sequence 的前提下认证解密 TLS 1.3 application data record，并以 `tls_session_secret` provenance 进入 plaintext 抽象；binding/stream/flow/provider core 已处理 application traffic secret lookup、record framing、ciphertext/plaintext offset、sequence、gap/poison、flow close/observation finish cleanup、绑定流密文抑制、terminal decrypt gap 后继续 suppression、上游 enforcement evidence 传播和未绑定流透传，但仍不声称已具备 automatic keylog/session decrypt live provider、live handshake/client-random extraction 或 agent 接入 | 第 11、31 节 |
