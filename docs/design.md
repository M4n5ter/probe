...
| `transparent_interception` | Available only for explicitly configured and probed inbound TPROXY | typed strategy/proxy plan、inbound TPROXY nftables/policy-route lifecycle、host owner lock、managed plain TCP relay、proxy runtime status/metrics、active proxy health probe，以及 `local_setup_projection` typed setup plan reporting 已接入；`xtask e2e-transparent-tproxy-loopback` 覆盖隔离 network namespace 中的真实 PREROUTING + managed relay 路径，`xtask e2e-transparent-tproxy-process-loopback` 在同一路径上覆盖 process-scoped setup-time listener proof。 | capability 只表示 Linux/root/nft/ip 层面的入站 TPROXY lifecycle 入口可执行，不表示 selector 可投影、flow classifier 已实现、MITM 已实现或外部 manifest 的最终 selector 已提前校验。`external` proxy mode 仍要求用户提供本地代理；超出 setup-time procfs listener proof 的 process-scoped setup、flow-aware setup 和 outbound MITM setup 当前仍 fail closed。 |
...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；后续要实现出站 MITM，应把 proxy self-bypass 契约、证书材料、MITM CA 注入、output redirect lifecycle 和 L7 分类作为同一个 executable backend 的门控条件。
...
- `enforcement.interception.strategy = "none"` 默认不要求额外能力；`"inbound_tproxy"` 要求本地显式 selector、proxy listen port、平台入口层面的 `transparent_interception` capability、`enforcement.mode = "enforce"`、live host capture，并且不能与 connection backend 同时启用，直到真实 composite execution backend 存在；`"outbound_mitm"` 当前会解析成 `transparent_interception` unavailable，因为它还缺少 proxy self-bypass 和 MITM lifecycle。
...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_mitm`，并在 runtime plan/status/check 中报告 proxy mode/listen port、独立 selector、`local_setup_projection`、`transparent_interception` capability、process/flow classifier capability 和 agent-owned proxy runtime snapshot；当前 `inbound_tproxy` 有 executable Linux lifecycle，且可选 `managed_tcp_relay` 由 agent 管理 plain TCP relay，配置化 active health probe 会主动检查显式 TCP endpoint 并在连续失败达到阈值后降级 health，`outbound_mitm` runtime 会 fail closed，不安装 output redirect 规则。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle、可选 managed TCP relay、基础运行态指标、配置化 active proxy health probe 与 relay-connect upstream connect telemetry 已接入，出站 MITM 执行 backend 推迟 | 入站用 nftables/TPROXY 透明转发到本地代理是当前可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理/MITM 需要 proxy self-bypass、证书注入、output redirect lifecycle 与 L7 分类一起建模后再安装 output redirect 规则，不能成为默认 capture 路线。 |
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；出站 MITM 先保留配置与 plan/status 表达但不安装 output redirect，直到 proxy self-bypass、证书注入、output redirect lifecycle 和 L7 MITM lifecycle 一起落地 | 第 11、18、32 节 |
