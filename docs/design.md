...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    enforcement decision 委托给本机 MITM proxy。仍需独立实现 client trust/install instructions、内置 TLS MITM 数据面和 proxy 内部动作执行。
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  proxy-side hook transport、真实 TLS MITM data-plane verdict/audit 和 managed-process health-transition audit 的 privileged/E2E
  覆盖仍是验收缺口。
...
RuntimePlan/agent proxy hook wiring 和本机 HTTP JSON hook transport 已进入控制面能力；
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、plaintext bridge 和 proxy-side hook wiring。 | 内置 TLS MITM data-plane、proxy hook privileged/E2E、proxy 内部动作执行、trust/install lifecycle。 |
...
和本机 HTTP JSON hook transport 属于控制面能力边界。trust/install lifecycle、真实 TLS MITM 数据面和 proxy
内部保护动作执行不属于当前能力边界。
