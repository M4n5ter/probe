...
| `remote_bundle` | `endpoint` 加可选 `max_body_bytes`。 | 对 endpoint 做一次 bounded GET；`max_body_bytes` 约束 response body；response body 是 TOML document，包含 `source` 字符串和 `[manifest]`；解析出的 `source` capped read 语义等同 1 MiB `main.lua`。 |
...
| --- | --- | --- |
| `local_directory` | 目录内包含 `manifest.toml` 和 `main.lua`。 | source entry 拒绝 symlink；通过 no-follow open、handle metadata 和 capped read 读取，避免把目录外文件纳入策略源；`main.lua` 受 executable source size limit 约束。 |
| `remote_bundle` | HTTP response body 是单个 TOML document。 | 执行一次 bounded GET；response body 受 `max_body_bytes` 约束；解析出的 `source` 继续受 executable source size limit 约束；不解包 archive，不解释远程路径，不支持 bundle-local module。 |
...
| `hooks` | Lua callback 列表。 | 使用 typed policy hook enum；TOML/JSON 边界表现为 callback name，例如 `on_http_request_headers`、`on_websocket_handoff`、`on_websocket_frame`。 |
...
通过后原子替换 active set，失败时保留旧 active set。后台 policy bundle file watcher 和动态控制面下发不属于该 contract。
...
remote enforcement manifest source 不代表动态控制面、长连接下发、主配置热替换或后台 watcher。
...
bypass mark 构造 marked plan，agent-owned webhook、remote policy bundle 和 remote enforcement manifest HTTP connector 也复用同一 mark，
...
- configured policy `remote_bundle` 在 `check`/`run`/`reload_policies` 中通过单次 GET 拉取 policy bundle TOML document；
  `max_body_bytes` 限制 HTTP response body，解析出的 `source` 继续受 executable source size limit 约束。
- enforcement remote manifest source 在 `check`/`run`/`reload_enforcement_policy` 中通过单次 GET 拉取 TOML manifest，
...
- `e2e-suite`
  ...
  - 覆盖：非特权 agent durable output、HTTP/SSE/WebSocket parser、flow-level gap/degraded 语义、
    typed provider loss event、policy、remote policy bundle、remote enforcement manifest、webhook/file exporter。
...
- `baseline`
  ...
  - 具体包括：plaintext feed、typed capture-event feed provider loss、SSE、flow-level gap、WebSocket、
    webhook/file exporter、remote policy bundle、remote enforcement manifest。
- `live-core`
...
| 配置 | 当前为单 TOML 主配置 + typed policy source；目录化拆分、目录热加载和 RemoteConfigSource 仍是目标态 | 第 19 节 |
