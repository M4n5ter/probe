...
`key_log_file` 和 `session_secret_file` 是 plaintext decrypt hint material，只能由 TLS plaintext plan 显式引用。`tls.plaintext.decrypt_hints.key_log_refs` 必须指向 `key_log_file` material，`tls.plaintext.decrypt_hints.session_secret_refs` 必须指向 `session_secret_file` material；引用不存在、空字符串、类型错配或同一 refs 列表内重复都会在基础配置校验阶段失败，避免引用 multiplicity 改变后续 secret lookup 语义。runtime plan 会把这些 refs 解析成 typed material plan，status 会对 configured plaintext refs 做 metadata-only source check。`check` 会进一步读取显式引用的 plaintext material：`key_log_file` 按 SSLKEYLOGFILE/NSS key log 语法核验三字段、label、context hex 和 secret hex，只输出 entries 与 label count 摘要；`session_secret_file` 按 JSON Lines 导入契约校验，每条记录要求 `protocol = "tls12" | "tls13"`、`secret_kind`、32-byte hex `client_random`、hex `secret`，并可带 32-byte hex `server_random`、`0x` + 2-byte hex `cipher_suite`、`not_before_unix_ns` 和 `not_after_unix_ns`。当前合法组合是 `tls12 + master_secret`，以及 `tls13 + client_handshake_traffic_secret | server_handshake_traffic_secret | client_application_traffic_secret | server_application_traffic_secret | exporter_secret`；不合法组合在导入边界 fail closed。check report 只输出 entries、protocol/kind counts 和 secret byte min/max 摘要。`session_secret_refs` 会在 agent live composition root 中启用 TLS 1.3 session-secret auto-binding wrapper；agent loader 在打开 primary live provider 前读取并解析 material 一次，然后只从可被当前 auto-binding provider 消费的 TLS 1.3 application traffic records 构建 lookup-unique store；exact duplicate live records 会被去重，不同 live records 若在相同 protocol、secret kind、client random 和有效时间窗口上重叠则 fail closed，避免 merged store 把唯一 lookup 变成 ambiguous 后静默透传密文。TLS 1.2 master、TLS 1.3 handshake/exporter 等非 live records 仍可进入 check summary，但不会进入当前 auto-binding provider store。`key_log_refs` 目前仍只提供导入、检查和状态事实，不会单独启用 live keylog decrypt，也不会改变 libssl uprobe attach 行为。未来 keylog 解密应作为独立 plaintext backend 接入。

当前 `session_secret_refs` 是 live auto-binding 的配置面，不是“仅导入 material”。当 runtime plan 选中 live host capture backend（eBPF 或 libpcap）时，agent preflight 会要求 auto-binding view 至少包含一条可被 `Tls13ApplicationDataDecryptor` 消费的 TLS 1.3 client/server application traffic secret：必须有 `cipher_suite` metadata、suite 在当前支持范围 `0x1301 | 0x1302 | 0x1303` 内，且 traffic secret 长度匹配对应 HKDF hash。TLS 1.2 `master_secret`、TLS 1.3 handshake/exporter secret 或缺少 `cipher_suite` 的 application traffic secret 可以作为 schema 边界被解析或在未来 backend 中使用，但不能单独启用当前 live auto-binding。`check` 与 `run` 复用同一 auto-binding applicability 判断；plaintext feed/replay/no-live mode 下 `check` 只做 material parse 与 secret-free summary，不因当前 live provider 不消费这些 records 而失败。
