...
- remote `max_body_bytes` 会进入 resolved runtime plan。
- remote `max_body_bytes` 只约束 enforcement manifest response body，不约束 Lua policy bundle。
...
- `main.lua` 是本地 bundle source，当前按 1 MiB 上限读取；远程 Lua bundle/package 分发尚未实现。
...
- remote source 的 `max_body_bytes` 是 HTTP response body 预算：
  校验后进入 resolved runtime plan。
- 该预算只保护 remote enforcement manifest 拉取，不表示已支持远程 Lua policy bundle 分发。
...
| `remote` | 保留 endpoint 与已解析的 `max_body_bytes`。 | `check`、`run` 和手动 `reload_enforcement_policy` 按需执行一次 HTTP GET。 |

remote source 不代表动态控制面、长连接下发、主配置热替换或后台 watcher。`max_body_bytes` 是远程 TOML manifest 的 HTTP response
body 预算：默认 16 MiB，配置校验拒绝 0 和超过 512 MiB 的值。该预算用于容纳较大的 remote manifest 并防止无界内存读取；它不是远程 Lua
policy bundle/package 分发能力。Lua policy bundle 仍由 `policies[].path` 指向本地 `manifest.toml + main.lua` 目录。
...
- offline `status` 只做 metadata-only 检查并把健康状态标为 degraded。remote source 在 offline status 中不发网络请求、不声称 manifest
...
- remote source 在 `check` 中通过单次 GET 拉取 TOML manifest，按 plan 中的 `max_body_bytes` 限制 response body，并复用同一套 manifest 校验。
- remote source 在 `run` 中通过单次 GET 拉取 TOML manifest，使用同一 body budget 和 manifest 校验。
- remote source 在手动 `reload_enforcement_policy` 中通过单次 GET 拉取 TOML manifest，使用同一 body budget 和 manifest 校验。
...
- remote source 在 offline status 中只报告 endpoint 和 `max_body_bytes` metadata。
