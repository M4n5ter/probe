...
- TLS plaintext 真实 ringbuf saturation 仍需要真正的 privileged/e2e 验收。
...
- TLS plaintext ringbuf saturation 仍需单独的 privileged E2E。
...
- TLS plaintext 真实 ringbuf saturation 仍需 privileged/e2e 覆盖。
...
| Process eBPF privileged E2E | `xtask e2e-ebpf-process-output-loss` | 真实 process eBPF output ringbuf saturation 产生 provider-scoped `capture_loss`，并向 active tracked flow 输出 unknown-offset `Gap`。 | TLS plaintext ringbuf saturation；durable/admin/prometheus 链。 |
...
- Provider loss
  ...
  - `e2e-ebpf-process-output-loss`，root/bpffs：
    ...
    不证明 TLS plaintext ringbuf saturation 或 durable/admin/prometheus 链。
...
TLS plaintext ringbuf saturation 仍需独立 privileged E2E；该场景必须复用相同的 provider-scoped `capture_loss` 与 flow gap 语义。
