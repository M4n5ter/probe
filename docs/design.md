...
| `libssl_uprobe` | Probed degraded/unavailable; degraded is explicitly usable for best-effort TLS plaintext | 已实现 process-scoped discovery/planning、strict TLS artifact preflight、eBPF libssl plaintext artifact contract、userspace Aya loader/provider、ringbuf sample decoder、procfs fd-to-flow resolver、startup attach sidecar builder，以及 primary live provider + TLS plaintext provider 的 capture multiplexer wiring；best-effort attach 只有在同一 process/library target 上完整 attach fd association recipe 与 plaintext recipe 后才启用 sidecar，半个 recipe 会 rollback 已 attach link；TLS producer 在 `SSL_set_fd` state 存在时置 `FD_VALID`，userspace 只在该标志存在时用 fd 做 procfs flow resolution；详情见 TLS plaintext 小节。 | 尚未实现 agent runtime 的动态发现/reconcile、detach 生命周期、process-exit 清理、强 fd ownership、async fan-in/backpressure 和 provider 级健康上报；producer 的 fd 关联仍是 best-effort，`FD_VALID` 表示可尝试归因，不等于强 socket ownership 证明，解析成功的 libssl bytes 仍保持 degraded。 |
...
当前实现不是动态 reconcile。当前 `agent run` 只做启动时一次性 procfs scan 和 startup attach plan，然后把 libssl plaintext provider 作为 best-effort sidecar 接入 `CaptureMultiplexer`；动态发现、detach/unattach 生命周期和 process-exit 清理仍属于缺口。
...
- Remaining lifecycle gaps：当前仍缺动态进程发现/reconcile、detach/unattach 生命周期、process-exit 清理、async fan-in/backpressure、provider 级健康上报和更强的 fd ownership。
...
`tls.plaintext.selector` 与 enforcement/policy selector 使用同一 AST，用于表达“只对某些应用做 TLS plaintext instrumentation”的 attach 范围。当前 runtime 会编译校验 selector，并在 startup scan/planning 阶段用 process-scope projection 做候选进程剪枝；尚未完成的是运行中的动态发现、reconcile 和 detach。
...
- 默认/显式 live capture：
  ...
  - TLS plaintext sidecar：libssl plaintext sample ABI、capture bridge、userspace uprobe loader/provider、agent startup sidecar builder、procfs fd-to-flow resolver 和带状态生命周期清理的 eBPF uprobe producer 已存在；startup sidecar 作为 best-effort provider 接入 multiplexer，运行期失败会被禁用并更新 online runtime status/capability reason，但不会停止 required primary capture。
  - Missing：仍未完成 agent runtime 的动态 attach lifecycle、process-exit cleanup、async fan-in/backpressure、lost-event feedback 和完整 kernel-side capture program。
...
3. startup libssl sidecar builder 可基于 selector 和 procfs scan 生成 attach plan。
4. best-effort attach readiness 要求同一 process/library target 上 fd association recipe 与 plaintext recipe 都完整。
5. flow 未解析时只输出 gap；解析成功的 libssl bytes 仍标记 degraded。
6. online admin status 能显示 TLS sidecar runtime pending/enabled/disabled，并在 disabled 时暴露原因。
...
7. dynamic process reconcile/detach 完成后，再把验收升级为动态 attach 生命周期覆盖。
