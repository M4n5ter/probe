...
| `libssl_uprobe` | Probed degraded/unavailable; degraded is explicitly usable for best-effort TLS plaintext | 已实现 process-scoped discovery/planning、strict TLS artifact preflight、libssl plaintext eBPF producer、userspace Aya loader/provider、ringbuf sample decoder、procfs fd-to-flow resolver、startup attach planning、periodic reconcile sidecar、online `last_reconcile` status 和 primary+TLS multiplexer wiring；attach safety 使用 target-scoped session commit 和 unsafe partial side effect fail-closed。详情见 TLS plaintext 小节。 | 仍是 degraded；`FD_VALID` 只表示可尝试 fd-based 归因，不等于强 socket ownership。剩余缺口见 [Remaining TLS lifecycle gaps](#remaining-tls-lifecycle-gaps)。 |
...
| userspace loader/provider | `capture::EbpfProcessObservationProbe` 可从 preflighted bytes load、attach process tracepoint specs、打开 ringbuf 和 allow map、解码 typed process observation，并在 provider 授权时把 connect observation 的 thread pid、fd 和 fd-table epoch 写入 `SSSA_ALLOWED_SOCKET_FDS`。`EbpfProcessObservationProvider` 可轮询 ringbuf，把 connect observation 经 TGID+thread PID+fd resolver 转成 `ConnectionOpened`，重试耗尽仍无法解析 connect 时输出 degraded `Gap`；provider 会用 bounded `(thread pid, fd)` 表跟踪 write attribution，仅在 selector 命中且 epoch 有效后授权内核读取后续 write sample，并在 sample 到达时输出 outbound always-degraded `CapturedBytes`，对截断或读失败部分输出 `Gap`，在同 thread descriptor close 命中时输出 best-effort `ConnectionClosed`。allow 写入失败时不留下本地 tracked flow，避免半应用状态。`capture::LibsslUprobePlaintextProvider` 可从 strict TLS artifact preflight、typed attach plan 和 flow resolver 打开，loader 按 attach request 只加载每个 uprobe program 一次，并可对多个 PID/library attach；ringbuf record 先解码为 TLS plaintext sample，再经 flow resolver 转成 `PlaintextEvent` bytes/gap；flow 未解析时只输出 gap，不把明文字节塞进未知 flow。 | 通用 lost-event conversion、强 socket lifetime close、完整 eBPF traffic capture 生命周期、provider metrics；TLS 侧剩余 lifecycle 缺口见 [Remaining TLS lifecycle gaps](#remaining-tls-lifecycle-gaps)。 |
...
| Runtime wiring | startup runtime builder 生成初始 attach result；capture concrete provider 已有动态 reconcile 方法，agent runtime 用 concrete sidecar 按 fixed interval 调度该入口。 |
...
6. flow 未解析时只输出 gap；解析成功的 libssl bytes 仍标记 degraded。
