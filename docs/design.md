...
| Policy | `policy_runtime` | 已支持 replay/live pipeline、多个 active bundle、manual reload、本地 bundle watcher 和 remote bundle polling；主配置 reload 与 policy state migration 仍缺失。 | 第 15-17 节 |
...
- `policy_runtime`
  ...
  - 缺口：主配置 reload、push/streaming 控制面下发和 policy state migration 未实现。
...
- `dry_run_enforcement`
  ...
  - 边界：不执行真实连接阻断；在线 reload 不等同于主配置 reload、push/streaming 控制面下发或 setup-time
    host rule 动态更新。
...
- 当前仍未实现主配置 reload、push/streaming 控制面下发或通用配置后台 reload worker。
...
- policy bundle reload 不支持主配置 reload、目录化配置热加载或 push/streaming 控制面下发。
...
- enforcement policy reload 不支持主配置 reload、目录化配置热加载或 push/streaming 控制面下发。
