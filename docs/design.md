...
- 不实现动态远程控制面、长连接下发、主配置热替换或后台 enforcement watcher；policy bundle 支持 admin 手动 reload
  和显式本地 bundle watcher。
- 不长期保存全量原始流量。
...
- `lua_jit`
  ...
  - 缺口：主配置 reload、远程控制面下发、enforcement policy watcher 和 policy state migration 未实现。
...
- 当前仍未实现主配置 reload、远程控制面下发、enforcement policy watcher 或通用后台 reload worker。
