...
| L7 MITM | `l7_mitm` | 显式 `inbound_tproxy_mitm` / `outbound_transparent_mitm` strategy 要求外部 MITM backend contract、typed material refs 和 TCP readiness probe；可选 capture-event plaintext bridge 把外部 MITM 明文接入同一 pipeline，feed openability 在 capture preflight 阶段 fail closed。agent 仍不管理 L7 proxy 进程。 | 第 11、18、32 节 |
...
- `l7_mitm`
  - Runtime status：默认 unavailable；显式配置 external L7 MITM backend contract、material contract 完整且
    TCP readiness probe 可达时可为 available。
  ...
  - external backend contract 由第 18 节的 MITM backend contract 定义，包含 backend kind、typed material refs
    和本机 listener readiness/health gate；配置 plaintext bridge 时，feed openability 属于 capture provider
    preflight gate。
  - 可选 `plaintext_bridge.mode = "capture_event_feed"` 使用 JSON-lines `CaptureEvent` feed 作为 external MITM plaintext bridge；
    ...
    `ready` 表示 preflight 已打开 feed，`active` 表示 provider 已挂入 live capture mux。
...
- 能力面：MITM strategy 同时要求 `transparent_interception` 与 `l7_mitm`；缺少 external backend contract、readiness target、
  listener readiness 或 material pair 时 fail closed；配置 plaintext bridge 时，feed openability 在 capture preflight 阶段 fail closed。
...
- plaintext bridge openability gate 属于 capture provider preflight，只证明配置的 feed 可打开；它不证明 external proxy 会写入事件，
...
- bridge 文件在 capture preflight 阶段打开一次并交给 build 阶段复用；打开失败 fail closed。
...
- 当前 preflight 覆盖 MITM plaintext bridge feed openability，并把打开的 bridge provider 交给 build 阶段复用。
...
- L7 MITM runtime metrics 输出 external backend health mode/check counters 和 plaintext bridge mode。
