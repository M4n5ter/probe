...
`sssa-probe` 的目标不是做一个传统抓包工具，而是做一个面向 Linux 主机的进程级流量探针。它需要在支持 eBPF 的机器上优先使用 eBPF 获取强进程归因和高性能采集能力；在 eBPF 不可用或能力不足时，自动降级到
...
- HTTP/1.x、SSE 和 WebSocket frame metadata 已可解析。
...
- `capture.deep_observe_selector` 可按方向授权 outbound `write(2)` / `sendto(2)` / `writev(2)` / `sendmsg(2)` 的
  single-buffer 或 bounded first-non-empty-iovec syscall argument sample。
- 同一 selector 授权 inbound `read(2)` / `recvfrom(2)` / `readv(2)` / `recvmsg(2)` 的 single-buffer 或
  bounded first-non-empty-iovec syscall result sample。
- 内核只在 userspace allow map 的 fd-table epoch 与 read/write direction mask 同时匹配时读取 payload。
- vector syscall 只在固定 scan limit 内跳过前导 zero-length iovec，并采样第一个 non-empty iovec 的 bounded prefix。
- 输出 bytes 始终标记 degraded；后续 iovec、scan limit 之外或 sample buffer 之外的字节用 gap 表达。
...
- libssl uprobe plaintext sidecar 已有 provider-level、startup agent、dynamic process attach/detach 和同 PID dynamic libssl load
  privileged loopback E2E。
- TLS artifact 使用单一 global state epoch fence：`SSSA_TLS_STATE_EPOCHS[0]` 防止旧 `SSL*` map state 跨 attach lifecycle 被解释。
...
- 强 socket lifetime。
- 同进程 libssl unload/remap replacement privileged E2E。
- agent-managed L7 MITM 代理进程生命周期与明文数据面。
- 完整 TCP 栈恢复。
- 强进程归因。
...
不允许静默降级。任何能力缺失必须出现在 capability matrix、metrics、admin API 和相关事件 envelope 中；active health 只聚合当前
...
| L7 MITM | `l7_mitm` | 显式 `inbound_tproxy_mitm` / `outbound_transparent_mitm` strategy 要求外部 MITM backend contract、typed material refs、TCP readiness probe，以及配置 plaintext bridge 时的 feed openability；可选 capture-event plaintext bridge 把外部 MITM 明文接入同一 pipeline。agent 仍不管理 L7 proxy 进程。 | 第 11、18、32 节 |
| Protocol | `http1`、`sse`、`websocket_handoff`、`websocket_frame` | WebSocket 只输出 handoff/frame metadata，不聚合完整 message，不解压 extension payload。 | 第 12-14 节 |
| Policy | `lua_jit` | 已支持 replay/live pipeline、多个 active bundle、manual reload；文件 watcher 和远程控制面未实现。 | 第 15-17 节 |
...
| Enforcement | `dry_run_enforcement`、`connection_enforcement` | dry-run 可用；真实 socket destroy 只在显式配置、root/procfs 能力满足且 owner 复核通过时执行。 | 第 18 节 |
| Transparent interception | `transparent_interception`、process/flow classifier capability | 入站 TPROXY 和出站透明代理已有 lifecycle；flow classifier、agent-managed L7 MITM lifecycle 和多数 process selector 投影仍缺失。 | 第 18、31 节 |
...
- `ebpf`
  ...
  - 已实现：kernel connect、accept/accept4、close/plain close_range 和 payload tracepoint observation。
  - 已实现：userspace `aya` loader、ringbuf decoder、TGID+thread PID+fd lookup 到 `ConnectionOpened` bridge。
  - 已实现：`capture.deep_observe_selector` 命中 flow 后，userspace 将 `(tgid, fd, flow-start fd-table epoch, read/write direction mask)`
    写入 allow map。
  ...
  - 已实现：inbound sample 只有 read direction 被授权时，才在 syscall enter 通过 allow+epoch gate 后固定 single-buffer
    或 first-non-empty iovec segment。
  - 已实现：inbound syscall exit 仅在成功返回后读取 enter-time segment 的用户 buffer bounded prefix，避免重新信任可变 iovec descriptor。
  - 已实现：vector syscall 在固定 scan limit 内跳过前导 zero-length iovec，只读取第一个 non-empty iovec 的 bounded prefix。
  - 已实现：返回长度超过已捕获 prefix、后续 iovec、scan limit 或 sample buffer 的部分表达为 gap。
  - 已实现：connect/accept fd 归因 retry 已拆成跨 poll 状态机，避免 provider 在 serial multiplexer 中长时间 sleep。
...
- `l7_mitm`
  - Runtime status：默认 unavailable；显式配置 external L7 MITM backend contract、material contract 完整且
    TCP readiness probe 可达、配置的 plaintext bridge feed 可打开时可为 available。
    在线 runtime 会继续按 backend readiness probe 的 `interval_ms` 周期检查同一 TCP endpoint，
    并在连续失败达到 `failure_threshold` 后把 agent health 降为 degraded。
  - `inbound_tproxy_mitm` 与 `outbound_transparent_mitm` 是显式 strategy，由 config、RuntimePlan、check report 和 validation 共同表达。
  - external backend contract 由第 18 节的 MITM backend contract 定义，包含 backend kind、typed material refs、
    本机 listener readiness/health gate，以及配置 plaintext bridge 时的 feed openability gate。
  - 可选 `plaintext_bridge.mode = "capture_event_feed"` 使用 JSON-lines `CaptureEvent` feed 作为 external MITM plaintext bridge；
  ...
  - agent 能把 selector 命中的连接重定向到 external proxy listener，并在 capability matrix/status/check 中表达该 contract。
  - agent 不管理 external L7 proxy 进程，不证明 proxy 已完成 TLS interception；没有配置 plaintext bridge 时，listener readiness 不能等同于明文 parser 数据面。
  ...
  - 边界：readiness/health probe 只证明配置的本机 TCP endpoint 可达；它不是外部进程 supervisor，不证明 IPv4/IPv6
  ...
  - 边界：capture-event plaintext bridge 只接收 external proxy 输出的统一 capture events；仍需独立实现 client trust/install instructions、
    L7 proxy lifecycle、proxy-side policy hook 和 MITM audit event。
...
- `http1` / `sse` / `websocket_handoff` / `websocket_frame`
  ...
  - 验收：`xtask e2e-websocket-plaintext-feed` 覆盖 HTTP Upgrade、101 response、WebSocket text frame、handoff/frame export、
    Lua hook、connection lifecycle 顺序、无 protocol error 和 durable spool 输出。
  - 边界：不聚合完整 WebSocket message，不解压 extension payload；不支持 HTTP/2、HTTP/3/QUIC。

策略、存储与外发能力：

- `lua_jit`
  - Runtime status：Degraded。
  - 已实现：LuaJIT policy runtime 已接入 replay/live pipeline，支持多个 active bundle 按配置顺序执行。
  ...
  - 缺口：文件 watcher、主配置 reload、远程控制面下发和 policy state migration 未实现。
...
- `dry_run_enforcement`
  ...
  - 已实现：在线 admin `reload_enforcement_policy` 可 validate-then-swap 当前 source 解析出的 active selector/profile；
    坏 manifest 不污染运行态。
  - 边界：不执行真实连接阻断；在线 reload 不等同于主配置 reload、远程控制面下发或 setup-time host rule 动态更新。
...
- `connection_enforcement`
  ...
  - 已实现：固定系统路径 `ss -K` TCP socket destroy backend、执行前 procfs socket owner 复核和 status/backend reporting。
  - 边界：默认 `backend = "none"` 不要求 connection capability。
  - 边界：probe 只确认 Linux、受信系统路径中的 `ss -K` 入口、root 执行上下文和 procfs socket attribution 入口。
  - 边界：每条 flow 仍可能因不是 live host observation、携带 observation-only evidence、当前 socket owner 无法复核或已不匹配、
    内核/namespace/匹配窗口而返回 `unsupported`。
...
- Runtime status：所选 executable Linux/root/nft/ip transparent interception lifecycle 可用时 available。
...
- setup 执行 `nft --check`。
- setup 获取 table-scoped `sssa_probe` owner lock。
...
- 出站 relay 在 upstream connect 前设置 `SO_MARK` 绕过自身 OUTPUT redirect。
- agent-owned webhook/control-plane HTTP connector 复用同一个 typed TCP socket mark 边界。
- 这避免 export/control-plane 出站连接被本机 OUTPUT redirect 自捕获。
...
- capability 不表示 L7 MITM proxy 进程已实现；external MITM listener、material contract 和可选 plaintext bridge
  由 `l7_mitm` capability 与 MITM backend contract 单独表达。
...
- 其它 process selector 和 flow-aware outbound setup 仍 fail closed。
- outbound transparent proxy 可承载显式 MITM。
- managed relay 只是 plain TCP transparent relay。
- external L7 MITM contract 可声明 CA/leaf/upstream trust material refs，并要求本机 listener readiness/health gate 可达。
- L7 MITM 还需要信任材料分发、独立 proxy lifecycle、proxy-side policy hook 和审计事件。
...
- `transparent_process_classifier`
  ...
  - 缺口：process-only、无 host boundary、wildcard local port、tcp6 listener 表不可读或不可解析、fd owner scan 不完整、
    未归因 listener、任一共享 listener holder 不匹配和 flow-aware selector 仍 fail closed。
...
- `transparent_flow_classifier`
  ...
  - 已实现：`not`、named ref，以及包含 classifier-only 或 unconstrained 分支的 `any` setup requirement 会落到该 capability。
  - 边界：还没有 flow-aware classifier 或 registry-backed setup resolution。
...
- ingress lane 当前写入 `sssa.probe.capture_event.origin.json` JSON framed `CaptureEvent`。
- export lane 当前写入 `sssa.probe.event_envelope.subject_origin.json` JSON framed `EventEnvelope`。
...
- `PlaintextSource` 显式区分 `external_plaintext_feed`、`libssl_uprobe` 与 `tls_session_secret` 等来源。
...
- 本轮只有在可用 records 能构成新的 lookup-unique store 时才替换当前 store。
- `session_secret_file` 是强 material。
- `session_secret_file` 缺失、为空或尚未 ready 会让本轮 refresh 保持 pending。
...
- 若全体 material 都不贡献 records，本轮仍是 pending。
- parse failure 会让本轮 refresh failed。
- ambiguous/conflicting lookup 会让本轮 refresh failed。
- 某个非空 `session_secret_file` 没有贡献任何可用于 live auto-binding 的 TLS 1.3 application traffic secret record 时，本轮 refresh failed。
...
- `initial pending`：
  ...
  只有没有任何其它 ready material 贡献 usable record 时，本轮才保持 pending。
...
- `CapturePipeline` 负责 capture event -> ingress journal -> per-flow parser -> policy -> enforcement audit -> export queue 的 replay/shared
  processing。
- ingress journal 现在写入完整 `CaptureEvent` JSON payload，而不是只写入 bytes payload。
...
- 配置了 storage retention 时，统一 worker 会在本轮启动恢复完成后再开始清理过期 ingress/export 连续前缀。
...
- 这些 counters 通过 Prometheus `sssa_pipeline_capture_loss_events_total` 暴露。
- 这些 counters 通过 Prometheus `sssa_pipeline_capture_lost_events_total` 暴露。
...
- 当前仍未实现主配置 reload、文件 watcher、远程控制面下发或后台 reload worker。
...
- parser 不为了 metadata 缓存完整 payload。
- 当前不聚合完整 WebSocket message。
- 当前不解压 extension payload。

#### Configured policy loader

- 已实现 configured policy loader 的 bundle 入口。
- `policies[].path` 必须指向目录。
- policy id 必须唯一。
- bundle 按 `manifest.toml` + `main.lua` 加载。
- manifest 目前支持 `id`、`version` 和 typed `hooks`。
...
- `status` 只做 bundle metadata/manifest 校验，不执行 Lua。
- `check` 和 `run` 会显式加载并执行初始化。
...
- 该 reload 不重读主 TOML。
- 该 reload 不切换 execution surface。
- 该 reload 不承担远程控制面语义。
- 如果当前执行面包含 transparent interception 这类 setup-time host rule owner，会 fail closed。
...
- Object preflight
  ...
  - 仍缺同进程 libssl unload/remap replacement lifecycle E2E、flow-specific lost-event reconstruction 和完整 kernel/socket-path capture 验收。
- Shared ABI
...
- outbound 支持 single-buffer 与 bounded first-non-empty-iovec syscall argument sample。
- inbound 支持 single-buffer 与 bounded first-non-empty-iovec syscall result sample。
...
- flow-start 携带的 `(tgid, fd, TGID epoch, direction mask)` 必须写入 allow map。
...
- 普通 `close(fd)` 只删除当前 TGID 下该 fd 的 allow。
- 普通 `close(fd)` 不再提升全 TGID epoch，避免同进程其它 fd close 误杀活跃 flow。
- epoch map 不做 LRU 淘汰。
- 容量耗尽、缺失 epoch 或 direction mask 不含当前方向时不授权深度采样。
- 这里选择漏采，而不是让旧 allow 复活或越过 selector。

fd 与 socket identity 边界：

- 当前模型不是强 socket identity proof。
- 当前模型不是 fd-table identity proof。
- 当前模型不是 kernel-copied payload proof。
- 当前只有成功 procfs fd lookup 后可选的 userspace `SO_COOKIE`。
- 当前没有 kernel-side socket cookie/fd-table identity。
- 因此 `(tgid, fd)` 只是接受主流 Linux process fd-table 共享模型，不能证明所有边界场景。

outbound syscall 采样语义：

- 当前覆盖 `write(2)`、`sendto(2)`、`writev(2)` 和 `sendmsg(2)`。
- enter tracepoint 在 allow gate 后立即持久化 single-buffer 或 bounded first-non-empty-iovec prefix。
...
- enter tracepoint 在 allow gate 后固定 single-buffer 或 first-non-empty iovec segment 的用户 buffer 和 readable segment length。
- exit tracepoint 仅在成功返回后，从 enter-time segment 读取 bounded prefix。
...
- 最多扫描固定数量的 iovec。
- 跳过前导 zero-length iovec。
- 只输出第一个 non-empty iovec 的 bounded prefix。
- 实际返回长度超过该 prefix、后续 iovec、scan limit 或 sample buffer 时，由 userspace bridge 输出 gap。
...
`CLOSE_RANGE_CLOEXEC` 只是设置 close-on-exec，不代表当前 fd 已关闭；`CLOSE_RANGE_UNSHARE` 会改变调用线程的 fd-table identity，在当前没有 fd-table
identity 的模型里也不能安全映射到 TGID 级关闭。因此二者都不会生成 `ConnectionClosed`，只通过 epoch 让后续深度采样保守降级。
...
- `agent` composition root 只在 libpcap provider 能按当前配置打开并安装 filter 时注入 available `Libpcap` descriptor；否则注入 unavailable
  ...
  connect/accept flow 按 bounded `(tgid, fd)` 表跟踪 payload attribution，并在后续同 TGID+fd descriptor close 或同 TGID close_range
  命中 tracked fd 时输出 best-effort `ConnectionClosed`；容量淘汰的 tracked flow 后续 close lifecycle 会被忽略，不会伪造 lifecycle。它不会读取已关闭 fd，
  也不等同于强 socket lifetime，dup/fork/fd passing、process exit、unshare fd-table 和 lost-event 语义仍需要后续 kernel-side socket
  cookie/fd-table identity/refcount、FIN/RST 或进程生命周期信号补强。`procfs_socket_attribution` 仍是 degraded capability：hidepid、权限、
...
- 已实现通用 `PlaintextEvent`：`bytes`、`gap`、`connection_opened`、`connection_closed`。事件带 `PlaintextSource`，当前代码路径支持
  `external_plaintext_feed`、`libssl_uprobe` 和 `tls_session_secret` provenance；转换为 `CaptureEvent` 时保留对应 `source`，
  `provider = plaintext`。`tls_session_secret` 默认不算 live host observation，不能直接作为 destructive enforcement 的强证据。
...
- Runtime status fact：
  ...
  process starttime、mapped/read path、device/inode identity 和 deleted 状态。
  如果后续 sidecar disabled，该字段保留最后一次成功结果用于排障。
  它只能说明最近一次 reconcile 的 target 集合，不能解释为当前 per-target provider health。
- Failure behavior：
...
- Attach atomicity：单个 libssl recipe 是 uprobe point 的局部 rollback 单元；best-effort 的 session link 提交单元是同一 process/library
  ...
  `SSSA_TLS_STATE_EPOCHS[0]` 存在且非零时才记录 TLS call；fd/offset state key 也携带 `state_epoch`。userspace 在初始 attach、有 stale
...
- per-target provider health：
  `last_reconcile` 只能说明最近一次成功 reconcile，不能证明当前每个 target 仍健康。
- async fan-in/backpressure：
  TLS sidecar 仍挂在 serial poll/mux 模型上，后续高吞吐需要独立 fan-in 与背压边界。
- TLS ringbuf saturation E2E：
  TLS plaintext output ringbuf 写失败已反馈为 provider-level `capture_loss`，
  但还缺人为制造真实 ringbuf saturation 的 privileged E2E。
- strong fd ownership：
  `FD_VALID` 只表示可尝试 fd-based resolution，不证明 fd 生命周期强绑定到当前 socket。
- same-process library unload/remap replacement e2e：
  `e2e-tls-plaintext-dynamic-library-loopback` 已证明同一 PID 在 agent ready 后动态加载 libssl 会被周期 reconcile attach，
  并产生该 PID 的 durable TLS plaintext HTTP/policy 输出。
  但本机 OpenSSL 3 带 `NODELETE`，该入口不证明 `dlclose` 后旧 libssl 映射消失、
  同进程 stale+new target 同时发生，或 replacement/remap 后的 epoch fence 真实行为。
...
- `SSSA_ALLOWED_SOCKET_FDS`：
  userspace allow map。
- `SSSA_FD_TABLE_EPOCHS`：
  TGID epoch hash map。
- `SSSA_PENDING_ACCEPTS`：
  保存 accept/accept4 enter-time sockaddr 指针。
- `SSSA_PENDING_WRITES`：
  保存已通过内核侧 allow+epoch+write-direction gate 的 enter-time pending outbound single-buffer/bounded first-non-empty-iovec sample。
- `SSSA_PENDING_READS`：
  保存已通过 allow+epoch+read-direction gate 的 inbound single-buffer/bounded first-non-empty-iovec attempt。
- `SSSA_PENDING_WRITE_SCRATCH`：
  per-CPU map，承载 enter-time bounded outbound snapshot。
- `SSSA_PROCESS_EVENT_SCRATCH` 与 `SSSA_PROCESS_READ_EVENT_SCRATCH`：
  per-CPU map，分别组装最终 outbound 和 inbound payload sample record。
- `SSSA_PROCESS_OUTPUT_LOSSES`：
  per-CPU map，累计 process output ringbuf write failure。
- 这些 scratch/loss maps 避免 BPF stack 撑爆。

process artifact 的 fd/epoch 语义：

- connect/accept record 携带 flow-start epoch。
- allow map key 是 `(tgid, fd)`。
- allow map value 是 fd-table epoch 加 read/write direction mask。
- 该模型符合主流 Linux 进程 fd-table 共享模型。
- outbound-only selector 不会触发 inbound buffer read。
- `close` 只删除 `(tgid, fd)` allow。
...
- 保存单一 global state epoch 的 `SSSA_TLS_STATE_EPOCHS[0]`。
...
- `SSSA_TLS_OUTPUT_LOSSES` per-CPU output loss counter map。
...
- plaintext return probe 最终写入 artifact 内的 `SSSA_EVENTS` ringbuf。
- ringbuf reserve 失败时递增 `SSSA_TLS_OUTPUT_LOSSES`。
- userspace 因而能看见 provider 级 TLS plaintext 丢失。

`xtask check-ebpf` 的 strict artifact 验证：

- process artifact 必须包含 10 个 process map。
- process artifact 必须包含 29 个 tracepoint program。
- process artifact 不得夹带 TLS maps/programs。
- TLS artifact 必须包含 `SSSA_EVENTS`、TLS call/fd/offset/state-epoch/scratch/output-loss maps。
...
Aya 做 symbol resolution 和 attach，并打开同一 artifact 内的 `SSSA_EVENTS` ringbuf 与 `SSSA_TLS_OUTPUT_LOSSES` per-CPU counter
...
- Runtime status：
  ...
  sequence、`observed_unix_ns`、精确 target counts 和来自 capture outcome 的 bounded target snapshots。
- Status overlay：
...
- 显式 external MITM contract 使用独立的 `[enforcement.interception.mitm]` 配置块。
...
- MITM contract 使用第 18 节定义的 backend readiness probe 表达 external listener 的本机 TCP readiness target。
- MITM backend 必须独立管理信任注入或证书分发、upstream TLS、self-bypass、L7 lifecycle、policy hook 和审计事件。
...
- `hooks` 使用 typed policy hook enum，TOML/JSON 边界仍表现为 Lua callback name，例如 `on_http_request_headers`、
  `on_websocket_handoff`、`on_websocket_frame`。
...
- bundle source entry 会拒绝 symlink，并通过 no-follow open、handle metadata 和 capped read 读取 `manifest.toml`/`main.lua`，避免把
  bundle 目录外文件静默纳入策略源。
- `main.lua` 复用现有 sandbox、instruction budget 和 memory limit；bundle-local `lib/`、checksum/signature、bundle 内 selector、
  capability requirement、state budget 和后台 policy bundle file watcher 尚未实现。
- 运行中的 pipeline 已经拥有共享 active policy runtime set；在线 admin `reload_policies` 会重新读取当前配置引用的 enabled bundles，在独立 Lua VM
  中加载并校验所有 manifest hooks，通过后原子替换 active set，失败时保留旧 active set。
- `policies[].path` 只接受 policy bundle directory。`replay --policy` 可直接读取单个 Lua 文件用于本地调试，但该入口不生成 configured policy source
  metadata，也不参与 `status`/`check` 的 bundle source contract。
...
- reload 不改变 enforcement mode、execution surface 或 backend。透明拦截这类 setup-time host rule surface 需要单独的 host mutation
  lifecycle；在它存在时 planner-only reload 会 fail closed。
...
- `RuntimePlan.enforcement` 是能力事实源：`mode_capability` 只表达 mode 自身的能力需求，当前只有 `dry_run` 需要 `dry_run_enforcement`；
  `execution_surfaces` 显式列出 `enforce` 使用的执行面；`connection.capability` 只由显式 connection backend 产生；
  ...
  和 `proxy.self_bypass` 契约决定；MITM backend 和证书材料由 `interception.mitm` 的 typed material refs 决定。Linux artifact
  planning 属于 `transparent-linux` artifact owner，agent runtime 可以消费该 artifact，但 artifact acceptance 本身不伪装成产品 E2E。
  `enforce` 表示允许真实执行，但必须配置且只能配置一个执行面，不再隐式等同于 connection enforcement。
...
"enforce"` 会要求至少配置一个执行面，当前可选执行面是显式 connection backend 或 transparent interception strategy。`enforce` 还要求 resolved
capture plan 为 live host capture；`replay` 和 `plaintext_feed` 即使能产生策略 verdict，也不能和真实 enforcement 组合成可运行计划。配置
...
该 probe 会检查 Linux、固定系统路径中的 `ss` 命令、`-K/--kill` 支持、root 执行上下文以及 procfs socket attribution 入口；全部满足时才把
`connection_enforcement` 标为 available。该 capability 表示 socket destroy 入口条件满足，不表示无副作用证明了当前内核会销毁任意目标 socket；该 backend 只接受
live host observation source（当前为 eBPF syscall、libpcap、libssl uprobe），拒绝 replay/mock/external plaintext feed
事件以避免恢复或离线输入误杀当前 socket。
...
也不会被复制进 planner 生成的 `EnforcementDecision.reason`。当前 eBPF single-buffer/bounded first-non-empty-iovec syscall
...
backend；backend 在调用 `ss -K` 前会重新读取 procfs socket owner，要求当前 flow 的 socket owner 与触发事件中的进程身份仍匹配；无法解析 flow 地址、找不到当前
owner、owner 与触发进程不一致或 procfs 读取失败时返回 `unsupported`，不会执行 destructive action。owner 复核通过后才按 flow 的 local/remote
address/port 调用 `ss -K`，单次调用有固定 timeout；命令成功但没有报告真实 socket 行时返回 `unsupported`，不会伪装成 `applied`；后端调用错误会作为 `failed`
enforcement decision 进入事件流。

当前也已实现外部 enforcement policy manifest 的第一版装配边界。`enforcement.policy.source` 可以指向单个 TOML manifest 文件、包含 `manifest.toml`
的目录，或一个 remote endpoint；`RuntimePlan.enforcement.policy_source` 会把目录 source 解析为实际 `manifest.toml` 路径并保留 remote
endpoint，后续 `check`、`run`、offline `status` 和在线 admin reload 都消费 plan，而不是重新解释 raw config。remote source 在
`check`/`run`/手动 `reload_enforcement_policy` 时按需 GET；它不代表动态控制面、长连接下发、主配置热替换或后台 watcher。manifest 当前字段为：
...
主配置中的 `enforcement.selector` 与 manifest 内的 `selector` 使用 AND 语义合成；因此可以把全局 agent 防护边界和外部下发的应用/服务边界分开管理。manifest 不执行代码，
只是声明保护 profile 和作用域；Lua policy 仍负责产生 typed verdict。`check`、`run` 和在线 admin `reload_enforcement_policy` 会显式读取并校验
manifest，本地 source 从文件读取，remote source 通过异步 HTTP GET 拉取 TOML，禁止重定向，并按 16 MiB 上限读取响应体，以容纳较大的远端策略包而不做无界内存读取；`run` 先完成完整
runtime plan validation，再构造 configured enforcement，因此 unsupported capability 不会提前读取本地 manifest，也不会访问 remote endpoint，
且缺少 executable backend factory 时不会继续加载 Lua policy。启动恢复 ingress journal 时不挂载 enforcement planner：恢复可以按当前 policy 重新产出
verdict/export 事件，但不会对历史 ingress 执行真实阻断或 dry-run enforcement decision，避免旧 four-tuple 在启动时误杀当前 socket。offline `status`
只做 metadata-only 检查并把健康状态标为 degraded，remote source 在 offline status 中不发网络请求、不声称 manifest 已加载，避免把 offline status
变成隐式执行或启用防护的入口；在线 admin status 使用当前 loaded manifest snapshot，报告 `loaded` 并携带 local path 或 remote endpoint source
origin。该 snapshot 只在 `run` 启动加载或 `reload_enforcement_policy` 成功后更新；普通 status 请求不会重新读取磁盘上可能已变化的 source，也不会重新访问 remote
endpoint。
...
- 运行面：显式启用的 external MITM backend contract，而不是 `managed_tcp_relay` 的布尔开关。
- 配置面：`inbound_tproxy_mitm` / `outbound_transparent_mitm` 是 config 和 RuntimePlan 的一等 strategy 名称。
- 能力面：MITM strategy 同时要求 `transparent_interception` 与 `l7_mitm`；缺少 external backend contract、readiness target、
  listener readiness、material pair 或配置的 plaintext bridge feed openability 时 fail closed。
- 生命周期面：external backend contract 只接受外部 L7 backend 监听端口；agent 负责 selector-scoped redirect 和 TCP endpoint
  readiness/health 检查，不负责启动、重启或升级 L7 proxy 进程。
- 证书面：独立引用 MITM CA pair、leaf certificate pair 和 upstream TLS trust material；自动生成 CA/leaf、client trust/install instructions
  不属于 external backend contract。
...
- L7 生命周期：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、policy hook、audit event 和 proxy process lifecycle contract。
- 验证面：必须有入站和出站的真实 agent E2E，证明 plaintext event、policy decision、proxy lifecycle、cleanup 和 failure audit 都进入统一 pipeline。

透明转发、plain TCP relay、external proxy reserved-mark contract、出站 host-rule lifecycle、显式 MITM strategy，以及 external MITM material
contract/readiness/health/plaintext bridge 的配置、plan、status、check 和 fail-closed 边界已经存在；agent-managed L7 proxy 仍未实现。

当前 agent composition root 已接入 Linux inbound TPROXY lifecycle：probe 检查 Linux、root、可信系统路径中的 `nft` 和 `ip`，当前执行层调用受信系统命令生成、
校验和安装 nftables/policy-route 状态，没有引入 `nftables` crate 作为 netlink API；nft table、inbound TPROXY route mark、outbound proxy
bypass mark 和 inbound TPROXY route table 不接受用户配置，执行层固定使用当前保留资源（`sssa_probe`、`0x53534101`、`0x53534102`、`53534`），并在
RuntimePlan、check 和 status 中作为 resolved fields 报告；`run` 在通过完整 RuntimePlan validation、加载外部 manifest 并得到最终 effective
selector 后，对最终 inbound nft 脚本执行 `nft --check`，再获取 `/run/sssa-probe/transparent-interception/sssa_probe.lock` 这个
table-scoped host owner lock，清理当前保留 host resources 的所有 IPv4/IPv6 policy routing 遗留；若配置 `managed_tcp_relay`，会在 lock
持有期间按同一 inbound TPROXY plan 的 rule family 启动透明 IPv4/IPv6 listener，然后安装 owned `inet` table 和 `fwmark -> route table`
policy routing。policy routing 只按实际 nft rule family 安装：IPv4-only selector 不触碰 IPv6 rule/route，IPv6-only selector 不触碰
IPv4，未限定 remote address 时同时安装两族。退出时删除 owned nftables table 并清理本次安装的 route/rule，再停止 managed relay 并释放 owner lock。setup
失败会 best-effort rollback；不会使用 `flush ruleset` 这类全局破坏性命令。
...
- 执行 `nft --check`。
- 获取同一个 `/run/sssa-probe/transparent-interception/sssa_probe.lock` table-scoped owner lock。
...
- agent-owned webhook/control-plane HTTP connector 会把 outbound proxy bypass mark 设置到连接 socket 上。
- 因此 planned webhook exporter 可与 outbound transparent proxy 共存。
...
当前 `transparent_interception` capability 的含义是“所选透明拦截 strategy 的 Linux/root/nft/ip executable lifecycle 可用”。它只由平台 probe
...
target，再通过可信 `ip -j address show` 在 activation 和每次 same-port active probe 前读取本机接口地址，只在目标地址族会命中实际 managed listener
时拒绝把本机非 loopback 地址作为自身 listen port 的 probe target，达到连续失败阈值后才把 agent health 降为 degraded；它不从 per-flow relay connect
结果推断健康。

managed relay 不加载 MITM CA、不注入证书、不把 MITM 作为默认 TLS plaintext 路线。它解析上游目标时通过显式 target recovery strategy 完成：入站 TPROXY
managed relay 使用 accepted socket 的 `local_addr()`，并拒绝转发到自身 listen port，避免 direct proxy connect 或错误规则造成自连递归；出站
transparent proxy managed relay 使用 Linux `SO_ORIGINAL_DST` / `IP6T_SO_ORIGINAL_DST` 恢复被 OUTPUT redirect 前的原始目标，并用本机地址
inventory 拒绝 original destination 指向本地 proxy listener 的自循环。TCP connect path 已抽到 `probe-io` 的 typed socket factory，可在
`connect(2)` 前设置 `SO_MARK`：入站 relay 使用 unmarked plan，出站 relay 从 outbound redirect artifact/lifecycle plan 的 proxy
bypass mark 构造 marked plan，agent-owned webhook/control-plane HTTP connector 也复用同一 mark，且不能复用 inbound TPROXY route mark。
...
- `requires_process_classifier`
  ...
  - 当前语义：本地 projection 只做静态 typed 预分类；执行层只对最终 effective setup rules 运行
    setup-time procfs listener proof。
  - 可执行条件：final `host_rule_boundary = host_rules`，且每个 scope 都包含显式 local ports。
    不满足时 fail closed。
...
执行层在加载外部 manifest 后，会把主 `enforcement.selector`、manifest selector 和 `enforcement.interception.selector` 的交集作为最终
effective setup selector 重新投影。Outbound UID/GID-only process-scoped setup 不走 classifier，而是作为 nft socket-owner host rule
安装；其它 process-scoped setup 只有在 process classifier capability 可用、存在 host-rule boundary、local ports 显式、当前每个目标 TCP
listener inode 的所有 holder 进程都可经完整 procfs owner scan 归因且明确命中 process selector 时才会安装 host rules。其它 process-scoped 情况和所有
flow-aware setup plan 都以 setup error fail closed，避免把“按应用拦截”的配置静默扩大成全机拦截。当前 executable inbound TPROXY 要求最终 rule set 中每个
scope 都具备显式 local port 集合；wildcard local port scope 会 fail closed，因为它会把 proxy listen port 也纳入拦截范围，而当前还没有完整 proxy
self-bypass lifecycle 或规则排除契约。远程 manifest 可以进一步收窄本地 scope，但不能成为安装 host rules 的唯一 scope。
...
audit/status contract 作为同一个 executable backend 的门控条件；完善 L7 MITM backend 时，还必须接入 trust/install 生命周期、proxy process
lifecycle、proxy-side L7 readiness、plaintext bridge hardening 和 L7 分类。

当前 `RuntimePlan` 只接受一个 execution surface；若同时配置 connection backend 和 transparent strategy，会在 plan validation 阶段 fail
...
evidence、当前 flow 仍能通过 procfs 复核为同一进程身份、随后仍能被 `ss -K` 匹配且内核实际执行 socket destroy 时销毁现有 TCP socket。WSL2
或受限内核可能满足命令和权限探测但对具体 socket 静默跳过，此时必须记录为 `unsupported`。
...
- eBPF object 路径放在 `[capture.ebpf] object_path` 下。这个字段作为 `aya-obj` object preflight 输入；process observation runtime 使用
  strict process artifact contract，校验 ringbuf、allow map、fd-table epoch map、pending accept、pending write/read、pending
  write scratch、process write/read event scratch 和 process output loss counter map shape，校验 connect、accept/accept4、close、
  fd lifecycle、single-buffer/bounded first-non-empty-iovec payload tracepoint sections，并拒绝夹带 TLS maps/programs 的混装
  object。高层用户态 `aya` process observation loader、connect/accept observation 到单个 `ConnectionOpened` `CaptureEvent` 的纯转换
  bridge、`capture.deep_observe_selector` 按方向授权的 outbound single-buffer/bounded first-non-empty-iovec syscall argument
  sample 和 inbound single-buffer/bounded first-non-empty-iovec syscall result sample 到 always-degraded
  ...
  event 的转换已存在。超出 bounded iovec scan/sample buffer 的 continuation、flow-specific lost-event reconstruction 和完整
...
- worker 随后用本轮剩余 `prune_batch_limit` 执行 max-records 清理。
- 因此 `max_records` 不是年龄清理的全局保底。
- `run` 会先完成启动恢复，再启动统一 storage retention worker。
- 这个顺序避免 worker 抢先清掉本轮 recovery 要读取的输入。
...
- `RuntimePlan.enforcement` 保留 `execution_surfaces`。
...
- 要求 `interception.mitm.backend = "external"`。
- 要求 `interception.mitm.backend_readiness_probe.target`。
...
- webhook exporter 与 `outbound_transparent_proxy` 可以共存。
- agent-owned webhook/control-plane socket 会继承同一个 outbound proxy bypass `SO_MARK`，避免被本机 OUTPUT redirect 自捕获。
- file exporter 仍可作为本地交接路径使用。

MITM backend contract：

- `interception.mitm.backend = "external"` 表示 agent 只负责 selector-scoped host redirect 和能力事实，不负责 L7 proxy 进程生命周期。
- `interception.mitm.backend_readiness_probe.target` 必须是 loopback IP socket address，且端口必须等于 `proxy.listen_port`；
  agent 使用 TCP connect 验证 external listener readiness。
- `interception.mitm.backend_readiness_probe.interval_ms`、`timeout_ms` 和 `failure_threshold` 必须落在有界范围内，
  且 `timeout_ms` 不能超过 `interval_ms`。
- agent 启动时用同一 TCP connect contract 做 readiness gate；在线 runtime 会按 `interval_ms` 周期继续检查，
  并在连续失败达到 `failure_threshold` 后把 agent health 降为 degraded。
- backend readiness/health probe 只证明配置的本机 TCP endpoint 可达；它不证明 IPv4/IPv6 全族监听、TPROXY original-destination
  recovery、TLS handshake、ALPN/SNI routing 或 policy hook。
- plaintext bridge openability gate 只证明配置的 feed 可打开；它不证明 external proxy 会写入事件，也不证明 plaintext event 语义正确。
- `interception.mitm.plaintext_bridge.mode = "capture_event_feed"` 表示 external MITM proxy 会把解密后的统一 `CaptureEvent`
...
- bridge 文件打开失败 fail closed；运行中读取错误按 best-effort sidecar disable，不拖垮主 live capture。
...
- 上游 trust anchors 是 external MITM proxy 的独立 upstream TLS trust input，不会影响 exporter mTLS 或 TLS decrypt hint。
...
- plan 保留 MITM backend、backend readiness probe、plaintext bridge 和 resolved CA/leaf/upstream trust material refs。
...
- wildcard local port 要等完整 proxy self-bypass lifecycle 或规则排除契约存在后再支持。
- final rule set 校验通过后执行 `nft --check`。
- 随后获取 `sssa_probe` table owner lock。
...
- final rule set 校验通过后执行 `nft --check`。
- 随后获取同一个 `sssa_probe` table owner lock。
...
- remote source 在 `check` 中通过单次 GET 拉取 TOML manifest，并复用同一套校验。
- remote source 在 `run` 中通过单次 GET 拉取 TOML manifest，并复用同一套校验。
- remote source 在手动 `reload_enforcement_policy` 中通过单次 GET 拉取 TOML manifest，并复用同一套校验。
...
- `run` 在构造 configured enforcement 前，先完成 capture provider 的 side-effect-free preflight。
- `run` 在启动 admin/export worker 前，先完成 capture provider 的 side-effect-free preflight。
- `run` 在打开 spool 前，先完成 capture provider 的 side-effect-free preflight。
- `run` 在启用 transparent interception 前，先完成 capture provider 的 side-effect-free preflight。
- 当前 preflight 覆盖 `key_log_refs` / `session_secret_refs` live auto-binding material 读取。
- 当前 preflight 覆盖 material 解析。
- 当前 preflight 覆盖 auto-binding view 构建。
- 当前 preflight 覆盖 lookup-unique 检测。
- 当前 preflight 覆盖 live-consumable 校验。
- 这个顺序避免 TLS material 错误时仍短暂安装 nftables/TPROXY host rules。
...
- external MITM backend readiness probe 必须满足 MITM backend contract。
...
runtime plan 也会把 `interception.mitm` refs 解析成 typed material plan，并保留 backend readiness probe，
...
- 当前落盘 payload 是带 record envelope 的 `SpoolPayload`。
...
- 当前内容包括 `sssa.probe.capture_event.origin.json` JSON framed `CaptureEvent`。
- 当前内容包括 `sssa.probe.event_envelope.subject_origin.json` JSON framed `EventEnvelope`。
...
- 报告 active policies 的 id/path/selector 是否配置。
- 报告 metadata-derived `policy_version`。
- 报告 policy bundle directory 是否具备可加载 metadata。
- 会解析各 bundle 的 `manifest.toml`。
- 会检查 `main.lua` metadata。
- 不会加载或执行 Lua source。
...
- remote source 在 offline status 中只报告 endpoint metadata。
...
- loaded manifest snapshot 保留 local path 或 remote endpoint source origin。
...
- transparent interception setup-time host rules 尚无在线 mutation owner 时，enforcement policy reload 会 fail closed。
...
- `capture`
  ...
  - 负责事件桥接：connect/accept 到 `ConnectionOpened` 的纯转换，best-effort descriptor close/close_range 到
    `ConnectionClosed` lifecycle event，unresolved connect/accept 到 degraded `Gap` 的 provider path。
  - 负责 bounded sample：selector-authorized outbound/inbound single-buffer 或 bounded first-non-empty-iovec sample 到
...
- `xtask e2e-websocket-plaintext-feed`
  ...
  - 断言 request/response/handoff/frame 顺序、frame metadata/fingerprint、Lua handoff/frame hook、lifecycle 顺序和无 protocol error。
...
- `xtask e2e-ebpf-process-loopback`
  ...
  - 覆盖 selector-authorized outbound single-buffer/bounded first-non-empty-iovec syscall argument sample。
  - 覆盖 selector-authorized inbound single-buffer/bounded first-non-empty-iovec syscall result sample。
  ...
  - vector 模式验证带 zero-length leading iovec 和 split payload 的 bounded first-non-empty-iovec `writev(2)` / `readv(2)` /
...
- 真实 ringbuf saturation 仍需要真正的 privileged/e2e 验收。
...
- TLS lifecycle 仍需要真正的 privileged/e2e 验收。
...
- 默认/显式 live capture：
  ...
  - eBPF observation：已有 `aya-obj` strict process artifact preflight、shared ABI、最小内核 object scaffold、高层用户态 `aya` loader、
    ...
    selector-authorized outbound single-buffer/bounded first-non-empty-iovec syscall argument sample 与 inbound
    single-buffer/bounded first-non-empty-iovec syscall result sample 到 always-degraded `CapturedBytes`/`Gap` 的 payload
  ...
  - TLS plaintext sidecar：libssl plaintext sample ABI、capture bridge、userspace uprobe loader/provider、agent startup attach
    ...
    attach、进程退出后 active target 清除/detach 的真实特权闭环。
...
| Enforcement / interception | enforcement、transparent、MITM bridge cases | user 或 root/net-admin | dry-run、manifest reload、TPROXY/transparent lifecycle、external MITM contract 配置/状态/readiness/health gate、MITM plaintext bridge live fan-in。 | 默认 destructive action、agent-managed L7 MITM proxy/data-plane。 |
...
- `--io-mode readv-writev` 使用 zero-length leading `IoSlice`/`IoSliceMut` 加 split payload。
- `readv-writev` 验证 bounded first-non-empty-iovec `writev(2)` / `readv(2)` path。
...
- 命中 selector 后，才能验证 selector-authorized outbound/inbound single-buffer/bounded first-non-empty-iovec syscall sample。
...
- `cargo run -p xtask --locked -- e2e-websocket-plaintext-feed`
  ...
  - 断言 WebSocket frame metadata/fingerprint。
  - 断言 handoff/frame Lua policy alert。
  - 断言无 protocol error。
  - 断言 ingress/export spool 均可读回。
...
- 它不代表主配置 reload、远程控制面 watch、setup-time transparent interception host rule mutation 或 destructive backend 已完成。
...
- 验证带 zero-length leading iovec 和 split payload 的 bounded first-non-empty-iovec `writev(2)` / `readv(2)`。
- 验证带 zero-length leading iovec 和 split payload 的 bounded first-non-empty-iovec `sendmsg(2)` / `recvmsg(2)`。
...
- 断言 selector-authorized degraded outbound single-buffer/bounded first-non-empty-iovec sample。
- 断言 selector-authorized degraded inbound single-buffer/bounded first-non-empty-iovec sample。
...
- 不覆盖同进程 libssl unload/remap replacement lifecycle。
...
- `e2e-tls-plaintext-loopback` 必须先运行 `cargo run -p xtask --locked -- ebpf-build` 和 `cargo build -p agent -p e2e-fixture
  ...
  output 的真实 agent 闭环；它仍不证明 keylog/session decrypt、同进程 libssl unload/remap replacement lifecycle 或强 socket ownership。
...
- `e2e-tls-plaintext-dynamic-library-loopback` 必须先运行 `cargo run -p xtask --locked -- ebpf-build` 和 `cargo build -p agent
  ...
  后先完成一次 reconcile 并确认该 PID 没有 active TLS target；随后 fixture `dlopen` 临时复制的 `libssl.so.3`，admin status 必须报告同一 PID 的该
  `mapped_path` 成为 active target，再触发 `SSL_set_fd` + `SSL_write_ex` 出站 HTTP request。fixture 在写后短暂保持 fd/proc live，给当前
  procfs fd-to-flow resolver 可见窗口。最终从 Fjall ingress/export 断言该 PID 的 `source = libssl_uprobe` plaintext HTTP request、
  HTTP parser output、configured policy alert 和无 policy runtime error。该入口证明同一 PID 动态 libssl load、mapped-path scoped
  attach、selector gate、fd-to-flow 归因和 durable parser/policy 链路；由于本机 OpenSSL 3 带 `NODELETE`，它不证明 `dlclose` 卸载旧映射、同进程
  stale+new replacement 或 state epoch 在真实 remap 下的行为。
...
- `xtask e2e-websocket-plaintext-feed` 通过同一真实 agent path 覆盖 HTTP Upgrade 到 WebSocket frame metadata、handoff/frame policy hooks、
  connection lifecycle 顺序、无 protocol error 和 durable output。
...
- 覆盖 allow-map 授权。
- 覆盖 bounded outbound single-buffer/bounded first-non-empty-iovec syscall argument sample。
- 覆盖 bounded inbound single-buffer/bounded first-non-empty-iovec syscall result sample。
...
- vector 模式覆盖带 zero-length leading iovec 和 split payload 的 bounded first-non-empty-iovec `writev(2)` / `readv(2)` /
...
- 当前没有用 privileged E2E 人为制造真实 ringbuf saturation。
...
- 完整 socket lifetime。
- flow-specific lost-event reconstruction。
- 同进程 libssl unload/remap replacement lifecycle。
...
- 覆盖 pending accept/read/write layout。
...
- 测试验证 process 的 10 个 map + 29 个 tracepoint program。
...
- 该 E2E 覆盖 selector-authorized outbound/inbound single-buffer/bounded first-non-empty-iovec payload sample degraded semantics。
...
- `xtask e2e-tls-plaintext-dynamic-library-loopback` 已验证同 PID dynamic libssl load。
...
- 同进程 libssl unload/remap replacement lifecycle 仍需 privileged/e2e 覆盖。
...
- eBPF flow-start fd resolution retry 会跨 poll 继续，且不在 poll path 内 sleep。
...
- `xtask e2e-tls-plaintext-dynamic-library-loopback` 覆盖同一 PID 先无 active TLS target、后动态加载 libssl mapped path、
  再被周期 reconcile attach 并产出 durable plaintext output。
- 真实 Aya attach/detach side effect 的同进程 libssl unload/remap replacement 路径仍需 privileged/e2e 覆盖。
...
- 真实 Aya attach/rollback/detach side effect 的动态进程和同 PID dynamic libssl load 路径已有 E2E 覆盖。
- 同进程 unload/remap replacement 路径仍待 privileged E2E 补齐。
...
- Planning / sidecar runtime：
  ...
  sidecar 到期后先 reconcile 再 poll provider，blocked planning 不继续 poll。
...
- Admin observable status：
  ...
  以及最近一次成功 reconcile 的 sequence、`observed_unix_ns`、精确 target counts 和 capture 层 bounded target snapshots。
- Failure overlay：
...
该验证仍不等同于 [Remaining TLS lifecycle gaps](#remaining-tls-lifecycle-gaps) 全部完成；dynamic process attach/detach 和同 PID
dynamic libssl load 已有 E2E，同进程 unload/remap replacement、per-target health、TLS ringbuf saturation E2E 和 strong
ownership 仍是后续边界。

当前已验证的 TLS plaintext material check 路径：capture 单元测试覆盖 SSLKEYLOGFILE/NSS key log parser 的 comment/blank line 跳过、已知
label、future uppercase label、三字段约束、context 长度、secret hex 校验和错误不泄漏 secret 值；key log material 现在会解析为 secret-bearing
...
长度、protocol/secret kind 非法组合、validity window、JSON data error 净化和错误不泄漏 secret 值；session secret material 现在会解析为 typed
...
当前仍没有 unbounded/general sequence resynchronization、TLS 1.2 key block decrypt 或 agent-managed MITM/proxy lifecycle。四条 privileged/live
...
- 校验 `content-type = application/x-protobuf` 和 `x-sssa-codec = gzip`。
...
`capture_loss` runtime metrics 的当前验证包括单元/集成和非特权 agent E2E：单元/集成层证明 provider loss event 不会只停留在 export queue、recovery
replay 不会污染 live runtime counters；`xtask e2e-capture-loss-event-feed` 进一步用真实 `agent run`、follow-mode typed
capture-event feed、admin Unix socket 和 Fjall spool 证明 deterministic provider loss event 会同时进入 durable ingress/export、
admin metrics JSON 和 Prometheus counters。它仍不证明真实 live ringbuf saturation 场景下内核/eBPF output loss counter 到 provider
loss event 的端到端同步。
...
webhook exporter、file exporter 和 remote enforcement manifest。`cargo run -p xtask --locked -- e2e-suite --list` 会列出全部
...
- `e2e-suite`
  ...
  - 覆盖：非特权 agent durable output、HTTP/SSE/WebSocket parser、flow-level gap/degraded 语义、
    typed provider loss event、policy、remote enforcement manifest、webhook/file exporter。
- `e2e-suite --profile product`
  ...
  - 覆盖：replay/plaintext、live libpcap HTTP/WebSocket、admin reload、eBPF process observation、
    TLS plaintext/material 和 transparent interception。
...
并在 root 且 `/sys/fs/bpf` 挂载为 bpffs 的 Linux 环境下执行。
...
- `baseline`
  ...
  - 具体包括：plaintext feed、typed capture-event feed provider loss、SSE、flow-level gap、WebSocket、
    webhook/file exporter、remote enforcement manifest。
- `live-core`
...
- `process-ebpf`
  - 默认权限：root/bpffs。
  - 覆盖：eBPF process observation loopback，作为进程级 attribution 的主机验收入口。
- `tls-plaintext`
  - 默认权限：root/bpffs。
  - 覆盖：libssl plaintext provider、startup/dynamic attach、target lifecycle 和同 PID dynamic libssl load。
- `transparent-interception`
  ...
  - 覆盖：nftables/TPROXY 入站 host rule、process-scoped setup-time listener proof、
    ...
    以及 UID/GID socket-owner-scoped outbound 路径。
- `linux-artifacts`
  - 默认权限：root/net-admin。
...
- Provider loss
  - `e2e-capture-loss-event-feed`，user：
    ...
    不证明真实 eBPF/TLS ringbuf saturation。
...
- Admin reload
  - `e2e-admin-policy-reload` / `e2e-admin-enforcement-reload`，root/CAP_NET_RAW：
...
- TLS plaintext
  - `e2e-tls-plaintext-target-lifecycle-loopback` / `e2e-tls-plaintext-dynamic-library-loopback`，root/bpffs：
    覆盖 libssl plaintext sidecar 的 target lifecycle、同 PID dynamic libssl load、fd-to-flow 归因和 durable parser/policy 链路。
    不证明强 socket ownership 或真实 unload/remap replacement。
...
- Transparent inbound
  - `e2e-transparent-tproxy-loopback` / `e2e-transparent-tproxy-process-loopback`，root/net-admin：
    覆盖隔离 network namespace 中的 nftables/TPROXY host rule lifecycle、managed relay 入站转发，
    以及 process-scoped setup-time listener proof。
    不证明规则安装后的 listener 动态追踪或 cgroup/owner mark classifier。
...
consumer，并且 parser 不会在 gap 后继续把缺失响应体伪装成完整 body。`e2e-capture-loss-event-feed` 已覆盖 deterministic provider-scoped
`capture_loss` 同时进入 durable export、admin metrics JSON 与 Prometheus counters。后续仍需要 live provider-level overload/loss
E2E：人为制造真实 eBPF/TLS ringbuf/provider loss 时必须产生 provider-scoped `capture_loss`，并证明该信号同样进入 durable export、admin
metrics/status 与 Prometheus counters。
...
- ABI/object/loader 层已验证 TLS artifact 通过 `SSSA_TLS_STATE_EPOCHS[0]` 表达 sidecar 级 TLS state lifecycle。
...
- 该入口不直接观察 `SSSA_TLS_STATE_EPOCHS` 写 `0` 或新 epoch。
...
- 覆盖随后 `dlopen` 临时复制的 libssl mapped path。
- 覆盖周期 reconcile attach 该 path。
- 覆盖通过 `SSL_set_fd` + `SSL_write_ex` 产出 durable plaintext HTTP request 和 policy alert。
- 该入口证明同 PID dynamic library load。
- 该入口不证明 `dlclose` 后旧 mapping 消失。
- 该入口不证明 OpenSSL `NODELETE` 场景下的 stale target 或真实 replacement/remap epoch 行为。
...
- 本地 source 从文件读取；remote source 通过一次 GET 拉取 TOML。
...
- `xtask e2e-remote-enforcement-policy` 已用真实 agent path 验证 remote source 拉取、selector 合成，以及 manifest protection profile 对 dry-run
  decision 的约束。
...
- L7 MITM runtime metrics 输出 external backend health mode/check counters。
...
- `outbound_transparent_proxy` 按第 18 节 proxy lifecycle contract 支持 host-rule OUTPUT/dstnat redirect、managed relay 或 external reserved-mark
  self-bypass。
- agent-owned webhook/control-plane HTTP connector 也会使用 outbound proxy bypass mark，使 planned webhook exporter 可与 outbound transparent proxy 共存。
- RuntimePlan/check/status 会表达 OUTPUT/dstnat artifact、proxy port 和 proxy bypass mark。
- RuntimePlan/check/status 会表达 external MITM backend contract、backend health、capture-event plaintext bridge 和 resolved MITM material refs。
...
MITM backend contract 已进入配置、规划、能力报告、readiness gate 和 backend health 覆盖；capture-event plaintext bridge 的 live provider
...
| 优先级 | 状态 | 目标 | 主要交付 | 验收信号 |
| --- | --- | --- | --- | --- |
| P0 | Active | 收敛主要缺口，避免 best-effort 被误读成强保证。 | 强 socket lifetime、flow loss、TLS ringbuf E2E、libssl remap E2E。 | capability、metrics、export 和 E2E 状态一致。 |
| P1 | Active | 完成真实防护路径的能力边界。 | connection eBPF enforcement、transparent lifecycle、destructive audit/recovery。 | 显式 backend、selector gate、capability gate 和审计链可验证。 |
| P2 | Active | 建立显式 L7 MITM backend。 | agent-managed L7 proxy lifecycle、MITM trust/install、plaintext bridge hardening、L7 policy hook、audit event。 | 默认全机 MITM 不可用；显式 MITM 有独立 health/status/audit/E2E。 |
| P3 | Next | 扩展协议与外发面。 | HTTP/2 parser、WebSocket message aggregation 设计、gRPC/Kafka/OTLP exporter、远程配置源。 | 新 provider/parser/exporter 通过现有 trait 接入，不破坏四平面边界。 |
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；入站 TPROXY/managed relay 与出站 host-rule/external self-bypass/owner-scoped paths 已接入；external L7 MITM contract 与 capture-event plaintext bridge 已可配置和报告，agent-managed L7 proxy 仍缺失 | 第 11、18、32 节 |
...
| enforcement policy reload | 在线 admin 手动触发当前 enforcement policy source 的 validate-then-swap；只替换 active policy snapshot，不重读主配置、不改变 execution surface | 第 17、18、28 节 |
...
| 当前拦截验收 | dry-run verdict、remote manifest、socket destroy audit、transparent TPROXY/relay/outbound lifecycle 与真实 agent outbound E2E 已覆盖；external MITM contract 已验证，完整 L7 proxy/data-plane 未完成 | 第 18、31 节 |
