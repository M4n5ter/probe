...
- 强 socket lifetime。
- 透明 HTTPS MITM E2E、upstream TLS relay、自动修改客户端 trust store 和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、`client_trust.mode = "operator_managed"`、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；`product_proxy` backend 从 typed bridge、policy hook 和 leaf material 生成 first-party proxy CLI，内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、dynamic flow feed、proxy-side hook deny 执行和 durable delegated decision。upstream TLS relay、透明 HTTPS MITM E2E、强原始归因和自动 client trust store 安装仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 边界：readiness/health probe 只证明配置的本机 TCP endpoint 可达；它不是外部进程 supervisor，不证明 IPv4/IPv6
    ...
    handshake、SNI/ALPN routing 或 plaintext extraction 成功。内置 `traffic-probe-mitm-proxy` 的 TLS listener
    单独证明下游 TLS server-side termination 可生成明文 HTTP feed；它不证明透明 HTTPS 规则路径或 upstream TLS relay。
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    同一产品 proxy 支持显式配置证书链和私钥来终止下游 TLS，并把解密后的 HTTP request/response 写入 bridge feed。
    仍需独立实现 upstream TLS relay、透明 HTTPS MITM E2E、强原始 socket/process attribution 和自动 client trust store 安装。
...
- capability 不表示 L7 MITM 数据面已实现；external/managed/product proxy MITM listener、material contract、product proxy TLS termination
...
- L7 MITM lifecycle audit 由 durable export 表达；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  backend-owned action report、正式 HTTP plaintext proxy deny 执行和 product proxy 下游 TLS termination 由对应测试覆盖。
- 自动 client trust store 安装、upstream TLS relay、透明 HTTPS MITM E2E 和强原始归因仍缺失。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  product proxy 的 HTTP plaintext deny 执行和下游 TLS termination 已可验证。透明 HTTPS verdict/audit 仍需要
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  `product_proxy` 参数合成由 RuntimePlan 测试覆盖；product proxy 下游 TLS termination 由 crate-level TLS client/server/feed 测试覆盖。
  透明 HTTPS MITM data-plane verdict/audit、upstream TLS relay、强原始归因和 managed-process health-transition audit 的 privileged/E2E
  覆盖仍是缺口。
...
managed backend HTTP data-plane canary、backend-owned action report、正式 HTTP plaintext proxy deny 执行和下游 TLS termination
构成可验证控制面能力；透明 HTTPS MITM E2E、upstream TLS relay、强原始归因和自动 client trust store 安装不属于当前控制面能力。
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、upstream TLS relay、非 HTTP plaintext deny 的保护动作执行和 L7 分类。
...
- 上游 trust anchors 是 MITM backend 的独立 upstream TLS trust input，不会影响 exporter mTLS 或 TLS decrypt hint。
  内置 product proxy 已支持下游 TLS termination；上游 trust anchors 仍是后续 upstream TLS relay 的输入契约。
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、operator-managed client trust contract、plaintext bridge、proxy-side hook wiring、typed execution evidence、managed backend HTTP data-plane canary、正式 HTTP plaintext proxy deny 执行、正式 proxy 下游 TLS termination、backend-owned action report，以及 gated Linux socket destroy destructive path。 | upstream TLS relay、透明 HTTPS MITM E2E、强原始归因、自动 client trust store 安装、pre-connect deny。 |
...
当前仍没有 unbounded/general sequence resynchronization、TLS 1.2 key block decrypt、内置透明 HTTPS MITM E2E 或 upstream TLS relay。四条 privileged/live
...
可由产品二进制持有。
它不替代透明 HTTPS MITM 规则路径验证，也不证明 upstream TLS relay。
自动 client trust store 安装、upstream TLS relay、透明 HTTPS MITM E2E 和强原始归因不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | 透明 HTTPS MITM E2E、upstream TLS relay、自动 client trust store 安装、完整 proxy-side 动作执行、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM lifecycle、operator-managed client trust contract 与 plaintext bridge 可配置和报告；`product_proxy` backend 从 typed leaf material refs 合成 first-party proxy TLS 参数，agent 侧 typed material refs 是 MITM material source of truth；透明 HTTPS MITM E2E 与 upstream TLS relay 仍缺失 | 第 11、18、32 节 |
