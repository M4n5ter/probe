...
- `l7_mitm`
  ...
  - external backend contract 由第 18 节的 MITM backend contract 定义，包含 backend kind、typed material refs、
    本机 listener readiness gate，以及配置 plaintext bridge 时的 feed openability gate。
  ...
  - 边界：readiness probe 不是周期性进程监督，不证明 IPv4/IPv6 全族 transparent listener 都可用，不证明
...
- external L7 MITM contract 可声明 CA/leaf/upstream trust material refs，并要求本机 listener readiness gate 可达。
- L7 MITM 还需要信任材料分发、periodic external proxy health、独立 proxy lifecycle、proxy-side policy hook 和审计事件。
...
- 生命周期面：external backend contract 只接受外部 L7 backend 监听端口；agent 负责 selector-scoped redirect，不负责启动或监督
  L7 proxy 进程。
...
- L7 生命周期：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、policy hook、audit event 和 periodic health/status contract。
...
contract/readiness/plaintext bridge 的配置、plan、status、check 和 fail-closed 边界已经存在；agent-managed L7 proxy 仍未实现。
...
flow-aware classifier、external proxy runtime health proof 或非 UID/GID-only 的 process-scoped outbound setup。出站透明代理要求该 capability
...
audit/status contract 作为同一个 executable backend 的门控条件；完善 L7 MITM backend 时，还必须接入 trust/install 生命周期、proxy health、
plaintext bridge 和 L7 分类。
...
- `interception.mitm.backend_readiness_probe.timeout_ms` 必须落在有界范围内。
- backend readiness probe 只证明配置的本机 TCP endpoint 可达；它不证明 IPv4/IPv6 全族监听、TPROXY original-destination
...
- RuntimePlan/check/status 会表达 external MITM backend contract、capture-event plaintext bridge 和 resolved MITM material refs。
...
| P2 | Active | 建立显式 L7 MITM backend。 | external backend contract、MITM CA/material、入站/出站 transparent MITM、proxy health、plaintext bridge、L7 policy hook。 | 默认全机 MITM 不可用；显式 MITM 有独立 health/status/audit/E2E。 |
