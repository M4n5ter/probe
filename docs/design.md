...
| `host-rules` | root/net-admin | 透明拦截 host rule lifecycle，目前覆盖 nftables/TPROXY 入站路径。 |
...
| 当前拦截验收 | dry-run typed verdict、remote external manifest GET、manifest selector 与主配置 selector 合成、process/traffic scoped dry-run decision、manifest profile unsupported audit、online enforcement manifest reload active planner swap/invalid reload retention/setup-time surface rejection、显式 backend 下的 owner-verified TCP socket destroy audit decision，以及透明拦截 nftables/TPROXY plan/probe/lifecycle、managed relay runtime metrics 和隔离 netns 入站截获/转发 E2E | 第 18、31 节 |
