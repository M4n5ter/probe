...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、operator-managed client trust contract、plaintext bridge、proxy-side hook wiring、typed execution evidence、managed backend HTTP data-plane canary、正式 HTTP plaintext proxy deny 执行、backend-owned action report，以及 gated Linux socket destroy destructive path。 | 内置 TLS MITM data-plane、upstream TLS relay、强原始归因、自动 client trust store 安装、pre-connect deny。 |
