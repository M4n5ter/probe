...
| `transparent_interception` | Available when the selected executable Linux/root/nft/ip transparent interception lifecycle is usable | typed strategy/proxy plan、按方向投影的 `local_setup_projection` reporting、inbound TPROXY nftables/policy-route lifecycle、host owner lock、managed plain TCP relay、proxy runtime status/metrics、active proxy health probe，以及 outbound transparent proxy host-rule lifecycle 已进入 RuntimePlan/check/status 和 agent runtime。`outbound_transparent_proxy` 当前只允许 `managed_tcp_relay`：agent 以 `transparent-linux` artifact spec 与最终 outbound host-rule scope 生成 OUTPUT/dstnat setup，并准备 managed relay plan；随后执行 `nft --check`、获取 table-scoped `sssa_probe` owner lock、best-effort 清理旧 owned table 和保留 policy routing 遗留、启动 agent-managed relay，再安装 nft 规则；失败会清理已应用状态并停止 proxy。RuntimePlan 显式拆分 inbound TPROXY route mark、outbound proxy bypass mark 和 inbound TPROXY route table，避免把“拦截后本地路由”和“代理自身出站绕过”混成一个 host resource。proxy relay 已把 target recovery 建模为显式策略：入站 TPROXY 使用 accepted socket `local_addr()`，出站透明代理使用 Linux `SO_ORIGINAL_DST` / `IP6T_SO_ORIGINAL_DST` resolver，并在 upstream connect 前设置 `SO_MARK` 绕过自身 OUTPUT redirect；relay 还会用本机地址 inventory 拒绝 original destination 指向本地 proxy listener 的自循环。`xtask e2e-transparent-tproxy-loopback` 覆盖隔离 network namespace 中的真实 PREROUTING + managed relay 路径，`xtask e2e-transparent-tproxy-process-loopback` 在同一路径上覆盖 process-scoped setup-time listener proof；`xtask e2e-transparent-outbound-proxy-loopback` 在隔离 network namespace 中启动真实 agent，验证 OUTPUT redirect、managed relay、original destination recovery、proxy bypass mark、upstream connect success 和 cleanup 的 data-plane 闭环；`xtask e2e-transparent-linux-outbound-redirect-artifact-netns` 是单独 Linux artifact acceptance case，在隔离 netns 中从 explicit artifact spec 与 outbound host-rule scope 生成 OUTPUT/dstnat artifact，并执行 `nft --check`、安装、规则列出断言和清理。agent 单元测试覆盖 outbound nft check failure 不会安装规则或启动 proxy，以及 outbound activation 会在安装前清理同 owner 下的保留 policy routing。 | capability 表示当前策略所需的 executable Linux transparent interception backend 可用，不表示 selector 一定可投影、process/flow classifier 已实现、MITM 已实现或外部 manifest 的最终 selector 已提前校验。`external` proxy mode 对出站透明代理仍 fail closed，直到存在可验证的外部 proxy self-bypass contract；webhook exporters 与出站透明代理组合仍 fail closed，直到 agent-owned outbound control-plane sockets 有可验证 bypass contract；process-scoped/flow-aware outbound setup 仍 fail closed，当前只有可投影 host-rule outbound scope 可执行。outbound transparent proxy 只是 plain TCP transparent relay，不等于 MITM；L7 MITM 还需要证书注入、信任材料分发和独立 proxy lifecycle。Linux artifact acceptance 证明 artifact 语法与 owned table cleanup，真实 agent outbound data-plane 由 `e2e-transparent-outbound-proxy-loopback` 覆盖。 |
...
- `enforcement.interception.strategy = "none"` 默认不要求额外能力；`"inbound_tproxy"` 要求本地显式 selector、proxy listen port、平台入口层面的 `transparent_interception` capability、`enforcement.mode = "enforce"`、live host capture，并且不能与 connection backend 同时启用，直到真实 composite execution backend 存在；`"outbound_transparent_proxy"` 当前要求 `managed_tcp_relay`、本地显式 selector、proxy listen port、平台入口层面的 `transparent_interception` capability、`enforcement.mode = "enforce"` 和 live host capture，并且同样不能与 connection backend 同时启用。由于 webhook exporter 当前无法给 agent-owned outbound control-plane sockets 设置 verified bypass mark，`outbound_transparent_proxy` 与 webhook exporters 组合会在 runtime validation fail closed；file exporter 仍可作为本地交接路径使用。RuntimePlan/check/status 可以通过真实 composition 路径表达 OUTPUT/dstnat artifact、proxy port 和 proxy bypass mark；agent setup scope 校验通过 `transparent-linux` artifact planner 把最终 outbound host-rule scope 渲染成 OUTPUT/dstnat nft artifact，并拒绝 wildcard remote port、process-scoped outbound setup 和 flow-aware outbound setup。
...
17. `e2e-transparent-outbound-proxy-loopback` 在 root/net-admin 环境下启动隔离 network namespace 中的真实 agent，以 host-rule outbound selector 安装 OUTPUT/dstnat redirect，启动 agent-managed relay，通过 loopback client/upstream 响应和 admin transparent proxy metrics 验证 relay accepted、original destination upstream connect success、proxy bypass mark 防自捕获和 shutdown cleanup。它证明 plain TCP outbound transparent proxy data-plane，不证明 webhook control-plane bypass、process/flow-scoped outbound setup 或 L7 MITM lifecycle。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle、可选 managed TCP relay、基础运行态指标、配置化 active proxy health probe、relay-connect upstream connect telemetry、出站 selector projection、Linux original-destination resolver、proxy pre-connect `SO_MARK` 原语、RuntimePlan/check/status outbound redirect artifact、shared Linux OUTPUT/dstnat artifact planning、agent outbound host-rule managed relay lifecycle、root/net-admin netns Linux artifact E2E 和真实 agent outbound data-plane E2E 已接入；L7 MITM 作为后续独立 backend | 入站用 nftables/TPROXY 透明转发到本地代理是已验证可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理当前只支持 host-rule scope + managed relay；`external` proxy self-bypass、agent control-plane outbound bypass、process/flow-scoped outbound setup 和 L7 MITM 证书/分类 lifecycle 仍是缺口。 |
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；relay 目标恢复已拆成显式策略，出站透明代理复用 Linux original-destination resolver 原语，proxy connect path 具备 pre-connect `SO_MARK` 原语；出站透明代理已有按 outbound selector 投影的 status/check 表达、RuntimePlan outbound redirect artifact、`transparent-linux` OUTPUT/dstnat artifact planning、Linux artifact acceptance、host-rule managed relay runtime activation 和真实 agent outbound data-plane E2E。`external` proxy self-bypass、agent control-plane outbound bypass、process/flow-scoped outbound setup 和 L7 MITM backend 的证书注入/L7 lifecycle 仍未完成 | 第 11、18、32 节 |
