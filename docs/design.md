...
当前已验证的 TLS plaintext material check 路径：capture 单元测试覆盖 SSLKEYLOGFILE/NSS key log parser 的 comment/blank line 跳过、已知 label、future uppercase label、三字段约束、context 长度、secret hex 校验和错误不泄漏 secret 值；session secret parser 单元测试覆盖 JSON Lines 导入、protocol/kind count、secret byte min/max、空 material 拒绝、client random 长度、protocol/secret kind 非法组合、validity window、JSON data error 净化和错误不泄漏 secret 值；agent check 测试覆盖显式 key log refs 与 `session_secret_file` refs 解析为 secret-free summary，以及非法 key log/session secret material 让 `check` fail closed。
