...
| `transparent_interception` | Available only when the executable inbound TPROXY Linux/root/nft/ip lifecycle is usable | typed strategy/proxy plan、按方向投影的 `local_setup_projection` reporting、inbound TPROXY nftables/policy-route lifecycle、host owner lock、managed plain TCP relay、proxy runtime status/metrics、active proxy health probe，以及 outbound MITM redirect preview 已进入 RuntimePlan/check/status；preview 记录 OUTPUT/dstnat、proxy port 与 proxy bypass mark，但不生成或安装 nft 脚本。`xtask e2e-transparent-tproxy-loopback` 覆盖隔离 network namespace 中的真实 PREROUTING + managed relay 路径，`xtask e2e-transparent-tproxy-process-loopback` 在同一路径上覆盖 process-scoped setup-time listener proof。 | capability 只表示入站 TPROXY executable lifecycle 的 Linux/root/nft/ip 前置条件满足。它不表示 selector 可投影、flow classifier 已实现、MITM 已实现、outbound redirect 可安装或外部 manifest 的最终 selector 已提前校验。`external` proxy mode 仍要求用户提供本地代理；超出 setup-time procfs listener proof 的 process-scoped setup、flow-aware setup 和 outbound MITM runtime install 当前仍 fail closed。outbound redirect preview 不等于 MITM：original-destination recovery、proxy self-bypass socket marking、证书注入和 L7 MITM proxy lifecycle 尚未接入。 |
...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；后续要启用出站 MITM runtime install，应把 proxy self-bypass socket marking、original-destination recovery、证书材料、MITM CA 注入、output redirect lifecycle 和 L7 分类作为同一个 executable backend 的门控条件。
...
| 显式透明代理/MITM backend | 入站 TPROXY lifecycle、可选 managed TCP relay、基础运行态指标、配置化 active proxy health probe、relay-connect upstream connect telemetry、出站 selector projection 与 RuntimePlan/check/status redirect preview 已接入，出站 MITM 执行 backend 推迟 | 入站用 nftables/TPROXY 透明转发到本地代理是当前可执行边界；`managed_tcp_relay` 只提供 agent-owned plain TCP relay，不等同于 MITM。出站透明代理/MITM 已能形成 typed redirect preview，但还需要 proxy self-bypass socket marking、original-destination recovery、证书注入、output redirect lifecycle 与 L7 分类一起落地后才可安装规则，不能成为默认 capture 路线。 |
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；出站 MITM 已有按 outbound selector 投影的 status/check 表达与 RuntimePlan redirect preview，但不安装 output redirect，直到 proxy self-bypass socket marking、original-destination recovery、证书注入、output redirect lifecycle 和 L7 MITM lifecycle 一起落地 | 第 11、18、32 节 |
