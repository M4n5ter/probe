...
| `transparent_flow_classifier` | Unavailable until a classifier backend is wired | RuntimePlan/check/status 已显式报告 flow classifier capability；`any`、`not` 和 named ref 的 setup requirement 会落到该 capability。 | 还没有 flow-aware classifier 或 registry-backed setup resolution；flow-aware transparent interception 仍 fail closed。 |
...
| `requires_flow_classifier` | transparent-interception-owned classifier selector view 和 `host_rule_boundary`。 | 当前 fail closed；`any`、`not` 和 named ref 需要 flow-aware classifier 或 registry-backed resolution。 |
