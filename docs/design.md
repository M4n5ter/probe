...
| Operator surface | admin/status/prometheus、ratatui TUI | TUI 是服务器本地 agent 工作台：复用或托管启动本地 agent，浏览选中进程事件，展示 provider/open-failure 诊断和 MITM 路线提示；policy source 与大型 MITM backend contract 不由 TUI 代替。 | 本节、Admin 章节 |
