...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、typed material refs 和 TCP readiness probe；真实 TLS MITM 数据面仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；仍需独立实现 client trust/install instructions、
    内置 TLS MITM 数据面、proxy-side policy hook 和 MITM audit event。
...
- L7 MITM 还需要信任材料分发、TLS MITM 数据面、proxy-side policy hook 和审计事件。
...
- `external` backend 由外部进程提供；`managed_process` backend 由 agent 启动直接子进程。
  显式 stop 和 panic/unwind drop 都会 best-effort 清理其进程组。
- MITM backend 必须独立管理信任注入或证书分发、upstream TLS、self-bypass、TLS/L7 protocol state、policy hook 和审计事件。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、policy hook 和 audit event。
- 验证面：入站 external/managed MITM backend 已有真实 agent E2E，证明 backend lifecycle、managed fixture
  readiness 连接后的 bridge feed fan-in、policy decision、managed process cleanup 和 runtime status；出站 MITM、
  proxy-side policy hook 和 failure audit 仍是验收缺口。
...
真实 TLS MITM 数据面、proxy-side policy hook、trust/install lifecycle 和 MITM audit event 不属于当前控制面能力。
...
lifecycle，以及 capture-event plaintext bridge live provider fan-in。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | TLS MITM 数据面、trust/install、L7 policy hook、audit event。 | 显式 MITM 有独立 health/status/audit/E2E。 |
