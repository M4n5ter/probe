...
- Runtime failure policy：配置/对象/ABI/program 形状错误仍 fail-closed。per-PID startup identity 读取中的消失进程、权限拒绝、malformed/racy proc metadata 会被视作 best-effort skip；全局 boot id、procfs 根路径等 dependency 错误不会被误当成单个 PID race。startup scan 没有目标、目标在 attach 前消失，或没有同一 process/library target 上完整的 fd association + plaintext recipe set 时，不会拖死 primary capture，而是记录 disabled TLS plaintext runtime state，并在 runtime status 与 `libssl_uprobe` capability reason 中保留禁用原因。若 attach 过程中已经可能执行过会写 TLS state map 的 uprobe，再遇到 target stale、无 plaintext 可提交、rollback 失败或 fatal program/object 形状错误，则不能安全降级为 disabled sidecar，必须 detach 已提交 links 并 fail-closed。
