...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；真实 TLS MITM 数据面、proxy 内部动作执行和 trust/install lifecycle 仍缺失。 | 第 11、18、32 节 |
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    仍需独立实现 client trust/install instructions、内置 TLS MITM 数据面和 proxy 内部动作执行。
...
- L7 MITM lifecycle audit 已进入 durable export；proxy-side hook contract、`delegated` outcome、RuntimePlan/agent wiring
  和本机 HTTP JSON hook transport 已接入。
- 信任材料分发、TLS MITM 数据面和 proxy 内部保护动作执行仍缺失。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理和 proxy 内部保护动作执行；
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  proxy-side hook transport、`delegated` planner outcome 和 durable decision readback 已由真实 agent/netns/spool E2E 覆盖。
  真实 TLS MITM data-plane verdict/audit、proxy 内部保护动作执行和 managed-process health-transition audit 的 privileged/E2E
  覆盖仍是缺口。
...
RuntimePlan/agent proxy hook wiring、本机 HTTP JSON hook transport 和 delegated decision readback 已进入控制面能力；
真实 TLS MITM 数据面、proxy 内部保护动作执行和 trust/install lifecycle 不属于当前控制面能力。
...
proxy-side L7 readiness、plaintext bridge hardening、proxy 内部保护动作执行和 L7 分类。
...
- plan 保留 MITM backend、backend readiness probe、plaintext bridge、policy hook 和 resolved CA/leaf/upstream trust material refs。
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、plaintext bridge 和 proxy-side hook wiring。 | 内置 TLS MITM data-plane、proxy 内部动作执行、trust/install lifecycle。 |

MITM lifecycle coverage 包含入站/出站 strategy plan/status、出站 selector redirect canary、
external backend、managed-process backend、capture-event plaintext bridge 和 HTTP JSON policy hook。
...
出站 selector redirect canary、MITM plaintext bridge live fan-in，以及本机 HTTP JSON policy hook delegated decision。
...
trust/install lifecycle、真实 TLS MITM 数据面和 proxy 内部保护动作执行不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | TLS MITM 数据面、trust/install、proxy 内部动作执行、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
