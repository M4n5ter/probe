...
- 强 socket lifetime。
- 透明 HTTPS MITM E2E、自动修改客户端 trust store、动态 SNI/ALPN routing 和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
| L7 MITM | `l7_mitm` | 显式 MITM strategy 要求 backend contract、`client_trust.mode = "operator_managed"`、typed material refs、TCP readiness probe、durable lifecycle audit、loopback HTTP JSON policy hook 和明确的 `l7_mitm_plaintext` data-plane provenance；`product_proxy` backend 从 typed bridge、policy hook、target recovery、leaf material 和 upstream trust material 生成 first-party proxy CLI，内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、upstream TLS relay、dynamic flow feed、proxy-side hook deny 执行和 durable delegated decision。透明 HTTPS MITM E2E、强原始归因、动态 SNI/ALPN routing 和自动 client trust store 安装仍缺失。 | 第 11、18、32 节 |
...
| Enforcement | `dry_run_enforcement`、`connection_enforcement` | dry-run 可用；真实 socket destroy 只在显式配置、root/procfs 能力满足、active loopback self-test 通过且 owner 复核通过时执行。 | 第 18 节 |
...
- `l7_mitm`
  ...
  - 边界：readiness/health probe 只证明配置的本机 TCP endpoint 可达；它不是外部进程 supervisor，不证明 IPv4/IPv6
    全族 transparent listener 都可用，不证明 transparent rule path 下的 TLS handshake、SNI/ALPN routing 或 plaintext extraction 成功。
    内置 `traffic-probe-mitm-proxy` 的 TLS listener 单独证明下游 TLS server-side termination 可生成明文 HTTP feed；
    crate-level upstream TLS relay 测试证明 Host-derived server name、导入 trust anchor 和明确 HTTP response framing
    可用于上游 TLS relay，但不证明 native root store 在宿主机上的证书覆盖，也不证明透明 HTTPS 规则路径。
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    同一产品 proxy 支持显式配置证书链和私钥来终止下游 TLS，并支持使用单一有效 HTTP Host 作为 upstream TLS server name；
    配置的 `upstream_server_name` 是对该 Host 的 pin，不允许与转发给上游应用的 Host 分离。
    upstream TLS root store 由 native roots 和导入 upstream trust anchors 组成；crate-level tests 覆盖导入 trust anchor 路径。
    仍需独立实现透明 HTTPS MITM E2E、强原始 socket/process attribution、动态 SNI/ALPN routing 和自动 client trust store 安装。
  ...
  - 验收：`xtask e2e-managed-mitm-proxy-policy-hook-plaintext-bridge-live-sidecar`
    覆盖正式 `traffic-probe-mitm-proxy` 作为 agent-managed backend 被启动、创建 capture-event feed、
    生成 dynamic L7 MITM plaintext flow、通过本机 HTTP JSON hook 接收 deny action、返回带 policy reason 的 403，
    并写入 durable delegated `EnforcementDecision`。
...
- L7 MITM lifecycle audit 由 durable export 表达；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  backend-owned action report、正式 HTTP plaintext proxy deny 执行和 product proxy 下游/上游 TLS relay 由对应测试覆盖。
- 自动 client trust store 安装、透明 HTTPS MITM E2E、动态 SNI/ALPN routing 和强原始归因仍缺失。
...
- Agent 可验证 MITM contract、material refs 和 TCP listener readiness，并把 selector 命中流量导向 configured listener。
  `managed_process` 只覆盖通用进程生命周期；`product_proxy` 额外从 typed readiness target、plaintext bridge、policy hook 和 leaf material
  生成 first-party proxy CLI。backend 的明文输出必须通过 plaintext bridge 或内置 product proxy 数据面进入 pipeline。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  product proxy 的 HTTP plaintext deny 执行、下游 TLS termination 和 upstream TLS relay 已可验证。透明 HTTPS verdict/audit 仍需要
  transparent rule path、TLS termination、policy hook 和 durable decision readback 的独立 E2E 证明，
  区别于已经存在的 backend lifecycle audit。
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  outbound `linux-original-destination` target recovery 和 outbound upstream socket mark；
  product proxy 下游 TLS termination 与 upstream TLS relay 由 crate-level TLS client/server/feed 测试覆盖。
  透明 HTTPS MITM data-plane verdict/audit、强原始归因和 managed-process health-transition audit 的 privileged/E2E
  覆盖仍是缺口。
...
managed backend HTTP data-plane canary、backend-owned action report、正式 HTTP plaintext proxy deny 执行和下游/上游 TLS relay
构成可验证控制面能力；透明 HTTPS MITM E2E、强原始归因、动态 SNI/ALPN routing 和自动 client trust store 安装不属于当前控制面能力。
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、动态 SNI/ALPN routing、非 HTTP plaintext deny 的保护动作执行和 L7 分类。
...
evidence、当前 flow 仍能通过 procfs 复核为同一进程身份、随后仍能被 `ss -K` 匹配且内核报告销毁了现有 TCP socket 时输出 `applied`。
WSL2 或受限内核即使提供 `ss --kill`，也可能无法实际销毁 loopback socket；这种环境应在 capability probe 阶段标记
...
- hook request JSON 包含 `requested_action`、完整 `verdict` 和触发该 verdict 的 `EventEnvelope`。
  ...
  | --- | --- | --- | --- | --- |
  | `delegated` | `executed_action` | `delegated` | policy requested action | MITM proxy 声明已执行请求的保护动作；`EnforcementDecision.execution` 记录 `proxy_side_hook`、`surface = "l7_mitm"`、`executed_action` 和代理 reason |
...
| Enforcement | enforcement、transparent、MITM cases | user/root | dry-run、manifest reload、transparent lifecycle、MITM lifecycle、operator-managed client trust contract、plaintext bridge、proxy-side hook wiring、typed execution evidence、managed backend HTTP data-plane canary、正式 HTTP plaintext proxy deny 执行、正式 proxy 下游/上游 TLS relay、backend-owned action report，以及 gated Linux socket destroy destructive path。 | 透明 HTTPS MITM E2E、动态 SNI/ALPN routing、强原始归因、自动 client trust store 安装、pre-connect deny。 |
...
`applied` decision 与被保护 fixture 非成功退出；WSL2/受限内核这类 `ss --kill` 可见但不销毁 loopback socket 的环境必须产生 suite-level
...
已在 status/health 中表达；provider-level firing/loss activity 已在 status/metrics 中表达；per-link kernel liveness、
...
当前仍没有 unbounded/general sequence resynchronization、TLS 1.2 key block decrypt、内置透明 HTTPS MITM E2E 或动态 SNI/ALPN routing。四条 privileged/live
...
backend-owned hook、downstream deny response，以及显式 TLS certificate/private-key 配置下的 downstream TLS termination
可由产品二进制持有；同一数据面验证 Host-derived server name、导入 trust anchor、HTTP response framing
和 upstream TLS relay。native root store 覆盖范围取决于宿主机 trust store，不作为 crate-level 数据面证据。
它不替代透明 HTTPS MITM 规则路径验证。
自动 client trust store 安装、透明 HTTPS MITM E2E、动态 SNI/ALPN routing 和强原始归因不属于当前能力边界。
...
| 优先级 | 状态 | 目标 | 主要交付 | 验收信号 |
| --- | --- | --- | --- | --- |
| P0 | Active | 收敛主要缺口，避免 best-effort 被误读成强保证。 | kernel socket-object lifetime、precise flow-specific lost-event reconstruction、TLS ringbuf saturation E2E、per-link kernel liveness beyond provider activity。 | capability、metrics、export 和 E2E 状态一致。 |
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | 透明 HTTPS MITM E2E、自动 client trust store 安装、动态 SNI/ALPN routing、完整 proxy-side 动作执行、data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM lifecycle、operator-managed client trust contract 与 plaintext bridge 可配置和报告；`product_proxy` backend 从 typed leaf/upstream material refs 合成 first-party proxy TLS 参数，agent 侧 typed material refs 是 MITM material source of truth；透明 HTTPS MITM E2E、动态 SNI/ALPN routing 和自动 client trust store 安装仍缺失 | 第 11、18、32 节 |
