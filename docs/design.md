...
- 已实现 `check` 的 TLS plaintext material 内容核验：对显式引用的 `key_log_file` 通过 `TlsMaterialFileStore` 读取有界 regular file，并按 NSS/SSLKEYLOGFILE 三字段语法解析 label、context 和 secret hex；解析阶段会构造 secret-bearing typed material store，但 check report 只输出 entries 与 label counts，不输出 secret bytes，也不在错误中泄漏 secret 字段值。`session_secret_file` 使用 JSON Lines 导入契约，字段级 schema 集中在 TLS 材料小节说明；`check` 会校验 protocol/secret kind 组合，只输出 entries、protocol/kind counts 和 secret byte min/max 摘要，JSON parse 错误只暴露类别和位置，不回显输入片段。key log 和 session secret lookup 在不能唯一选择 material 时会显式返回 ambiguous，而不是按文件顺序隐式选择第一条；session secret record decrypt core 当前支持 TLS 1.3 application traffic secret + cipher suite metadata 下的 protected record authenticated decrypt，可通过 `Tls13SessionSecretPlaintextAdapter` 输出统一 plaintext event，可由 `Tls13SessionSecretHandshakeObserver` 从明文 TLS 1.3 ClientHello/ServerHello capture bytes 提取 client random、server random、selected cipher suite、handshake message offset 和完成该 message 的 TLS record 边界，可由 `Tls13SessionSecretStreamAdapter` 消费有序 ciphertext chunk/gap 后维护 TLS record stream、sequence 和 plaintext offset，可由 `Tls13SessionSecretFlowBindingPlanner` 将已知 client random、typed application traffic secret kind、typed lookup time 与 flow cursor 显式解析成 binding，可由 `Tls13SessionSecretFlowDecryptor` 在该 typed binding 后消费统一 `CaptureEvent` 输出 plaintext events，并可由 `Tls13SessionSecretDecryptingProvider` 包装任意 capture provider，对已显式绑定 stream 输出 `tls_session_secret` plaintext `CaptureEvent`、抑制对应密文、保留未绑定流原样、在 terminal decrypt gap 后继续抑制该 stream 后续密文，并把上游 enforcement evidence/propagation 传播到生成的 plaintext bytes/gap；`Tls13SessionSecretAutoBindingProvider` 可在观察到 ClientHello 后用 authenticated application record 自动绑定 session secret candidate，并对首个候选 TLS record 跨 `CapturedBytes` 的场景做 bounded pre-bind buffering；若后到 unrelated/pass-through event 会越过 held raw，则先释放 held raw 以保护 provider 输出顺序；agent composition root 现在会在 live primary provider 外层按 `session_secret_refs` 加载有界 regular file material、合并 session secret records、调度 `session_secret_file` runtime refresh，并接入 `Tls13SessionSecretAutoBindingProvider`；missing/empty 初始 material 会以 pending state 启动，运行期 pending 或失败 refresh 保留当前 store 并继续 primary capture。该路径只覆盖 session-secret auto-binding，不等同于 keylog-derived live provider、unbounded/general sequence resync 或 TLS 1.2 解密。
- `session_secret_file` runtime refresh 的 store replacement 是 all-or-nothing 不变量：多 `session_secret_refs` 中只要有任意一个 configured file 缺失、为空或尚未 ready，本轮 refresh 就是 pending，必须保留当前 store；解析失败、或某个非空 configured file 没有贡献任何可用于 live auto-binding 的 TLS 1.3 application traffic secret record，本轮 refresh 就是 failed，同样不替换当前 store。这样避免用部分 material 静默替换完整 store，导致已受管流的后续解密/防护上下文隐性丢失。
- TLS session-secret auto-binding 的 pre-auth candidate 只保存 `client_random`、traffic secret kind、lookup time、flow/direction 和 stream cursor 这类 typed lookup intent；在后续 application record probe 时才用当前 store 解析 material，并且只有 authenticated decrypt 成功后才创建带 record snapshot 的 flow binding。probe 会在固定 sequence candidate window 内认证试探；若以非零 TLS application sequence 绑定，绑定流会先输出 `tls_session_secret` plaintext gap，表达更早的 application plaintext 已不可用。已绑定 stream 会继续使用认证成功时冻结的 material record，不会被后续 `session_secret_file` refresh 悄悄改写；未认证 candidate 和 waiting-for-material candidate 则可以从后续 refresh 得到的当前 store 中受益。provider store replacement 只替换 material store，并在同一个 keyed candidate set 内把可用的 `WaitingForMaterial` entry 激活为 `Probing`；候选容量、FIFO、gap/close 清理和 buffered raw release 都只有一个 owner，避免 refresh 半应用或 pending/probing 两套规则分叉。
...
`session_secret_file` runtime refresh 不变量：

| 状态/动作 | 不变量 | 对 active store 的影响 |
| --- | --- | --- |
| `not_configured` | 没有配置 `session_secret_refs`。 | 无 store。 |
| `disabled` | 配置了 refs，但当前 capture mode 不是 live host capture，auto-binding 不执行。 | 无 store。 |
| initial pending | configured refs 缺失、为空或尚未 ready。 | 无 store，candidate 只保留 lookup intent。 |
| active | 全部 configured refs ready，且每个非空 ref 都贡献至少一个可用于 live auto-binding 的 TLS 1.3 application traffic secret。 | 生成新 active store，并激活 matching waiting candidate。 |
| later pending | 新一轮 refresh 缺失、为空或未 ready。 | 保留旧 active store。 |
| failed | parse failure、ambiguous lookup 或非空 ref 没有 live auto-binding record。 | 保留旧 active store，更新 `last_failure`。 |
| authenticated bind | candidate 用当前 store 认证解密 application record 成功。 | flow binding 冻结认证成功时的 material record；后续 refresh 不改写该 binding。 |
...
| keylog/session decrypt hints | Session-secret live auto-binding when live host capture and explicit `session_secret_refs` are configured; keylog remains material/check only | TLS plaintext plan 已拆成 instrumentation 与 decrypt hints：instrumentation 表达 selector、capability requirement、`libssl_uprobe_object_path` 和 reconcile runtime；decrypt hints 表达 keylog/session secret material refs。`check` 可输出 libssl uprobe artifact path metadata，并可对显式 `key_log_file` refs 做 SSLKEYLOGFILE 语法核验和 secret-free 摘要，对显式 `session_secret_file` refs 做 JSON Lines session secret schema 校验并输出 secret-free 摘要。capture 层已有 `Tls13ApplicationDataDecryptor`、`Tls13SessionSecretPlaintextAdapter`、`Tls13SessionSecretHandshakeObserver`、`Tls13SessionSecretFlowBindingPlanner`、`Tls13SessionSecretStreamAdapter`、`Tls13SessionSecretFlowDecryptor`、`Tls13SessionSecretDecryptingProvider` 和 `Tls13SessionSecretAutoBindingProvider`，可用 typed `TlsSessionSecretRecord` 中的 TLS 1.3 application traffic secret 与 `cipher_suite` 派生 AEAD key/IV，对 protected record 做 authenticated decrypt，把 application data 输出为 `PlaintextEvent(source = tls_session_secret)`；agent live composition root 会在 live host capture 且配置 `session_secret_refs` 时通过 `TlsMaterialFileStore` 读取有界 regular file、解析多个 `session_secret_file`，只从可消费的 TLS 1.3 application traffic records 构建 lookup-unique auto-binding store，并按 `tls.plaintext.decrypt_hints.refresh_interval_ms` 重读 material；missing/empty 初始 `session_secret_file` 会以 pending state 启动，运行期 pending 或失败 refresh 会保留当前 store 并继续 primary capture，只有全部 configured refs 都 ready、且每个非空 ref 都至少贡献一个可用于 live auto-binding 的 TLS 1.3 application traffic secret record 时才替换 provider store。然后 agent 把 primary live provider 包装为 `Tls13SessionSecretAutoBindingProvider`，未配置 session secret refs 或非 live host capture 时 primary provider 保持原样；handshake observer 可在 bounded active stream budget 和 bounded captured hello body budget 下从明文 TLS 1.3 ClientHello/ServerHello 提取 client random、server random、selected cipher suite、handshake message offset 和完成该 message 的 TLS record 边界，并支持 handshake message 跨 TLS record 分片重组；明显非 TLS record、超出 hello body budget、application data record、gap 和 close 会清理 observer stream state，用于后续 live binding candidate；binding planner 在调用方已经知道 client random、typed application traffic secret kind、typed lookup time、flow/direction 和 stream cursor 时，将 material lookup 的 missing/ambiguous 显式 fail closed，且 handshake/exporter/master secret kind 在 candidate 类型上不可表达；stream adapter 在按 flow/direction 有序输入 ciphertext chunk/gap 时维护 ciphertext offset、TLS record framing、sequence number 与 plaintext offset；flow decryptor 要求调用方先用 typed binding/cursor 显式绑定 flow + direction + session secret record + stream cursor，再通过统一 `CaptureEvent` 入口输出 plaintext events；decrypting provider 可包装任意 capture provider，对已显式绑定 stream 抑制密文并输出 `tls_session_secret` plaintext `CaptureEvent`，未绑定流保持原样；auto-binding provider 在观察到 TLS 1.3 ClientHello 后创建双方向 application traffic secret candidate，candidate 数量有固定上界；material 未到达时会保留 typed pending candidate，pre-auth candidate 只保存 lookup intent，并在后续 application record probe 时解析当前 store；它会在 outer application_data record 上用 bounded sequence candidate window 做 authenticated decrypt，成功后才绑定当前 flow/direction 并冻结该 binding 的 material record；若绑定 sequence 非零，会先输出 `tls_session_secret` plaintext gap 表达早期 application plaintext 不可用；候选 record 跨 `CapturedBytes` 时会先缓冲到完整 record；若后到 unrelated/pass-through event 会越过 held raw，则先释放 held raw；非 application_data record 和认证失败都受 bounded resync budget 约束，未绑定的密文保持原始 capture event；provider 的 suppression registry 不等同于 decryptor active stream，decryptor 因 offset mismatch、上游 gap、record 过大或认证失败进入 terminal gap 后，provider 仍继续抑制该显式绑定 stream 的后续密文，直到真实 close 或 observation finish；生成的 plaintext bytes/gap 会继承最强上游 enforcement evidence 与 propagation，真实 close 前若 suppressed ciphertext 升级为 observation-only flow evidence，会先发 carrying gap 再发 plaintext close。offset mismatch、上游 gap、record 过大、认证失败、close 时未完成 TLS record 或 observation finish 时已吞掉的未完成 TLS record 都会转换为 plaintext gap 并清理对应 decryptor stream，其中 observation finish 不伪造 connection close；plaintext offset 溢出不会静默饱和为最大 offset，也不会半推进 TLS sequence；支持 `0x1301`、`0x1302`、`0x1303`，错误路径不会泄漏 secret。 | unbounded/general record sequence resync 与 keylog live provider 仍缺失；runtime capability matrix 报告本地 session-secret record decrypt core；live host capture path 在 `session_secret_refs` 配置后启用 session-secret auto-binding wrapper，但只有 material refresh 在 application record 被处理前产出可消费 TLS 1.3 application traffic secret、或后续 record 在 bounded sequence candidate window 内认证命中时才能解密该 record；已透传密文不会回溯，material 后到且当前 chunk 不在 TLS record 边界时仍可能只能 raw 透传；keylog material 缺 cipher suite/record sequence 仍不能单独完成解密；证书/私钥不会被误称为通用解密材料；`e2e-tls-session-secret-auto-binding-loopback` 覆盖预置 `session_secret_file`，`e2e-tls-session-secret-material-refresh-auto-binding-loopback` 覆盖 ClientHello 后写入 `session_secret_file` 的 privileged/live auto-binding；live keylog tail 仍缺失。 |
...
`key_log_file` 和 `session_secret_file` 是 plaintext decrypt hint material，只能由 TLS plaintext plan 显式引用。`tls.plaintext.decrypt_hints.key_log_refs` 必须指向 `key_log_file` material，`tls.plaintext.decrypt_hints.session_secret_refs` 必须指向 `session_secret_file` material；引用不存在、空字符串、类型错配或同一 refs 列表内重复都会在基础配置校验阶段失败，避免引用 multiplicity 改变后续 secret lookup 语义。runtime plan 会把这些 refs 解析成 typed material plan，status 会对 configured plaintext refs 做 metadata-only source check，并把 `tls.plaintext.decrypt_hints.refresh_interval_ms` 传入 live decrypt hint wrapper。`check` 会进一步读取显式引用的 plaintext material：`key_log_file` 按 SSLKEYLOGFILE/NSS key log 语法核验三字段、label、context hex 和 secret hex，只输出 entries 与 label count 摘要；`session_secret_file` 按 JSON Lines 导入契约校验，每条记录要求 `protocol = "tls12" | "tls13"`、`secret_kind`、32-byte hex `client_random`、hex `secret`，并可带 32-byte hex `server_random`、`0x` + 2-byte hex `cipher_suite`、`not_before_unix_ns` 和 `not_after_unix_ns`。当前合法组合是 `tls12 + master_secret`，以及 `tls13 + client_handshake_traffic_secret | server_handshake_traffic_secret | client_application_traffic_secret | server_application_traffic_secret | exporter_secret`；不合法组合在导入边界 fail closed。check report 只输出 entries、protocol/kind counts 和 secret byte min/max 摘要。`session_secret_refs` 会在 agent live composition root 中启用 TLS 1.3 session-secret auto-binding wrapper；agent loader 在打开 primary live provider 前读取并解析 material，后续按 refresh interval 重读同一 refs，并只从可被当前 auto-binding provider 消费的 TLS 1.3 application traffic records 构建 lookup-unique store；missing/empty 初始 `session_secret_file` 会以 pending store 启动，支持 ClientHello 先到、material 后到的同一握手后续 application record auto-binding；若早期 application record 已透传，后续 record 只能在 bounded sequence candidate window 且当前 chunk 从 TLS record 边界开始时被认证绑定，绑定时会先输出 plaintext gap，不回溯重写已透传密文；非空但非法的初始 material 仍 fail closed，运行期 pending 或失败 refresh 则保留当前 store 并继续 primary capture，只有读到新的可用 store 才替换 provider store。exact duplicate live records 会被去重，不同 live records 若在相同 protocol、secret kind、client random 和有效时间窗口上重叠则 fail closed，避免 merged store 把唯一 lookup 变成 ambiguous 后静默透传密文。TLS 1.2 master、TLS 1.3 handshake/exporter 等非 live records 仍可进入 check summary，但不会进入当前 auto-binding provider store。`key_log_refs` 目前仍只提供导入、检查和状态事实，不会单独启用 live keylog decrypt，也不会改变 libssl uprobe attach 行为。未来 keylog 解密应作为独立 plaintext backend 接入。

当前 `session_secret_refs` 是 live auto-binding 的配置面，不是“仅导入 material”。当 runtime plan 选中 live host capture backend（eBPF 或 libpcap）时，当前 auto-binding view 只消费可被 `Tls13ApplicationDataDecryptor` 使用的 TLS 1.3 client/server application traffic secret：必须有 `cipher_suite` metadata、suite 在当前支持范围 `0x1301 | 0x1302 | 0x1303` 内，且 traffic secret 长度匹配对应 HKDF hash。TLS 1.2 `master_secret`、TLS 1.3 handshake/exporter secret 或缺少 `cipher_suite` 的 application traffic secret 可以作为 schema 边界被解析或在未来 backend 中使用，但不能单独驱动当前 live auto-binding。`check` 仍验证当前文件内容并对非空但不可用于 live auto-binding 的 session-secret refs fail closed；`run` 允许 missing/empty `session_secret_file` 作为 pending material 启动，用 runtime refresh 接收稍后写入的 usable record。plaintext feed/replay/no-live mode 下 `check` 只做 material parse 与 secret-free summary，不因当前 live provider 不消费这些 records 而失败。
...
- `run` 在构造 configured enforcement、启动 admin/export worker、打开 spool 和启用 transparent interception 前，会先完成 capture provider 的 side-effect-free preflight；当前该 preflight 覆盖 `session_secret_refs` live auto-binding material 读取、解析、auto-binding view 构建、lookup-unique 检测和 live-consumable 校验。这个顺序避免 TLS material 错误时仍短暂安装 nftables/TPROXY host rules。
...
agent check 测试覆盖显式 key log refs 与 `session_secret_file` refs 通过 typed store 解析为 secret-free summary、合并后重叠的 `session_secret_file` refs 复用 live loader 语义并让 `check` fail closed，以及非法 key log/session secret material 让 `check` fail closed；agent TLS decrypt hint loader 单元测试覆盖无 `session_secret_refs` 时保持 primary provider、配置 `session_secret_refs` 后包装 auto-binding provider、empty `session_secret_file` 可作为 pending live material 启动、运行期 refresh 读到损坏 material 时保留当前 store、运行期 refresh 读到 empty material 时不清空当前 store、多 ref refresh 遇到缺失/empty 或非 live ref 时不部分替换 active store、多个 `session_secret_file` 合并为同一 store、exact duplicate records 去重、同一 lookup key 的重叠记录 fail closed，以及非法 material fail closed 且不泄漏 secret。该边界为后续 keylog/session decrypt provider 提供 typed lookup、TLS 1.3 handshake observation、record decrypt、plaintext bridge、capture-layer ciphertext stream adapter、显式 flow-binding decrypt core 和 provider adapter 能力，且 `Tls13SessionSecretAutoBindingProvider` 单元测试覆盖 ClientHello 后保留不可解密外层 application_data、store 在 ClientHello 后到达仍可绑定后续 application record、store 在首条 application record 已 raw 透传后到达时可用 bounded sequence candidate window 绑定第二条 application record 并先输出 plaintext gap、pre-auth candidate 在认证绑定前使用刷新后的当前 material、认证成功后的 bound stream 保留已冻结 material、同一 `CapturedBytes` 中途绑定、首个 application record 跨 `CapturedBytes` 时先缓冲再绑定，并在认证成功的 application record 处输出 plaintext。当前仍没有 keylog-derived cipher suite inference、unbounded/general sequence resynchronization、TLS 1.2 key block decrypt 或 live keylog provider；`e2e-tls-session-secret-auto-binding-loopback` 已覆盖预置 `session_secret_file` 的 privileged/live auto-binding，`e2e-tls-session-secret-material-refresh-auto-binding-loopback` 已覆盖 ClientHello 后写入 `session_secret_file` 的 privileged/live auto-binding；两者都不证明 keylog-derived live provider。
...
| TLS 路线 | 非 MITM 优先，当前已有 explicit key log/session secret material typed store、secret-free check summary、TLS 1.3 session-secret protected-record decrypt core、TLS 1.3 handshake observer、`PlaintextEvent` bridge、capture-layer binding planner、ciphertext stream adapter、显式 flow-binding decrypt core、显式绑定 decrypting provider adapter、capture-layer automatic binding provider、agent live session-secret auto-binding wrapper、`session_secret_file` runtime refresh 与 libssl uprobe demo；material store 可按 client random 等非 secret key 查找 secret，session secret core 可在有 cipher suite metadata 和 sequence 的前提下认证解密 TLS 1.3 application data record，并以 `tls_session_secret` provenance 进入 plaintext 抽象；handshake observer 可从 capture bytes 中提取 TLS 1.3 ClientHello/ServerHello 的 client random、server random、selected cipher suite、handshake message offset 和完成该 message 的 TLS record 边界，并支持 handshake message 跨 TLS record 分片重组；binding/stream/flow/provider core 已处理 application traffic secret lookup、record framing、ciphertext/plaintext offset、sequence、gap/poison、flow close/observation finish cleanup、绑定流密文抑制、terminal decrypt gap 后继续 suppression、上游 enforcement evidence 传播和未绑定流透传，auto-binding 在 ClientHello 后通过 authenticated application record 绑定候选，支持 material 后到时激活 pending candidate、pre-auth candidate 用当前 store 做 probe-time lookup、bounded sequence candidate window 绑定 material 后到时的后续 application record、非零 sequence 绑定前输出 plaintext gap、认证绑定后冻结 material record、首个候选 record 跨 `CapturedBytes` 的 bounded pre-bind buffering，并在后到 unrelated/pass-through event 前释放 held raw 以保护 provider 输出顺序；runtime refresh 对多 ref 保持 all-or-nothing，缺失/empty/parse failure/非 live ref 都不会部分替换 active store；仍不声称已具备 keylog live provider、unbounded/general sequence resync、TLS 1.2 解密、live keylog tail 或已透传密文回溯解密 | 第 11、31 节 |
