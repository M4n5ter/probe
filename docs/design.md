...
本文档是架构事实源，同时记录当前实现状态。当前仓库已经进入 V0/V1 骨架实现阶段：已建立 Rust workspace，完成 replay 驱动的 HTTP/1.x + SSE parser、LuaJIT policy runtime、Fjall export queue、protobuf batch envelope、pluggable compression codec 和 HTTP webhook exporter。尚未实现 live capture provider、ingress journal、libpcap fallback、eBPF 主路径、TLS uprobe provider 和真实 enforcement。
...
- 已实现 replay CLI，用单向输入文件驱动 parser、policy、spool 和可选 webhook exporter。
- 已实现 capability matrix，但 live capture 相关能力必须标记 unavailable；policy/webhook 目前只在 replay pipeline 中可用；durable spool 目前只覆盖 export queue，因此标记 degraded。
- 已实现 selector AST 的基础形态：`match`、`all`、`any`、`not`、`ref`，命名 selector 通过 registry 编译解析。
- 当前 `FjallSpool` 存储的是带 schema 的 `SpoolPayload`；V0 schema 为 JSON framed `EventEnvelope` payload；protobuf batch envelope 通过显式 payload schema 标记该格式。这是过渡契约，不等同于最终 protobuf event envelope。
...
- 当前只实现 export queue；ingress journal 尚未实现，因此 capability matrix 只能把 durable spool 标记为 degraded，而不是完整 available。
- 当前落盘 payload 是带 schema 的 `SpoolPayload`。V0 内容是 JSON framed `EventEnvelope`，用于 replay/debug/export 闭环。该格式必须通过 `payload_schema` 明示，不能伪装成最终 protobuf event schema。
...
- `agent`：主二进制、生命周期、配置加载、admin API。
...
| durable spool 当前实现 | Fjall export queue + `DurableSpool` trait + schema-aware `SpoolPayload`；ingress journal 未实现，能力标记 degraded | 第 21 节 |
