...
| `export_queue` | Available | export lane、per-sink cursor、schema-aware payload、protobuf batch envelope，以及按 planned sinks cursor 下界执行的 acked-prefix cleanup 已存在。 | retention deadline 尚未进入完整运行态。 |
| `webhook_exporter` | Degraded | `run` 可连续 drain planned sinks，支持固定间隔、全局/每 sink batch 预算、single sink timeout、per-sink exponential backoff、cursor-safe export queue cleanup、trust anchors 和 client identity refs。 | retention deadline 尚未实现。 |
...
- planned export worker drain 和 tail drain 会在每轮 sink 尝试后，按当前 cursor-owning planned sinks 的 cursor 最小值有界清理 export queue 中已被全部 planned sinks 确认的前缀；失败或未推进的 sink 会把 cleanup 下界保持在自己的 cursor 上，因此不会破坏 at-least-once 投递，也不会把单次 cleanup 变成无界删除事务。未来如果引入 fire-and-forget exporter，应先在 export plan 中分出不参与 retention 下界的 sink 类型，而不是复用该 cursor 集合。
...
| cleanup | 所有 cursor-owning planned sinks ack 后，按 planned sinks cursor 下界有界清理 export queue 前缀。 | retention deadline 未完成前不做超时丢弃。 |
...
- 当前已实现 per-sink cursor、per-sink batch quota、acked-prefix cleanup 和内存态 exponential backoff，避免某个慢或失败 sink 阻塞其它 sink；未来 retention deadline 和在线 backoff 状态进入 status 后，再把 failed/degraded 语义扩展到这些运行态。
...
- retention deadline 尚未实现；当前 worker 采用配置化固定间隔、全局/每 sink 每轮 batch budget、配置化 sink timeout 和内存态 per-sink exponential failure backoff 的 best-effort drain，并在每轮 drain 后按 planned sinks cursor 下界有界清理已全部确认的 export queue 前缀。tail drain 仍会尽力尝试所有 sink，不使用后台 worker 的 failure backoff，但同样会执行 cursor-safe bounded cleanup。
