...
- 交互模型以 `TuiAction` 为边界。键盘事件和鼠标事件同级地翻译为同一组 action；渲染层输出 hit map，
  tab、字段行、进程行和控制按钮都通过 hit target 驱动鼠标交互。
...
- `unix_http_exporter`
  ...
  - 验收：`xtask e2e-unix-http-exporter` 覆盖真实 agent bounded run tail drain，经 Unix domain socket HTTP
    发送 deflate-compressed protobuf batch，校验 custom header、batch range/id/schema/event set、structured ACK、sink cursor 前进且无 pending records。
...
- 校验每个 batch 的 custom header。
- 校验按 batch sequence 返回的 JSON ack。
- 校验 local-sidecar sink cursor 前进，且该 sink 没有 pending records。
...
- `e2e-suite`
  ...
  - 覆盖：非特权 agent durable output、HTTP/SSE/WebSocket parser、flow-level gap/degraded 语义、
    ...
    remote enforcement manifest、webhook/file/Unix HTTP exporter。
...
- Export
  ...
  - `e2e-unix-http-exporter`，user：
    ...
    structured ACK 后 local-sidecar sink cursor 前进且无 pending records。
