...
| keylog/session decrypt hints | Planned config with explicit check validation plus available `tls_session_secret_record_decrypt` core | TLS plaintext plan 已拆成 instrumentation 与 decrypt hints：instrumentation 表达 selector、capability requirement、`libssl_uprobe_object_path` 和 reconcile runtime；decrypt hints 表达 keylog/session secret material refs。`check` 可输出 libssl uprobe artifact path metadata，并可对显式 `key_log_file` refs 做 SSLKEYLOGFILE 语法核验和 secret-free 摘要，对显式 `session_secret_file` refs 做 JSON Lines session secret schema 校验并输出 secret-free 摘要。capture 层已有 `Tls13ApplicationDataDecryptor` 和 `Tls13SessionSecretPlaintextAdapter`，可用 typed `TlsSessionSecretRecord` 中的 TLS 1.3 application traffic secret 与 `cipher_suite` 派生 AEAD key/IV，对单条 protected record 做 authenticated decrypt，并把 application data 输出为 `PlaintextEvent(source = tls_session_secret)`；runtime capability matrix 会把单 record decrypt core 报告为 `tls_session_secret_record_decrypt`，但这只表示本地 core 可用；支持 `0x1301`、`0x1302`、`0x1303`，错误路径不会泄漏 secret。 | 尚未实现 live TLS record stream provider、按 flow/client random/material/time 的 lookup、record sequence resync 或把该 bridge 接入 agent composition root；keylog material 缺 cipher suite/record sequence 仍不能单独完成解密；证书/私钥不会被误称为通用解密材料。 |
...
| TLS 路线 | 非 MITM 优先，当前已有 explicit key log/session secret material typed store、secret-free check summary、TLS 1.3 session-secret protected-record decrypt core 到 `PlaintextEvent` bridge 与 libssl uprobe demo；material store 可按 client random 等非 secret key 查找 secret，session secret core 可在有 cipher suite metadata 和 sequence 的前提下认证解密单条 TLS 1.3 application data record，并以 `tls_session_secret` provenance 进入 plaintext 抽象，但仍不声称已具备 keylog/session decrypt live provider | 第 11、31 节 |
