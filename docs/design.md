...
| Profile | 默认权限 | 覆盖范围 |
| --- | --- | --- |
| `baseline` | user | 非特权 replay/plaintext/export/policy regression：plaintext feed、SSE、WebSocket、webhook/file exporter、remote enforcement manifest。 |
...
后续 overload E2E 覆盖 metrics 中无静默 gap；如人为制造过载，则出现明确 degraded/gap。
