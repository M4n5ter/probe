...
- 强 socket lifetime。
- 自动修改客户端 trust store、ALPN-aware routing、hostname-to-upstream target routing
  和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
- `product_proxy` backend 从 typed bridge、policy hook、target recovery、CA 或 leaf TLS material
  和 upstream trust material 生成 first-party proxy CLI。
- 内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、
  ...
  server name、dynamic flow feed、proxy-side hook deny 执行、透明入站 HTTPS 规则路径和 durable
  delegated decision。
- 强原始归因、ALPN-aware routing、hostname-to-upstream target routing、自动 client trust store 安装
  和通用 transparent allow-path upstream routing matrix 仍缺失。
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    透明入站 HTTPS E2E 覆盖 product proxy deny path：fresh network namespace client 经 veth/TPROXY
    进入 product proxy，使用 operator-managed CA trust 完成下游 TLS handshake，触发 HTTP JSON hook
    的 deny verdict，返回代理侧 403，并写入 `l7_mitm_plaintext` feed 与 durable delegated decision。
    强原始 socket/process attribution、ALPN-aware routing、hostname-to-upstream target routing、
    自动 client trust store 安装和通用 transparent allow-path upstream routing matrix 仍缺失。
...
- L7 MITM lifecycle audit 由 durable export 表达；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  和透明入站 HTTPS product proxy deny path 由对应测试覆盖。
- 自动 client trust store 安装、ALPN-aware routing、hostname-to-upstream target routing、强原始归因
  和通用 transparent allow-path upstream routing matrix 仍缺失。
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  ...
  deny path 已可验证。透明入站 HTTPS E2E 同时覆盖 transparent rule path、TLS termination、policy hook
  和 durable decision readback；通用 allow-path upstream routing、ALPN-aware routing 和 hostname-to-upstream
  target routing 仍是独立缺口。
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  product proxy 透明入站 HTTPS data-plane verdict/audit 由 privileged netns E2E 覆盖；
  强原始归因、managed-process health-transition audit、ALPN-aware routing、hostname-to-upstream target routing
  和通用 transparent allow-path upstream routing matrix 的 privileged/E2E 覆盖仍是缺口。
...
以及 product proxy 透明入站 HTTPS deny path 构成可验证控制面能力；强原始归因、ALPN-aware routing、
hostname-to-upstream target routing、自动 client trust store 安装和通用 transparent allow-path upstream routing matrix
不属于当前控制面能力。
...
- 已证明 managed backend HTTP data-plane canary、正式 HTTP plaintext proxy deny 执行、正式 proxy
  ...
  server name、product proxy 透明入站 HTTPS deny path、backend-owned action report，以及 gated Linux
  socket destroy destructive path。
- 不证明 ALPN-aware routing、hostname-to-upstream target routing、强原始归因、自动 client trust store
  安装、通用 transparent allow-path upstream routing matrix 或 pre-connect deny。
...
透明入站 HTTPS product proxy deny path 由 privileged netns E2E 覆盖，证明 TPROXY 规则路径、
下游 TLS handshake、HTTP JSON hook deny、代理侧 403、plaintext bridge 和 durable delegated decision
属于同一可执行路径。
自动 client trust store 安装、ALPN-aware routing、hostname-to-upstream target routing、强原始归因
和通用 transparent allow-path upstream routing matrix 不属于当前能力边界。
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | trust store、ALPN/host routing、transparent allow path、proxy action 和 data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
- P2 聚焦自动 client trust store 安装、ALPN-aware routing、hostname-to-upstream target routing、
  通用 transparent allow-path upstream routing matrix、完整 proxy-side 动作执行和 data-plane audit。
...
| MITM/透明代理目标 | 透明拦截使用 typed plan；MITM lifecycle、trust、bridge、product proxy TLS 和透明入站 HTTPS deny path 的事实源见对应章节。 | 第 11、18、32 节 |
