...
`e2e-gap-plaintext-feed` 已覆盖非特权 flow-level gap 不被静默吞掉：gap 既进入 durable ingress，又以 degraded export envelope 暴露给后续 consumer，并且 parser 不会在 gap 后继续把缺失响应体伪装成完整 body。后续仍需要 live provider-level overload/loss E2E：人为制造 ringbuf/provider loss 时必须产生 provider-scoped `capture_loss` 或等价 degraded 信号，并在 admin metrics/status 中可观察。
