...
| `libssl_uprobe` | Probed degraded/unavailable, not selectable as enabled TLS plaintext yet | 已实现 process-scoped discovery/planning、strict TLS artifact preflight、eBPF libssl plaintext artifact contract、userspace Aya loader/provider、ringbuf sample decoder 和 flow-resolver bridge；详情见 TLS plaintext 小节。 | 尚未实现 agent runtime 的动态发现、attach/unattach 生命周期、与 process provider lifecycle/flow resolver 的组合 wiring；producer 在缺少 process-generation/lifecycle owner 前不会设置 `FD_VALID`；即使 artifact preflight 成功，capability 也最多为 degraded，`tls.plaintext.enabled = true` 仍 fail-closed。 |
...
- Runtime gap：已实现 plaintext sample 事件契约、可编译的 eBPF libssl uprobe producer、capture 层 userspace Aya loader/provider，以及 agent registry 对 `tls.plaintext.libssl_uprobe_object_path` 的 strict TLS artifact preflight；尚未接入 agent runtime 的动态 attach lifecycle 和 process lifecycle/flow resolver 组合 wiring。
...
当前已验证的 libssl plaintext ABI/adapter/loader 路径：`ebpf-abi` 单元测试覆盖 process event 与 TLS plaintext event 的 wire kind、header layout、record layout、field offset、encode/decode、invalid direction reject、read-failed payload reject、TLS map spec 表、TLS state layout，以及公共 event header decoder 能在只读 header prefix 时识别 TLS plaintext record；process observation decoder 遇到 TLS plaintext record 会明确拒绝为非 process event。`core` selector 单元测试覆盖完整 flow match、directionless match、process-scope projection 的保守候选语义，以及 canonical Linux proc stat parser 对 comm 中括号的处理。`capture` 单元测试覆盖 wire event 到内部 TLS plaintext sample 的归一化、provider 可从 sample source 读取样本，保留 process、SSL pointer、可选 fd、direction、stream offset 和 captured bytes；flow resolver 命中时由 bridge 统一构造 flow 并输出 `source = libssl_uprobe` 的 plaintext bytes，同一 TCP connection 的多个 sample 即使 timestamp 不同也保持同一 `FlowIdentity`；实际截断 sample 会输出 degraded chunk 和 gap；truncated flag-only 不产生零宽 gap；read-failed sample 只输出 gap；flow 未解析时只输出 gap，不把明文字节伪装成可解析 flow，且外发 reason 不包含 `ssl_pointer`；live provider 遇到 idle sample source 时会继续等待而不是向 pipeline 报告 EOF。loader 单元测试覆盖 process-scoped attach plan 到 process generation/path/symbol/program/kind request 的展开、空计划 fail-fast、超出 `pid_t` 范围或 PID 0 的 fail-before-Aya、attach target identity stale 时 fail-before-Aya、Aya uprobe/uretprobe kind 与 attach kind 的匹配，以及 missing object 在 object preflight 阶段失败；discovery/attach planning helper 单元测试覆盖 process generation 读取与主 discovery 路径复核、selector process prefilter、缺失 generation 拒绝、重复 generation 去重、PID reuse/generation mismatch 拒绝、per-PID discovery error 不阻断其它候选进程、discovery process PID 一致性校验，以及多个 discovery report 的 process groups/degraded reasons 合并。`ebpf-object` 单元测试用从 ABI spec 派生的 TLS plaintext ELF fixture 验证 5 个 map 和 13 个 uprobe/uretprobe program 可被 `aya-obj` inventory 解析并满足 contract；`xtask check-ebpf` 已验证真实 `ebpf-program` process artifact 与 `ebpf-tls-plaintext` TLS artifact 分别满足 strict contract。config/runtime/status/check 测试覆盖 `tls.plaintext.libssl_uprobe_object_path` 的解析、无效路径/错误 provider 组合 fail-closed、RuntimePlan 透传、status/check metadata 输出；agent registry 测试覆盖缺 TLS object path、无效 TLS object、TLS object preflight 成功但 runtime wiring 未完成时保持 degraded，以及缺 procfs socket attribution 时 unavailable。该验证仍不等同于 privileged libssl uprobe attach e2e 或 agent runtime wiring 已完成。
