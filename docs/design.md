...
| Profile | 默认权限 | 覆盖范围 |
| --- | --- | --- |
| `baseline` | user | 非特权 replay/plaintext/export/policy regression：plaintext feed、WebSocket、webhook/file exporter、remote enforcement manifest。 |
...
11. `e2e-tls-session-secret-auto-binding-loopback` 在 root/CAP_NET_RAW 下验证预置 `session_secret_file` material、真实 `agent run`、live libpcap、TLS 1.3 ClientHello/ServerHello observation、session-secret auto-binding、decrypted `tls_session_secret` plaintext、HTTP parser、Lua policy 和 Fjall durable output 的闭环。
12. `e2e-tls-session-secret-material-refresh-auto-binding-loopback` 在 root/CAP_NET_RAW 下验证 `session_secret_file` 初始为空、fixture 先完成 ClientHello/ServerHello 交换且不发送 application record，通过 admin pipeline metrics 确认 handshake-only 阶段已有 capture 进展后，才原子替换写入 material；随后 admin status 等待 `session_secret_refresh.generation` 进入 active，最终 durable spool readback 证明同一 flow 的 live libpcap ClientHello/ServerHello 已进入 ingress，后续 material refresh 激活 pending candidate，application record 再被 authenticated decrypt，并进入 HTTP parser、Lua policy 和 Fjall durable output。
13. `e2e-tls-keylog-auto-binding-loopback` 在 root/CAP_NET_RAW 下验证预置 `key_log_file` material、同一 flow 的 ServerHello selected cipher suite 补齐 keylog-derived record、authenticated auto-binding、ciphertext suppression、HTTP parser、Lua policy 和 Fjall durable output。
14. `e2e-tls-keylog-material-refresh-auto-binding-loopback` 在 root/CAP_NET_RAW 下验证 keylog 初始只有未以 `\n` 提交的 partial traffic secret line，fixture 先完成 ClientHello/ServerHello 交换且不发送 application record，通过 admin pipeline metrics 确认 handshake-only 阶段已有 capture 进展后，再补齐完整 line；最终 durable spool readback 证明同一 flow 的 live libpcap ClientHello/ServerHello 已进入 ingress，runtime refresh 激活 pending candidate，随后 application record 被 authenticated decrypt。第 11-14 项都不证明已经透传出去的密文可回溯解密。
15. `e2e-transparent-tproxy-loopback` 在 root/net-admin 环境下验证隔离 network namespace 中的 nftables/TPROXY host rule lifecycle 与 managed relay 入站转发。
16. `e2e-transparent-tproxy-process-loopback` 在 root/net-admin 环境下复用同一隔离 network namespace/veth/TPROXY/managed relay 路径。它先用不匹配的进程名 selector 启动 agent，断言 agent 在 readiness 前 fail closed、失败原因来自 process selector mismatch，且 nft/policy routing 没有遗留；随后把透明拦截 selector 收窄到持有目标 listener 的 `xtask` 进程名，验证 setup-time procfs listener classifier 能把 process-scoped setup 安全降解为可执行 host rules 并完成真实转发。它不证明规则安装后的 listener 变化可动态追踪，也不证明 cgroup/owner mark classifier 已存在。
17. `e2e-transparent-outbound-proxy-loopback` 在 root/net-admin 环境下启动隔离 network namespace 中的真实 agent，以 host-rule outbound selector 安装 OUTPUT/dstnat redirect，启动 agent-managed relay，同时配置 webhook exporter。用 loopback client/upstream 响应和 admin transparent proxy metrics 验证业务连接 relay accepted、original destination upstream connect success、proxy bypass mark 防自捕获和 shutdown cleanup；用 webhook receiver batch 和精确 relay metrics 验证 agent-owned webhook control-plane socket 通过 bypass mark 直连，没有被 OUTPUT redirect 捕获成第二条 relay。它证明 plain TCP outbound transparent proxy data-plane 与当前 webhook control-plane bypass，不证明非 UID/GID-only 的 process selector、flow-aware outbound setup 或 L7 MITM lifecycle。
18. `e2e-transparent-outbound-external-proxy-loopback` 在 root/net-admin 环境下启动隔离 network namespace 中的真实 agent 和测试外部 proxy，以 host-rule outbound selector 安装 OUTPUT/dstnat redirect，但 agent 不启动 managed relay。测试外部 proxy 对 upstream socket 设置保留 `SO_MARK`，用 loopback client/upstream 响应、webhook receiver batch、admin transparent proxy metrics 为零和 cleanup 验证遵守 reserved-mark contract 的外部 proxy 能绕过 agent-owned OUTPUT redirect，且 agent-owned webhook control-plane socket 同样不会被自捕获。它证明 external self-bypass contract 的可执行路径，不证明 agent 会管理外部 proxy lifecycle、持续检查任意外部 proxy 健康或完成 L7 MITM/certificate lifecycle。
19. `e2e-transparent-outbound-owner-proxy-loopback` 在 root/net-admin 环境下启动隔离 network namespace 中的真实 agent，以 UID/GID-only outbound process selector 生成 `meta skuid`/`meta skgid` socket-owner host rule，并启动 agent-managed relay。测试先以匹配 UID/GID 的 owner 身份运行 client，验证该业务连接被 OUTPUT redirect 捕获；随后在同一 upstream 地址/端口以非匹配 owner 运行 client，验证它直连 upstream 且 agent relay metrics 仍只有一条 accepted relay。它证明 UID/GID socket-owner-scoped outbound selector 可以成为 executable host rule，不证明 PID/name/exe/cmdline/systemd/container 等其它 process selector 已有动态 classifier。
20. `e2e-transparent-linux-outbound-redirect-artifact-netns` 在 root/net-admin 环境下从 explicit `transparent-linux` artifact spec 与 outbound host-rule set 生成 OUTPUT/dstnat artifact，再执行真实 `nft --check`、安装、规则列表断言和 cleanup 验证。它属于 `linux-artifacts` profile，不启动 agent，不证明 agent runtime outbound proxy self-bypass、original destination recovery 或证书/MITM lifecycle。
