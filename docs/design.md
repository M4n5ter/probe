...
- 强 socket lifetime。
- 自动修改客户端 trust store、ALPN-based multi-protocol routing、DNS-discovered upstream route selection
  和完整 L7/TLS proxy 分类。
- 完整 TCP 栈恢复。
- 强进程归因。
...
- 内置 `traffic-probe-mitm-proxy` 覆盖 HTTP plaintext data-plane、显式下游 TLS termination、
  ...
  server name、HTTP/1.1 ALPN advertise/gate、显式 exact / suffix-wildcard host-to-upstream route、
  dynamic flow feed、
...
- 强原始归因、ALPN-based multi-protocol routing、DNS-discovered upstream route selection、
...
- `l7_mitm`
  ...
  - 边界：capture-event plaintext bridge 只接收 MITM backend/feed 提供的统一 capture events；policy hook transport 只证明 agent 能把
    ...
    多个 wildcard 命中时最长 suffix 优先；route 未命中时回退 configured 或 recovered original destination。
    ...
    强原始 socket/process attribution、ALPN-based multi-protocol routing、DNS-discovered upstream route selection、
    自动 client trust store 安装，以及除 WebSocket 101 tunnel 以外的非 HTTP transparent allow-path matrix 仍缺失。
...
- L7 MITM lifecycle audit 由 durable export 表达；proxy-side hook contract、`delegated` outcome、typed execution evidence、
  ...
  exact / suffix-wildcard host-to-upstream route selection、透明入站/出站 HTTPS product proxy routed allow/deny path
  和透明 HTTPS WebSocket tunnel path 由对应测试覆盖。
- 自动 client trust store 安装、ALPN-based multi-protocol routing、DNS-discovered upstream route selection、
...
- L7 数据面：必须拥有 HTTP/TLS protocol state、ALPN/SNI/CONNECT 处理、upstream TLS relay 和保护动作执行点；
  ...
  ALPN-based multi-protocol routing、DNS-discovered upstream route selection 和非 HTTP transparent
...
- 验证面：入站和出站 MITM strategy 的 external/managed-process backend 均有真实 agent E2E，
  ...
  由 privileged netns E2E 覆盖；WebSocket inbound case 覆盖 `*.suffix` route pattern 进入真实 transparent path；
  强原始归因、managed-process health-transition audit、ALPN-based multi-protocol routing、DNS-discovered
...
构成可验证控制面能力；强原始归因、ALPN-based multi-protocol routing、DNS-discovered upstream
...
完整 TLS proxy-side L7 readiness、plaintext bridge hardening、ALPN-based multi-protocol routing、DNS-discovered upstream route selection、
...
- 已证明 suffix-wildcard upstream route selection 的 exact 优先、longest-suffix 优先和 inbound transparent
  WebSocket path 覆盖。
- 不证明 ALPN-based multi-protocol routing、DNS-discovered upstream route selection、强原始归因、
...
ALPN-based multi-protocol routing 或 DNS-discovered upstream route selection。四条 privileged/live
...
- Product MITM
  ...
  - 边界：不证明自动 client trust store 安装、ALPN-based multi-protocol routing、DNS route discovery 或
...
DNS-discovered upstream route selection、强原始归因，以及除 WebSocket 101 tunnel 以外的
...
| P2 | Active | 完成显式 L7 MITM backend 数据面。 | trust store、ALPN multi-protocol routing、DNS-discovered route selection、非 HTTP transparent allow path、proxy action 和 data-plane audit。 | 显式 MITM 有独立 health/status/audit/E2E。 |
...
- P2 聚焦自动 client trust store 安装、ALPN-based multi-protocol routing、DNS-discovered upstream route
