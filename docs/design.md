...
- `RuntimePlan.enforcement` 是能力事实源：`mode_capability` 只表达 mode 自身的能力需求，当前只有 `dry_run` 需要 `dry_run_enforcement`；`execution_surfaces` 显式列出 `enforce` 使用的执行面；`connection.capability` 只由显式 connection backend 产生；`interception.capability` 只由可执行透明拦截 strategy 产生，当前是入站 TPROXY。`outbound_mitm` 只产生 redirect preview，不把 preview 伪装成 capability requirement。`enforce` 表示允许真实执行，但必须配置且只能配置一个执行面，不再隐式等同于 connection enforcement。
...
后续要实现真正 process-scoped transparent interception，应引入 cgroup/owner mark 或 proxy-side classification，而不是把 process selector 丢掉；后续要启用出站 MITM runtime install，应把已有 proxy pre-connect `SO_MARK`、已存在但未接入 lifecycle 的 original-destination resolver、证书材料、MITM CA 注入、output redirect lifecycle 和 L7 分类作为同一个 executable backend 的门控条件。
...
- 不把透明代理/MITM 作为默认核心拦截路径；当前只实现显式启用的 Linux nftables/TPROXY 规则生命周期，并可选启动 agent-managed plain TCP relay。后续 MITM backend 必须显式启用、能力门控、按进程/应用 selector 生效，并独立管理证书材料、proxy self-bypass lifecycle、output redirect lifecycle 和 L7 分类，不能复用 TLS material 或 libssl uprobe instrumentation 的概念。
...
| MITM/透明代理预留 | 透明拦截 strategy 已作为 enforcement typed plan 建模；入站已有 Linux nftables/TPROXY + policy routing lifecycle，可选启动 agent-managed plain TCP relay，并暴露基础 relay runtime metrics、配置化 active proxy health probe 与 relay-connect upstream connect telemetry；relay 目标恢复已拆成显式策略，出站 MITM 可复用 Linux original-destination resolver 原语，proxy connect path 已具备 pre-connect `SO_MARK` 原语；出站 MITM 已有按 outbound selector 投影的 status/check 表达与 RuntimePlan redirect preview，但不安装 output redirect，直到证书注入、output redirect lifecycle 和 L7 MITM lifecycle 一起落地 | 第 11、18、32 节 |
