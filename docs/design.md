...
8. `enforcement.interception.strategy` 可表达 `inbound_tproxy` 与 `outbound_mitm`，并在 runtime plan/status 中报告独立 selector 和 `transparent_interception` capability；当前仅 `inbound_tproxy` 有 executable Linux lifecycle，`outbound_mitm` runtime 会 fail closed，不安装 output redirect 规则。
