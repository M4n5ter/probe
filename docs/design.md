...
- 已实现 `capture` crate 的核心 provider 模型：
  ...
  - `CaptureMultiplexer` 支持 required 与 best-effort slot。primary live provider 放在 required slot；TLS plaintext sidecar 放在 best-effort slot。required provider 失败会停止 capture；best-effort provider 运行期失败会进入 disabled 状态，在 capability/runtime status reason 中保留失败原因，并继续 primary capture。
...
| `libssl_uprobe` | Probed degraded/unavailable; degraded is explicitly usable for best-effort TLS plaintext | 已实现 process-scoped discovery/planning、strict TLS artifact preflight、eBPF libssl plaintext artifact contract、userspace Aya loader/provider、ringbuf sample decoder、procfs fd-to-flow resolver、startup attach sidecar builder、attach target reconcile diff model，以及 primary live provider + TLS plaintext provider 的 capture multiplexer wiring；best-effort attach 只有在同一 process/library target 上完整 attach fd association recipe 与 plaintext recipe 后才启用 sidecar，半个 recipe 会 rollback 已 attach link；TLS producer 在 `SSL_set_fd` state 存在时置 `FD_VALID`，userspace 只在该标志存在时用 fd 做 procfs flow resolution；详情见 TLS plaintext 小节。 | 尚未实现 agent runtime 的周期性发现、live attach/detach reconcile executor、process-exit 清理、强 fd ownership、async fan-in/backpressure 和 provider 级健康上报；producer 的 fd 关联仍是 best-effort，`FD_VALID` 表示可尝试归因，不等于强 socket ownership 证明，解析成功的 libssl bytes 仍保持 degraded。 |
...
当前实现不是运行中动态 reconcile。当前 `agent run` 只做启动时一次性 procfs scan；startup attach 被建模为从空 attach state 到当前 attach plan 的第一次 reconcile，然后把 libssl plaintext provider 作为 best-effort sidecar 接入 `CaptureMultiplexer`。reconcile report 只暴露“attach/detach side effect 全部成功后才可提交”的 attach state，避免后续动态 executor 把计划状态误当成已生效状态。后续重复扫描、对新增 target 执行 attach、对 stale target 执行 detach/unattach 和 process-exit 清理仍属于缺口。
...
- Attach plan / loader：`capture::LibsslUprobeAttachPlan` 是 process-scoped 结构：process generation + 内部 procfs verifier source -> library targets -> recipes。recipe 只保存 symbol，角色和 attach point list 都由 `ebpf-abi` 中的 `EBPF_TLS_LIBSSL_UPROBE_SPECS` 派生，避免 `symbol`、角色、program name 和 probe kind 漂移出无效状态。每个 `LibsslUprobeAttachPoint` 都包含 library symbol、BPF program name、entry/return kind、symbol role 和 offset；`SSL_set_fd`、`SSL_clear` 和 `SSL_read/write` family 都规划 entry + return attach point；`SSL_free` 只有 entry attach point。attach offset 当前保持 `0` 并使用 Aya 的 symbol resolution。loader 会先 strict preflight TLS artifact，再把 attach plan 展开为 program/library symbol/process generation/target library request；attach execution 已从 ringbuf/sample probe 路径拆到独立 plaintext attach module，并通过 `LibsslUprobeAttachSession` 持有 loaded BPF program set 和按 process/library target 分组的 attached links；每次 attach 调用返回 readiness summary。probe drop 会通过该 session 对剩余 target links 做 draining best-effort detach；后续运行中 attach/detach executor 应复用同一个 session 边界，避免重复 load 已加载 program 或重新拼接 link 归属。每次 Aya attach 前都会重新读取 `/proc/<pid>/stat` 复核 process starttime，并用 `O_NOFOLLOW` 打开当前 `/proc/<pid>/root/...` read path 复核 device/inode identity，避免 selector、discovery 与 attach 之间的 PID reuse 或 stale/replaced library 被静默信任。
- Runtime wiring：agent 侧已实现 startup libssl plaintext runtime builder。它读取 `RuntimePlan`，要求 live capture mode，重新编译 TLS selector，先用 `ProcfsAttributor::probe` 确认全局 procfs dependency，再枚举并识别当前进程，调用 capture 层 attach planning 生成 startup attach plan，并用 `LibsslUprobeAttachState::reconcile` 把 startup 建模为 empty-state 初次 reconcile，最后用 `LibsslUprobePlaintextProvider` + procfs TGID/thread PID/fd flow resolver 构造 TLS plaintext sidecar。live capture factory 在 primary provider 外层包一层 `CaptureMultiplexer`，把 primary packet/process event 和 libssl plaintext event 送入同一个 pipeline；primary provider 是 required slot，TLS plaintext sidecar 是 best-effort slot。
- Runtime failure policy：配置/对象/ABI/program 形状错误仍 fail-closed。per-PID startup identity 读取中的消失进程、权限拒绝、malformed/racy proc metadata 会被视作 best-effort skip；全局 boot id、procfs 根路径等 dependency 错误不会被误当成单个 PID race。startup scan 没有目标、目标在 attach 前消失、库 identity 变 stale、单个 attach 失败，或没有同一 process/library target 上完整的 fd association + plaintext recipe set 时，不会拖死 primary capture，而是记录 disabled TLS plaintext runtime state，并在 runtime status 与 `libssl_uprobe` capability reason 中保留禁用原因。
- Attach atomicity：best-effort attach 的基本单位是 libssl recipe，而不是单个 uprobe attach point。每个 recipe 会先复核 process generation 和 mapped library identity，再逐点 attach；如果同一 recipe 中后续 attach 失败，本 recipe 已成功的 link 会反向 rollback。rollback 失败属于 live side effect 泄漏风险，必须 fail-closed。只有同一 process/library target 上 fd association recipe 与 plaintext recipe 都完整成功后，sidecar 才能进入 enabled 状态；只完成 lifecycle recipe、只有 plaintext recipe 或半个 recipe 都不能启用 sidecar。best-effort loader 会把这种 target-level readiness 作为 link 保留边界：如果没有任何可运行 target，会在返回 disabled 前 detach 已 attach links；如果部分 target 可运行、部分 target 只有半套 recipe，则只保留可运行 target 的 links；如果 prune 失败，attach session 会 best-effort drain 自身并 fail-closed。best-effort provider 进入 enabled 前会在 attach/prune 完成后打开 ringbuf，并按 ringbuf 容量、固定 TLS event 大小和少量 idle settle 次数有界丢弃启动窗口 backlog，覆盖已排队或 Aya `Busy` head record 的 stale plaintext，同时不追逐 retained target 的持续新事件；这仍是 best-effort hygiene，不是替代 target-level attach executor/generation fence 的强隔离。如果 ringbuf 打开失败，会先 detach 已保留 links 再返回错误。strict loader 没有 unretained target，不做 backlog drain。
- Remaining lifecycle gaps：当前仍缺运行中周期性进程发现、执行 attach/detach 的 reconcile loop、process-exit 清理、async fan-in/backpressure、provider 级健康上报和更强的 fd ownership。
...
TLS plaintext config/runtime/status/check plan 已能表达 provider、selector、capability requirement、`libssl_uprobe_object_path` 和 keylog/session secret refs。agent registry 会对已配置的 TLS plaintext object 做 strict TLS artifact preflight，并把 host/object/contract/procfs socket attribution 失败写入 `libssl_uprobe` capability reason；如果这些前置条件都满足，能力仍保持 degraded 而不是 available，因为当前只支持 startup attach sidecar。`tls.plaintext.enabled = true` 对 `libssl_uprobe` 要求 live capture mode，拒绝 unavailable capability，但允许 degraded best-effort capability 显式启用。best-effort attach disabled reason 会保留首个 attach/validation 失败，避免把 stale target、attach permission 和 partial recipe 全部压成无信息状态；online admin status 会额外暴露 TLS sidecar runtime pending/enabled/disabled 状态，并在 disabled 时把 status capability matrix 中的 `libssl_uprobe` overlay 为 unavailable。`external_plaintext_feed` available 只表示 agent 已能消费外部已解密明文 feed。
...
`tls.plaintext.selector` 与 enforcement/policy selector 使用同一 AST，用于表达“只对某些应用做 TLS plaintext instrumentation”的 attach 范围。当前 runtime 会编译校验 selector，并在 startup scan/planning/reconcile 阶段用 process-scope projection 做候选进程剪枝；尚未完成的是运行中的动态发现、attach/detach reconcile loop。
...
- TLS plaintext uprobe object 路径放在 `[tls.plaintext] libssl_uprobe_object_path` 下，而不是复用 `[capture.ebpf] object_path`。前者必须满足 strict TLS plaintext artifact contract，后者必须满足 strict process observation artifact contract；两个 artifact 不能混装。agent registry 会用该路径探测 `libssl_uprobe` capability：缺路径、host prerequisites、object read/parse、TLS contract 或 procfs socket attribution 失败都会进入 unavailable reason；TLS artifact 和 procfs attribution 都通过时报告 degraded，因为当前只支持 startup attach sidecar，只有 attach target diff/reconcile 模型，尚未实现运行中 dynamic process reconcile/detach executor。`tls.plaintext.enabled = true` 要求 live capture mode 和非 unavailable `libssl_uprobe` capability；如果 startup scan 后没有 attachable target、target attach race 导致无可用 probe，或没有同一 process/library target 上完整的 fd association + plaintext recipe set，agent 会继续 primary capture，并通过 disabled TLS plaintext runtime state 暴露 unavailable capability reason。
...
- 默认/显式 live capture：
  ...
  - TLS plaintext sidecar：libssl plaintext sample ABI、capture bridge、userspace uprobe loader/provider、agent startup sidecar builder、attach target reconcile diff model、procfs fd-to-flow resolver 和带状态生命周期清理的 eBPF uprobe producer 已存在；startup sidecar 作为 best-effort provider 接入 multiplexer，运行期失败会被禁用并更新 online runtime status/capability reason，但不会停止 required primary capture。
  - Missing：仍未完成 agent runtime 的周期性 dynamic attach/detach executor、process-exit cleanup、async fan-in/backpressure、lost-event feedback 和完整 kernel-side capture program。
...
- Poll、mux 与 reconcile：`CapturePoll` 单元测试覆盖 finite provider 的 finished、live provider idle、已消费输入但未产出事件的 progress 语义，eBPF connect retry 跨 poll 继续且不在 poll path 内 sleep；`CaptureMultiplexer` 单元测试覆盖某个 source finished 后继续轮询其它 active source、active source idle 后继续产出事件、progress 不触发 idle/finished、best-effort source 错误不会停止 required source、disabled best-effort capability 会保留失败原因，并且可通知 runtime state owner 更新 online status；`LibsslUprobeAttachState` 单元测试覆盖 empty-state startup reconcile、新 target attach plan filtering、retained target 不重复 attach、stale target reporting，以及 report 只通过成功后状态 accessor 暴露可提交 state 的边界。真正的 attach/detach side effect 成败提交顺序仍需等动态 executor 落地后补事务级测试。
- Attach loader：loader 单元测试覆盖 process-scoped attach plan 到 process generation/path/symbol/program/kind/recipe request 的展开、空计划 fail-fast、超出 `pid_t` 范围或 PID 0 的 fail-before-Aya、strict attach target identity stale 时 fail-before-Aya、Aya uprobe/uretprobe kind 与 attach kind 的匹配、best-effort attach 必须具备同一 process/library target 上完整的 fd association recipe 与 plaintext recipe、resolvable target set、disabled reason 保留首个 attach failure，以及 missing object 在 object preflight 阶段失败。attach request 展开、pid 校验、attach kind 匹配和 best-effort summary 的单元测试跟随独立 plaintext attach module；ringbuf decode 与 object preflight 测试留在 probe module。运行时 attach 使用 recipe 级 atomicity：半个 recipe 失败会 rollback 已 attach link；attach session 会持久记录 loaded program set 和 target links，best-effort load 只保留具备 target-level readiness 的 links，prune 失败时 session 会 best-effort drain 并返回错误，best-effort enabled 前会 bounded-drain/settle ringbuf 启动 backlog，probe drop 会通过 session 显式 best-effort detach 剩余 links。真实 Aya attach/rollback/detach side effect、ringbuf backlog drain 的 privileged 行为和运行中 dynamic target detach loop 的 privileged/e2e 覆盖仍待补齐。
- Runtime/config/status：config/runtime/status/check 测试覆盖 `tls.plaintext.libssl_uprobe_object_path` 的解析、无效路径/错误 provider 组合 fail-closed、RuntimePlan 透传、status/check metadata 输出，runtime 测试还覆盖 libssl TLS plaintext 要求 live capture mode、unavailable fail closed、degraded best-effort capability 可显式启用。agent registry 测试覆盖缺 TLS object path、无效 TLS object、TLS object preflight 成功但仍保持 degraded，以及缺 procfs socket attribution 时 unavailable；agent TLS plaintext runtime 单元测试覆盖 startup process scan 跳过消失进程与 racy/invalid process metadata，同时不把全局 procfs dependency error 误当成 per-PID race，disabled TLS plaintext build 会记录 unavailable reason，以及 bounded `(pid, process starttime, SSL*, TCP connection)` flow-start tracking 对同一 SSL connection 保持稳定、对 PID generation/connection 变化产生新 identity。startup runtime builder 现在把初次 attach 作为 empty-state reconcile 进入同一模型，但还没有运行中的 attach/detach executor。online admin status 会报告 TLS plaintext runtime sidecar 的 pending/enabled/disabled 状态；sidecar disabled 时，status capability matrix 和 `tls.plaintext.capability` 都会把 `libssl_uprobe` overlay 为 unavailable 并保留原因，整体 health 降级但 primary capture 不被误判为失败。

该验证仍不等同于 privileged libssl uprobe attach e2e、运行中动态 process reconcile/detach executor 或强 fd ownership 已完成。
...
3. startup libssl sidecar builder 可基于 selector 和 procfs scan 生成 attach plan，并作为 empty-state reconcile 产出待 attach target。
4. attach reconcile model 可区分 new、retained 和 stale targets，且只暴露 attach/detach side effect 成功后才可提交的 attach state，避免 retained target 在后续动态阶段被重复 attach，或把 planned state 当作 applied state。
...
2. selector 命中该进程。
3. startup attach 成功挂载 `SSL_set_fd`、`SSL_clear`、`SSL_free` 与 `SSL_read/write` family uprobes。
4. 捕获带 `FD_VALID` 的明文字节并解析到 TCP flow。
5. 明文字节进入 HTTP parser。
6. 事件标注 plaintext source、confidence 和 degraded reason。
7. dynamic process reconcile/detach executor 完成后，再把验收升级为动态 attach 生命周期覆盖。
