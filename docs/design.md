...
- 已实现 capability matrix；`procfs_attribution` 和 `procfs_socket_attribution` 分别按本机 `/proc/<pid>` 与 `/proc/net/tcp`、可选 best-effort `/proc/net/tcp6`、proc root 探测结果标记 degraded/unavailable。eBPF capture provider 已接入基于 `aya-obj` 的 object preflight：默认 agent descriptor 会先执行 host probe，把 Linux/BTF vmlinux、bpffs mount 和 `unprivileged_bpf_disabled` 状态写入 eBPF reason；宿主前提满足后，才读取可选的 `capture.ebpf.object_path` 并解析 object，报告 typed programs/maps、按显式 expected contract 校验 expected ringbuf map、tracepoint program name 和 raw tracepoint section，或明确解析失败原因。preflight 不调用 `aya::Ebpf::load_file`，避免普通 capability 探测创建内核 map、加载 BTF 或触发 verifier；object 读取使用 `NOFOLLOW`、打开后 regular-file metadata 校验和 64 MiB 上限，避免 symlink race 或超大文件拖垮 fallback 路径。仓库已有 `ebpf-abi` 共享 C ABI event envelope、`ebpf-object` dependency-light object preflight crate 和 `ebpf-program` kernel crate scaffold；当前内核程序通过 `aya-ebpf` 产生 `tracepoint/syscalls/sys_enter_connect` ringbuf observation event，只包含当前安全 scaffold 已能提供的进程身份和 command 字段，不声明 syscall args、lost-event 或最终 `CaptureEvent` 转换能力。高层用户态 `aya` loader 还没有引入；它要和 provider wiring、typed eBPF event 到 `CaptureEvent` 的转换、共享 hardened object open 边界一起落地，避免暴露半成品 API 或绕过 preflight 安全读取。由于完整 eBPF provider 和完整 kernel-side capture program 还没有实现，eBPF provider 即使 object preflight 和 contract 校验成功也必须标记 unavailable，不能被 `auto` 或显式 `ebpf` 选择为可运行 provider。libssl uprobe/keylog TLS 解密和真实 enforcement 相关能力仍必须标记 unavailable；libssl uprobe 当前只实现了尚未接入 provider/loader 的 attach candidate discovery 代码：可读取 `/proc/<pid>/maps`，识别 executable libssl/boringssl 映射，保留 mapped path、`/proc/<pid>/root` read path、typed mapped file identity（device major/minor + inode）和 deleted 状态，打开 library 后用 fd metadata 校验仍匹配 maps 中的 mapped file identity，再使用 ELF symbol table 确认已定义的 `SSL_read`、`SSL_write`、`SSL_read_ex`、`SSL_write_ex` text symbols，并输出 per-PID candidate；deleted mapping、不可读 library、过大 library、mapped file identity 不匹配或非 ELF library 都进入 typed degraded reason。TLS plaintext 已有 config/runtime/status plan 边界，但还没有实际 Aya uprobe loader、ring buffer、SSL fd/socket 关联或 plaintext event provider。`external_plaintext_feed`、HTTP/1、SSE、WebSocket handoff、dry-run enforcement 是可用能力。`ingress_journal` 和 `durable_spool` 仍标记 degraded，因为当前恢复是 at-least-once，且 durable parser checkpoint 和 processing provenance 还不完整；但它已经覆盖 persisted bytes、gap 和 connection lifecycle event。webhook exporter 已可在 `run` 中连续 drain planned sinks，并支持配置固定间隔、全局/每 sink 每轮 batch 预算、单 sink timeout、失败后的 fixed backoff，以及用于 trust anchors 和 client certificate/private key identity 的 per-exporter TLS material refs；但 adaptive/exponential backoff 和 retention deadline 尚未实现，因此仍标记 degraded。dry-run enforcement 记录策略保护意图但不执行真实阻断；外部 enforcement manifest 可以限制保护动作集合并提供额外 selector，`check`/`run` 会显式加载，offline `status` 做 metadata-only 检查，online admin status 报告运行中 loaded manifest。libpcap 能力按本机设备和权限探测结果标记 available/unavailable。默认 `auto` capture 不会静默选择 replay 或 plaintext feed 作为 live provider；`run` 在无 live capture provider 时 fail closed，显式 `plaintext_feed` 会从 JSON-lines feed path 读取已解密明文 event 并接入 replay 共用的 pipeline；policy selector 会在 runtime plan / configured policy load 阶段校验，并在 pipeline 中限制 policy 执行范围；`check` 用于查看 resolved plan 并显式加载 policy / enforcement planner，`status` 用于查看 health/spool/exporter/metrics 快照。
- 已实现 selector AST 的基础形态：`match`、`all`、`any`、`not`、`ref`，命名 selector 通过 registry 编译解析。
- 当前 `FjallSpool` 存储的是带 typed schema 的 `SpoolPayload`；代码内使用 `SpoolPayloadSchema` enum，落盘和 protobuf `payload_schema` 字段仍写稳定 wire string。`CaptureEvent` 和 `EventEnvelope.kind` 都是 serde tagged payload enum；`EventType` enum 显式镜像稳定 discriminant，用于 event id、`EventKind::name()` 和 policy hook 映射，并用测试锁住它与 serde `type` tag 的一致性。ingress lane 当前写入 JSON framed `CaptureEvent`，export lane 当前写入 JSON framed `EventEnvelope`。reader 只返回已越过 lane durable high-water sequence 的条目，避免并发 exporter 读到已 commit 但尚未 `SyncAll` 持久化完成的事件；`status` 使用同一 durable high-water 生成 spool/export lag 快照。protobuf batch envelope 通过显式 payload schema 标记该格式。这是过渡契约，不等同于最终 protobuf event envelope。当前 replay 不把文件输入归因到 agent 自身，而使用 synthetic replay identity、保留 PID/TGID `0` 和 0 confidence。
...
当前实现状态：agent 已有 eBPF host probe，检查运行平台是否为 Linux、`/sys/kernel/btf/vmlinux` 是否为 regular file、`/sys/fs/bpf` 是否为目录且 Linux `statfs` filesystem magic 是否为 bpffs，并读取 `/proc/sys/kernel/unprivileged_bpf_disabled` 作为权限/安全姿态提示，其中 `0` 表示 unprivileged BPF enabled，`1` 表示 permanently disabled，`2` 表示 disabled 但管理员仍可再改。宿主前提满足后，agent 会读取 `capture.ebpf.object_path` 并通过 `aya-obj` 做 eBPF object preflight，报告 object 是否能解析、其中的 typed programs/maps，以及是否满足显式 expected contract：默认需要 `SSSA_EVENTS` ringbuf map、`sssa_sys_enter_connect` tracepoint program 和 `tracepoint/syscalls/sys_enter_connect` raw section。`ebpf-abi` 已定义共享 ringbuf event envelope 和 userspace decoder，当前 decoder 只支持 connect tracepoint observation record；`ebpf-object` 承载 dependency-light object preflight，`ebpf-program` 已用 `aya-ebpf` 提供最小内核 object scaffold，`xtask check-ebpf` 负责在具备 nightly、rust-src 和 `bpf-linker` 的环境中对 eBPF crate 执行 fmt、BPF target clippy、locked release build 和 object contract preflight；`xtask ebpf-build` 只做 locked build 与 preflight。高层用户态 `aya` loader、agent provider wiring、typed eBPF event 到 `CaptureEvent` 的转换和完整 kernel-side capture program 还没有完成，因此 `ebpf` provider descriptor 仍不会进入 available；`auto` 不会选择 eBPF，显式 `capture.selection = "ebpf"` 会 fail closed 并给出 object、contract 或未实现 provider/event conversion 的原因。
...
- `agent` composition root 只在 libpcap provider 能按当前配置打开并安装 filter 时注入 available `Libpcap` descriptor；否则注入 unavailable descriptor 并输出原因，`runtime` 只消费 descriptor 做 plan。live `LibpcapProvider` 由 `agent` 注入 procfs TCP process resolver；resolver 通过共享 `TcpConnection`、`/proc/net/tcp`、可读取且可解析时的 `/proc/net/tcp6` 和 fd socket inode best-effort 归因，短 TTL 复用 socket snapshot，成功时提高 attribution confidence，失败时回退到 synthetic unknown identity。`procfs_socket_attribution` 仍是 degraded capability：hidepid、权限、PID/fd race 和 net namespace 边界会导致 lookup 失败；lookup 错误进入 capture degradation reason，未匹配则以 0 confidence 表达。
...
当前实现状态：已实现 libssl uprobe attach candidate discovery 的第一步，但尚未接入 runtime provider/loader。`capture::LibsslUprobeTargetDiscovery` 可按 PID 读取 `/proc/<pid>/maps`，跳过非 executable 和非文件映射，识别 libssl/boringssl shared object，保留 mapped path、`/proc/<pid>/root` read path、typed mapped file identity（device major/minor + inode）和 deleted 状态，并通过单次打开的 fd 校验当前 read path 仍匹配 maps 中的 mapped file identity，然后用 `object` crate 解析 ELF symbol table，确认当前支持的 `SSL_read`、`SSL_write`、`SSL_read_ex`、`SSL_write_ex` 已定义 text symbol 集合。deleted mapping、不可读 library、超过当前大小上限的 library、mapped file identity 不匹配或非 ELF library 都进入 typed discovery degraded reasons，而不是把它们伪装成可 attach target。TLS plaintext config/runtime/status plan 已能表达 provider、selector、capability requirement 和 keylog/session secret refs，但尚未实现 Aya uprobe loader、动态 attach lifecycle、SSL fd/socket 关联、ring buffer 和 plaintext event provider，因此 capability matrix 中 `libssl_uprobe` 必须保持 unavailable；`external_plaintext_feed` available 只表示 agent 已能消费外部已解密明文 feed。
...
`key_log_file` 和 `session_secret_file` 是 plaintext decrypt hint material，只能由 TLS plaintext plan 显式引用。`tls.plaintext.key_log_refs` 必须指向 `key_log_file` material，`tls.plaintext.session_secret_refs` 必须指向 `session_secret_file` material；引用不存在、空字符串或类型错配都会在基础配置校验阶段失败。runtime plan 会把这些 refs 解析成 typed material plan，status 会对 configured plaintext refs 做 metadata-only source check。当前 `keylog` plaintext provider 是保留 provider，runtime validation 在启用时 fail closed；禁用状态下保留 refs 只用于配置/status 可见性，不代表解密已启用。`libssl_uprobe` provider 不使用 keylog/session secret refs，因此只要当前 provider 是 `libssl_uprobe`，配置这些 refs 就会被拒绝。
...
规划事件类型包括 TLS metadata、分协议 body chunk 细分、WebSocket frame/message、HTTP/2 stream event 等；这些不能冒充当前 policy bundle 可用事件。
...
3. Lua policy bundle 根据 manifest 注册 hook，例如 `on_connection_opened`、`on_connection_closed`、`on_http_request_headers`、`on_http_response_headers`、`on_http_body_chunk`、`on_sse_event`、`on_websocket_handoff`、`on_opaque_stream`、`on_gap`、`on_protocol_error`。
...
- `id`。
- `version`。
- `selector`：可选 core typed selector。
...
- verdict action 不在当前 protection profile 的 `protective_actions` 中时输出 `unsupported`，而不是静默当作已防护。
- 真实 `enforce` 模式在当前实现中必须 fail closed，因为还没有可执行阻断后端。
...
- eBPF object 路径放在 `[capture.ebpf] object_path` 下。这个字段作为 `aya-obj` object preflight 输入，但不等同于 eBPF provider 已可运行；高层用户态 `aya` loader、agent provider wiring、typed event conversion 和完整 kernel-side capture program 完成前，preflight 成功仍然是 unavailable capability，并保留未实现原因。
...
- 当前 replay/live pipeline 先将完整 `CaptureEvent` 写入 ingress journal，再解析成 `EventEnvelope` 写入 export queue；`recover_ingress_journal_until_idle` 可以从 durable ingress journal 按 sequence 顺序重放 `CaptureEventJson` payload，重放 parser/policy/enforcement/export 阶段；`agent run` 会先执行该恢复，再打开 capture provider。
...
- export queue 对已经写入的 `EventEnvelope` 提供 at-least-once 投递。
- 已写入 export queue 的 event 有稳定 `event_id`。
- batch 有稳定 `batch_id`。
- 接收端可按 id 去重已入队事件的重试投递。
- 当前 ingress recovery 重新生成的语义重复事件不保证复用同一个 `event_id`，不能把这类重复交给接收端 event-id 去重。
- 不承诺 exactly-once。
...
- `capture`：capture provider trait、replay provider、external plaintext feed provider、libpcap provider、eBPF host preflight、未来用户态 `aya` loader 和 libssl/keylog provider adapter；eBPF provider wiring 和 typed event conversion 完成后再把 loader 接入可选 live provider。
...
- `ebpf-abi`：userspace/kernel-space 共享 C ABI event envelope、program/map contract 常量。
- `ebpf-object`：dependency-light `aya-obj` parser preflight、typed map/program inventory 和 artifact contract 校验。
- `ebpf-program`：排除在默认 host workspace 之外的 Aya eBPF 内核程序 crate，由 `xtask check-ebpf` 显式校验，由 `xtask ebpf-build` 显式构建。
...
- 默认/显式 live capture：按 `auto`、`ebpf`、`libpcap` 解析 plan；当前实际可运行的是 libpcap，eBPF 已有 `aya-obj` object preflight、shared ABI 和最小内核 object scaffold，但还没有高层用户态 `aya` loader、provider wiring、typed event conversion 和完整 kernel-side capture program，因此不会被选中。
...
| 当前实现真实连接级阻断 | 推迟 | 会抢占观测闭环、TLS demo 和 fallback 交付主线。 |
...
| TLS 路线 | 非 MITM 优先，当前优先完成 libssl uprobe demo | 第 11、31 节 |
...
| WebSocket | 当前识别 Upgrade 并定义 handoff，不完整解析 frame | 第 13 节 |
