...
pub(super) fn prepare_health_probe(
...
) -> Option<TransparentProxyHealthProbePlan> {
    ...
    Some(TransparentProxyHealthProbePlan {
        target: *target,
...
mod tests {
    ...
    fn disabled_health_probe_has_no_runtime_plan() {
        assert!(
            prepare_health_probe(&TransparentInterceptionProxyHealthProbePlan::Disabled).is_none()
        );
    }
