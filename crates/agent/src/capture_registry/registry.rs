...
use crate::transparent_interception::{
    TransparentInterceptionFlowClassifier, TransparentInterceptionProcessClassifier,
};
...
fn provider_registry_with_capture_descriptors(
    ...
    let transparent_flow_classifier =
...
fn ebpf_provider_descriptor_from_object_report(
    ...
    CaptureProviderDescriptor::degraded(
        ...
        CaptureProviderBuilder::Ebpf,
        format!(
            "eBPF object preflight via aya-obj succeeded ({}), procfs socket attribution is usable, live fd lookups can carry optional SO_COOKIE when pidfd_getfd is permitted and the duplicated fd inode still matches, and the process observation provider can emit connect and accept/accept4 flow-start observations with descriptor leases, prefer procfs-resolved socket metadata when available, fall back to degraded observed flows when kernel tracepoints provide a remote endpoint before procfs resolution succeeds, seed startup and flow-start process payload allowance from procfs selectors, recover tracked flows at payload time from descriptor leases when a flow-start event was missed, authorize process lifecycle payload sampling through direct TGID resolution or tracepoint-provable observed process metadata, bind flow-start payload capture through descriptor/socket allow maps, emit selector-authorized always-degraded outbound single-buffer and bounded multi-iovec prefix syscall argument samples and inbound single-buffer and bounded multi-iovec prefix syscall result samples bound to descriptor generation, descriptor-generation close/plain close_range lifecycle events, userspace tracked-flow displacement as event-local terminal provider-state boundary gaps, output ring-buffer failure conversion to degraded capture_loss events, conservative unknown-offset gap fan-out to active tracked payload flows, and per-tracepoint kernel firing counters, but payload beyond the bounded multi-iovec scan/sample or fixed verifier-friendly append slots, precise flow-specific lost-event reconstruction, kernel socket-object lifetime, and complete kernel traffic capture are not implemented",
            object.summary(),
        ),
    )
}
...
mod tests {
    ...
    fn ebpf_provider_descriptor_exposes_degraded_observation_provider_after_object_preflight() {
        ...
        assert!(reason.contains("per-tracepoint kernel firing counters"));
        ...
        assert!(reason.contains("flow-start payload capture through descriptor/socket allow maps"));
