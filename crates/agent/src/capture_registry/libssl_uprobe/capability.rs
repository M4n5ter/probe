...
fn capability_from_object_report(
    ...
    CapabilityState::degraded(
        CapabilityKind::LibsslUprobe,
        format!(
            "eBPF TLS plaintext object preflight via aya-obj succeeded ({}), procfs socket attribution is usable, and agent libssl plaintext sidecar wiring can run startup attach plus periodic process scan/reconcile with last-reconcile counters and observed timestamp, but per-target provider health, strong fd ownership, low-latency provider multiplexing, and privileged dynamic lifecycle e2e coverage remain best-effort",
            object.summary()
        ),
    )
}
...
mod tests {
    ...
    fn capability_stays_degraded_until_stronger_lifecycle_guarantees_exist() {
        ...
        assert!(reason.contains("last-reconcile counters and observed timestamp"));
